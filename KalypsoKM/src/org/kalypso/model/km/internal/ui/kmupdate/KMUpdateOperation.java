...
public class KMUpdateOperation implements ICoreRunnableWithProgress
{
  ...
  private IStatus[] calculateChannel( final KMChannel kmChannel, final KalininMiljukovType km ) throws Exception
  {
    ...
    for( int i = 0; i < kmParameter.size(); i++ )
    {
      ...
      m_featureChanges.add( new FeatureChange( kmParameterFE, rkvT, kForeland ) ); //$NON-NLS-1$
      m_featureChanges.add( new FeatureChange( kmParameterFE, KMParameter.PROP_RNF, nValid ) ); //$NON-NLS-1$
      m_featureChanges.add( new FeatureChange( kmParameterFE, rnvPT, nForelandValid ) ); //$NON-NLS-1$
      m_featureChanges.add( new FeatureChange( kmParameterFE, qrkPT, qSum ) ); //$NON-NLS-1$
      m_featureChanges.add( new FeatureChange( kmParameterFE, cPT, alpha ) ); //$NON-NLS-1$
