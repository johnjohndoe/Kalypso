 ...
 * 
 */
public class ExecuteTelemacKalypsoSimulation
{
  ...
  /**
   * Create execute request to TelemacKalypsoSimulation WPS
   * 
   * @throws URISyntaxException
   *           if any of the inputs is not a valid URI
   */
  public ExecuteTelemacKalypsoSimulation( final String serviceEndpoint, final IWPSObserver wpsObserver, final String pStrExe, final URI pTelemacModelPath )
  {
    m_serviceEndpoint = serviceEndpoint;
    m_wpsObserver = wpsObserver;
    m_inputs = createInputs( pStrExe, pTelemacModelPath );
  }
  ...
  public IStatus run( final IProgressMonitor monitor )
  {
    ...
    try
    {
      ...
      outputs.add( TelemacKalypsoSimulation.OUTPUT_RESULTS );

      // run the simulation
      m_manager = VFSUtilities.getNewManager();
      m_wpsRequest = new DefaultWPSProcess( TelemacKalypsoSimulation.ID, m_serviceEndpoint, m_manager );
...
//    {
//      return StatusUtilities.statusFromThrowable( e, Messages.getString( "org.kalypso.kalypsomodel1d2d.sim.TelemacCalculation.3" ) ); //$NON-NLS-1$
//    }
...
//      e.printStackTrace();
  ...
  public FileObject getResultsDir( ) throws Exception
  {
    if( m_resultsDir == null )
    {
      ...
      final Object[] resultArray = m_wpsRequest.getResult( TelemacKalypsoSimulation.OUTPUT_RESULTS );
      if( resultArray == null )
        return null;
