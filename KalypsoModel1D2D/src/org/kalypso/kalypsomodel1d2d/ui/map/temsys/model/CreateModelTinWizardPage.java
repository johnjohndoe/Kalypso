  ...
  protected void doHandlePageChangeing( final PageChangingEvent event )
  {
    ...
    if( event.getCurrentPage() == this && event.getTargetPage() == getNextPage() )
    {
      ...
      if( m_execute.isOK() )
      {
        ...
        m_statusComposite.setStatus( new Status( IStatus.OK, KalypsoModel1D2DPlugin.PLUGIN_ID, Messages.getString( "org.kalypso.kalypsomodel1d2d.ui.map.temsys.model.CreateModelTinWizardPage.6" ) ) ); //$NON-NLS-1$
      }
      else
      {
        event.doit = false;
