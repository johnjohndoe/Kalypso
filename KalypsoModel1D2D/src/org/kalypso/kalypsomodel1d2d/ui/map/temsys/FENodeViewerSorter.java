...
{
  private final Map<TableColumn, Boolean> m_sortMap = new HashMap<>();

  public FENodeViewerSorter( final TableColumn defaultColumn )
  {
    setCurrentColumn( defaultColumn );
  }

  /**
   * Pushs the current sortorder in a map which key is the table-column.
   *
   * @param column
   */
  public void pushSortCriteria( final TableColumn column )
  {
    if( m_sortMap.get( column ) == null )
    {
      m_sortMap.put( column, new Boolean( true ) );
    }
    else
    {
      final boolean newSort = !m_sortMap.get( column ).booleanValue();
      m_sortMap.put( column, new Boolean( newSort ) );
    }
  }
   ...
   */
  public boolean isDescending( final TableColumn column )
  {
    boolean returnValue = true;
    if( this.m_sortMap.get( column ) != null )
    {
      returnValue = this.m_sortMap.get( column ).booleanValue();
    }
    else
    {
  ...
  public int compare( final Viewer viewer, final Object obj1, final Object obj2 )
  {
    int rc = -1;
    // get the data
    final IFE1D2DNode data1 = (IFE1D2DNode) obj1;
    final IFE1D2DNode data2 = (IFE1D2DNode) obj2;
   ...
   */
  public void setCurrentColumn( final TableColumn currentColumn )
  {
