...
public class Add2DElementsCommand implements ICommand
{
  ...
  public Add2DElementsCommand( final GMLWorkspace discretisationModel, final List< ? extends GM_PolygonPatch> elements, final IProgressMonitor monitor )
  {
  ...
  public void process( ) throws Exception
  {
    try
    {
      final SubMonitor monitor = SubMonitor.convert( m_progressMonitor );
      monitor.beginTask( getDescription(), m_elements.size() );
  ...
  {
    return m_log.asMultiStatusOrOK( getDescription() );
  }
  ...
  private void createElementFromRing( final GM_PolygonPatch ring )
  {
    final GM_Position[] positions = ring.getExteriorRing();
    final String srsName = ring.getCoordinateSystem();

    final GM_Point[] points = new GM_Point[positions.length - 1];
    for( int i = 0; i < points.length; i++ )
    {
      final GM_Position position = positions[i];
      points[i] = GeometryFactory.createGM_Point( position, srsName );
      final IFE1D2DNode node = m_discModel.findNode( points[i], IFEDiscretisationModel1d2d.CLUSTER_TOLERANCE );
      if( node != null )
  ...
  {
    final IFE1D2DNode[] nodes = createNodes( points );
  ...
  private IFE1D2DNode[] createNodes( final GM_Point[] points )
  {
    final IFE1D2DNode[] nodes = new IFE1D2DNode[points.length];

    for( int i = 0; i < points.length; i++ )
    {
      nodes[i] = m_discModel.createNode( points[i] );
  ...
  private void createElement( final IFE1D2DEdge[] edges )
  {
