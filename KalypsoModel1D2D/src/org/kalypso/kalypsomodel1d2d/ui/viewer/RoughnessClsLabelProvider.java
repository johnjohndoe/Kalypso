...
/**
 * Label provider for {@link org.kalypso.kalypsosimulationmodel.core.roughness.IRoughnessCls} This label provides can
 * have two states:
 * <ul>
 * <li/>{@link #ROUGHNESS_ONLY} to provide label for tables showing only roughnesses
 * <li/>{@link #ROUGHNESS_WITH_CORRECTION} to provides labels for tables showing corrected roughnesses *
 * </ul>
 *
 * @author Madanagopal
 ...
 *
 */
public class RoughnessClsLabelProvider extends LabelProvider implements ITableLabelProvider
{
  /**
  ...
  /**
   * State key for providing label to tables showing only corrected roughnesses
   *
   */
  ...
  {
    if( m_configState == ROUGHNESS_ONLY )
    {
      return getColumnTextForRoughnessOnly( element, columnIndex );
    }
    else if( m_configState == ROUGHNESS_WITH_CORRECTION )
    {
      return null;
    }
    else
  ...
  /**
   * Sets the config state for this label provide. legal values are:
   * <ul>
   * <li/>{@link #ROUGHNESS_ONLY}
   * <li/>{@link #ROUGHNESS_WITH_CORRECTION}
   * </ul>
   */
  public void setConfigState( final int configState ) throws IllegalArgumentException
  {
    if( !(configState != ROUGHNESS_ONLY || configState != ROUGHNESS_WITH_CORRECTION) )
    {
      throw new IllegalArgumentException( "configState value is illegal:" + configState ); //$NON-NLS-1$
    }

    m_configState = configState;
  }

  private final String getColumnTextForRoughnessOnly( final Object element, final int columnIndex )
  {
    if( element instanceof IRoughnessCls )
    {
      // return ((IRoughnessCls)element).getName();
  ...
  private static final String toUnknownColSpecStr( )
  {
