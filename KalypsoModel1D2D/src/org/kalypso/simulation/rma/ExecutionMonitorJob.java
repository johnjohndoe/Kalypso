      ...
      canceledLoop: while( !monitor.isCanceled() )
      {
        ...
        if( m_executeResponse != null )
        {
          ...
          else if( processFailed != null )
          {
            ...
            status = new Status( IStatus.ERROR, KalypsoModel1D2DPlugin.PLUGIN_ID, messages );
            monitor.done();
            break canceledLoop;
          }
          ...
          {
            status = new Status( IStatus.WARNING, KalypsoModel1D2DPlugin.PLUGIN_ID, Messages.getString( "ExecutionMonitorJob_2" ) ); //$NON-NLS-1$
          }
