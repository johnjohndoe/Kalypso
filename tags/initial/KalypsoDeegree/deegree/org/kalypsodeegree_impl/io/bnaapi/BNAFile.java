 ...
 * Data are returned as a {@link BNAFeatureSet}
 * <p>
 * @version 2003.08.04
...
public class BNAFile {
    ...
    private void loadFile( RandomAccessFile raf ) throws IOException {
        ...
        while ( true ) {
            ...
            //			if(len-pos < 3)
            //			{
            //				System.out.println("TRIMMING OUT FILE TAIL ("+(len-pos)+" bytes left)");
            //				break;
            //			}
            // READ HEADER
            ...
            //...
            // End of delimiters check. Cant' parse any further. Maybe there is only garbage left.
            if ( heads == null ) {
                // Check to see if remaining chars can be safely skipped
                raf.seek( pos );

                if ( !skippable( raf ) ) {
                    System.out.println( "Cannot continue parsing file. " + ( len - pos ) + 
                                        " bytes left out." );
                }
    ...
    /**
     * Search for quote-delimited headers
     * We are looking for a header with this format:
     *          "name1", "name2", ... , numpoints
     * The split array will be something like this:
     *                       0    1   2   3   4            len-1
     *    "A","B",...,n => null / A / , / B / , / ... / , / n
    *
     * @param    header The header to be parsed
     *
     * @return   a String[] with the parsed headers (last element in array will be the numpoints)
     *           or null if the header is not parsable by quotes.
     *
     */
    private String[] getHeaderByQuote( String header ) throws IOException {
        ...
        if ( ( heads.length >= 5 ) && // we need at least 2 header fields + numpoint
             ( ( heads.length % 2 ) == 1 ) ) // it must be an odd number: i.e: 1"2"3"4"5
        {
    ...
    /**
     * Search for tab-delimited headers
     * We are looking for a header with this format:
     *       name1 TAB name2 TAB ... TAB numpoints
     *
     * The split array will be something like this:
     *        0   1        len-1
     *          A / B / ... / n
    *
     ...
     *
     * @return   a String[] with the parsed headers (last element in array will be the numpoints)
     *           or null if the header is not parsable by tabs.
     */
    ...
    /**
     * Search for space-delimited headers
     * We are looking for a header with this format:
     *       name1 TAB name2 TAB ... TAB numpoints
     *
     * The split array will be something like this:
    ...
    */
     ...
     * This methods reads <I>n</I> line from file <I>raf</I>.
     * In each line there's a pair of coords, comma- or tab-separated.
     *
     * @param    raf   The input file
     * @param    n     Number of points (file lines) to be read
     *
    ...
    private BNAGeometry readGeometry( RandomAccessFile raf, int n ) throws IOException {
        ...
        for ( int i = 0; i < n; i++ ) {
            ...
            GM_Position point = 
            	GeometryFactory.createGM_Position( Double.parseDouble( coords[0] ), 
