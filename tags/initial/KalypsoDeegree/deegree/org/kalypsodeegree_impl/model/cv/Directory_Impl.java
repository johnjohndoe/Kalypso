...
class Directory_Impl implements Directory {
    ...
    Directory_Impl( String resource, GM_Envelope boundingBox, Directory[] directories, 
                    String[] extensions, double widthCRS, 
                    double heightCRS ) {
        this.resource = resource;
        this.boundingBox = boundingBox;
     ...
     * of <tt>Directory</tt> will contain one or more <tt>Directory</tt> instances with a
    ...
    public Tile[] getTiles( GM_Envelope bbox ) throws CoverageCreationException {
        ...
        //double dia = Math.sqrt(bbox.getWidth()*bbox.getWidth() + bbox.getHeight()*bbox.getHeight());
        ArrayList list = null;
        ...
        try {
            ...
            while ( x1 < boundingBox.getMax().getX() ) {
                ...
                while ( y1 < boundingBox.getMax().getY() ) {
                    GM_Envelope env = GeometryFactory.createGM_Envelope( x1, y1, x1 + widthCRS, 
                                                                 y1 + heightCRS );
                    ...
                    if ( env.intersects( bbox ) ) {
                        ...
                        list.add( new Tile_Impl( new URL( "file:///" + resource + "/" + fl ), env, 
                                                 level ) );                        
    ...
    /** returns the height of the tiles contained within the diirectory in measurement
     * of its CRS
     *
     */
    ...
    /** returns the width of the tiles contained within the diirectory in measurement
     * of its CRS
     *
     */
