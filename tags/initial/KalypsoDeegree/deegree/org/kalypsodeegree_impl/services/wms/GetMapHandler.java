...
class GetMapHandler {
	...
	private double calcScale() throws Exception {
		...
		GM_Position min = GeometryFactory.createGM_Position(
                    bbox.getMin().getX() + dx * ( request.getWidth() / 2d - 1d), 
                    bbox.getMin().getY() + dy * ( request.getHeight() / 2d - 1d));
		GM_Position max = GeometryFactory.createGM_Position(
                    bbox.getMin().getX() + dx * ( request.getWidth() / 2d), 
                    bbox.getMin().getY() + dy * ( request.getHeight() / 2d));
	...
	protected synchronized void increaseCounter() {
		count++;
        if ( count == themes.length) notifyAll();
	}

	/**
	 * performs a GetMap request and retruns the result encapsulated within
	 * a <tt>WMSGetMapResponse</tt> object. <p>
	 ...
	 * to the client as requested (GetMap-Request EXCEPTION-Parameter).
	 *     
	...
	public WMSGetMapResponse performGetMap() throws WebServiceException {
		...
		for (int i = 0; i < layers.length; i++) { 
            ...
            if ( layers[i] instanceof NamedLayer ) {
                org.deegree.services.wms.capabilities.Layer lay = 
                    capabilities.getCapability().getLayer(layers[i].getName());
	 ...
	 *
	 * @param namedLayer layer to get the styles for
	 */
	 ...
	 */
	private boolean validate(org.deegree.services.wms.capabilities.Layer layer, String name) throws WebServiceException {
		...
		if (!tmp) {
			throw new InvalidSRSException("layer: " + name + " can't be " + 
                                          "delievered in SRS: " + request.getSrs());
		}
		// check scale
	...
	private WMSGetMapResponse createResponse(Exception e) {
		...
		if ("application/vnd.ogc.se_inimage".equals(format)) {
			BufferedImage bi = new BufferedImage(request.getWidth(), request.getHeight(), BufferedImage.TYPE_INT_ARGB);
			Graphics g = bi.getGraphics();
			g.setColor(Color.BLUE);
			g.drawString(e.toString(), 5, 20);
			content = bi;
		} else if ("application/vnd.ogc.se_blank".equals(format)) {
			content = new BufferedImage(request.getWidth(), request.getHeight(), BufferedImage.TYPE_INT_ARGB);
		} else {
	...
	private WMSGetMapResponse renderMap() {
		...
		if (mime.equals("image/svg+xml")) {
			response = WMSProtocolFactory.createWMSGetMapResponse(request, exce, ((SVGGraphics2D)g).getRoot());
		} else {
	...
	/**
	 * prints a copyright note at left side of the map bottom. The copyright
	 * note will be extracted from the WMS capabilities/configuration
	 * @param g graphic context of the map 
	 * @param heigth height of the map in pixel
	 */
	...
	private class ServiceInvokerForNL extends Thread implements OGCWebServiceClient {
		 ...
		 */
		ServiceInvokerForNL(org.deegree.services.wms.capabilities.Layer layer, UserStyle[] styles, int index) {
			this.layer = layer;
			this.styles = styles;
			this.index = index;
		}                
			...
			if (ds != null) {
				...
				try {
					...
					OGCWebServiceException exce = 
                        new OGCWebServiceException_Impl("ServiceInvokerForNL: " + 
		...
		private OGCWebServiceEvent createGetFeatureRequest(DataSource ds) throws Exception {
			...
			if (query == null) {
                ...
                if (filter instanceof ComplexFilter) {	
					...
					org.deegree.services.wfs.filterencoding.Operation op = 
                        ((ComplexFilter)filter).getOperation();
 ...
 //				double[] delev = request.getElevation(); // this is the elevation array
 //				if(delev != null)
 //					rpl.addParameter("elevation", delev); //dunno how to parse it
		...
		/**
		 * creates a getMap request for requesting a cascaded remote WMS 
		 * considering the getMap request and the filterconditions defined in 
		 * the submitted <tt>DataSource</tt> object
		 * The request will be encapsualted within a <tt>OGCWebServiceEvent</tt>.
		...
		private OGCWebServiceEvent createGetMapRequest(DataSource ds) throws XMLParsingException {
			...
			IDGenerator idg = IDGenerator.getInstance();
			gmr = WMSProtocolFactory.createGetMapRequest(version, "" + idg.generateUniqueID(), lys, elevation, 
                sampleDim, format, request.getWidth(), request.getHeight(), request.getSrs(), 
                request.getBoundingBox(), tranparency, bgColor, request.getExceptions(), 
                time, null, null, vendorSpecificParameter);
			...
			try {
				...
				if (res.getException() != null) {
					...
					if (res instanceof WMSGetMapResponse) {
						...
						OGCWebServiceException exce = 
                            new OGCWebServiceException_Impl("ServiceInvokerForNL: " + layer.getName(), 
				...
				Debug.debugException(e, " - ");
				OGCWebServiceException exce = new OGCWebServiceException_Impl("ServiceInvokerForNL: " + layer.getName(), e.toString());
				putTheme(index, exce);
			}
		...
		private void handleGetCoverageResponse(WCSGetCoverageResponse response) throws Exception {
			...
			GC_GridCoverage gc = new ImageGridCoverage(rop.getAsBufferedImage(), request.getBoundingBox(), reqCRS, false);
			org.deegree.graphics.Layer rl = MapFactory.createRasterLayer(layer.getName(), gc);
	...
	private class ServiceInvokerForUL extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                Debug.debugException(e, " - ");
                OGCWebServiceException exce = 
                    new OGCWebServiceException_Impl("ServiceInvokerForUL: " + layer.getName(), 
        ...
        private void handleWFS() throws Exception {
            ...
            org.deegree.graphics.Layer fl = 
                MapFactory.createFeatureLayer( layer.getName(), reqCRS, fc );
			putTheme(index, MapFactory.createTheme(layer.getName(), fl, styles));
            increaseCounter();
            Debug.debugMethodEnd();
        }
