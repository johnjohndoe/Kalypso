	...
	public static Filter buildFromDOM(Element element) throws FilterConstructionException {
		...
		if (!"http://www.opengis.net/ogc".equals(element.getNamespaceURI())) {
			throw new FilterConstructionException(
				"Element <" + element.getNodeName() + "> is not in the correct namespace: " + 
		...
		if ( firstElement.getLocalName().toLowerCase().equals("featureid") ) {
			...
			if ( WFSGNS.equals( firstElement.getNamespaceURI() ) ) {
				...
				if ( (!relationTypeAttribute.equals("BT")) && 
                     (!relationTypeAttribute.equals("NT")) && 
                     (!relationTypeAttribute.equals("RT")) ) {
					throw new FilterConstructionException(
						"Bad ThesaurusFilter: Attribute relationType is \"" + 
                        relationTypeAttribute + "\". Must be \"BT\", \"NT\" or \"RT\".");
				}

				int hierarchyLevelsAttribute = Integer.parseInt(firstElement.getAttribute("hierarchyLevels"));
				if (hierarchyLevelsAttribute <= 0) {
					throw new FilterConstructionException(
						"Bad ThesaurusFilter: Attribute hierarchyLevels is \"" + 
                        hierarchyLevelsAttribute + "\". Must be greater than zero.");
				}
