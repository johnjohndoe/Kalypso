...
/**
 * defines the encapsulating of the WCS GetCoverage operation<p>
...
//	private TimeExtent timeExtent = null;
    ...
    private WCSGetCoverageRequest_Impl(String version, String id, HashMap vendorspecificParameters,
                                       String layer, String crs, String responseCrs,
                                       GM_Envelope boundingBox, RangeParamList rpl, String format,
                                       String interpolationMethod, String exceptions) {
...
//        }
    ...
    WCSGetCoverageRequest_Impl(String version, String id, HashMap vendorspecificParameters,
                               String layer, String crs, String responseCrs,
                               GM_Envelope boundingBox, RangeParamList rpl,
                               double resX, double resY, double resZ,
							   String format, String interpolationMethod, String exceptions) {
        this(version, id, vendorspecificParameters, layer, crs, responseCrs,
			 boundingBox, rpl, format, interpolationMethod, exceptions);
    ...
    WCSGetCoverageRequest_Impl(String version, String id, HashMap vendorspecificParameters,
							   ...
							   String format, String interpolationMethod,
                               String exceptions) {
        this(version, id, vendorspecificParameters, layer, crs, responseCrs, boundingBox,
             rpl, format, interpolationMethod, exceptions);
        this.width = width;
        this.height = height;
        this.depth = depth;
...
//     * must match one or more RangeID elements under the selected coverage layer.
//     * <p>
...
//    }
     ...
     * Service Elements section. <p>
     * GetCoverage requests must use this parameter to specify the coordinate
     * reference system in which the query (BBOX) is expressed. The values of
     * this request parameter must be one of those defined in a SRS or
     * QuerySRS/SRS element under the requested coverage layer.
     */
     ...
     * in the SRS parameter.<p>
     * For any part of the coverage domain that is partly or entirely contained
     * in the Bounding Box defined by BBOX, the server should return coverage
...
// ETJ removed
//    /**
//     * If the Capabilities XML defines an ElevationExtent on the queried coverage
//     * layer, GetCoverage queries may use an ELEVATION parameter to constrain the
...
//     * RANGE parameter) with one of the RangeAxis names defined on that range set
...
//     * defined on the selected coverage layer, then the constraint can be expressed
//     * simply as: <p>
...
//    }
    ...
    /**
     * GetCoverage queries for gridded coverages may request coverage replies
     * with a specific grid size. The parameters WIDTH, HEIGHT, DEPTH define the
     * grid size (number of gridpoints or cells) along the three axes of the grid.
     * <p>
     * Either these parameters or RESX, RESY, RESZ are normally required for grid
     * coverage layers. However, if the Capabilities XML reports only the Interpolation
     * method None for the queried coverage layer, then GetCoverage queries must
     * be for the full native resolution of the data: they may not use RESX,
     * RESY, RESZ or WIDTH, HEIGHT, DEPTH to change the coverage resolution.
     * In this case, BBOX alone is used for subsetting.<p>
     ...
     * given in SRS or RESPONSE_SRS.<p>
     ...
     * third axis of that spatial reference system.<p>
     ...
     * resolution. In this case, BBOX alone is used for subsetting. <p>
     ...
     * application/vnd.ogc.se_xml by listing it in a <Exceptions><Format>
     * element in its Capabilities XML. The entire MIME type string in <Format>
     * is used as the value of the EXCEPTIONS parameter. <p>
     * Errors are reported using Service Exception XML, as specified in WMS 1.1
     * Section 6.7 and Annex A.3.  This is the default exception format if none
    ...
    public String getRequestParameter() throws WebServiceException {
        ...
        sb.append( "&bbox=" )
			.append( bbox.getMin().getX() ).append( "," )
			.append( bbox.getMin().getX() ).append( "," )
			.append( bbox.getMax().getX() ).append( "," )
