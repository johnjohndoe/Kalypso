...
public class OverviewMetadataListener extends FreeSearchListener
{
  ...
  protected void validateRequest( RPCWebEvent event ) throws CatalogClientException
  {
    ...
    bbox = GeometryFactory.createGM_Envelope( minx.doubleValue(), miny.doubleValue(), maxx
        .doubleValue(), maxy.doubleValue() );
   ...
   *              will be throwns if it is impossible to create a GetRecord
   *              request
   */
  ...
  /**
   * validates the result of the full get record request and returns an
   * <tt>Object</tt> depending on the results content.
   * 
   * @param result
   *          result to a full GetRecord request
  ...
  protected Object validateResult( HashMap result ) throws CatalogClientException
  {
    ...
    try
    {
      ...
      DetailedMetadata mdMetadata = MetadataFactory
          .createISO19115Full( (Element)nl.item( 0 ), null );
