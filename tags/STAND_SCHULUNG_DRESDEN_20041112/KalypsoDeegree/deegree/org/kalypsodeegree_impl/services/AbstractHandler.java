...
abstract public class AbstractHandler implements Handler
{
  ...
  public Map inProgressList = null;
  ...
  /**
   * @see AbstractHandler#registerHandler(Handler)
   */
   ...
   * If for minimum one Handler was found that is responsible the method returns
   * <tt>true</tt> otherwise <tt>false</tt> will be returned.
   */
  ...
  /**
   * sets time in seconds the waits for a response of a WMSService that serves a
  ...
  /**
   * sets the time in seconds the handler keeps on running the request and
   * response loops when no more requests or responses are registered to be
   * performed. A value of -1 (default) indicates that the loops will be stopped
   * immidiatly if no more requests or responses are registered to be performed.
   */
  ...
  abstract public class Loop extends Thread
  {
    ...
    /**
     * this method will be called after a loop has been stopped. As default no
     * action is performed; but the method can be overwritten by an extending
     * class to perform some clean-up actions for example.
     */
