    ...
    try
    {
      for( int i = 0; i < types.length; i++ )
      {
        if( parent.isKnownFeatureType( types[i] ) )
        {
          ...
          try
          {
            org.deegree.services.wfs.configuration.FeatureType ft = config
                .getFeatureType( types[i] );
      ...
      response[0] = WFSProtocolFactory.createWFSDescribeFeatureTypeResponse( request, at, null,
          schema );
    }
    catch( Exception e )
    {
      ...
      OGCWebServiceException exce = new OGCWebServiceException_Impl(
          "CDescribeFeatureType: performRequest", e.toString() );
