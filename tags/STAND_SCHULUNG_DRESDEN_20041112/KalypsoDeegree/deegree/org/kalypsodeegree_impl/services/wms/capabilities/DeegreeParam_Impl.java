...
public class DeegreeParam_Impl implements DeegreeParam, Marshallable
{
  ...
  DeegreeParam_Impl( int cacheSize, int maxLifeTime, int requestTimeLimit, float mapQuality,
      URL defaultOnlineResource, String rootDirectory, int maxMapWidth, int maxMapHeight,
      String copyRight, GazetteerParam gazetteer, URL schemaLocation, URL dtdLocation, Proxy proxy )
  {
  ...
  /**
   * returns the URL where to access the gazetteer service associated with the
   * WMS
   *  
   */
  ...
  public String exportAsXML()
  {
    ...
    sb
        ...
        .append( XMLTools.validateCDATA( rootDirectory ) )
        .append( "</RootDirectory>" )
        .append(
            "<DefaultOnlineResource xmlns:xlink=\"http://www.w3.org/1999/xlink\" xlink:type=\"simple\" xlink:href=\"" )
        .append( NetWorker.url2String( defaultOnlineResource ) ).append( "\"/>" ).append(
            "<CacheSize>" ).append( cacheSize ).append( "</CacheSize>" ).append( "<MaxLifeTime>" )
        .append( maxLifeTime ).append( "</MaxLifeTime>" ).append( "<RequestTimeLimit>" ).append(
            requestTimeLimit ).append( "</RequestTimeLimit>" ).append( "<MapQuality>" ).append(
            mapQuality ).append( "</MapQuality>" ).append( "<MaxMapWidth>" ).append( maxMapWidth )
        .append( "</MaxMapWidth>" ).append( "<MaxMapHeight>" ).append( maxMapHeight ).append(
            "</MaxMapHeight>" );
    if( gazetteer != null )
    {
      sb.append( "<Gazetteer>" ).append( ( (Marshallable)gazetteer ).exportAsXML() ).append(
          "</Gazetteer>" );
    }
    if( schemaLocation != null )
    {
      sb
          ...
          .append( NetWorker.url2String( schemaLocation ) ).append( "\"/>" ).append(
              "</SchemaLocation>" );
    }
    if( schemaLocation != null )
    {
