...
/**
 * A BNA geometry is a point or an ordered sequence of points. Points are stored
 * as GM_Positions. Ordered sequence can be a ring if the last point has the
 * same coords of the first one. If a ring has _bLine = false, then it's a
 * surface, else it's a polygon.
 * <p>
 * 
 * @version 2003.08.04
...
public class BNAGeometry
{
   ...
   * 
   * @param isLine
   *          a boolean telling if this is a line or a surface.
   */
  ...
  {
    return ( _points.size() > 1 ) && _bLine
        && ( (GM_Position)_points.get( 0 ) ).equals( _points.get( _points.size() - 1 ) );
  }
