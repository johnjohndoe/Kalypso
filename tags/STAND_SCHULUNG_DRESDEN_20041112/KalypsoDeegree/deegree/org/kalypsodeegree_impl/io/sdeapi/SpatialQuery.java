...
/**
 * This class handles a complete ArcSDE request: If instanciated, the class can
 * open a connection/instance of the specified ArcSDE server, set a bounding box
 * as a spatial filter to query the defined layer. The resultset of the query
 * contains the geometries as well as the tabular data associated with them. The
 * table is stored as a deegree Table object whereas the geometries are stored
 * as an array of deegree GM_Objects. Depending on the datatype of the
...
public class SpatialQuery
{
      ...
      throws SeException
  {
    ...
    GM_Envelope layerBBox = GeometryFactory.createGM_Envelope( layer.getExtent().getMinX(), layer
        .getExtent().getMinY(), layer.getExtent().getMaxX(), layer.getExtent().getMaxY() );
    ...
    if( query != null )
    {
      ...
      SeExtent extent = new SeExtent( query.getMin().getX(), query.getMin().getY(), query.getMax()
          .getX(), query.getMax().getY() );
  ...
  /**
   * Runs a spatial query against the opened layer using the specified spatial
   * filter. <br>
   * throws SeException
   */
  public Table runSpatialQuery( String[] cols ) throws SeException
  {
    ...
    if( spatialFilter != null )
    {
      ...
      else
      {
        try
        {
          deegreeTable = new Table_Impl( layer.getQualifiedName(), new String[]
          { "NONE" }, new String[]
          { "java.lang.String" }, 2 );
        }
        catch( Exception e )
  ...
  /**
   * Initialize Table object - used with first row of the SpatialQuery This
   * method sets the TableName, TableColumnNames and their DataTypes <br>
   * throws SeException
   */
  ...
  /**
   * CreateGeometry - used with every row of the SpatialQuery Depending on the
   * layers' geometries datatype different operations are made to create the
   * appropriate object. <br>
  ...
  private void createGeometry( SeShape shape, ArrayList list ) throws SeException
  {
    ...
    case SeShape.TYPE_LINE:
      ...
      for( int pt = 0; pt < numPoints; pt++ )
      {
        gmSimpleLinePosition[pt] = GeometryFactory.createGM_Position( points[pt].getX(), points[pt]
            .getY() );
      }
    ...
    case SeShape.TYPE_MULTI_LINE:
      ...
      for( int partNo = 0; partNo < numParts; partNo++ )
      {
        ...
        GM_Position[] gmMultiSimpleLinePosition = new GM_Position[shape
            .getNumPoints( partNo + 1, 1 )];
        int i = 0;
    ...
    case SeShape.TYPE_POLYGON:
      ...
      if( numSubParts > 1 )
      {
        ...
        for( int subPartNo = 1; subPartNo < numSubParts; subPartNo++ )
        {
          ...
          GM_Position[] gmPolygonPosition = new GM_Position[shape.getNumPoints( 1, subPartNo + 1 )];
          int i = 0;

          for( int pt = subPartOffset[subPartNo]; pt < lastPoint; pt++ )
          {
            gmPolygonPosition[i] = GeometryFactory.createGM_Position( points[pt].getX(), points[pt]
                .getY() );
            i++;
          }
      ...
      try
      {
        GM_Surface gmSurface = GeometryFactory.createGM_Surface( gmPolygonExteriorRing,
            gmPolygonInteriorRings, new GM_SurfaceInterpolation_Impl(), null );
        list.add( gmSurface );
      }
      catch( GM_Exception gme )
      {
    ...
    case SeShape.TYPE_MULTI_POLYGON:
      ...
      {
        GM_Position[] gmMultiPolygonExteriorRing = new GM_Position[shape.getNumPoints( partNo + 1,
            1 )];
        ...
        {
          partEnd = subPartOffset[partOffset[partNo + 1]]; //Otherwise scan to
          // the offset of next
          // part
        }
        ...
        for( ; ( pointNo < points.length ) && ( pointNo < partEnd ); pointNo++ )
        {
          ...
          if( exterior )
          {
            gmMultiPolygonExteriorRing[i] = GeometryFactory.createGM_Position( points[pointNo]
                .getX(), points[pointNo].getY() );
          ...
          else
          {
            ...
            // assign the size of the second dimension of
            ...
            gmMultiPolygonInteriorRings[subPartIndex][i] = GeometryFactory.createGM_Position(
                points[pointNo].getX(), points[pointNo].getY() );
            i++;
          }
        } // End for
        ...
        try
        {
          gmMultiPolygonSurface[partNo] = GeometryFactory.createGM_Surface(
              gmMultiPolygonExteriorRing, gmMultiPolygonInteriorRings,
      ...
      try
      {
        GM_MultiSurface gmMultiSurface = GeometryFactory
            .createGM_MultiSurface( gmMultiPolygonSurface );
