...
{
  /**
   * The valid coordinate range of a grid coverage. The lowest valid grid coordinate is zero. A grid with 512 cells can
   * have a minimum coordinate of 0 and maximum of 512, with 511 as the highest valid index.
   */
  GC_GridRange getGridRange();

  /**
   * The math transform allows for the transformations from grid coordinates to real world earth coordinates. The
   * transform is often an affine transformation. The coordinate system of the real world coordinates is given by the
   * {@link org.opengis.cv.CV_Coverage#getCoordinateSystem}method. If no math transform is given, this attribute will
   * be <code>null</code>.
   */
