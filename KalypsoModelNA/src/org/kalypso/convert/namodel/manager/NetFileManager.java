...
public class NetFileManager extends AbstractManager
{
  ...
  private void readNodeList( final LineNumberReader reader ) throws Exception
  {
    ...
    while( (line = reader.readLine()) != null )
    {
      ...
      final Set set = propCollector.entrySet();
    ...
    for( final Feature node : nodeFEs )
    {
      ...
      if( branchingFE != null )
      {
        ...
        if( branchingFT == kontEntnahmeFT || branchingFT == ueberlaufFT || branchingFT == verzweigungFT )
        {
          ...
          {
            final String message = String.format( "Inconsistent net: Node '%s' with branch has no upstream channel.", node.getName() );
            throw new SimulationException( message );
          }

          if( downStreamChannelFE == null )
          {
            final String message = String.format( "Inconsistent net: Node '%s' with branch has no downstream channel.", node.getName() );
            throw new SimulationException( message );
          }
  ...
  public void writeFile( final AsciiBuffer asciiBuffer, final GMLWorkspace workspace, final GMLWorkspace synthNWorkspace ) throws Exception
  {
    ...
    for( final Iterator iter = rootNetElements.iterator(); iter.hasNext(); )
    {
  ...
  public void appendNodeList( final GMLWorkspace workspace, final List nodeCollector, final AsciiBuffer asciiBuffer ) throws Exception, Exception
  {
    final IFeatureType kontEntnahmeFT = workspace.getGMLSchema().getFeatureType( NaModelConstants.NODE_VERZW_ENTNAHME );
