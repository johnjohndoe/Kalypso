...
public class UpdateSimulationWorker
{
  ...
  public IStatus execute( IProgressMonitor monitor ) throws CoreException
  {
    ...
    try
    {
      ...
      final CatchmentModelRunner catchmentModelRunner = new CatchmentModelRunner();
      catchmentModelRunner.executeCatchmentModel( m_simulation, control, model, control.getGeneratorN(), Catchment.PROP_PRECIPITATION_LINK, ITimeseriesConstants.TYPE_RAINFALL, new SubProgressMonitor( monitor, 20 ) );
      catchmentModelRunner.executeCatchmentModel( m_simulation, control, model, control.getGeneratorT(), Catchment.PROP_TEMPERATURE_LINK, ITimeseriesConstants.TYPE_MEAN_TEMPERATURE, new SubProgressMonitor( monitor, 20 ) );
      catchmentModelRunner.executeCatchmentModel( m_simulation, control, model, control.getGeneratorE(), Catchment.PROP_EVAPORATION_LINK, ITimeseriesConstants.TYPE_MEAN_EVAPORATION, new SubProgressMonitor( monitor, 20 ) );
