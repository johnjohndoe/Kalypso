 ...
 */
public class TimeseriesChartJob extends Job
{
  private final URL m_template;

  private final IChartModel m_model;

  public TimeseriesChartJob( final URL template, final IChartModel model )
  {
    super( "TimeseriesChartJob" ); //$NON-NLS-1$
  ...
  protected IStatus run( final IProgressMonitor monitor )
  {
    ...
    {
      return new Status( IStatus.ERROR, KalypsoUIRRMPlugin.getID(), ex.getLocalizedMessage(), ex );
    }
    finally
    {
      monitor.done();
  ...
  private void init( )
  {
    try
    {
      final ChartTypeHandler handler = new ChartTypeHandler( m_template );
