...
public class NaModelHelper
{
  ...
  public static int addRHBinCatchment( CommandableWorkspace modelworkspace, Feature catchment, FeatureType rhbFT,
      Feature measureRhbFE )
  {
    ...
    try
    {
      ...
      final double maxVol = depth.doubleValue() / 3 * m_factorHecto
          * ( Math.pow( lo, 2d ) + Math.pow( lu, 2d ) + Math.sqrt( Math.pow( lo, 2d ) * Math.pow( lu, 2d ) ) );
      final ZmlInlineTypeHandler typeHandler = (ZmlInlineTypeHandler)MarshallingTypeRegistrySingleton.getTypeRegistry()
          .getTypeHandlerForTypeName( "inline.zml.kalypso.org:ZmlInlineWVQType" );
      ...
      IObservation obs = new SimpleObservation( null, null, "RHB WVQ-Bezeihung", true, null, new MetadataList(), axis,
          model );
      ...
      propteries.put( FeatureFactory.createFeatureTypeProperty( STORAGE_CHANNEL_SV_PROP, NaModelConstants.NS_NAMODELL,
          Double.class.getName(), true, null ), new Double( 0 ) );
      //    get linked features
      ...
      int posNode = 0;
      final FeatureType nodeFT = modelworkspace.getFeatureType( NODE_ELEMENT_NAME );
      ...
      // Insert new links
      final AddRelationCommand addStorgeChannelNodeLink = new AddRelationCommand( modelworkspace, channels[posChannel],
          LINK_CHANNEL_DOWNSTREAMNODE, 0, downStreamNode );
  ...
  private static Double getVolume( double waterlevel, double lenght, double slope, double depth, double factorHecto )
  {
    double value = ( 4
        / 3
        * Math.pow( slope, 2d )
        * ( Math.pow( waterlevel, 2d ) - 3 * depth * Math.pow( waterlevel, 2d ) + 3 * Math.pow( depth, 2d )
            * waterlevel ) + 2 * slope * lenght * ( Math.pow( waterlevel, 2d ) - 2 * depth * waterlevel ) + Math.pow(
        lenght, 2d )
        * waterlevel )
        * factorHecto;
