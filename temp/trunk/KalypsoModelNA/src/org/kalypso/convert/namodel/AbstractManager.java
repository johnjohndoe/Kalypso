...
public abstract class AbstractManager
{
    ...
    public void createProperties(HashMap propCollector, String line,
            int formatLine) throws Exception
    {
        createProperties(propCollector, line, m_asciiFormat[formatLine]);
    }

    public void createProperties(HashMap propCollector, String line,
            String formatLine) throws Exception
    {
        line = line + "                                                ";
        ...
        } else
        {
            for (int i = 0; i < m.groupCount(); i++)
            {
                ...
                propCollector.put(name, FeatureFactory.createFeatureProperty(
                        name, value));
            }
        }
    }
    ...
    private String getPairRegExp(String fPair, List nameCollector)
    {
        ...
        if (m.matches())
        {
            ...
            return regExpChar + "{1," + (max - decimal - 1) + "}"
                    + decimalPoint + regExpChar + "{" + (decimal) + "}";
        }
        throw new UnsupportedOperationException();
    }
...
//        System.out.println(s[0]);
        ...
        if (m.matches())
        {
            ...
            value = value.trim();
            return (result.replace(max - value.length(), max, value))
                    .toString();
        }
        throw new UnsupportedOperationException();
        ...
        while (it.hasNext())
        {
            ...
            if (ft.getProperty(feProp.getName()) != null)
                ...
                System.out.println("property does not exist: >"
                        + feProp.getName() + "=" + feProp.getValue() + "<");
        }
    }

    public static String createFormatLine(String name, String format,
