...
public class KrigingReader
{
  ...
  private final Pattern BLOCK = Pattern.compile( ".*BLOCK:.+?(" + doublePattern + ").+?("
      + doublePattern + ").+" );
  ...
  public AbstractFilterType createFilter( final Feature feature, final String geoPropName )
  {
    ...
    if( elements.isEmpty() )
      throw new InvalidParameterException( "Raster ist zu grob, Keine zuordnung fuer "
  ...
  private AbstractFilterType createFilter( List krigingElements )
  {
    ...
    System.out.println( krigingElements.size() + " rasterpoints are withing geometry. (min is"
        + m_min + ")" );
    ...
    try
    {
      NOperationFilter nOperationFilter = filterFac.createNOperationFilter();
      ...
      for( Iterator iter = map.values().iterator(); iter.hasNext(); )
      {
        ...
        //                type.setHref(WeisseElsterConstants.PREFIX_LINK_OMBROMETER_Niederschlag
        // + rel.getId());
  ...
  public List parse( Reader reader )
  {
    ...
    try
    {
      final CS_CoordinateSystem srs = ConvenienceCSFactory.getInstance().getOGCCSByName(
          "EPSG:31467" );
      ...
      while( ( line = lineReader.readLine() ) != null )
      {
        ...
        if( m1.matches() )
        {
          ...
          e = new KrigingElement( GeometryFactory.createGM_Point( x, y, srs ) );
          result.add( e );
        }
        else if( m2.matches() )
        {
  ...
  public VirtualRepository createRepositoryConf( Feature[] features, String geoPropName )
