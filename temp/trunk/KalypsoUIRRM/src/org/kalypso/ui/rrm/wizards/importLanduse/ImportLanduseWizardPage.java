...
public class ImportLanduseWizardPage extends WizardPage implements SelectionListener, ModifyListener
{
  ...
  private Combo cmbLanduseProperty;

  private Combo cmbSealingFactorProperty;

  private Combo cmbDrainageTypeProperty;
  ...
  public void createControl( final Composite parent )
  {
    ...
    new Label( container, SWT.NONE ).setText( Messages.getString( "ImportLanduseWizardPage.14" ) ); //$NON-NLS-1$
    cmbSealingFactorProperty = new Combo( container, SWT.BORDER | SWT.READ_ONLY );
    final GridData gd1 = new GridData();
    gd1.horizontalAlignment = GridData.FILL;
    gd1.widthHint = 75;
    gd1.horizontalSpan = 2;
    cmbSealingFactorProperty.setEnabled( false );
    cmbSealingFactorProperty.setLayoutData( gd1 );

    // drainage type property combo box
    new Label( container, SWT.NONE ).setText( Messages.getString( "ImportLanduseWizardPage.15" ) ); //$NON-NLS-1$
    cmbDrainageTypeProperty = new Combo( container, SWT.BORDER | SWT.READ_ONLY );
    final GridData gd2 = new GridData();
    gd2.horizontalAlignment = GridData.FILL;
    gd2.widthHint = 75;
    gd2.horizontalSpan = 2;
    cmbDrainageTypeProperty.setEnabled( false );
    cmbDrainageTypeProperty.setLayoutData( gd2 );
  ...
  private void addListeners( )
  {
    ...
    cmbLanduseProperty.addSelectionListener( this );
    cmbSealingFactorProperty.addSelectionListener( this );
    cmbDrainageTypeProperty.addSelectionListener( this );
      ...
      try
      {
        if( isTextNonEmpty( txt_InputFile ) )
        {
          ...
          if( file.exists() && file.isFile() && file.canRead() )
          {
            ...
            shape.close();
            cmbLanduseProperty.setItems( propertyNames );
            cmbLanduseProperty.select( 0 );
            cmbLanduseProperty.setEnabled( true );
            cmbSealingFactorProperty.setItems( propertyNames );
            cmbSealingFactorProperty.select( 0 );
            cmbSealingFactorProperty.setEnabled( true );
            cmbDrainageTypeProperty.setItems( propertyNames );
            cmbDrainageTypeProperty.select( 0 );
            cmbDrainageTypeProperty.setEnabled( true );
          }
          else
          {
            cmbLanduseProperty.setEnabled( false );
  ...
  {
    return cmbLanduseProperty.getText();
  }

  public String getSealingFactorProperty( )
  {
    return cmbSealingFactorProperty.getText();
  }

  public String getDrainageTypeProperty( )
  {
    return cmbDrainageTypeProperty.getText();
  }
