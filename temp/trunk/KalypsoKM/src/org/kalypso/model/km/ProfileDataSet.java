  ...
  private AbstractKMValue getBankFull( final AbstractKMValue[] kmValues ) throws Exception
  {
    ...
    for( int i = 0; i < kmValues.length; i++ )
    {
      ...
      if( km.getAlpha() < 1 && i == 0 )
        throw new Exception( "Kein Bordvoller Abfluss für den Strang vorhanden, minimaler Abfluss übersteigt den bordvollen Abfluss." );
      else if( km.getAlpha() < 1 && i < 2 )
        throw new Exception( "Nicht genug Abflüsse unter dem bordvollen Abfluss für den Strang vorhanden (bordvoller Abfluss ist erster oder zweiter ermittelter Abfluss im Strang)." );
      else if( km.getAlpha() < 1 && i > 0 )
        return kmValues[i - 1];
    }
    throw new Exception( "Bordvoller Abfluss konnte für den Strang nicht korrekt ermittelt wrden. Überprüfe Profile oder der Bordvolle Abfluss wird im Strang nie erreicht." );
  }
