...
public class DWDRasterGeoLayer
{
  ...
  {
    double mx = m_xRaster.getValueAt( pos ) / DWDRaster.SCALE;
    double my = m_yRaster.getValueAt( pos ) / DWDRaster.SCALE;
  ...
  class UpdateFeatureSurfaceGeometry implements FeatureVisitor
  {
    public boolean visit( Feature f )
    {
      ...
      final FindSomeNearestVisitor visitor = new FindSomeNearestVisitor( centerPos, Double.MAX_VALUE, 9, GEO_PROP_POINT );
      ...
      for( int i = 0; i < result.length; i++ )
      {
        ...
        final GM_Position newPos = GeometryUtilities.createGM_PositionAverage( new GM_Position[]
        {
            centerPos,
            p2 } );
        cellEnv = cellEnv.getMerged( newPos );
      }
      final GM_Object geom;
      try
      {
        geom = GeometryFactory.createGM_Surface( cellEnv, point.getCoordinateSystem() );
        FeatureProperty property = FeatureFactory.createFeatureProperty( GEO_PROP_SURFACE, geom );
