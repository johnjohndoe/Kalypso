...
!***********************************************************************************************
      ...
      subroutine s0rc(text0,number,lt)
!c
!c     s$rc konvertiert eine real-groesze in eine asci-string
!c     dabei werden nur lt-werte hinter dem komma in den string uebernommen
!c     die lt-variable nimmt folgende werte an:
!c
!c        bei eintritt in die subroutine: lt = anzahl werte hinter kommo
!c
!c        beim verlassen der  subroutine: lt = laenge des strings 'text0'
!c                                             in anzahl character
!c
      character*80 text0
      real    number
!c
!c     konvertieren der real-groesze in einen asci-string
!c
      write(text0,'(f15.6)') number
      call u0ljst(text0)
      ipunkt=index(text0,'.')
      if (ipunkt.ne.0)goto 20
      write(1,1000)
1000  format(/'variabel keine real-groesze(upg s$rc)!!')
!      call writeFooter()
      stop 1010
 20   if(lt.eq.0)then
          lt=ipunkt-1
