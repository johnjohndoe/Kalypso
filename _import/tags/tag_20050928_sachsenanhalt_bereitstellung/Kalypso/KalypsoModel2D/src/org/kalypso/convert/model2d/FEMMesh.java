...
public class FEMMesh
{
  ...
  /**
   * gets the properties of the <MeshPolygon>and separates it in the parameters necessary for the ascii file of the
   * simulation programm
   * 
   * @param ws
   * @param rootFeature
  ...
  public void getMeshProperties( GMLWorkspace ws, Feature rootFeature )
  {
    ...
    for( int i = 0; i < meshFEs.size(); i++ )
    {
      try
      {
        ...
        GM_Surface geomProperty = (GM_Surface)meshFE.getProperty( "polygonProperty" );

        GM_SurfaceBoundary surfaceBoundary = geomProperty.getSurfaceBoundary();
  ...
  private void getEdgesOfPolygon( int idFE, GM_Position[] position )
  {
    ...
    if( nodeIdOfPoly.length == 5 )
    {
      ...
      System.out.println( "MeshPolygon " + idFE + " is not consisting of three or four edges" );
  ...
  private void addFEParameter( int k, Feature meshFE )
  {
    ...
    String fe = k + "  " + roughOld + "  " + roughInTS + "  " + order + "  " + dm + "\n";
    feList.add( fe );
   ...
   * @param dm
   */
  private StringWriter setFEStringWriter( int k, String roughOld, String roughInTS, String order, String dm )
  {
