...
public class ArraysTest extends TestCase
{
    ...
    {
        double[] d = { 1.002, 1.002, 2.3, 4.5, 6.7, 5.000333, 5.000334, 5.000336, 8 };

        assertEquals( d.length, 9 );

        double[] d1 = Arrays.removeDupicates( d, .0000000001 );
        System.out.println( "Remove dup1:" + Arrays.dump( d1 ) );
        assertEquals( d1.length, 8 );

        double[] d2 = Arrays.removeDupicates( d, .0001 );
        System.out.println( "Remove dup2:" + Arrays.dump( d2 ) );
        assertEquals( d2.length, 6 );

        double[] d3 = Arrays.removeDupicates( d, .0000005 );
    ...
    public void testImplode()
    {
    	...
    	String s1 = Arrays.implode(splits, ".", 0, 8);
    	
    	assertEquals( s1, test );
    	
		String s2 = Arrays.implode(splits, ".", 2, 7);
		
		assertEquals( s2, "3.4.5.6.7.8" );
		
		String s3 = Arrays.implode(splits, "", 0, 8);
