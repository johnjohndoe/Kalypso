...
public class InvariantCheckBoundaryConditions implements ICalculationValidateInterface
{
  ...
  public List<IProblem> checkAllInvariants( final ICalculationUnit calculationUnit )
  {
    ...
    {
      final String message = String.format(Messages.getString("org.kalypso.kalypsomodel1d2d.ui.calculationUnitView.invariants.InvariantCheckBoundaryConditions.0"), calculationUnit.getName()); //$NON-NLS-1$
      invResults.add( new ProblemDescriptor( null, message, calculationUnit, calculationUnit ) );
    }
    else if( size == 0 )
    {
      final String message = String.format( Messages.getString("org.kalypso.kalypsomodel1d2d.ui.calculationUnitView.invariants.InvariantCheckBoundaryConditions.1"), calculationUnit.getName()); //$NON-NLS-1$
      invResults.add( new ProblemDescriptor( null, message, calculationUnit, calculationUnit ) );
    }
