...
// Timestamp to = Timestamp.valueOf( "2003-06-01 00:00:00" );
      ...
      final KiWWDataProviderRMIf myServerObject = (KiWWDataProviderRMIf) Naming.lookup( rmiUrl );
      ...
      final HashMap auth = myServerObject.getUserAuthorisation( domain, "em", "123abc", "mydomain.kisters.de", null );
  ...
  private static void wqTest( final KiWWDataProviderRMIf myServerObject, final HashMap auth )
  {
    try
    {
      ...
      final HashMap ratingTables = myServerObject.getRatingTables( auth, KiWWDataProviderInterface.OBJECT_TIMESERIES, new Long[] { new Long( 1024002488 ) }, ts );
...
// }
  ...
  public static void gettsDatafromHash( final HashMap map, final String tsid )
  {
    final HashMap timeseries = (HashMap) map.get( KiWWDataProviderInterface.KEY_TIMESERIES );
    final HashMap ts_id = (HashMap) timeseries.get( tsid );

    final LinkedList tsdata = (LinkedList) ts_id.get( KiWWDataProviderInterface.KEY_TSDATA );
    int i = 0;
