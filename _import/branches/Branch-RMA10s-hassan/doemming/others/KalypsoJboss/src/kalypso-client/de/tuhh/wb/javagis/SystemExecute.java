...
public class SystemExecute extends Thread
{
    ...
    public SystemExecute(String command,File workingDir)
    {
	super(command);
	this.myCommand=command;
	this.myWorkingDir=workingDir;

	this.myCommand="endlos.exe";
	this.myWorkingDir=new File("/data");
    }
	...
	try
	    {
		String commandLine=myWorkingDir.getPath()+System.getProperty("file.separator")+myCommand;
		System.out.println("EXECUTE-Dir: "+myWorkingDir.getPath());
		...
		try
		    {
			Process process=(Runtime.getRuntime()).exec(commandLine,dummy,myWorkingDir);
