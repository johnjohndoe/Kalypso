...
public class GisNetModel
{
    ...
    private Vector myObjectIdListVector;
    ...
    public Image getBufferedMap(java.awt.Component component, Transformation trafo,int size)
    {
	...
	for(int i=0;i<myGisRelationClasses.size();i++)
		...
		for(int n=0;n<idList.size();n++)
			...
			g.drawLine((int)spSrc.getX(),(int)spSrc.getY(),(int)spDest.getX(),(int)spDest.getY());
			...
			g.drawString("#"+rId.toString(),(int)cx,(int)cy);
		    }
	    }
	...
	for(int i=0;i<myGisObjectClasses.size();i++)
		...
		for(int n=0;n<idList.size();n++)
			...
			g.drawImage(symbol,(int)sp.getX()-xOffset,(int)sp.getY()-yOffset,null);
		    }
	    }
	return mapBuffer;
    }
