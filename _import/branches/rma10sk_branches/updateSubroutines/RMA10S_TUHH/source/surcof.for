cipk  last update may 23 2005 fix wind direction bug
...
CIPK  LAST UPDATE JAN 21 2002 REVISE TO CREATE VELOCITY COMPONENTS 
cipk  last update oct 10 2001 reformualte to improve integration for type 3 and 4 elements
CIPK  LAST UPDATE MAR 26 2001  ADD SIDF FOR GLOBAL LOADING AS SURFACE FLOW
...
C     Last change:  WP   22 Jul 2008    5:03 pm
...
C      DXS=0.
C      DYS=0.
CIPK OCT01 CHANGE TO CATCH K=1 CASE
      DO 100 K = 1, NCN
cipk oct98 update to f90
        N=NOP(NN,K)
...
C-
  ...
  145 CONTINUE
C-
C.....COMPUTE U, V, H AND THEIR DERIVATIVES.....
C-
CIPK AUG95 ADD LOGIC TO GET SAL/TEMP/SED
      U = 0.0
      V = 0.0
      W=0.
      SALT=0.
...
cipk oct98 update to f90
      MR=NOP(NN,M)
      W=W+XN(M)*VVEL(MR)
CIPK JAN02 CHANGE TO VELOCITY COMPONENTS
...
CIPK JAN02      U=U+XN(M)*VEL(1,MR)
CIPK JAN02      V=V+XN(M)*VEL(2,MR)
CIPK JAN02      SALT=SALT+XN(M)*VEL(ICK,MR)
CIPK JAN02      DUDX=DUDX+DNX(M)*VEL(1,MR)
CIPK JAN02      DUDY=DUDY+DNY(M)*VEL(1,MR)
CIPK JAN02      DVDX=DVDX+DNX(M)*VEL(2,MR)
CIPK JAN02      DVDY=DVDY+DNY(M)*VEL(2,MR)
  270 CONTINUE
      H = 0.0
      DHDT=0.
      SPC=0.
...
cipk may05 fix wind direction bug
c        SIGMAX=SIGMAX+XM(M)*((SIGMA(MR,1)+stress(mr,1))*CX
c     +                        +(SIGMA(MR,2)+stress(mr,2))*SAN)
c        SIGMAY=SIGMAY+XM(M)*(-(SIGMA(MR,1)+stress(mr,1))*SAN
c     +                        +(SIGMA(MR,2)+stress(mr,2))*CX)
...
C      PD=P-RHO*GRAV*ZA*H
  ...
  350 CONTINUE
      GO TO 500
C-
C      PROCESS TYPE 1(SURFACE) ELEMENTS
C-
  400 IA=1-NDF
      DO 430 N=1,NCN
      IA=IA+NDF
  ...
  447 CONTINUE
CIPK MAR01  ADD SIDF FOR GLOBAL LOADING AS SURFACE FLOW
      F(IA)=F(IA)-XM(M)*AMW*(XHT*(DHDX*U+DHDY*V+DHDT)/XHT - SIDFQ)
CIPK MAY04 USE SIDFQ
  450 CONTINUE
...
cipk aug98 divide by xht in lines above
      ENDDO
CIPK AUG95 END ADDITIONS
  500 CONTINUE
  525 N1=2
      N1A=1
  550 CONTINUE
...
c      PDN(1)=PRESS(N1)-DEN(N1)*GRAV*VEL(3,N1)*(ELEV-CORD(N1,3))/
c     +         (ELEV-AO(N1))
c      PDN(3)=PRESS(N3)-DEN(N3)*GRAV*VEL(3,N3)*(ELEV-CORD(N3,3))/
c     +         (ELEV-AO(N3))
      DO 750 N=1,4
      XM(1)=1.-AFACT(N)
      XM(2)=AFACT(N)
...
c      ZC=XN(1)*CORD(N1,3)+XN(2)*CORD(N2,3)+XN(3)*CORD(N3,3)
C      RHO=1000.-ZC
C      RHOS=1000.
...
CIPK AUG02 TEST FOR SHALLOW OR NEGATIVE DEPTH TO SET STRESS TO ZERO.
  ...
  715 CONTINUE
C-
C......Surface elements apply boundary conditions for continuity equation
C-
      IA=3-2*NDF
      DO 725 M=1,2
      IA=IA+NDF*2
  ...
  723 CONTINUE
c      F(IA)=F(IA)-XM(M)*AMW*DHDX*U
C
CIPK MAR01  ADD SIDF FOR GLOBAL LOADING AS SURFACE FLOW USE SIDFQ MAY04
...
CIPK AUG95 END ADDITIONS
      GO TO 750
C
C     End elements apply boundary pressures
C
  730 CONTINUE
      Z=(1.-AFACT(N))*CORD(N1,3)+AFACT(N)*CORD(N3,3)
c      ZA=(ELEV-Z)/XHT
      SPC=(1.-AFACT(N))*SPEC(N1,3)+AFACT(N)*SPEC(N3,3)
c      P=(1.-AFACT(N))*PRESS(N1)+AFACT(N)*PRESS(N3)
      H=VEL(3,N1)
c      PD=(1.-AFACT(N))*PDN(1)+AFACT(N)*PDN(3)
C      PD=P-RHO*GRAV*ZA*H
...
c
          H=XM(1)*VEL(3,N1)+XM(2)*VEL(3,N3)
c          P=XM(1)*PRESS(N1)+XM(2)*PRESS(N3)
c          PD=(1.-AFACT(N))*PDN(1)+AFACT(N)*PDN(3)

c          ZC=XN(1)*CORD(N1,3)+XN(2)*CORD(N2,3)+XN(3)*CORD(N3,3)
C          P=GRAV*ZC*(ZC/2.-1000.)
C          PD=GRAV*ZC**2/2.
C          RHO=1000.-ZC
C          RHOS=1000.
c          PD=P-RHO*GRAV*ZA*H
...
cipk oct98 update to f90
      N1=NOP(NN,N)
CIPK OCT01  ADD AGD TRANSFORMATION
 ...
 1001 CONTINUE
C-
C......APPLY VERTCAL SHAPE FUNCTION FACTORS
C-
      JA=1-NDF
      DO 1010 JJ=1,NCN
      J=NOP(NN,JJ)
