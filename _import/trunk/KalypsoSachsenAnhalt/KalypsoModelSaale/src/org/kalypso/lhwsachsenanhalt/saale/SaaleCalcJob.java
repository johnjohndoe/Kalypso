...
public class SaaleCalcJob implements ICalcJob
{
  ...
  private File createInputFiles( final File tmpdir, final ICalcDataProvider inputProvider ) throws CalcJobServiceException, Exception
  {
...
//    new File( hwvordir, "WQ" ).mkdir();
  ...
  private void writeStammdaten( final File hwvordir, final GMLWorkspace modellWorkspace ) throws IOException, JAXBException, GmlConvertException
  {
    ...
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/1_pegel_std.gmc" ), new UrlUtilities(), stammdatURL, externData );
    
    m_logger.info( "Schreibe: Stammdat/met_geb.std" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/2_met_geb_std.gmc" ), new UrlUtilities(), stammdatURL, externData );
    
    m_logger.info( "Schreibe: Stammdat/egmpar.hwp" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/3_egmpar_hwp.gmc" ), new UrlUtilities(), stammdatURL, externData );
    
    m_logger.info( "Schreibe: Stammdat/tl.std" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/4_tl_std.gmc" ), new UrlUtilities(), stammdatURL, externData );
    
    m_logger.info( "Schreibe: Stammdat/ts.std" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/5_ts_std.gmc" ), new UrlUtilities(), stammdatURL, externData );
    
    m_logger.info( "Schreibe: Stammdat/wlmpar.hwp" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/6_wlmpar_hwp.gmc" ), new UrlUtilities(), stammdatURL, externData );

    m_logger.info( "Schreibe: Stammdat/hwsteu.stv" );
    GmlConvertFactory.convertXml( getClass().getResource( "resources/gml2hwvor/7_hwsteu_stv.gmc" ), new UrlUtilities(), stammdatURL, externData );
  }
  
  private void writeDaten( final File datendir, final GMLWorkspace modellWorkspace, final URL context ) throws IOException
  {
    final IUrlResolver resolver = new UrlUtilities();
    
    writeObservations( modellWorkspace, "Durchfluß", "PegelCollectionAssociation/PegelMember", new File( datendir, "Q_dat.vor" ), TimeserieConstants.TYPE_RUNOFF, resolver, context );
    writeObservations( modellWorkspace, "Niederschlag", "PegelCollectionAssociation/PegelMember[Gebiet]", new File( datendir, "P_dat.vor" ), TimeserieConstants.TYPE_RAINFALL, resolver, context );
    writeObservations( modellWorkspace, "Schnee", "PegelCollectionAssociation/PegelMember[Gebiet]", new File( datendir, "SNDAT.vor" ), TimeserieConstants.TYPE_RAINFALL, resolver, context );
    writeObservations( modellWorkspace, "Temperatur", "TempCollectionAssociation/TempMember", new File( datendir, "TLDAT.vor" ), TimeserieConstants.TYPE_TEMPERATURE, resolver, context );
    writeObservations( modellWorkspace, "Inhalt", "SpeicherCollectionAssociation/SpeicherMember", new File( datendir, "Tsdat.vor" ), TimeserieConstants.TYPE_VOLUME, resolver, context );
  }

  private void writeObservations( final GMLWorkspace workspace, final String linkProperty, final String featurePath, final File file, final String axisType, final IUrlResolver resolver, final URL context ) throws IOException
  {
  ...
  private File writeResults( final File tmpdir, final File datendir ) throws CalcJobServiceException
  {
    ...
    try
    {
      new HWVORBatch().convert( new String[] { datendir.getAbsolutePath(), resultdir.getAbsolutePath() } );
