...
public class VersionDialog
	...
	implements ActionListener, VersionListener, InternalFrameListener {
	...
	private void initMask() {
		switch (myStatus) {
			...
			case COPY :
				jProject.setText(
					myVersionAccess.getProjectName(myThemeKey, myVersionId));
				jTheme.addItem(myThemeKey);
				jState.setText(
					myVersionAccess.getState(myThemeKey, myVersionId));
				jName.setText(myVersionAccess.getName(myThemeKey, myVersionId));
				jDescription.setText(
					myVersionAccess.getDescription(myThemeKey, myVersionId));
				break;
			default :
				break;
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("ok".equals(command)) {
			final String project = jProject.getText();
			final String themeKey = (String) jTheme.getSelectedItem();
			final String state = jState.getText();
			final String name = jName.getText();
			final String description = jDescription.getText();

			final Component gp = Main.viewManager.getGlassPane();

			switch (myStatus) {
				case CREATE :
					Thread t_create = new Thread(new Runnable() {
						public void run() {
							try {
								System.out.println("create Version");
								myVersionAccess.createVersion(
									project,
									themeKey,
									state,
									name,
					...
					t_create.start();
					WaitingDialog waitDialog_create =
						WaitingDialog.getInstance();
					gp.setVisible(true);
					dispose();
					break;
				case RENAME :
					Thread t_rename = new Thread(new Runnable() {
						public void run() {
							try {
								System.out.println("rename Version");
								myVersionAccess.renameVersion(
									myThemeKey,
									myVersionId,
									project,
									state,
					...
					t_rename.start();
					WaitingDialog waitDialog_rename =
						WaitingDialog.getInstance();
					gp.setVisible(true);
					dispose();
					break;
				case COPY :
					...
					Thread t_copy = new Thread(new Runnable() {
						public void run() {
							try {
								System.out.println("copy Version");
								myVersionAccess.copyVersion(
									myThemeKey,
									myVersionId,
									project,
									state,
					...
					t_copy.start();
					WaitingDialog waitDialog_copy = WaitingDialog.getInstance();
					gp.setVisible(true);
