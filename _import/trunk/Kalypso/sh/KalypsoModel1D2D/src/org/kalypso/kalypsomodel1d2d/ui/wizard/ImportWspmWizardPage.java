...
public class ImportWspmWizardPage extends ListSelectionWizardPage
{
  ...
  public void setCalculation( final TuhhCalculation calculation )
  {
    ...
    final List< ? > reaches = (List< ? >) m_calculation.getReach().getFeature().getProperty( TuhhReach.QNAME_PROP_REACHSEGMENTMEMBER );
    setInput( reaches );

    setCheckedElements( reaches.toArray( new Object[reaches.size()] ) );

    setStatus( StatusUtilities.createStatus( IStatus.INFO, "Gew‰hlte Berechnungsvariante: " + calculation.getName(), null ) );
  }
