...
public class FeatureWrapperCollection<FWCls extends IFeatureWrapper2> extends AbstractFeatureBinder implements IFeatureWrapperCollection<FWCls>
{
   ...
   */
  public FeatureWrapperCollection( Feature featureCol, Class<FWCls> fwClass, QName featureMemberProp )
  {
    ...
    Assert.throwIAEOnNull( featureCol, "Parameter featureCol must not be null" );

    Assert.throwIAEOnNull( featureMemberProp, "Parameter featureMemberProp must not be null" );
  ...
  @SuppressWarnings("unchecked")
  public FeatureWrapperCollection( Feature parentFeature, QName childQName, QName featureMemberProp, Class<FWCls> fwClass ) throws IllegalArgumentException
  {
    super( createSubfeature( parentFeature, childQName, featureMemberProp ), childQName );

    Assert.throwIAEOnNull( parentFeature, "Parameter parentFeature must not be null" );
    Assert.throwIAEOnNull( childQName, "Parameter childQName must not be null" );
  ...
  private static Feature createSubfeature( Feature parentFeature, QName childQName, QName featureMemberProp )
  {
    try
    {
  ...
  public <T extends FWCls> T addNew( final QName newChildType, final Class<T> classToAdapt )
  {
    ...
    try
    {
      feature = Util.createFeatureForListProp( featureList, featureMemberProp, newChildType );
  ...
  public <T extends FWCls> T addNew( final QName newChildType, final String newFeatureId, final Class<T> classToAdapt )
  {
    ...
    try
    {
      ...
      feature = Util.createFeatureWithId( newChildType,// newFeatureQName,
  ...
  public <T extends FWCls> T addNew( final int index, final QName newChildType, final Class<T> classToAdapt )
  {
    ...
    try
    {
      final Feature feature = Util.createFeatureForListProp( featureList, featureMemberProp, newChildType );
      final T wrapper = (T) feature.getAdapter( classToAdapt );
      if( wrapper == null )
      {
        throw new IllegalArgumentException( "Feature not adaptable:" + "\n\tfeatureType=" + newChildType + "\n\tadapatble type=" + fwClass );
...
// IFeatureWrapper2 wrapper = (IFeatureWrapper2) f.getAdapter(fwClass);
    FWCls wrapper = FeatureHelper.getFeature( featureCol.getWorkspace(), featureList.remove( index ), fwClass );
    return wrapper;
  }
