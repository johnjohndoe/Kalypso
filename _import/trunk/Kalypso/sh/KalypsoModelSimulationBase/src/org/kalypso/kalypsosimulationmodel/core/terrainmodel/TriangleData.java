...
class TriangleData implements SurfacePatchVisitable
{
  ...
  /**
   * Given 3 coordinate this methode return the equation of a plan
   * containing those points.
   * The return equation as the form:
   *  z = Q*x+P*y+O
   *  The coefficients Q, P amd O are return as array
   *  
   *  @param  coords coordinate of  3 plane points 
   *  @return the cooeficients of the plane equation z = Q*x+P*y+O
   *            as array of double {Q,P,O} 
   * 
   */
  public static final double[] calculateTrianglePlaneEquation(Coordinate[] coords)
  {
    ...
    if(coords.length<3)
    {
      throw new IllegalArgumentException(
          "Param coord which represent the point of a triangle must "+
          "have a minimum length of 3: current length="+
          coords.length);
    }
...
//      C=-C;
  ...
  public void aceptSurfacePatches( GM_Envelope envToVisit, SurfacePatchVisitor surfacePatchVisitor ) throws GM_Exception
  {
    ...
    GM_Surface surfacePatch = org.kalypsodeegree_impl.model.geometry.GeometryFactory.createGM_Surface( 
                                      exterior, 
                                      HMOTerrainElevationModel.NO_INTERIOR, 
                                      3,
