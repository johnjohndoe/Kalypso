...
public class PageOutputFeature extends WizardPage implements SelectionListener, ModifyListener
{
	private final static int DEFAULT_EXPANSION_LEVEL = 2;
	...
	public PageOutputFeature(String pageName, String title, ImageDescriptor titleImage)
	{
	...
	public void widgetSelected(SelectionEvent e)
	{
		if (e.widget == m_browseButton)
		{
			...
			final KalypsoResourceSelectionDialog dialog = new KalypsoResourceSelectionDialog(getShell(), null, "Ausw‰hlen einer GML Datei",
					new String[] { "gml" }, container, new ResourceSelectionValidator());
			dialog.open();
	...
	void validateFeaturePathFromSelection(final IStructuredSelection selection)
	{
		...
		if (firstElement instanceof Feature)
		{
			...
			// guess title
			{
				title = (String) feature.getProperty(new QName("name"));
			}
			catch (Exception e)
			{
				// nothing
			}
			if (title == null || title.length() < 1) title = AnnotationUtilities.getAnnotation(ft).getLabel();
			pathList.add(featurepath.toString());
			titleList.add(title);
			if (feature.getFeatureType().getProperty(KalypsoModelSimulationBaseConsts.SIM_BASE_PROP_ROUGHNESS_LAYER_POLYGON) != null)
			...
			if (firstElement instanceof FeatureAssociationTypeElement)
			{
				...
				for (int i = 0; i < associationFeatureTypes.length; i++)
				{
					...
					final FeaturePath path = new FeaturePath(parentFeaturePath, ftp.getQName().getLocalPart() + "[" + ft.getQName().getLocalPart() + "]");
		...
		m_pathList = pathList;
		// Feature f = (Feature)
		// m_workspace.getFeatureFromPath((String)m_pathList.get(0));
		setPageComplete(!m_pathList.isEmpty() && (m_selectedFeature != null));
	}
