...
public class WorkflowViewContentProvider implements ITreeContentProvider {

	private IActivity EMPTY[] = {};

	private IWorkflow workflow;

	private IWorkflowPart root;
	...
	public Object[] getElements(Object inputElement) {
		...
		if (inputElement instanceof IWorkflow) {
			...
			root = workflow;
			return workflow.getTaskGroups().toArray();
		} else if (inputElement instanceof ITaskGroup[]) {
			final ITaskGroup taskGroup = ((ITaskGroup[]) inputElement)[0];
			this.root = taskGroup;
			return taskGroup.getSubTasks().toArray();
		} else {
			return EMPTY;
		}
	}
