...
public class WorkflowDB implements IWorkflowDB {
	...
	final static private Logger logger = Logger.getLogger(WorkflowDB.class);

	private List<IWorkflowDBChangeListerner> dbListener = new ArrayList<IWorkflowDBChangeListerner>();

	final private Model dbModel;

	final private File dbDescResource;

	final private URL dbDescURL;

	public WorkflowDB(File dbDescRes) throws IOException {
		dbDescURL = dbDescRes.toURL();
	...
	public IWorkflowData createWorkflowData(String id, String type,
			IWorkflowData parent) {

		logger.info("creating data for :" + id);
		if (type == null) {
			throw new IllegalArgumentException(
					"Arguments type must not be null: id=" + id + " type="
							+ type + " parent=" + parent);
		}
	...
	public IWorkflowData derivedWorkflowData(IWorkflowData parent,
			String childId) {
	...
	public void link(IWorkflowData subject, IWorkflowData object,
			EWorkflowProperty prop) {
		Schema.createStatement(dbModel, subject, object, prop);
	}

	public void unlink(IWorkflowData subject, IWorkflowData object,
			EWorkflowProperty prop) {
	...
	public boolean persist() {
		try {
			...
			FileOutputStream outStream = new FileOutputStream(dbDescResource);
