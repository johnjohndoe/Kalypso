...
import org.kalypso.afgui.model.internal.Help;
...
/**
 * 
 * A utility java representation of the workflow schema. It contains a jena
 * model for the schema, and the element declared in the schema.
 * 
 * The vocabulary elements, resources and properties, are all static final
 * field. If their initialization fails because of an exception, this exception
 * is store in {@link #PROBLEM_FLAG} are all element are consequently set to
 * null.
 * 
...
final public class Schema {
	...
	static final private Logger logger = Logger.getLogger(Schema.class);

	public static final String SCHEMA_NS = "http://www.tu-harburg.de/wb/kalypso/schema/workflow#";

	final static public String WORKFLOW_SCHEMA_FOLDER = "schema";

	final static public String WORKFLOW_SCHEMA_FILE = WORKFLOW_SCHEMA_FOLDER
			+ "/workflow_schema.xml";
	...
	final static public String URI_PROP_HAS_ACTIVITY = SCHEMA_NS
			+ "hasActivity";
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_ACTIVITY = new IResourceWrapperConstructor() {
		public IWorkflowConcept construct(Object resource) {
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_WORKFLOW = new IResourceWrapperConstructor() {
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_HELP = new IResourceWrapperConstructor() {
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_TASK_SEQ = new IResourceWrapperConstructor() {
	...
	final static public String URI_CLASS_WORKFLOW_STATUS = SCHEMA_NS
			+ "WorkflowStatus";

	final static public Resource CLASS_WORKFLOW_STATUS = null;

	final static public String URI_CLASS_ACTIVITY_STATUS = SCHEMA_NS
			+ "ActivityStatus";

	final static public Resource CLASS_ACTIVITY_STATUS = null;

	//
	final static public String URI_CLASS_WORKFLOW_DATA = SCHEMA_NS
			+ "WorkflowData";
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_TASK = new IResourceWrapperConstructor() {
	...
	final static public IResourceWrapperConstructor CONSTRUCTOR_TASK_GROUP = new IResourceWrapperConstructor() {
	...
	// ActivitySeq
	final static public String URI_CLASS_ACTIVITY_SEQ = SCHEMA_NS
			+ "ActvitySeq";

	final static public Resource CLASS_ACTIVITY_SEQ;

	final static public IResourceWrapperConstructor CONSTRUCTOR_ACTIVITY_SEQ = new IResourceWrapperConstructor() {
	...
	// hasActivities
	final static public String URI_PROP_HAS_ACTIVITIES = SCHEMA_NS
			+ "hasActivities";
	...
	// hasTaskGroups
	final static public String URI_PROP_HAS_TASK_GROUPS = SCHEMA_NS
			+ "hasTaskGroups";
	...
	final static public Map<Property, EActivityRelationship> REL_STATEMENT_PROP_MAP = null;
	...
	final static public String URI_PROP_HAS_LOCATION = SCHEMA_NS
			+ "hasLocation";

	final static public String URI_PROP_IS_DERIVED_FROM = SCHEMA_NS
			+ "isDerivedFrom";

	final static public Property PROP_IS_DERIVED_FROM;

	// executedWorkflowPart
	final static public String URI_PROP_EXECUTING_WORKFLOW_PART = SCHEMA_NS
			+ "executingWorkflowPart";

	final static public Property PROP_EXECUTING_WORKFLOW_PART;

	// processedResource
	final static public String URI_PROP_PROCESSED_RES = SCHEMA_NS
			+ "processedResource";

	final static public Property PROP_PROCESSED_RES;

	// hasSubRTContext
	final static public String URI_PROP_HAS_SUB_RT_CONTEXT = SCHEMA_NS
			+ "hasSubRTContext";
	...
	/**
	 * Holds the throwable which describt a failure while initialising the class
	 * static fields. A value null signal an error free initialisation
	 */
	...
	static {
		...
		try {
			...
			// File resFolder=
			// new
			// File(Schema.class.getResource(WORKFLOW_SCHEMA_FOLDER).getPath());

			InputStream iStream = Schema.class
					.getResourceAsStream(WORKFLOW_SCHEMA_FILE);// new
			// FileInputStream(new
			// File(resFolder,WORKFLOW_SCHEMA_FILE));
			schemaModel.read(iStream, "");
			String propUris[] = { URI_PROP_IS_ROOT, URI_PROP_HAS_ACTIVITY,
					URI_PROP_HAS_NAME, URI_PROP_EXE_STATE, URI_PROP_PART_OF,
					URI_PROP_HAS_A, URI_PROP_DEPENDS_ON, URI_PROP_FOLLOWS,
					URI_PROP_HAS_HELP, URI_PROP_HAS_TYPE,
					URI_PROP_HAS_ACTIVITIES, URI_PROP_EXECUTING_WORKFLOW_PART,
					URI_PROP_PROCESSED_RES, URI_PROP_HAS_SUB_RT_CONTEXT,
					URI_PROP_HAS_TASKS, URI_PROP_HAS_TASK_GROUPS,
					URI_PROP_IS_DERIVED_FROM };

			String resUris[] = { URI_CLASS_ACTIVITY, URI_CLASS_WORKFLOW,
					URI_CLASS_HELP, URI_CLASS_WORKFLOW_STATUS,
					URI_CLASS_ACTIVITY_STATUS, URI_CLASS_WORKFLOW_DATA,
					URI_CLASS_ACTIVITY_SEQ, URI_CLASS_TASK, URI_CLASS_TASK_SEQ };
			...
			PROP_EXECUTING_WORKFLOW_PART = propMap
					.get(URI_PROP_EXECUTING_WORKFLOW_PART);
			...
			Property tempProps[] = { PROP_HAS_A, PROP_DEPENDS_ON, PROP_PART_OF };
			ACTIVITY_LINK_PROPS = Collections.unmodifiableList(Arrays
					.asList(tempProps));
			...
			Map<Property, EActivityRelationship> tempRelPropMap = new HashMap<Property, EActivityRelationship>(
					propRelStatePair.length);

			for (Object[] pair : propRelStatePair) {
				tempRelPropMap.put((Property) pair[0],
	...
	private static final EnumMap<EWorkflowProperty, Property> computeEWorkflowPropToJena() {
		...
		EnumMap<EWorkflowProperty, Property> toPropertyMap = new EnumMap<EWorkflowProperty, Property>(
				EWorkflowProperty.class);

		for (Object[] pair : eWorklflowPropToJena) {
			// logger.info("pair0="+(EWorkflowProperty)pair[0]+
			// " pair1="+pair[1].getClass());
			toPropertyMap.put((EWorkflowProperty) pair[0],
	...
	private static Map<Object, IResourceWrapperConstructor> computeRTClassMap() {
		Object[][] eWorklflowPropToJena = { { IHelp.class, CONSTRUCTOR_HELP },
		...
		Map<Object, IResourceWrapperConstructor> map = new Hashtable<Object, IResourceWrapperConstructor>();

		for (Object[] pair : eWorklflowPropToJena) {
			map.put(pair[0], (IResourceWrapperConstructor) pair[1]);
		}
		logger.info(map);
		return Collections.unmodifiableMap(map);
	}

	private static final void fillPropMap(Map<String, Property> propMap,
			String[] uris) {
		Property curProp;
		for (String uri : uris) {
	...
	private static final void fillResMap(Map<String, Resource> propMap,
			String[] uris) {
		Resource curRes;
		for (String uri : uris) {
	...
	final static public EActivityRelationship toEActivityRelationship(
			Property prop) {
		return REL_STATEMENT_PROP_MAP.get(prop);
	}

	final static public RDFNode getProperty(Resource resource,
	...
	final static public String getStringProperty(Resource resource,
			EWorkflowProperty prop) {
		Statement stm = resource.getProperty(toJenaProperty(prop));
		if (stm == null) {
			return null;
	...
	final static public void createStatement(Model model,
			IWorkflowData subject, IWorkflowData object, EWorkflowProperty prop) {
		model.add((Resource) subject.getModelObject(), toJenaProperty(prop),
				(Resource) object.getModelObject());
	}

	final static public void removeStatement(Model model,
			IWorkflowData subject, IWorkflowData object, EWorkflowProperty prop) {
		model.removeAll((Resource) subject.getModelObject(),
				toJenaProperty(prop), (Resource) object.getModelObject());
	}

	static final public List<IWorkflowData> getWorkflowDataByType(Model model,
			String type) {
	...
	final static public IWorkflowData createWorkflowData(Model model,
			IWorkflowData parent, String childId) {
		Resource res = model.createResource(toURIString(model, childId),
				CLASS_WORKFLOW_DATA);
		...
		if (parent != null) {
			try {
				...
				model.add(res, PROP_IS_DERIVED_FROM, (Resource) parent
						.getModelObject());
			} catch (Throwable th) {
				logger.error("New Res=" + res + " parent=" + parent
						+ " propIsDevFrom=" + PROP_IS_DERIVED_FROM, th);
			}
		}
		return new WorkflowData(res);
	}

	final static public IWorkflowData derivedWorkflowData(Model model,
			IWorkflowData parent, String childId) {
		Resource res = model.createResource(toURIString(model, childId),
				CLASS_WORKFLOW_DATA);
		model.createStatement(res,
	...
	final static public List<IWorkflowData> getRootWorkflowDataByType(
		...
		for (; it.hasNext();) {
			...
			if (!res.hasProperty(PROP_HAS_TYPE)
					&& !res.hasProperty(PROP_IS_DERIVED_FROM)) {
		...
		if (resource == null) {
			...
			if (stm == null) {
				...
				if (objectNode.isLiteral()) {
					...
					throw new RuntimeException("Help Type not supported yet:"
							+ objectNode);
				}
		...
		if (resource == null) {
			...
			if (stm == null) {
				...
				try {
					...
					logger.error("No activities sequence found", th);
					return Collections.emptyList();
				}
			}
		}
		...
		if (resource == null) {
			...
			if (stm == null) {
				...
				try {
					...
					EActivityExeState state = EActivityExeState.valueOf(stm
							.getString());
					if (state == null) {
	...
	public static IWorkflowPart getExecutingWorkflowPart(Resource resource) {
		if (resource == null) {
			...
			if (stm == null) {
				logger.info("resource has no executing workflowpart:"
						+ resource);
				return null;
			} else {
				try {
					...
					IResourceWrapperConstructor c = RT_CLASS_MAP.get(typeStm
							.getResource());
	...
	final static public IWorkflowPart toWorkflowPart(Resource resource) {
		if (resource == null) {
			...
			try {
				...
				logger.error("Could not create wrapper class for:" + resource,
						th);
				return null;
			}
	...
	final static public <T extends IWorkflowPart> T asWorkflowPart(
			Resource resource, Class<T> cls) {
		if (resource == null) {
			return null;
		} else {
			Resource type = resource.getProperty(RDF.type).getResource();
