...
public class WorkflowView extends ViewPart {
	final static private Logger logger = Logger.getLogger(WorkflowView.class);
	...
	private IWorklistChangeListener wfceListener = new IWorklistChangeListener() {

		public void worklistChanged() {
			logger.info("worklist changed");
	...
	private ISelectionChangedListener scListener = new ISelectionChangedListener() {
	...
	public void createPartControl(Composite parent) {
		workflow = KalypsoAFGUIFrameworkPlugin.getDefault().getWorkflowSystem()
				.getCurrentWorkFlow();
		m_connector = WorkflowConnector.getConnector();
		m_connector.addWorklistChangeListener(wfceListener);
		...
		makeActions();
		Action[] actions = new Action[] { doAction, getDataAction,
				getHelpAction, previousAction, nextAction, topAction, upAction,
				downAction };

		MenuBarCreator.createMenubar(
		// getViewSite().getPart(),
				this.getViewSite(), (IDoubleClickListener) null, actions, null,// pageBook,//treeView,
	...
	private void createTableTreeView(Composite parent) {
		...
		ttView.setLabelProvider(new TableLabelProvider(
	...
	private void createTreeColumn(Composite parent) {
		...
		Tree tree = tv.getTree();// new Tree(parent, SWT.BORDER |
	...
	private void makeActions() {
		ImageRegistry reg = KalypsoAFGUIFrameworkPlugin.getDefault()
				.getImageRegistry();
		...
		doAction = new Action() {
			public void run() {
				...
				IStructuredSelection sel = (IStructuredSelection) view
						.getSelection();
		...
		doAction.setImageDescriptor(reg.getDescriptor(EActivityAction.DO
				.toString()));
		...
		getHelpAction = new Action() {
			public void run() {
				IStructuredSelection sel = (IStructuredSelection) view
						.getSelection();
				IActivity activity = (IActivity) sel.getFirstElement();
				// workflow.updateWorkflow(activity, EActivityAction.GET_HELP);
		...
		getHelpAction.setImageDescriptor(reg
				.getDescriptor(EActivityAction.GET_HELP.toString()));
		...
		nextAction.setImageDescriptor(reg.getDescriptor(EActivityAction.NEXT
				.toString()));
		// /////////////////////////////////////////////
		...
		previousAction.setImageDescriptor(reg
				.getDescriptor(EActivityAction.PREVIOUS.toString()));
		// /////////////////////////////////////////////
		upAction = new Action() {
			public void run() {
				IStructuredSelection sel = (IStructuredSelection) view
		...
		upAction.setImageDescriptor(reg.getDescriptor(EActivityAction.UP
				.toString()));
		// /////////////////////////////////////////////
		...
		downAction.setImageDescriptor(reg.getDescriptor(EActivityAction.DOWN
				.toString()));
		// /////////////////////////////////////////////
		...
		topAction.setImageDescriptor(reg.getDescriptor(EActivityAction.TOP
				.toString()));
		// /////////////////////////////////////////////
		...
		getDataAction.setImageDescriptor(reg
				.getDescriptor(EActivityAction.SHOW_DATA.toString()));
	}
}
