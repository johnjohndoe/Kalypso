...
public class WorkflowData implements IWorkflowData {
	final static private Logger logger = Logger.getLogger(WorkflowData.class);
	...
	public String getLocation() {
		return Schema.getStringProperty(resource,
				EWorkflowProperty.HAS_LOCATION);
	...
	public boolean hasLinkedWorkflowData(EWorkflowProperty prop) {
		...
		logger.info("PropertyLinkToGet=" + jenaProp + " Res="
				+ resource.getModel());
		if (prop == null) {
			...
			boolean hasProp = resource.getModel().contains(null, jenaProp,
					resource);// resource.hasProperty(jenaProp);
	...
	public List<IWorkflowData> getLinkedWorkflowData(EWorkflowProperty prop) {
		...
		if (prop == null) {
			...
			StmtIterator stmIt = resource.getModel().listStatements(null,
					jenaProp, resource);//
			// Properties(jenaProp);
			List<IWorkflowData> list = new ArrayList<IWorkflowData>();
