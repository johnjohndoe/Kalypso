...
public class WorkflowSystem implements IWorkflowSystem {
	final static private Logger logger = Logger.getLogger(WorkflowSystem.class);
	...
	// public IWorkflow createWorkflow(URL specURL, URL statusURL) throws
	// IOException
	// {
	// if(specURL==null)
	// {
	...
	/**
	 * To get the workflow for the given resource. If their is a worflow in the
	 * system for this resource then it is returned otherwise a new one is
	 * created
	 * 
	 * @param resouceType
	 * @return
	...
	public IWorkflow getCurrentWorkFlow() {
		if (currentWorkflow == null) {
			currentWorkflow = new WorkflowImpl(mergedModel
					.getResource(TestRDFModel.WORKFLOW_SH));
		}
