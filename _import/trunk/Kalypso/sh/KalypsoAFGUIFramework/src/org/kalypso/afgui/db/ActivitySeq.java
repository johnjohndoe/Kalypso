...
class ActivitySeq implements IWorkflowConcept, List<IActivity> {
	...
	public ActivitySeq(Resource resource) {
		if (resource instanceof Seq) {
			...
			throw new IllegalArgumentException("Resource not a sequence:"
					+ resource);
		}
	}
	...
	public void add(int index, IActivity element) {
		...
		if (element == null) {
			...
			if (res == null) {
				throw new IllegalArgumentException(
						"No resource wrapped in element");
			} else {
				seq.add(res);
				return;
			}
	...
	public <T> T[] toArray(T[] a) {
		if (a == null) {
			...
			if (a instanceof IActivity[] || a instanceof Object[]) {
