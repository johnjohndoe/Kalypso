...
public class UpdateTinsOperation implements ICoreRunnableWithProgress
{
  ...
  {
    final SubMonitor progress = SubMonitor.convert( monitor, "Daten aktualisieren", m_tinReferences.length );
    try
    {
  ...
  private void updateTinReference( final ITinReference ref, final SubMonitor monitor ) throws Exception
  {
    ...
    switch( sourceType )
    {
      ...
      case shape:
        throw new UnsupportedOperationException( "FloodModeller: Wasserspiegelimport aus Shape-Dateien wird noch nicht unterst¸tzt." );
        // break;
    ...
    final Feature refFeature = ref.getWrappedFeature();
