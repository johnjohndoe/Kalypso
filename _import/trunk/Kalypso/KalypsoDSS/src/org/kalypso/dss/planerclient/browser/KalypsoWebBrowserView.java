...
public class KalypsoWebBrowserView extends ViewPart implements IBrowserViewerContainer, IKalypsoLinkHandler
{
  ...
  private LocationListener locationListener = new LocationAdapter()
  {
    ...
    public void changed( LocationEvent event )
    {
      ...
      {
        String linkStr = link.replaceFirst( IKalypsoLinkHandler.PERSPECTIV_PROTOCOL, "" );
        handlePerspectiveProtocol( linkStr );

      }
      else if( link.startsWith( IKalypsoLinkHandler.LAUNCH_PROTOCOL ) )
      {
        MessageDialog.openConfirm( getSite().getShell(), "Not implemented yet", "Das launch-protokol ist noch nicht implementiert." );
      }
    }
  };
