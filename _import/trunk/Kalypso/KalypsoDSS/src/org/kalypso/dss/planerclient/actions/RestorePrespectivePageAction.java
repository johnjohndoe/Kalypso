...
/**
 * Diese Action wird nur einmal ausgeführt und steht als Platzhalter für die
 * z.B. aus dem Intro-screen aufgerufener Einstiegsklick.
 */
public class RestorePrespectivePageAction extends Action implements
	...
	public void run(IAction action) {
		...
		final WorkbenchPage activePage = (WorkbenchPage) workbench
				.getActiveWorkbenchWindow().getActivePage();

		final PerspectiveRegistry registry = (PerspectiveRegistry) workbench
		...
		try {
			...
			PerspectiveDescriptor realDesc = (PerspectiveDescriptor) registry
					.findPerspectiveWithId(perspectiveID);
			IPerspectiveDescriptor perspectiveDesc = activePage
					.getPerspective();
			...
			IViewPart part = activePage
					.findView(KalypsoWebBrowserView.WEB_BROWSER_VIEW_ID);
			if (part instanceof KalypsoWebBrowserView) {
				((KalypsoWebBrowserView) part)
						.setURL(KalypsoDSSPlugin.START_URL);
			}
