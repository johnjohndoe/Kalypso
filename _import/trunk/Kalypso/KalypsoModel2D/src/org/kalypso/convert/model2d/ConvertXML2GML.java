...
import simulation2d.FpParams;
import simulation2d.ObjectFactory;
import simulation2d.Simulation2D;
import simulation2d.impl.FpParamsImpl;
import simulation2d.impl.Simulation2DTypeImpl;
...
public class ConvertXML2GML {
    ...
    private static String XSI_schemaLocation_NS = "http://elbe.wb.tu-harburg.de/2dModel http://troubadix.wb.tu-harburg.de/lupp/2dgml.xsd";
    ...
    private static StringBuffer sbTEST = new StringBuffer();
    ...
    */
    ...
    private static void setComment(Simulation2D sim) {
        if (sim.getComment()!= null) comment = sim.getComment().getCom();
    }
    ...
    /**
     * sets the ar value and its components 
     * (id, p1, p2, e_left, e_right, midSideNode)
     * 
     * @param sim
     */
    ...
    /**
     * sets the parameter of fe (id, roughnessOld, roughnessInTimeWarp, orderOfProcessing,defaultMass)
     * 
     * @param sim
     */
    ...
    /**
     * sets the rk parameters (id, roughness, color, param1, param2, param3, param4)
     * @param sim
     */
    private static void setRK(Simulation2D sim) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if(type.getParam1() != null) param1[j] = type.getParam1().intValue();
            if(type.getParam2() != null) param2[j] = type.getParam2().intValue();
            if(type.getParam3() != null) param3[j] = type.getParam3().intValue();
            if(type.getParam4() != null) param4[j] = type.getParam4().intValue();
        }
    }
    ...
    */
     ...
     * @param i
     */
    private static void createFPParamsNode(Document doc, Element e, int i, boolean exists){
	    ...
	    try{
	        ...
	        point.setAttribute("srsName", "http://www.opengis.net/gml/srs/epsg.xml#31467");
            ...
            System.out.println("Error occurred in generating geometry element by converting to gml file");
        }
       ...
       if(exists == true){            
	        ...
	        tg1.appendChild(XMLHelper.createTextNode(doc, ""+timeGradient1[i]));
	        e.appendChild(tg1);
	        
	        Element tg2 = doc.createElement("timeGradient2");
	        tg2.appendChild(XMLHelper.createTextNode(doc, ""+timeGradient2[i]));
	        e.appendChild(tg2);
	        
	        Element tg3 = doc.createElement("timeGradient3");
	        tg3.appendChild(XMLHelper.createTextNode(doc, ""+timeGradient3[i]));
	        e.appendChild(tg3);
	        
	        
	        Element dfoog1 = doc.createElement("degreeOfFreedomOfOldTimeGradient1");
	        dfoog1.appendChild(XMLHelper.createTextNode(doc, ""+degreeOfFreedomOfOldTimeGradient1[i]));
	        e.appendChild(dfoog1);
	        
	        Element dfoog2 = doc.createElement("degreeOfFreedomOfOldTimeGradient2");
	        dfoog2.appendChild(XMLHelper.createTextNode(doc, ""+degreeOfFreedomOfOldTimeGradient2[i]));
	        e.appendChild(dfoog2);
	        
	        Element dfoog3 = doc.createElement("degreeOfFreedomOfOldTimeGradient3");
	        dfoog3.appendChild(XMLHelper.createTextNode(doc, ""+degreeOfFreedomOfOldTimeGradient3[i]));
	        e.appendChild(dfoog3);
	        
	        Element tgtw1 = doc.createElement("TimeGradientOfFormerTimeWarps1");
	        tgtw1.appendChild(XMLHelper.createTextNode(doc, ""+timeGradientOfFormerTimeWarps1[i]));
	        e.appendChild(tgtw1);
	        
	        Element tgtw2 = doc.createElement("TimeGradientOfFormerTimeWarps2");
	        tgtw2.appendChild(XMLHelper.createTextNode(doc, ""+timeGradientOfFormerTimeWarps2[i]));
	        e.appendChild(tgtw2);
	        
	        Element tgtw3 = doc.createElement("TimeGradientOfFormerTimeWarps3");
	        tgtw3.appendChild(XMLHelper.createTextNode(doc, ""+timeGradientOfFormerTimeWarps3[i]));
	        e.appendChild(tgtw3);
        }
    }
    ...
    private static void setPolygonOfElements(Simulation2D sim){
        ...
        for (int i = 0; i < arList.size(); i++) {
            ...
            int meshPolygonIdLeft = (int)Math.round(Double.parseDouble(ar[i][3]));
            int meshPolygonIdRight = (int)Math.round(Double.parseDouble(ar[i][4]));
            
            MeshPolygon myMeshPolygon = null;
            
            if (outPolygons.size() == 0){
                myMeshPolygon = new MeshPolygon();
                myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][1])));
		        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][2])));
		        
		        if(outPolygons.size() <= meshPolygonIdLeft)outPolygons.setSize(meshPolygonIdLeft+1);
		        outPolygons.setElementAt(myMeshPolygon, meshPolygonIdLeft);
            }
            if (outPolygons.size() > 0 && meshPolygonIdLeft > 0){
                if((outPolygons.size()-1) < meshPolygonIdLeft)outPolygons.setSize(meshPolygonIdLeft+1);
	            if (outPolygons.elementAt(meshPolygonIdLeft) != null  ){
	                myMeshPolygon = (MeshPolygon)outPolygons.elementAt(meshPolygonIdLeft);
	                myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][1])));
			        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][2])));
			        
			        myMeshPolygon.setAR((int)Math.round(Double.parseDouble(ar[i][3])),(int)Math.round(Double.parseDouble(ar[i][1])), (int)Math.round(Double.parseDouble(ar[i][2])));
	           } else {	                
	                myMeshPolygon = new MeshPolygon();
	                myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][1])));
			        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][2])));
            ...
            if (outPolygons.size() > 0 && meshPolygonIdRight > 0){
                ...
                if (outPolygons.elementAt(meshPolygonIdRight) != null){
                    myMeshPolygon = (MeshPolygon)outPolygons.elementAt(meshPolygonIdRight);
			        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][1])));
			        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][2])));
			        
			        myMeshPolygon.setAR((int)Math.round(Double.parseDouble(ar[i][4])),(int)Math.round(Double.parseDouble(ar[i][1])), (int)Math.round(Double.parseDouble(ar[i][2])));
	            } else {
	                myMeshPolygon = new MeshPolygon();
	                myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][1])));
			        myMeshPolygon.setPoint((int)Math.round(Double.parseDouble(ar[i][2])));
    ...
    private static void createPolygonNode(Document doc, Element e, int i){
        ...
        //TODO dynamic srs
        polygonElement.setAttribute("srsName", "http://www.opengis.net/gml/srs/epsg.xml#31467");
        ...
        try{
		    ...
		    if (mp != null){
	            ...
	            if(coordsPoint1 != null && coordsPoint2 != null && coordsPoint3 != null
	                    && idMP[2]!= 0.0){
	                coordsPoint1 = getCoordinates(idMP[0]);
		            coordsPoint2 = getCoordinates(idMP[1]);
		            coordsPoint3 = getCoordinates(idMP[2]);
	                
	                coordinatesP1 = coordsPoint1[0]+","+coordsPoint1[1]+","+coordsPoint1[2];
	    		    coordinatesP2 = coordsPoint2[0]+","+coordsPoint2[1]+","+coordsPoint2[2];
	    		    coordinatesP3 = coordsPoint3[0]+","+coordsPoint3[1]+","+coordsPoint3[2];
	    		    
	    		    coordinates = coordinatesP1+" "+coordinatesP2+" "+coordinatesP3+" "+coordinatesP1;
	            }
	            ...
	            if (mp.getVector().size() == 4){
	                ...
	                coordinatesP4 = coordsPoint4[0]+","+coordsPoint4[1]+","+coordsPoint4[2];

	                coordinates = coordinatesP1+" "+coordinatesP4+" "+coordinatesP3+" "+coordinatesP2+" "+coordinatesP1;
	            }
		    }
		    if (coordinates.equalsIgnoreCase("")){
                System.out.println("m"+i+" has no edges: error in ascii file defining this feature");
                remove = true;
            }else remove = false;
    ...
    private static void createElementsNode(Document doc, Element e, int i){
        ...
        try{
	        ...
	        //TODO catches negativ roughness
//	        if(ro[i]< 0){
//	            ro[i] = 0-ro[i];
//	        }
//	        if(rtw[i]< 0){
...
//	        }
	        ...
	        roElem.appendChild( XMLHelper.createTextNode(doc, ""+roughness[ro[i]]) );
	        e.appendChild(roElem);
	        
	        Element rtwElem = doc.createElement("roughnessInTimeWarp");
	        rtwElem.appendChild( XMLHelper.createTextNode(doc, ""+roughness[rtw[i]]) );
            ...
            System.out.println("error in roughness parameter: negativ values in ascii file");
        }
    }
    ...
    private static void createGML(String outFile, boolean exists){
        ...
        try{
	         ...
	         * feature point
	         */
	        Element fpCollMember = doc.createElement("featurePointCollectionMember");
...
//	            meshElement.setAttribute("fid", "m"+j);
     ...
     public void startXML2GML(String inFile, String outFile, 
             String ns, String sl, boolean existsDetailedFP){
         try{             
             StringBuffer sb = new StringBuffer();
             ObjectFactory fac = new ObjectFactory();
             Unmarshaller unmarshaller = fac.createUnmarshaller();
             
             Simulation2D sim = (Simulation2D) unmarshaller.unmarshal(new File(inFile));
             ...
             System.out.println("Error in converting xml file to gml file -> JAXB Exception: ");
             e.printStackTrace();
         }
     }
