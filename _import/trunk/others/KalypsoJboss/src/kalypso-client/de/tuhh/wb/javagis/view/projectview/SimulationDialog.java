...
public class SimulationDialog
	...
	implements ActionListener, InternalFrameListener, KalypsoXmlImportListener
//, VersionListener
{
	...
	public SimulationDialog(
		...
		Object vId) {
		super(
			I18n.get("PV_SD_Title") + versionAccess.getFullName(themeKey, vId),
			true,
			true,
			true,
			true);
	...
	private void initMask() {
		...
		jTargetDir.setToolTipText(
			I18n.get("PV_SD_jTargetDirTT") + targetDir.getPath());
		...
		if ("chooseTargetDir".equals(command)) {
			int returnVal =
				fileChooser.showDialog(
					this,
					I18n.get("PV_SD_jfileChooserText"));
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				targetDir = fileChooser.getSelectedFile();
				jTargetDir.setText(
					I18n.get("PV_SD_jTargetDir") + targetDir.getPath());
				jTargetDir.setToolTipText(
					I18n.get("PV_SD_jTargetDirTT") + targetDir.getPath());
			}
		}
	...
	private void runSimulation() {
		try {
			...
			if (!xmlTempDir.exists())
				xmlTempDir.mkdirs();
			File simulationCaseFile =
				new File(xmlTempDir, "simulationCase.xml");
			myVersionAccess.xmlExport(
				myThemeKey,
				myVersionId,
				simulationCaseFile);
