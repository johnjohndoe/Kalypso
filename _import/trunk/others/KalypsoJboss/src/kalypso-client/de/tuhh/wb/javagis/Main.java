...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			props.setProperty(
				"bce_url",
				"jdbc:edbc://134.28.87.20:WB7/elbe::abwb/INGRES");
	...
	public static void connectBCE() throws Exception {
		...
		Database.init(
			props.getProperty("bce_driver"),
			props.getProperty("bce_url"),
			props.getProperty("bce_user"),
			props.getProperty("bce_pass"));
		LogView.print("connect to ingresdb: " + props.getProperty("bce_url"));
		/*
			...
			if (trim > 5)
				text =
					text.substring(0, trim)
						+ "<br>"
						+ text.substring(trim, text.length());
		}
