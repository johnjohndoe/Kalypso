...
public class Version implements ActionListener {
	...
	private Version(String themeKey, Object vId) {
		this.topicName = "Editable" + themeKey + vId;
		this.myThemeKey = themeKey;
		this.myVersionId = vId;
		this.myGisObjectClasses = new Hashtable();
		this.myGisRelationClasses = new Hashtable();
		isEditable=false;//setEditable(false);
		System.out.println("new Version, topicname:"+topicName);
		...
		for (int i = 0; i < model.objectSize; i++) {
			...
			GisObjectClass gisObjectClass =
				new GisObjectClass(this, model, elementTable, i);
		...
		for (int i = 0; i < model.relationSize; i++) {
			...
			GisRelationClass gisRelationClass =
				new GisRelationClass(this, model, elementTable, i);
			myGisRelationClasses.put(
				gisRelationClass.getKey(),
				gisRelationClass);
		}
	}
	...
	private void setEditable(boolean status) {
		if(isEditable==status)
			return;
		isEditable = status;
		//System.out.println("Version.setEditable("+status+")");
		if (isEditable) {
	...
	*/
	...
	public void setVersionDescription(String description) {
		Main.versionClass.setVersionDescription(
			myThemeKey,
			myVersionId,
			description);
	}
	...
	public Vector getVectorSets(int et, Object eId)
		...
		return Main.versionClass.getVectorSets(
			myThemeKey,
			myVersionId,
			et,
			eId);
	...
	public void setSimplePropertyValue(
		int et,
		Object eId,
		int pos,
		Object value) {
		Main.versionClass.setSimplePropertyValue(
			myThemeKey,
			et,
			eId,
			pos,
	...
	public void createRelation(
		...
		String destKey,
		Object destId) {
		Main.versionClass.createRelation(
			myThemeKey,
			myVersionId,
			rt,
			srcKey,
	...
	public void actionPerformed(ActionEvent event) {
		...
		System.out.println("Version.actionPerformed("+action+")");
