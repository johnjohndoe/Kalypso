...
public class FloodRiskAnalysis {
	 ...
	 * @throws Exception
	 */
	public static RectifiedGridCoverage defineRisk(
			RectifiedGridCoverage annualDamageGrid,
			RectifiedGridCoverage landuseGrid, Hashtable riskClassTable ) throws Exception {
		LogView.println("Calculate FloodRiskGrid...");
		RectifiedGridDomain floodRisk_gridDomain = new RectifiedGridDomain(
				annualDamageGrid.getGridDomain().getOrigin(null), annualDamageGrid
						.getGridDomain().getOffset(), annualDamageGrid
						.getGridDomain().getGridRange());
		Vector annualDamage_rangeSetData = annualDamageGrid.getRangeSet()
				.getRangeSetData();
		...
		for (int i = 0; i < annualDamage_rangeSetData.size(); i++) {
			Vector annualDamage_rowData = (Vector) annualDamage_rangeSetData
					.get(i);
			...
			for (int j = 0; j < annualDamage_rowData.size(); j++) {
				if (annualDamage_rowData.get(j) != null
					...
					if (riskClass != null) {
						floodRisk_rowData.addElement(new Double(riskClass
								.doubleValue()));
					} else {
			 ...
			 * annualDamage_rangeSetData.size() + " calculated");
			 */
		}//for i
		RangeSet floodRisk_rangeSet = new RangeSet(floodRisk_rangeSetData, null);
		RectifiedGridCoverage floodRiskGrid = new RectifiedGridCoverage(
				floodRisk_gridDomain, floodRisk_rangeSet);
