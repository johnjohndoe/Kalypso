...
public class SelectWidget extends GisMapEditorWidgetActionDelegate
{
    ...
    public JComponent getOptionDialog(  )
    {
        return null;
    }
    ...
    public void paint( Graphics g )
    {
        ...
        else if( //JMSelectOptionPanel.getInstance(  ).hasSnapEnabled(  ) && 
            cursorPoint != null )
        {
            int radius = 10;//JMSelectOptionPanel.getInstance(  ).getPixelRadius(  );
            g.drawRect( (int)cursorPoint.getX(  ) - radius, (int)cursorPoint.getY(  ) - radius, 2 * radius, 2 * radius );
        }
    }
...
{
              ICommand command=new JMMarkSelectCommand(mapModell.getActiveTheme(), GeometryFactory.createGM_Position( g1x, g1y ) ,mySelectionId);
...
} else // dragged
            {
                ...
                {
                  ICommand command= new JMMarkSelectCommand(mapModell.getActiveTheme(), GeometryFactory.createGM_Envelope( 
                      minX, minY, maxX, maxY ), withinStatus,mySelectionId ) ;
