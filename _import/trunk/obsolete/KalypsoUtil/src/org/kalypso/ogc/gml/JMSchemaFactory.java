 ...
 * DOCUMENT ME!
 *
 * @author vdoemming To change the template for this generated type comment go to Window&gt;Preferences&gt;Java&gt;Code
...
public class JMSchemaFactory
{
	...
	public static FeatureType[] createFeatureTypes(JMSchema schema)
		throws Exception
	{
		NodeList nl =
			schema.getSchema().getElementsByTagNameNS(
				"http://www.w3.org/2001/XMLSchema",
				"element");
		nl =
			XMLTools.reduceByAttribute(nl, "substitutionGroup", "gml:_Feature");
		...
		for (int i = 0; i < nl.getLength(); i++)
		{
			JMFeatureTypeBuilder builder =
				new JMFeatureTypeBuilder(schema, nl.item(i));
...
//				System.out.println("ELEMENT_REF");
				{
					JMAttribute refAttribute =
						new JMAttribute(
							schema,
							XMLTools.getAttributeNode(node, "ref"));
...
//					System.out.println("RESTRICTION_TYPE");
					if (element.hasAttribute("base"))
					{
						JMAttribute baseAttribute =
							new JMAttribute(
								schema,
								XMLTools.getAttributeNode(node, "base"));
						collector.setTypeName(baseAttribute);
					}
					else
						throw new Exception(
							"unknown Restriction: " + XMLTools.toString(node));
...
//					System.out.println("ENUMERATION_TYPE");
					if (element.hasAttribute("value"))
					{
						JMAttribute valueAttribute =
							new JMAttribute(
								schema,
								XMLTools.getAttributeNode(node, "value"));
						collector.addEnumerationObject(
							valueAttribute.getValue());
					}
					else
						throw new Exception(
							"unknown Enumeration: " + XMLTools.toString(node));
...
//				System.out.println("IGNORED_TYPE");
	...
	private static int getType_ElementNode(JMSchema schema, Node node)
		throws Exception
	{
		Element element = (Element) node;
...
/*
		...
		Node node)
	{
		JMAttribute nameAttribute =
			new JMAttribute(schema, XMLTools.getAttributeNode(node, "name"));
		...
		boolean nullable = true; // todo: support nullable
...
*/
		...
		throws Exception
	{
		...
		JMAttribute typeAttribute =
			new JMAttribute(jmSchema, XMLTools.getAttributeNode(node, "type"));
		String type =
