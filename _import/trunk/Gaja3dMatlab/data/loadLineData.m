function [ boundaries ] = loadLineData( file, hasheader, params  )
%% LOADLINEDATA Loads line data from shape (.shp) or text (.txt, .xyz) file
...
%% switch file extension
    ...
    switch(ext)
        case SHPEXT
            ...
            info = shapeinfo(file);
            if(~(strcmp(info.ShapeType, 'PolyLine')))
                error('Shapefile %s does not have line geometry.', file);
            end
            %read shapefile
        ...
        case {TXTEXT, XYZEXT}
            ...
            boundaries = struct('Geometry', 'Line', 'BoundingBox', B, 'X', X, 'Y', Y, 'ID', ID);
    end
%%
end %LOADLINEDATA
