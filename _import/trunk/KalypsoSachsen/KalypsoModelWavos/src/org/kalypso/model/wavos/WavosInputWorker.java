...
public class WavosInputWorker
{
  ...
  public static File createNativeInput( final File tmpDir, final ISimulationDataProvider inputProvider, final Properties props, final File nativeInDir, final String flussName, final Map metaMap ) throws Exception
  {
    try
    {
      ...
      final InputStream openStream = awerteZip.openStream();
  ...
  private static void writeBauwerkeOffen( final GMLWorkspace wks, final File flussBinDir, final Properties props ) throws Exception
  {
    ...
    for( final Iterator iter = bauwerkeList.iterator(); iter.hasNext(); )
    {
      ...
      if( urlConTest != null )
      {
        ...
        String zeile = String.format( Locale.ENGLISH, "%-14s", kenn + "_aus" );
        String suffix;
        for( int ii = 0; ii < tplValues.getCount(); ii++ )
        {
          final boolean polderOffen = ((Boolean) tplValues.getElement( ii, axWerte )).booleanValue();
          final Date datum = (Date) tplValues.getElement( ii, axDatum );
          if( ii == 0 )
          {
            // wenn direkt am Anfang offen, dann eine Stunde vor
            // Anfang schon offen
            final Date newDate = WavosUtils.addHour( datum, -intervalAmount );
            zeile = zeile + String.format( Locale.ENGLISH, " %s", WavosUtils.createWavosDate( newDate ) );
          }
          if( !(zustand == polderOffen) )
          {
            ...
            zeile = zeile + String.format( Locale.ENGLISH, "%28s", WavosUtils.createWavosDate( datum ) );

            pWrtrOffen.println( zeile );
            final Date newDate = WavosUtils.addHour( datum, intervalAmount );

            zeile = String.format( Locale.ENGLISH, "%-14s%s", kenn + suffix, WavosUtils.createWavosDate( newDate ) );
            zustand = polderOffen;
          }
        }
      }
  ...
  private static void writeProFile( final GMLWorkspace wks, final File flussBinDir, final String flussName ) throws Exception
  {
    ...
    for( final Iterator iter = pegelList.iterator(); iter.hasNext(); )
    {
      ...
      if( allowed )
      {
        pWrtrPro.println( String.format( Locale.ENGLISH, "%4s%3s", pegel.getProperty( m_kenn ), pegel.getProperty( m_projektionType ) ) );
      }
    }
  ...
  private static void writeInput( final GMLWorkspace wks, final File inputDir, final Properties props, final Map metaMap ) throws Exception
  {
    ...
    {
      final Feature pegel = (Feature) iter.next();
      final String lfdNum = String.format( Locale.ENGLISH, "%05d", pegel.getProperty( m_lfdNum ) );
    ...
    while( iter.hasNext() )
    {
      ...
      projektionsZeile = projektionsZeile + String.format( Locale.ENGLISH, "%6s", allowed ? pegel.getProperty( m_projektionType ).toString() : "0" );
    }
    ...
    for( int i = countMeasValues; i < countForecastValues + countMeasValues; i++ )
    {
      ...
      while( iterObs.hasNext() )
      {
        ...
        if( valList.size() > i )
          ...
          zeile = zeile + String.format( Locale.ENGLISH, "%6.0f", new Double( -1 ) );
      }
      pWrtrInput.println( zeile );
      c.add( Calendar.HOUR_OF_DAY, intervalAmount );
    }
  ...
  public static HashMap<String, Object> parseControlFile( final URL gmlURL ) throws SimulationException
  {
    try
    {
