...
public class WavosInputWorker {
	...
	public static File createNativeInput(final File tmpDir,
			final ISimulationDataProvider inputProvider, Properties props,
			final File nativeInDir, final String flussName, final Map metaMap)
			throws Exception {
		try {
			...
			m_logger
					.info("\tLese Steuerparameter: " + controlGmlURL.toString());
			...
			// .../input.par
			// .../shiftvor.par
			m_logger.info("\tSchreibe " + WavosConst.FILE_WAVOS_PAR + " und "
					+ WavosConst.FILE_SHIFTVOR_PAR);
			...
			final File awerteTargetDir = new File(flussDir,
					WavosConst.DIR_AWERTE);
			...
			final URL awerteZip = new URL(awerteURLDir,
					WavosConst.FILE_AWERTE_ZIP);
			...
			throw new SimulationException(
					"Fehler beim Erzeugen der Eingabedateien", e);
		}
	}
	 ...
	 * @param props
	 */
	private static void writeParFiles(final File exeDir, final Properties props)
			throws Exception {
		// Schreibe input.par und shiftvor.par
		final Date forecastDate = (Date) props
				.get(WavosConst.DATA_STARTFORECAST_DATE);
		final boolean useAWerte = ((Boolean) props
				.get(WavosConst.DATA_USE_AWERTE)).booleanValue();
		final File fleInputPar = new File(exeDir, WavosConst.FILE_WAVOS_PAR);
		final Writer wrtrInputPar = new OutputStreamWriter(
				new FileOutputStream(fleInputPar), WavosConst.WAVOS_CODEPAGE);
		...
		// der
		// Vorhersage
		final String mergeCasePath = (String) props
				.get(WavosConst.DATA_MERGE_CASE_PATH);
		...
		final File fleShiftvorPar = new File(exeDir,
				WavosConst.FILE_SHIFTVOR_PAR);
		final Writer wrtrShiftvorPar = new OutputStreamWriter(
				new FileOutputStream(fleShiftvorPar), WavosConst.WAVOS_CODEPAGE);
	...
	private static void writeBatchFile(final File exeDir, final String flussName)
			throws Exception {
		// schreibe start.bat
		...
		// output.log
		pWrtrInputPar
				.println("bin\\dr_wavos_kalypso.exe " + flussName
						+ "\\bin\\objekte_" + flussName + ".dat < "
						+ WavosConst.FILE_WAVOS_PAR + " > "
						+ WavosConst.FILE_WAVOS_LOG);
		...
		pWrtrInputPar.println("xcopy " + "\"" + flussName + File.separator
				+ WavosConst.DIR_VORHER + "\"" + " " + "\"" + flussName
				+ File.separator + WavosConst.DIR_VORHER_SAVE + "\" /y/i"); // /
		// c
		...
		// shiftvor.log
		pWrtrInputPar.println("bin\\shiftvor.exe " + flussName
				+ "\\bin\\objekte_shiftvor.dat < "
				+ WavosConst.FILE_SHIFTVOR_PAR + " > "
				+ WavosConst.FILE_SHIFTVOR_LOG);

		IOUtils.closeQuietly(pWrtrInputPar);
		IOUtils.closeQuietly(wrtrInputPar);
	}

	private static void writeTimeseriesAndAtFiles(final Properties props,
			final File flussDir, final Map metaMap) throws Exception {
		// workspace holen
		...
		tafelDir.mkdirs();
		FeatureVisitorZml2WavosAT.writeAT(wks, tafelDir, (URL) props
				.get(WavosConst.DATA_GML_CONTEXT));
	 ...
	 * @param props
	 */
	private static void writeBauwerkeOffen(final GMLWorkspace wks,
			final File flussBinDir, final Properties props) throws Exception {
		final File fleOffen = new File(flussBinDir, "bauwerke.offen");
		final Writer wrtrOffen = new OutputStreamWriter(new FileOutputStream(
				fleOffen), WavosConst.WAVOS_CODEPAGE);
		final PrintWriter pWrtrOffen = new PrintWriter(wrtrOffen);

		final FeatureList bauwerkeList = (FeatureList) wks
				.getFeatureFromPath("bauwerkMember");

		pWrtrOffen
				.println("# Wann sind welche Bauwerke offen? Ein Bauwerk kann auch in mehreren Zeilen vorkommen");
		pWrtrOffen.println("#");
		ITuppleModel tplValues = null;
		final URL url = (URL) props.get(WavosConst.DATA_GML_CONTEXT);
		final int intervalAmount = ((Integer) props
				.get(WavosConst.DATA_INTERVAL_AMOUNT)).intValue();
		for (final Iterator iter = bauwerkeList.iterator(); iter.hasNext();) {
			...
			final TimeseriesLinkType tsLink = (TimeseriesLinkType) bauwerk
					.getProperty(m_propZmlLinkPolder);
			final String href = tsLink.getHref();
			final URL zmlUrl = m_urlresolver.resolveURL(url, href);
			final URLConnection urlConTest = UrlUtilities
					.connectQuietly(zmlUrl);

			if (urlConTest != null) {
				final IObservation obsZml = ZmlFactory.parseXML(zmlUrl, kenn);
				tplValues = obsZml.getValues(null);
				final IAxis axDatum = ObservationUtilities.findAxisByType(
						tplValues.getAxisList(), TimeserieConstants.TYPE_DATE);
				final IAxis axWerte = ObservationUtilities.findAxisByType(
						tplValues.getAxisList(),
						TimeserieConstants.TYPE_POLDER_CONTROL);
				boolean zustand = false;

				String zeile = String.format(Locale.ENGLISH,"%-14s",  kenn
						+ "_aus" );
				String suffix;
				for (int ii = 0; ii < tplValues.getCount(); ii++) {
					boolean polderOffen = ((Boolean) tplValues.getElement(ii,
							axWerte)).booleanValue();
					Date datum = (Date) tplValues.getElement(ii, axDatum);
					if (ii == 0) {
						// wenn direkt am Anfang offen, dann eine Stunde vor
						// Anfang schon offen
						final Date newDate = WavosUtils.addHour(datum,
								-intervalAmount);
						zeile = zeile
								+ String.format(Locale.ENGLISH," %s", WavosUtils
										.createWavosDate(newDate));
					}
					if (!(zustand == polderOffen)) {
						...
						zeile = zeile
								+ String.format(Locale.ENGLISH,"%28s", WavosUtils
										.createWavosDate(datum));

						pWrtrOffen.println(zeile);
						final Date newDate = WavosUtils.addHour(datum,
								intervalAmount);

						zeile = String.format(Locale.ENGLISH,"%-14s%s", kenn + suffix,
	 ...
	 * @param flussName
	 */
	private static void writeProFile(final GMLWorkspace wks,
			final File flussBinDir, final String flussName) throws Exception {
		final File flePro = new File(flussBinDir, flussName + ".pro");
		final Writer wrtrPro = new OutputStreamWriter(new FileOutputStream(
				flePro), WavosConst.WAVOS_CODEPAGE);
		final PrintWriter pWrtrPro = new PrintWriter(wrtrPro);

		final FeatureList pegelList = (FeatureList) wks
				.getFeatureFromPath("pegelMember");
		pWrtrPro
				.println("# Elbe.pro: Projektion: 0=nein, 1 oder W = auf W, Q= auf Q ");
		...
		for (final Iterator iter = pegelList.iterator(); iter.hasNext();) {
			...
			final boolean allowed = propAllowed == null ? false
					: ((Boolean) propAllowed).booleanValue();

			if (allowed) {
				pWrtrPro.println(String.format(Locale.ENGLISH,"%4s%3s", pegel
						.getProperty(m_kenn), pegel
						.getProperty(m_projektionType)));
			}
		}
	...
	private static void writeInput(final GMLWorkspace wks, final File inputDir,
			final Properties props, final Map metaMap) throws Exception {
		final URL url = (URL) props.get(WavosConst.DATA_GML_CONTEXT);
		// eine Datei (Endung aus Datum des letzten Messwerts)
		final Date startForecast = (Date) props
				.get(WavosConst.DATA_STARTFORECAST_DATE);
		final int intervalAmount = Integer.parseInt(props.get(
				WavosConst.DATA_INTERVAL_AMOUNT).toString());

		final String nmeInput = WavosUtils.createInputFleName(startForecast);
		final File fleInput = new File(inputDir, nmeInput);
		final Writer wrtrInput = new OutputStreamWriter(new FileOutputStream(
				fleInput), WavosConst.WAVOS_CODEPAGE);
		...
		pWrtrInput.println(" "
				+ props.get(WavosConst.DATA_SIMULATION_DURATION).toString()
				+ "  tszurueck");
		pWrtrInput.println(" "
				+ props.get(WavosConst.DATA_FORECAST_DURATION).toString()
		...
		for (final Iterator iter = pegelList.iterator(); iter.hasNext();) {
			final Feature pegel = (Feature) iter.next();
			final String lfdNum = String.format(Locale.ENGLISH,"%05d", pegel
					.getProperty(m_lfdNum));
			pegelMap.put(lfdNum, pegel);

			final TimeseriesLinkType tsLink = (TimeseriesLinkType) pegel
					.getProperty(m_propZmlLink);
			final String href = tsLink.getHref();
			final URL zmlUrl = m_urlresolver.resolveURL(url, href);
			final URLConnection urlConTest = UrlUtilities
					.connectQuietly(zmlUrl);
		...
		String kennZeile = String.format(Locale.ENGLISH,"%17s", "");
		String stundenZeile = String.format(Locale.ENGLISH,"%18s", "");
		String projektionsZeile = String.format(Locale.ENGLISH,"%17s", "");
		...
		while (iter.hasNext()) {
			...
			final boolean zuflussVorhersage = property == null ? false
			...
			kennZeile = kennZeile + String.format(Locale.ENGLISH,"%6s", kenn);
			stundenZeile = stundenZeile + String.format(Locale.ENGLISH,"%6s", "0");
			final Object propAllowed = pegel.getProperty(m_projektionAllowed);
			final boolean allowed = propAllowed == null ? false
					: ((Boolean) propAllowed).booleanValue();

			projektionsZeile = projektionsZeile
					+ String.format(Locale.ENGLISH,"%6s", allowed ? pegel.getProperty(
							m_projektionType).toString() : "0");
		}
		pWrtrInput.println(" 0  nvorweiter");
		pWrtrInput
				.println(" ##### Anzahl Stunden bekannt: Werte hier nicht aendern");
		pWrtrInput.println(kennZeile);
		pWrtrInput.println(stundenZeile);
		pWrtrInput
				.println(" ##### Projektionen: Werte koennen hier geaendert werden");
		pWrtrInput.println(kennZeile);
		pWrtrInput.println(projektionsZeile);
		pWrtrInput
				.println(" ##### Wasserstaende: Werte koennen geaendert werden");
		pWrtrInput.println(kennZeile);

		// TODO? Zeitversatz (Sommer/Winter)
		final Calendar c = Calendar.getInstance();
		c.setTime(startForecast);
		final int countMeasValues = ((Integer) props
				.get(WavosConst.DATA_COUNT_MEASURED_VALUES)).intValue();
		...
		// m_AxisToBeWritten)
		final Date simStart = (Date) (props
				.get(WavosConst.DATA_SIMULATION_START));
		c.setTime(simStart);
		...
		for (int i = 0; i < countMeasValues; i++) {
			...
			while (iterObs.hasNext()) {
				...
				zeile = zeile + String.format(Locale.ENGLISH,"%6.0f", valList.get(i));
			}
			pWrtrInput.println(zeile);
			c.add(Calendar.HOUR_OF_DAY, intervalAmount);
		}
		pWrtrInput.println(kennZeile);
		final int countForecastValues = ((Integer) props
				.get(WavosConst.DATA_COUNT_FORECAST_VALUES)).intValue();
		for (int i = countMeasValues; i < countForecastValues + countMeasValues; i++) {
			...
			while (iterObs.hasNext()) {
				...
				if (valList.size() > i)
					...
					zeile = zeile + String.format(Locale.ENGLISH,"%6.0f", new Integer(-1));
			}
			pWrtrInput.println(zeile);
			c.add(Calendar.HOUR_OF_DAY, intervalAmount);
		}
	...
	private static void copyAndUnzipRechenkern(final File targetDir,
			final int zipType) throws SimulationException {
		URL urlZip = null;
		...
		try {
			...
			case WavosConst.ZIP_TYPE_BIN: {
				urlZip = WavosInputWorker.class
						.getResource(WavosConst.RECHENKERN_BIN_ZIP);
				break;
			}
			case WavosConst.ZIP_TYPE_DATA: {
				urlZip = WavosInputWorker.class
						.getResource(WavosConst.RECHENKERN_DATA_ZIP);
				break;
			}
			}
	...
	public static HashMap<String, Object> parseControlFile(final URL gmlURL)
		...
		try {
			...
			// String
			final Object objMergeCasePath = controlFeature
					.getProperty("mergeCasePath");
			...
			// int [h]
			final Integer forecastDur = (Integer) controlFeature
					.getProperty("forecastDuration");
			dataMap.put(WavosConst.DATA_FORECAST_DURATION, forecastDur);

			// int [d]
			final Integer simulationDur = (Integer) controlFeature
					.getProperty("simulationDuration");
			dataMap.put(WavosConst.DATA_SIMULATION_DURATION, simulationDur);

			// int [h]
			final Integer intervalAmount = (Integer) controlFeature
					.getProperty("intervalAmount");
			dataMap.put(WavosConst.DATA_INTERVAL_AMOUNT, intervalAmount);

			// boolean
			final Object useAWerteProp = controlFeature
					.getProperty("useAWerte");
			final Boolean useAWerte = useAWerteProp == null ? Boolean.FALSE
					: ((Boolean) useAWerteProp);
			...
			cal.add(CalendarUtilities.getCalendarField("DAY_OF_MONTH"),
					-simulationDur.intValue());
			Date simulationStart = cal.getTime();

			simulationStart = DateUtils.truncate(simulationStart,
			...
			// oder so...
			cal.setTime(startForecastTime);
			final Integer cntMeasVals = new Integer((1 + simulationDur
					.intValue() * 24 + cal.get(Calendar.HOUR_OF_DAY))
			...
			final Integer cntForecastVals = new Integer(forecastDur.intValue()
					/ intervalAmount.intValue());
