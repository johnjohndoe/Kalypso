...
CMemTrackFile::CMemTrackFile(CFrameWnd* pWnd) : CMemFile()
{
	m_nLastPercent = -1;
	m_dwLength = 0;
	m_pFrameWnd = pWnd;
...
UINT CMemTrackFile::Read(void FAR* lpBuf, UINT nCount)
{
	UINT n = CMemFile::Read(lpBuf, nCount);
	if (m_dwLength != 0)
		OutputPercent((int)((GetPosition()*100)/m_dwLength));
	return n;
}

void CMemTrackFile::Write(const void FAR* lpBuf, UINT nCount)
{
...
void CMemTrackFile::OutputString(LPCTSTR lpsz)
{
	if (m_pFrameWnd != NULL)
	{
		m_pFrameWnd->SetMessageText(lpsz);
...
void CMemTrackFile::OutputPercent(int nPercentComplete)
{
	if (m_pFrameWnd != NULL && m_nLastPercent != nPercentComplete)
	{
		m_nLastPercent = nPercentComplete;
...
CMemOEMFile::CMemOEMFile(CFrameWnd* pWnd) : CMemTrackFile(pWnd)
{
}

UINT CMemOEMFile::Read(void FAR* lpBuf, UINT nCount)
{
	UINT n = CMemTrackFile::Read(lpBuf, nCount);
	if (m_bOEMToANSI)
		OemToCharBuffA((const char*)lpBuf, (char*)lpBuf, n);
	else
		CharToOemBuffA((const char*)lpBuf, (char*)lpBuf, nCount);
...
void CMemOEMFile::Write(const void FAR* lpBuf, UINT nCount)
{
//	if (!m_bOEMToANSI)
//		CharToOemBuffA((const char*)lpBuf, (char*)lpBuf, nCount);
	CMemTrackFile::Write(lpBuf, nCount);
}
...
CConverter::CConverter(LPCSTR pszLibName, CFrameWnd* pWnd) : CTrackFile(pWnd)
{
	...
	else
	{
		...
		if (m_pInitConverter != NULL)
		{
			...
			str.MakeUpper();
			VERIFY(m_pInitConverter(AfxGetMainWnd()->GetSafeHwnd(), T2A(str.GetBuffer(str.GetLength()))));
			str.ReleaseBuffer();
		}
	}
}
