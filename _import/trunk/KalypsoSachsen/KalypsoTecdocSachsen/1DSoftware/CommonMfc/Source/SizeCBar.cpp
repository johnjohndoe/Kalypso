...
void CSizingControlBar::OnLButtonDown(UINT nFlags, CPoint point)
{
  if (m_pDockBar != NULL)
  {
    // start the drag only if there is no gripper
...
void CSizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
  if (m_pDockBar != NULL)
  {
    // toggle docking only if there is no gripper
    ASSERT(m_pDockContext != NULL);
    if ( !m_bGripper )
...
void CSizingControlBar::OnNcLButtonDblClk( UINT nHitTest, CPoint point )
{
...
void CSizingControlBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    // make room for Gripper if activatet
    if ( m_bGripper )
    {
      switch ( nDockBarID )
      {
      case AFX_IDW_DOCKBAR_TOP:
      case AFX_IDW_DOCKBAR_BOTTOM:
        rc.DeflateRect( m_cxGripper, 0, 0, 0 );
        break;

      case AFX_IDW_DOCKBAR_LEFT:
      case AFX_IDW_DOCKBAR_RIGHT:
        rc.DeflateRect( 0, m_cxGripper, 0, 0 );
        break;
      }; // switch
    }; // if m_bGripper
...
void CSizingControlBar::OnNcPaint()
{
  ...
  // GripperRect ausrechnen
  switch ( m_nDockBarID )
  {
  case AFX_IDW_DOCKBAR_TOP:
  case AFX_IDW_DOCKBAR_BOTTOM:
    rcDraw.right = rcDraw.left + m_cxGripper;
    break;
    
  case AFX_IDW_DOCKBAR_LEFT:
  case AFX_IDW_DOCKBAR_RIGHT:
    rcDraw.bottom = rcDraw.top + m_cxGripper;
    break;
  };
  
  if ( m_bGripper )
    NcPaintGripper(&mdc, rcDraw);
...
void CSizingControlBar::NcPaintGripper(CDC* pDC, CRect& rcGripper)
{
  if( m_dwStyle & CBRS_ORIENT_HORZ )
  { // gripper at left
    
    // draw close box
    int width = rcGripper.Width();
    m_rectClose = CRect( rcGripper.left + 2, rcGripper.top + 3,
      rcGripper.right, rcGripper.top + width );
    m_rectGrip = rcGripper;
    ...
    CRect gripRect( rcGripper.left + width / 2 - 2, rcGripper.top + width + 2, 
      rcGripper.left + width / 2, rcGripper.bottom - 3 );
    pDC->Draw3dRect( gripRect, m_clrBtnHilight, m_clrBtnShadow );
    gripRect.OffsetRect( 4 , 0 );
    pDC->Draw3dRect( gripRect, m_clrBtnHilight, m_clrBtnShadow );
  }
  ...
  { // gripper at top
    ...
    // draw close box
    int height = rcGripper.Height();
    m_rectClose = CRect( rcGripper.right - height - 1, rcGripper.top + 2, 
      rcGripper.right - 3, rcGripper.bottom - 1 );
    m_rectGrip = rcGripper;
    ...
    CRect gripRect( rcGripper.left + 3, rcGripper.top + height / 2 - 2, 
      rcGripper.right - height - 1, rcGripper.bottom - height / 2 );
    pDC->Draw3dRect( gripRect, m_clrBtnHilight, m_clrBtnShadow );
    gripRect.OffsetRect( 0 , 4 );
    pDC->Draw3dRect( gripRect, m_clrBtnHilight, m_clrBtnShadow );
  };
};
...
UINT CSizingControlBar::OnNcHitTest(CPoint point)
{
  ...
  if (!IsFloating())
  {
    ...
    point.Offset(-rcBar.left, -rcBar.top);
    if ( m_rectClose.PtInRect( point ) )
      return HTSYSMENU;
    if ( m_rectGrip.PtInRect( point ) )
      return HTCAPTION;
  };
