!     Last change:  WP   26 Aug 2005    9:38 am
!--------------------------------------------------------------------------
...
!**   -----------------------                                           
!**   - eb2ks(iprof,hv,rg,rg1,q,q1,itere1,nstat,hr,nknot,jw8)           
...
grenztiefe: do
  ...
  IF (lein.eq.3) then
    write (jw8, 2001) 'I', 'HR', 'FROUDZAHL', 'DELTA H', 'ABSCHNITT'
  ENDIF
  ...
  iteration: DO i = 1, itmax_grnzh
    ...
    !write (jw8,*) 'In GRNZH. Zeile 257, hr = ', hr

    CALL uf (hr, hi, xi, s, indmax, nfli, nfre)

    !write (jw8,*) 'In GRNZH. Zeile 260, fges = ', fges
    ...
    !write (jw8,*) 'In GRNZH. Zeile 310, vor call eb2ks.'
    ...
    !write (jw8,*) 'In GRNZH. Zeile 325, nach call erfroud. FROUD = ', froud


    IF (abs (froud-1.) .lt.1.e-3) then
                                                                        
      !**         SCHREIBEN IN KONTROLLFILE
      IF (lein.eq.3) write (jw8, 2002) i, hr, froud
    ...
    ELSE
      ...
      !write (jw8,*) 'HR=', hr, '  HRA=', hra, '  HRB=',hrb
      ...
      !write (jw8,*) 'In GRNZH. Zeile 353, vor call eb2ks.'
      ...
      IF (ifg.eq.1) then
        ...
        !**  BERECHNUNG NACH DARCY
        CALL eb2ks (iprof, hv, rg, rg1, q, q1, itere1, nstat, hra, nknot, jw8)
      ...
      !write (jw8,*) 'In GRNZH. Zeile 371, nach call erfroud. FROUDA = ', frouda
      ...
      !write (jw8,*) 'In GRNZH. Zeile 383, vor call eb2ks.'
      IF (ifg.eq.1) then
        nstat = 1
        itere1 = i
                                                                        
        !** BERECHNUNG NACH DARCY
        CALL eb2ks (iprof, hv, rg, rg1, q, q1, itere1, nstat, hrb, nknot, jw8)
      ...
      !write (jw8,*) 'In GRNZH. Zeile 400, nach call erfroud. FROUDB = ', froudb
      ...
      !**        SCHREIBEN IN KONTROLLFILE
      IF (lein.eq.3) then
        WRITE (jw8, 2003) i, hr, froud, dif, indfl
  ...
  IF (froudn .lt. 1.e-6 .and. froudn .gt. - 1.e-6) then
    ...
    !**    SCHREIBEN IN KONTROLLFILE
    IF (lein.eq.3) write (jw8, 2006) hgrenz, froud
  ...
  !**   SCHREIBEN IN KONTROLLFILE
  IF (lein.eq.3) then
    WRITE (jw8, 2007) 'I', 'HR', 'FROUDZAHL', 'DELTA H'
  ENDIF
  ...
  !**   SCHREIBEN IN KONTROLLFILE
  IF (lein.eq.3) then
    WRITE (jw8, 2008) ieb, hr, froud, dif
  ENDIF
  2008 format (1X, I3, 3F12.4)
                                                                        

  IF (abs (froud-1.) .lt.1.e-3) then
      IF (lein.eq.3) write (jw8, '(''grenztiefe im '',i3,''ten schritt b&
  ...
  ENDIF
      ...
      IF ( (froud-1.) * (froudn - 1.) .lt.0.) then 
        ...
        IF (abs (froud-1.) .lt.0.05) then 
          ...
          IF (lein.eq.3) then 
      WRITE (jw8, '(''abbruch der iteration in schritt '',i4,           &
     &     '' mit'',/,''froud= '',f12.4,''bei hr = '',f12.4)') ieb, frou&
     &d, hr                                                             
            WRITE (jw8, '(''abbruch der iteration : dif < 0.05'')') 
      WRITE (jw8, '(''--> aus grenztiefe mit hr = '',f12.4,/,           &
...
!**         SCHREIBEN IN KONTROLLFILE                                   
          IF (lein.eq.3) then 
            WRITE (jw8, '(''keine grenztiefe ermittelbar !!!'')') 
      WRITE (jw8, '(''--> weiter mit dem optimalsten '',                &
     &  ''wert:'',/,'' hr = '',f12.4,/,''froud = '',                    &
     &   f12.4,/)') hgrenz, froud                                       
          ENDIF 
          RETURN 
...
!         abspeichern der guenstigsten werte                            
 ...
 6060 IF (iterfr.lt.ianz) goto 6040 
                                                                        
!**    SCHREIBEN IN KONTROLLFILE                                        
      IF (lein.eq.3) then 
      WRITE (jw8, '(''keine grenztiefe gefunden !!! '',/,               &
...
!**   SCHREIBEN IN KONTROLLFILE                                         
      IF (lein.eq.3) then 
        WRITE (jw8, '(/,'' keine konvergenz --> andere iteration'')') 
      WRITE (jw8, '('' i    hr    froud    dif'')') 
      ENDIF 
...
!**              BERECHNUNG NACH STRICKLER                              
          CALL eb2ks (iprof, hv, rg, rg1, q, q1, itere1, nstat, hrb,    &
          nknot, jw8)                                                   
...
!**            SCHREIBEN IN KONTROLLFILE                                
          IF (lein.eq.3) then 
      WRITE (jw8, '('' keine grenztiefe bis rohrscheitel!! '',          &
...
!**        SCHREIBEN IN KONTROLLFILE                                    
        IF (lein.eq.3) then 
          dif = froud-1. 
          WRITE (jw8, '(i3,3(f12.4))') i, hr, froud, dif 
        ENDIF 
...
!**            SCHREIBEN IN KONTROLLFILE                                
            IF (lein.eq.3) then 
              ...
              WRITE (jw8, '('' --> anfang mit hr = '',f15.3)') hra 
            ENDIF 
...
!**   WO IST DAS ABBRUCHKRITERIUM, RETURN, LABEL 9999                   
end do grenztiefe
...
!**            SCHREIBEN IN KONTROLLFILE                                
      IF (lein.eq.3) write (jw8, '(''grenztiefe im '',i3,               &
     &                       ''ten schritt berechnet:''                 &
     &                 ,/,''hr = '',f6.2,1x,''bei froud '',f6.4)') i, hr&
     &, froud                                                           
...
!           print *,'wie ist das moeglich, das ich hierher komme !'     
!           print *,'stop in grnzh.f77 '                                
!           stop                                                        
