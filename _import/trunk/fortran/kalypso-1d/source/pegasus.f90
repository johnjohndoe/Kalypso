...
IF (ITERATIONSART=='EXACT ') then
  ...
  WRITE (UNIT_OUT_LOG, '('' in pegasus : '')')
  WRITE (UNIT_OUT_LOG, '('' gesuchte nullstelle zwischen '',f15.3,'' und '', f15.3,/,&
            & '' --> weiter mit hr = '',f15.3)') h1x, h2x, hr
  WRITE (UNIT_OUT_LOG, '('' f1 = '',f15.3,'' f2 = '',f15.3)') f1, f2
  WRITE (UNIT_OUT_LOG, '(//,'' iteration mittels pegasus-verfahren : '',//,  &
...
DO 100 jsch = 1, 50
  ...
  ELSE
    ...
    ! **********************************************************
    !JK           SCHREIBEN IN KONTROLLFILE
    WRITE (UNIT_OUT_LOG, '(6f15.3)') hr, hrneu, h1x, f1, h2x, f2
    ...
    ELSE
      ...
      ELSE
        hr = h1x
        !JK                 SCHREIBEN IN KONTROLLFILE
        WRITE (UNIT_OUT_LOG, '('' wsp gefunden : hr= '',f15.3,  &
          &       '' hrneu = '',f15.3)') hr, f1
...
ELSE
  ...
  !JK      SCHREIBEN IN KONTROLLFILE
  WRITE (UNIT_OUT_LOG, '('' keine konvergenz in pegasus '')')
