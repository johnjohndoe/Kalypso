...
CHARACTER(LEN=nch80) :: file_laengs     ! Variable f¸r Laengschnitt.txt
...
!MD  Q_Abfrage = 'IN_SCHLEIFE'  : Aueﬂere Abflussschleife am Bauwerke bzw. Berechnung
...
Hauptschleife: DO i = 1, maxger
  ...
  !WP Wenn gelesene Station grˆﬂer als Endstation, dann Programm beenden
  ...
  IF (BERECHNUNGSMODUS /= 'BF_UNIFORM') then
    ...
    ELSE IF (ibridge.eq.'b' .and. MIT_BRUECKEN) then
     ...
     !MD - NOCH in Arbeit ----------------------------------------------------------
      ...
      IF (BERECHNUNGSMODUS == 'REIB_KONST') then
        ...
        !MD  Sichern der Werte aus der ‰usseren Abflussschleife
        isch_out = isch
        nr_q_out = nr_q
        q_out = q

        !MD  Belegung der neuen Werte f¸r die innere Abflussschleife
        qvar_in = MIN_Q
        in_count = 1
       ...
       !MD  CYCLE Hauptschleife
            ...
            write (UNIT_OUT_QBRUECKE, 912) out_Qin_PROF(nprof-1,nr_q_out,nr_q)%stat,                     &
                             & out_Qin_PROF(nprof,nr_q_out,nr_q)%qges, q_out, out_Qin_PROF(nprof,nr_q_out,nr_q)%wsp,  &
      ...
      !----------------------------------------------------------------
        ...
        !MD  Rueckholen der Werte fuer die ‰ussere Abflussschleife
        isch = isch_out
        nr_q = nr_q_out
        q = q_out

        !MD zuerst: Profilnummer zur¸ck !!!!!!
        nprof = nprof_save
    ...
    ELSEIF (iwehr.eq.'w'.and. MIT_WEHREN) then
      ...
      !MD  Neu !!   Neu !!    Neu !!
      !MD  Interne-Schleife f¸r Bauwerke ueber alle Abfluesse mit einem Unterwasserstand
      ...
      IF (BERECHNUNGSMODUS == 'REIB_KONST') then
        ...
        WRITE (UNIT_OUT_WEHR, '(5x,'' Q_OW '',6x,''h_ow'',5x,''v_ow'',7x,''he_ow'',3x,''mue'',6x,  &
               &''Q-wehr'',2x,''h-ue'',5x,''A-ue'',4x,''‹-Art'',13x,''he-wehr'',4x,''h_uw'',5x,''v_uw'',7x,''he_uw'')')
        ...
        DO isch = 1, anz_q
          ...
          ! -----------------------------------------------------------------------------------
          ! Erzeuge Pfad- und Dateinamen f¸r 'Laengs_QWehre.txt'
