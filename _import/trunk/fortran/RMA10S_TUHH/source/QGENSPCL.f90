!     Last change:  WP    8 Nov 2007   11:11 am
!ipk  last update sep 04 2004 add error message
!IPK  LAST UPDATE DEC 10 2002 rearrange boundaries near the inflow level, reset magnitude
!ipk  last update feb 09 2001 allow for zero length cc lines
      ...
      SAVE
!-
!-..... Generate specified total flow boundary conditions for 3-d layer
!-
      DIMENSION QQAL(3)
!-
      ALLOCATABLE IQSPCL(:)
...
!
         ...
         ENDIF
!
!ipk oct98         VM = SQRT(VEL(1,NA)**2+VEL(2,NA)**2)
!
!         VEL(1,NA)=VM*COS(BEETA)
!         VEL(2,NA)=VM*SIN(BEETA)
      ELSE
!-
...
!
               DO 630 K=N1,NV
                  IF (IQSPCL(K) == 1)  THEN
                     SPEC(K,1) = QFCTR * Q1
!FES   1/16/95                     SPEC(K,2) = QFCTR * Q2
!FES   1/16/95 NEXT LINE ADDED AS PER R.RACHIELE RECOMMENDATION
                     SPEC(K,2) =  Q2
!ipk dec02
