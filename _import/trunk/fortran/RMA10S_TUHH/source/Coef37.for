...
cipk mar94 end change
        ...
        if(drdyc .lt. -1.e-8) dfact=10.
        if(drdyc .lt. -1.e-7) dfact=100.
      ENDIF
      if(maxn .eq. 1  .and. ntx .eq. 1) then
        DFCT(NN)=DFACT
cipk nov99 refine test to catch zero dfct
      elseif(dfct(nn) .eq. 0.) then
        dfct(nn)=dfact
      else
        dfact=dfct(nn)
      endif
...
CC      IF(ISLP .EQ. 1  .AND.  (IVLP .EQ. 0  .AND.  IHLP .EQ. 0)) THEN
...
CIPK JAN97      ELSEIF(IEDSW .EQ. 4) THEN
      ELSEIF(IEDSW .EQ. 4  .OR. IEDSW .EQ. 2) THEN
...
C-
  ...
  135 CONTINUE
      AMW = WAITX(I) * DETJ
      AREA(NN)=AREA(NN)+AMW
CIP NOV97      IF(NTX .EQ. 0) GO TO 500
C-
...
c
CIPK NOV97
      IF(IOPTZD .EQ. 3  .AND. NTX .EQ. 1) THEN
...
cc          IF(NN .EQ. 1) WRITE(76,*)'MKTEMP',GRATE,SRCSNK,GRATE*S+SRCSNK
        ELSE
          GRATE=0.
          SRCSNK=0.
        ENDIF
...
cipk jan97      FRN=AMW*(H*(U*DSDX+V*DSDY)+WUT*DSDZ+DIFX*DHDX*DSDX+DIFY*DHDY*DSDY)
     ...
     +    -AMW*SIDF(NN)*(SIDQ(NN,ICK-3)-S)
      IF(SIDF(NN) .EQ. 0.  .AND.  SIDQ(NN,ICK-3) .GT. 0.) THEN
