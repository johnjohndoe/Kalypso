...
c     Skip out if inactive point
         ...
         IF(NN < NVERTM) THEN 
           DO KK=NN+1,NVERTM 
             K=NKEY(KK) 
             IF(K /= 0) THEN 
               IF(XUSR(N) == XUSR(K)) THEN 
                 IF(YUSR(N) == YUSR(K)) THEN 
                   NKEY(KK)=0 
                 ENDIF 
               ELSE 
                 GO TO 200 
...
cIPK JAN94      INTEGER*2 NKEY                                          
  ...
  200 I = IB 
  210 K = I + ID 
  220 IF( A(NKEY(I)) <= A(NKEY(K)) ) GO TO 250 
      NKT = NKEY(K) 
      NKEY(K) = NKEY(I) 
      J = I 
  230 K = J - ID 
      IF( K < 1 ) GO TO 240 
      IF( A(NKT) > A(NKEY(K)) ) GO TO 240 
      NKEY(J) = NKEY(K) 
      J = K 
      GO TO 230 
  240 NKEY(J) = NKT 
...
c     Setup to form new triangles                                       
      ...
      DO J=1,NEDGE 
        IF(J < NEDGE) THEN 
          DO K=J+1,NEDGE 
            IF(IEDGE(1,K) == IEDGE(1,J)) THEN 
              IF(IEDGE(2,K) == IEDGE(2,J)) THEN 
                IEDGE(1,J)=0 
                IEDGE(2,J)=0 
                IEDGE(1,K)=0 
                IEDGE(2,K)=0 
              ENDIF 
            ELSEIF(IEDGE(1,K) == IEDGE(2,J)) THEN 
              IF(IEDGE(2,K) == IEDGE(1,J)) THEN 
                IEDGE(1,J)=0 
                IEDGE(2,J)=0 
                IEDGE(1,K)=0 
                IEDGE(2,K)=0 
