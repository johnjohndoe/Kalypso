!     Last change:  NIS  15 May 2008    8:35 pm
!ipk  last update FEB 26 2007 REFINE TEST FOR SUBMERGENCE
!ipk  last update nov 28 2006 allow for 1-d control structures
!ipk  last update JUNE 22 2005 ADD TO rma10
!ipk  last update JULY 30 2004 fix bug in 2-d/1-d test
!IPK  LAST UPDATE JUL 7 2004 ALLOW FOR 1-d ELEMENTS RESTORE TRANSELV 
!ipk  last update Dec 13 2000 correct for types other 10
      SUBROUTINE SUBSET
!-
!
      USE BLK10MOD
...
!
!     This indicates a boundary node
!
            ISUBM (N) = 0
          ELSE
            ISUBM(N)=-1
...
!
!IPK JUL04 Use TRC to allow for units
      IF(GRAV > 30.) THEN
        TRC=0.25
      ELSE
        TRC=0.1
...
!IPK JUN05
!ipk outnov06           IF(NCTREF(IMAT(NN)) == 0) THEN
!ipk dec00
!
!tabular data control structure element            
            if (njt (imat (nn) - 900) == 10) then
!ipk outnov06              if(maxn /= 1) go to 332
!ipk outnov06              ISUBMEL(nn)=0
!ipk jul04
...
!
!IPK JUL04            IF(WSLL(NOP(NN,1)) < WHGT(NOP(NN,1))+.35) THEN
...
!
!IPK JUL04            IF(WSLL(NOP(NN,3)) < WHGT(NOP(NN,3))+.35) THEN
...
!
!IPK JUL04            IF(WSLL(NOP(NN,5)) < WHGT(NOP(NN,5))+.35) THEN
...
!
!IPK JUL04            IF(WSLL(NOP(NN,7)) < WHGT(NOP(NN,7))+.35) THEN
...
!
                  ...
                  ENDIF
!
!          set isubm for all nodes around a weir element
                  DO L=1,NCORN(NN)
!ipk oct00 
                  ...
                  ENDDO
                  ISUBMEL(nn)=1
!IPK NOV06 ISUBMEL =1 MEANS SUBMERGED
                  GO TO 332
!
...
!
  332           CONTINUE
!             WRITE(75,*) 'SUBSET2',NN,ISUBMEL(NN),ISUBM(24)
!
...
!
      ENDDO
!
!ipk oct00  reset to for submerged elements
!
      do n=1,ne
      ...
      enddo 
!       IF(MAXN == 1) THEN
!         WRITE(145,*) 'IBN AND ISUBM'
!         DO N=1,NP
!           WRITE(145,'(3I8,F12.3)') N,IBN(N),ISUBM(N),ALFA(N)   
!         ENDDO
!       ENDIF
!
      RETURN
