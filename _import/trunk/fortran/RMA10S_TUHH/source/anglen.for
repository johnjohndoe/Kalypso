...
CIPK DEC00
        IF (IMAT(LL) .GE. 900  .AND. IGTP(LL) .EQ. 0)  GOTO 90
C
          N1 = NOPS(LL,1)
          N3 = NOPS(LL,3)
          X(1) = CORD(N1,1)
...
C.... DETERMINE LENGTH AND ANGLE OF MAJOR AXIS
      IF(NOP(LL,7) .EQ. 0) THEN
C.... TRIANGLES
         IF(L1.EQ.1 .AND. L2.EQ.3) THEN
            XX = X(1) - (X(3)+X(5))/2.
            YY = Y(1) - (Y(3)+Y(5))/2.
         ELSEIF(L1.EQ.1 .AND. L2.EQ.2) THEN
            XX = X(3) - (X(1)+X(5))/2.
            YY = Y(3) - (Y(1)+Y(5))/2.
         ELSE
            XX = X(5) - (X(1)+X(3))/2.
...
C.... QUADRILATERALS
         IF(L1.EQ.1 .AND. L2.EQ.2) THEN
            XX = X(3) - (X(1)+X(5))/2.
            YY = Y(3) - (Y(1)+Y(5))/2.
         ELSEIF(L1.EQ.2 .AND. L2.EQ.3) THEN
            XX = X(5) - (X(3)+X(7))/2.
            YY = Y(5) - (Y(3)+Y(7))/2.
         ELSEIF(L1.EQ.3 .AND. L2.EQ.4) THEN
            XX = X(7) - (X(1)+X(5))/2.
            YY = Y(7) - (Y(1)+Y(5))/2.
         ELSEIF(L1.EQ.1 .AND. L2.EQ.4) THEN
            XX = X(1) - (X(3)+X(7))/2.
            YY = Y(1) - (Y(3)+Y(7))/2.
         ELSEIF(L1.EQ.1 .AND. L2.EQ.3) THEN
...
C
      ANG=ATAN2(YY,XX)
cccc      IF(EEXXYY(1,LL).LT.0.) THEN
         XLA=SQRT(XX**2+YY**2)
         YLA=AREA(LL)/XLA
         IF(NOP(LL,7) .EQ. 0) YLA=YLA*2.
         IF(XLA.LT.YLA) THEN
            TA=AREA(LL)
            IF(NOP(LL,7).EQ.0) TA=TA*2.
            XLA=SQRT(TA)
            YLA=XLA
            ANG=0.
         ENDIF
...
c          t2 = 02.5 * yla/tt
c
c          if (mod(imat(ll),1000) .eq. 3)  then
c             t1 = 01.0 * xla/tt
c             t2 = 01.0 * yla/tt
c          endif
c
c
c
         IF(EEXXYY(1,LL).LT.T1) EEXXYY(1,LL)=T1
         IF(EEXXYY(2,LL).LT.T2) EEXXYY(2,LL)=T2
         IF(EEXXYY(3,LL).LT.T1) EEXXYY(3,LL)=T1
         IF(EEXXYY(4,LL).LT.T2) EEXXYY(4,LL)=T2
         IF(EEXXYY(5,LL).LT.T3) EEXXYY(5,LL)=T3
         IF(EEXXYY(6,LL).LT.T3) EEXXYY(6,LL)=T3
C
C  Convert for metric problems
C
         IF (GRAV .LT. 32.)  THEN
