!     Last change:  WP   22 Jul 2008    5:05 pm
!ipk  last update sep 19 2003 switch units for col.dat
!IPK  LAST UPDATE JAN 15 2002 ALLOW FOR FLOWS INTO COLLAPSING ELEMENTS
!     New routine November 12 1999
!
      SUBROUTINE TWODSW
      USE BLKHMOD
      USE BLK10MOD
...
!
!
!     Routine to detect threed elements below threshold for wetting/drying
!
!-
!     Work through all the surface nodes      
!-
      DO N=1,NPM
!
!     Test for 3-D
...
!
!
!     Establish threshold if it is not defined
!
        IF(TRANSIT == 0) THEN
          TRANSITT= ADT(N)-(ADB(N)+ADT(N))/2.*(1.-AKP(N))
!          if(n == 1) write(75,*) ' transit=',transitt
        ELSE
          TRANSITT=TRANSIT
        ENDIF
!
!     Is depth below the threshold
!
          IF(VEL(3,N) < TRANSITT) THEN
!
!     Test whether collapse already active, if so skip by
!
            IF(ICOLLAP(N) == 1) GO TO 300
!
!     We have a candidate to set collapse switch and integrate the velocities
!
            CALL AVEPARAM(N,1)
          ELSE
            K=NREF(N)
  ...
  300   CONTINUE
      ENDDO
!
!     Now set up elements to be collapsed 
!
      DO N=1,NE
!IPK JAN02  INITIALIZE SIDFF
        SIDFF(N)=0.
        IF(SIDF(N) /= 0.) THEN
          WRITE(75,*) 'N,SIDF(N),TVOL(N)',N,SIDF(N),TVOL(N)
        ENDIF
!
!     Intialize   ICOLLAPE=1 says this element is connected 
!     to node with depth below TRANSIT
!
        ICOLLAPE(N)=0
        IF(IMAT(N) > 0) THEN
          NCN=NCORN(N)
!
!     Loop on the nodes looking for  all depths below TRANSIT
!
!IPK APR01 ADD BETTER TEST FOR SIDE ELEMENTS
          ICNT=0
          IF(NETYP(N) == 13 .OR. NETYP(N) == 14) THEN
            IF(ICOLLAPE(NRELSF(N)) == 1) THEN
              ICOLLAPE(N)=1
            ENDIF
          ELSE
            DO M=1,NCN
              NN=ABS(NSURF(NOP(N,M)))
              IF(ICOLLAP(NN) == 1) THEN
                ICNT=ICNT+1
              ENDIF
            ENDDO 
!rrr MAY00          IF(ICNT > 0) THEN        
! if already depth averaged, skip
            IF (ICNT > 0     .AND.                                      &
     &         (netyp(n) /= 15 .AND. netyp(n) /= 16 .AND.               &
     &          netyp(n) /= 17 .AND. netyp(n) /= 6  .AND.               &
     &          netyp(n) /= 7 .AND. netyp(n) /= 8) ) THEN
!
              ICOLLAPE(N)=1
!
!ipk jan02  Look for side flows to translate to surface element
!
...
!
     ...
     &       voldx,voldy,polds,poldt,poldu,vdotx,vdoty,pdots,pdott,     &
     &         pdotu,vdotox,vdotoy,pdotos,pdotot,pdotou,dep1
      ...
      K=NREF(N)
      L=NREF(N)+NDEP(N)-2
!      write(78,*) n,nsurf(n),k,l,depscl,depsclo
      IF(NSURF(N) < 0) THEN
        LL=1
      ELSE
        LL=2
...
!
          ...
          DEPSUM=DEPSUM+DEP1
!          write(78,*) 'aveparm',mm,m1,m2,vel(4,mm),vel(4,m1),vel(4,m2),
!     +            vel(3,mm),vel(3,m2),dep1
     ...
     &        (VDOTO(6,MM)+4.*VDOTO(6,M1)+VDOTO(6,M2))/6.*DEP1
      ...
      DO M=K,L
        ...
        ICOLLAP(MM)=ICSW
!          write(78,*) 'aveparm-2',mm,vel(4,mm),vel(3,mm)
!
      ENDDO
!
      RETURN
