!IPK  LAST UPDATE MAR 05 2006 USE AND INPUT NODAL PROPS
...
!*******************      
!
!-ipk   This subroutine reads in all rates for sand
!
!IPK MAR06
      INTEGER ISMTREF(100)
      REAL  SDT(100)
...
!IPK MAR06
      ...
      CONP=1.-POSA
!
!     Initialize METRIC to 0 for english units
!
      METRIC = 0
!IPK MAR06 MOVED      LBASE=0
      CLABL ='   SAND '
!IPK MAR06
      CALL KINVIS
!     Sand is simulated read global parameters
!
!      IF(LSAND > 0) THEN
!
      CALL GINPT(LIN,ID,DLIN)
!
!IPK MAR06 ADD LOOP TO GET ELEMENT TYPE DATA
  250 CONTINUE
      LBASE=0
      N1=1
      N9=9
...
!
          READ(DLIN,1020)                                               &
!IPK MAR06     +    SACLL,SACUL,ASACI,SGSA,GSF,CLDE,CLER,VSAND,AMANN
...
!        ENDIF
        NSACI=ASACI+0.5
!
!IPK AUG00 PRINT SAND SETTLING LATER
!
...
!-
!IPK FEB95        VSAND=VSAND/86400.
!IPK DEC05
!IPK MAR06 MOVED        POSA=0.4
!IPK MAR06 MOVED        CONP=1.-POSA
!
        CALL GINPT(LIN,ID,DLIN)
!
        IF(ID(1:3) == 'DIA') THEN
...
! HN 23 Jul 2009: Reading polynomial factors , describing pore pressure distribution.         
        ...
        ENDIF  
!
!IPK MAR06        RHOBS=1000.*CONP*SGSA
!
        ...
        DO 1100  I=1,NSACI
          ISAND=I+LBASE
!IPK MAR06 UPDATE FOR ELEMENT VALUES
          SDT(ISAND)=GMSAC
          GMSAC=GMSAC*SACI
 1100   CONTINUE
        LBASE=LGS
...
!
!
!IPK MAR06 COMPUTE SETTLING VELOCITY IF NOT ENTERED
!
        D50M=D50T/1000.
        IF(VSANDT == 0.) THEN
          RNU=XNU(1)
...
!ipk mar06     
!      write(lout,'(8x,''Maximum allowed sand potential'',e15.4)') gpmaxt
        DO N=1,NE
!        IF(IMAT(N) > 0) THEN 
!HN 6.07.2009 use initial values for all elements and not only for active ones.
          IF(IMAT(N) /= 0) THEN       
          DO I=1,NLMTP
  ...
  350     CONTINUE          
        ...
        GO TO 250
      ENDIF
!IPK MAR06 MOVED        CALL KINVIS
!
!IPK MAR06        CALL GINPT(LIN,ID,DLIN)
!
!IPK MAR06 MOVED        IF(ID(1:3) == 'DIA') THEN
!IPK MAR06 MOVED          READ(DLIN,1030) d35,d50,d90,ismode,GPMAX
!IPK MAR06 MOVED          if(gpmax == 0.) gpmax=1.e+6
!IPK MAR06 MOVED        ELSE
!IPK MAR06 MOVED          WRITE(75,*) 'EXPECTED DIAMETER DATA - NONE FOUND'
!IPK MAR06 MOVED          WRITE(*,*) 'EXPECTED DIAMETER DATA - NONE FOUND'
!IPK MAR06 MOVED          STOP
!IPK MAR06 MOVED        ENDIF
!
!
!IPK MAR06 END CHANGES
!
        DO L=1,NMAT
          RCAE(L)=0.
          RWAE(L)=0.
...
!IPK MAR06
 ...
 6001     FORMAT(/10X,'ELEMENT TYPE',I10/                               &
     ...
     &            10X,'WAVE    RELATED ROUGHNESS',F24.4,' (M)')
        ...
        ENDIF
!
!IPK JUN02 DISTRIBUTE ROUGHNESS TO NODAL VALUES
!
        DO N=1,NE
          IF(IMAT(N) > 0) THEN
            DO L=1,NCORN(N)
...
!ipk mar06      write(lout,'(8x,''Maximum allowed sand potential'',e15.4)') gpmax
!wlp jul96 end additions      
!      END IF
!
  200 CONTINUE
