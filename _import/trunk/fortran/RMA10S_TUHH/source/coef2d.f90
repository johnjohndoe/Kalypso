...
!ipk  last update feb 26 2006 correst logic for loads from 2-d structures
...
!     Last change:  MD    9 Jun 2009    2:05 pm
...
!  N*DNY DU
      T3=AMS*H*S
!ipk jan97     +  -H/XHT*(DAODZ*(EPSXZ+(DRDZ+DSDX)*C2)+DRDX*DAODX*2.*C2))
! N*N  DV
!IPK MAR01 ADD DRAG      T4=AMS*(H*(DRDZ-OMEGA)+TFRIC*UBF*R*S*UBF*VBF)
...
! N*NX  DV
      T8=0.
!ipk jan97      T8=-AMS*H/XHT*(DAODZ*(EPSXZ+C3*(DRDZ+DSDX))+DAODX*DRDX*C3*2.) 
! N*NY  DV
      T8A=0.
!ip jan97      T8A=-AMS*H/XHT*(DAODZ*C4*(DRDZ+DSDX)+ DAODX*DRDX*C4)
! NX*NX DV
      T9= AMS*2.*H*DRDX*C3
! NX*NY DV
      T9A = AMS*2.*H*DRDX*C4
...
!
!ipk jan97     1  -AMT*(EPSX*DRDX*DAODX+EPSXZ*(DRDZ+DSDX)*DAODZ)
!  NY*M DH
...
! N*NX DV
      T2=AMS*AKY*H*R
!ipk jan97     +  -H/XHT*(DAODX*(EPSZX+(DRDZ+DSDX)*C3)+DAODZ*DSDZ*2.*C3))
! N*NY DV
      T3=AMS*H*S
!ipk jan97     +  -H/XHT*DAODZ*(EPSZ+DSDZ*2.*C4)+DAODX*(DRDZ+DSDX)*C4)
! N*N  DU
...
! NY*NX DV
      T6A=AMS*H*C3*2.*DSDZ
! N*NX DU
!ipk jan97      T7=-AMS*H/XHT*(DAODX*C1*(DRDZ+DSDX) +DAODZ*DSDZ*2.*C1)
      T7=0.
! N*NY DU
!ipk jan97      T7A=-AMS*H/XHT*(DAODX*(EPSZX+(DRDZ+DSDX)*C2)+DAODZ*DSDZ*2.*C2)
      T7A=0.0
! NX*NX DU
...
!
!ipk jan97     1  -AMT*(EPSZX*(DRDZ+DSDX)*DAODX+EPSZ*DSDZ*DAODZ)
!  NX*M DH
