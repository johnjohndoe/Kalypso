!     Last change:  MD   30 Jul 2009    2:04 pm
!IPK  LAST UPDATE APR 20 2006 ADD INPUT OPTION FOR TLAYM
!IPK  LAST UPDATE NOV 19 1997
!rrr  last update Aug 30 1997 test for max number of layers
!ipk  last update Aug 6 1997 change format
!IPK  LAST UPDATE JUNE 2 1997
!IPK  LAST UPDATE APR 7 1997 FIX LABEL AND FORMAT 
!ipk  last update June 4 1996
      SUBROUTINE SPROP
!-
      USE BLK10MOD
      USE BLKSEDMOD
...
!
!ipk nov97 add alternate reading logic
      CHARACTER (LEN = 8) ::  IDA
...
!      EXIST IN THE SYSTEM AND TRANSFERS.
!      THE INITIAL BED PROFILE AND BED PROPERTIES ARE READ IN BY
!      SUBROUTINE BED.
!
!******************************************************************
!
!ipk  set IBNK =0 for the moment until bank shear is implemented
!
...
!MD: Parameter auch fuer momentan trockene Elemente und Knoten!!        
        IF(IMAT(N) /= 0 .AND. ISVL == 3) THEN
          DO I=1,VSETMAX
            IF(NI_MAT(I) == ABS(IMAT(N)) .OR. NI_MAT(I) == 0) THEN
              NIMAT = NI_MAT(I)
              DO JJ=1,NCORN(N)
                J=NOP(N,JJ)
            ...
            Endif
          EndDo
        ElseIF(IMAT(N) /= 0 .AND. ISVL == 2) THEN
          DO JJ=1,NCORN(N)
             J=NOP(N,JJ)
             CRCON1(J)=CRCON1(1)
             CRCON2(J)=CRCON2(1)
...
!
!..................................................................
!...      NEW BED PROPERTIES. SAME DRY MASS IN EACH LAYER.
!.................................................................
      CALL GINPT(LIN,ID,DLIN)
      NN=0
!
...
!MD: Daten ueber alle Boden-Layer L      
      DO L=1,NLAYTM 
      IF(ID(1:4) == 'CSS4') THEN
!IPK APR06 ADD INPUT OPTION FOR TLAYM
!IPK MAY06   ADD EROSTM
...
!MD:  Layer auch fuer momentan trockene Elemente und Knoten!!        
        IF(IMAT(N) /= 0) THEN
          DO I=1,NLMTP
            IF(IMTREF(I) == 0 .AND. I > 1) GO TO 350
...
!
      ...
      Endif
!
!ipk June 1996      IF(IBNK /= 0) THEN
!ipk June 1996        READ(IIQ,5010) BNKCD,BNKGAB,BNKERC,BRKS
!ipk June 1996        WRITE(LOUT,6030) BNKCD,BNKGAB,BNKERC,BRKS
!ipk June 1996      ENDIF
!
      IF(ID(1:6) /= 'ENDSED') THEN
...
!ipk aug97 change format 6025 FORMAT(I13,F19.2,F24.2,F20.3,F20.2)
...
! 6032 FORMAT(1H1,30X,'SEDIMENT PROPERTIES'/)
!
 6033 FORMAT(//10X,'SETTLING VELOCITIES...'///20X,                      &
!IPK APR 97 FIX LABEL AND FORMAT
