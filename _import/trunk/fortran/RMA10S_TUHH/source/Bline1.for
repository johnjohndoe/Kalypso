C     Last change:  JAJ  12 Jun 2006   10:22 pm
...
cipk  last update Sep 3 1996   Add ocean boundary capability
      SUBROUTINE BLINE(NTR) !NiS,may06:
...
C
      ...
      IOD(N)=0
  180 CONTINUE
      !NiS,may06: during iteration NTR.gt.1
...
C-
        !NiS,may06: 2D/3D-part: transition elements with normally ncn.eq.5 are here declared with ncn.eq.3, so only 1D
            IF(NCN .GT. 3) THEN
...
C-
      DO 800 N=1,NEM
        ...
        IF(IMAT(N) .LT. 901  .OR.  IMAT(N) .GT. 5000) THEN
          ...
          N3=NOPS(N,5)
!NiS,may06: testing
!          write(*,*) 'nodes: ', N1,n2,n3
!          WRITE(*,*) 'angles: ', alfa(n1), alfa(n2), alfa(n3)
!-
          ...
          ENDIF
!NiS,may06: testing
!          WRITE(*,*) 'adifs: ', adif(n1), adif(n2), adif(n3)
!          WRITE(*,*) 'ntr: ', ntr
!-
          IF(NTR .GT. 0) THEN
            K=NREF(N2)+1
