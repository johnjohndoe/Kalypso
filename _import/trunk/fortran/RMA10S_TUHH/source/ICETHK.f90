...
!
      ...
      ELSE
        ...
        ENDDO
!
!     Loop on the surface nodes, using the elements
!       
        DO J=1,NEM
          if(imat(j) > 0) then
            nmt=mod(imat(j),100)
          ...
          DO MM=1,7,2
            ...
            IBNTHER(N)=1
!
!     First get a simplified average velocity and water temperature
!       
     ...
     &                +(VEL(5,M+1)+VEL(5,M-1))/6.)
            ...
            TMN=TMN/FDIV
!
!         test for appropiate equation and solve for QW
!
!            write(172,'(i5,f8.2,2i5,4f15.6)') 
!     +          dayofy,tet,maxn,n,icethk(n),vel(5,n),tmed,tmn
...
!                tslop=(-VEL(5,N)+vel(5,nmid+1))
!     +          /(cord(n,3)-cord(nmid+1,3))
!                nbot=ndep(n)+nref(n)-1
!                tslop=(-VEL(5,N)+vel(5,nbot))
!     +          /(cord(n,3)-cord(nbot,3))
              ELSE
                tslop=(-3.*tmed+4.*vel(5,nmid)-vel(5,nmid+1))           &
     &          /(cord(n,3)-cord(nmid+1,3))
!                tslop=(-tmed+vel(5,nmid+1))
!     +          /(cord(n,3)-cord(nmid+1,3))
!                nbot=ndep(n)+nref(n)-1
!                tslop=(-tmed+vel(5,nbot))
!     +          /(cord(n,3)-cord(nbot,3))
              ENDIF
              ...
              ELSE
                ...
                QICE(N)=-QW/1000.
!
!           Note that qice is in units of KJ/m2/sec
!
              ENDIF
            endif
!
...
!
!            write(174,'(i5,f8.2,2i5,4f15.8)') 
!     +          dayofy,tet,maxn,n,dhdt*1000000.,newthk,cdhdt,tslop
!
            IF(HTR == 0.) THEN
...
!
!            write(174,'(i5,f8.2,2i5,3f15.6)') 
!     +          dayofy,tet,maxn,n,dhdt*1000000.,newthk,cdhdt0
!
            IF(NEWTHK > 0.) THEN
              ICETHK(N)=NEWTHK
            ELSE
...
!c      n2=nops(j,2)
!c      n3=nops(j,3)
!c      WRITE(175,'(''ICETHK'',i5,f7.1,6i5)') 
!c     +   DAYOFY,TET,maxn,j,n1,n2,n3,itpas
!c      WRITE(175,'(3f15.8,f8.3,3f15.8,3f8.3)') 
!c     +ICETHK(n1),ICETHK(n2),icethk(n3),TMN,Qice(n1),Qice(n2),Qice(n3)
!c     + ,vel(5,n1),vel(5,n2),vel(5,n3)
        ENDDO
      ENDIF
      RETURN
      END
