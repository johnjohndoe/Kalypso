...
!ipk  last update apr 05 2006   make a first call to get sed initial conditions
!rrr  Last update Aug 23 1997
      SUBROUTINE BED
      USE BLK10MOD
      USE BLKSEDMOD
      USE BLKSANMOD
...
!**********************************************************************
!
!       THIS SUBROUTINE READS THE INITIAL BED PROFILE AND ELEVATION DATA 
!
!
!*******************************************************************
!-
...
!
      IF(NB == 0) GO TO 10
!
!ipk aug05 update to allow redefinition of bed
!
...
!MD: TM = Total Mass [kg/m²] ueber alle Layer        
      ENDDO
!ipk apr06   make a first call to get sed initial conditions
!
...
!
   10 CONTINUE
!IPK AUG05      IF(NB == 0) THEN
!IPK SEP05        CALL GINPT(LIN,ID,DLIN)
!IPK AUG05      ENDIF
        IF(ID(1:3) == 'CB ') THEN
...
!-
        ...
        IF(NL > 0) THEN
          ...
          WRITE(LOUT,6015)
          DO I=1,NL
!rrr aug97 update test to 3 characters
            IF(ID(1:3) /= 'CBL') THEN
...
!
!   -------------------------------------------------
!     READ INITIAL BED ELEVATION DATA
!   -------------------------------------------------
      IF (IELEV == 1) THEN
