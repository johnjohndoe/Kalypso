...
CIPK MAY04 RESET ELEMENT INFLOW
     ...
     +            + (cord(n3,2) - cord(n1,2))*SIN(thnn))
      ...
      do k=1,13
        !A(h)
        ah(n1) = ah(n1) + apoly(n1,k) * vel(3,n1)**(k-1)
        ah(n3) = ah(n3) + apoly(n3,k) * vel(3,n3)**(k-1)
        !QSch(h)
        qh(n1) = qh(n1) + qpoly(n1,k) * vel(3,n1)**(k-1)
        qh(n3) = qh(n3) + qpoly(n3,k) * vel(3,n3)**(k-1)
      end do
      ...
      !Sf
      sfnod(n1) = sfwicht(n1) * qhalt(n1)
     +          * ABS(qhalt(n1)) / (qh(n1)**2) * qgef(n1)
      sfnod(n3) = sfwicht(n3) * qhalt(n3)
     +          * ABS(qhalt(n3)) / (qh(n3)**2) * qgef(n3)
     ...
     +             dqdtaltzs(nn,2)
      ...
      do k=2,13
        ...
        dahdh(n3) = dahdh(n3) + (k-1) * apoly(n3,k) * h3**(k-2)
        !dQSch(h)/dh
        dqhdh(n1) = dqhdh(n1) + (k-1) * qpoly(n1,k) * h1**(k-2)
        dqhdh(n3) = dqhdh(n3) + (k-1) * qpoly(n3,k) * h3**(k-2)
      end do
      ...
      do k=3,13
        ...
        d2ahdh(n3) = d2ahdh(n3) + (k-1) * (k-2) * apoly(n3,k)*h3**(k-3)
        !d2QSch(h)/dh2
        d2qhdh(n1) = d2qhdh(n1) + (k-1) * (k-2) * qpoly(n1,k)*h1**(k-3)
        d2qhdh(n3) = d2qhdh(n3) + (k-1) * (k-2) * qpoly(n3,k)*h3**(k-3)
      end do
...
cipk nov97
      ...
      qqint(nn,i)   = qhalt(n1)  * xm(1)  + qhalt(n3)  * xm(2)
!      qqintdx(nn,i) = qhalt(n1)  * dmx(1) + qhalt(n3)  * dmx(2)
      ...
      aint2(i)      = 0.0
      do j = 1, 13
        aint1(i) = aint1(i) + apoly(n1, j) * hhint(nn,i)**(j-1)
        aint2(i) = aint2(i) + apoly(n3, j) * hhint(nn,i)**(j-1)
      enddo
     ...
     +                + (j-1) * apoly(n1,j) * hhint(nn,i)**(j-2)
       daintdh2(i) = daintdh2(i)
     +                + (j-1) * apoly(n3,j) * hhint(nn,i)**(j-2)
      end do
        dareaintdh(nn,i) =
     ...
     +         + (j-1) * (j-2) * apoly(n1, j) * hhint(nn,i)**(j-3)
        d2aintdh2(i) = d2aintdh2(i)
     +         + (j-1) * (j-2) * apoly(n3, j) * hhint(nn,i)**(j-3)
      end do
        d2areaintdh(nn,i) =
...
!     +                  + xm(2) * dahdh(n3)))
     ...
     +                  * dhintdt(nn,i)
      ...
      dvintdx(nn,i)  = dmx(1) * qhalt(n1) / ah(n1)
     +               + dmx(2) * qhalt(n3) / ah(n3)
     ...
     +              + qpoly(n1,j) * hhint(nn,i)**(j-1)
        qschint2(i) = qschint2(i)
     +              + qpoly(n3,j) * hhint(nn,i)**(j-1)
      end do
     ...
     +               + (j-1) * qpoly(n1,j) * hhint(nn,i)**(j-2)
        dqsintdh2(i) = dqsintdh2(i)
     +               + (j-1) * qpoly(n3,j) * hhint(nn,i)**(j-2)
      end do
     ...
     +            + (j-1) * (j-2) * qpoly(n1,j) * hhint(nn,i)**(j-3)
      d2qsidh2(i) = d2qsidh2(i)
     +            + (j-1) * (j-2) * qpoly(n3,j) * hhint(nn,i)**(j-3)
      end do
...
!       ESTIFM(NA,NA+2) = ESTIFM(NA,NA+2) - PPL * SPEC(N1,3) / 2.
 ...
 1050 CONTINUE
     ...
     +                        '         Knoten: ',  nop(nn,1), nop(nn,3)
        WRITE(*,'(a18, 1x,f9.7)')   '   Sohlgefaelle: ', sbot(nn)
        WRITE(*,'(a18,2(1x,f9.2))') '         Laenge: ',    xl(1), xl(3)
        WRITE(*,*) area(nn)
        WRITE(*,'(a18,2(5x,f5.2))') '   Wassertiefen: ', h1, h3
        WRITE(*,'(a18,2(4x,f6.3))') '     Schl-kurve: ',  qh(n1), qh(n3)
        WRITE(*,'(a18,2(1x,f9.3))') '   akt. Abfluss: ',
     +                                              qhalt(n1), qhalt(n3)
        WRITE(*,'(a18,2(4x,f6.3))') '   Fliessgeschw: ',
     +                                            vel_res(1), vel_res(2)
        WRITE(*,'(a18,2(4x,f6.3))') '  Fliessflaeche: ',  ah(n1), ah(n3)
        !WRITE(*,*) 'xl = ', xl(3)
        WRITE(*,'(a18,2(1x,f9.7))') '   Reibgefaelle: ',
     +                                              sfnod(n1), sfnod(n3)

        !WRITE(*,*) sfnod(n1), sbot(nn), qrandb, qh(n1), qgef(n1)
        WRITE(*,'(a18,2(4x,f6.3))') '          dahdh: ',
     +                                              dahdh(n1), dahdh(n3)
        WRITE(*,'(a18,2(1x,f9.3))') '        d2ahdh2: ',
     +                                            d2ahdh(n1), d2ahdh(n3)
        WRITE(*,'(a18,2(1x,f9.7))') '       dsfnoddh: ',
     +                                        dsfnoddh(n1), dsfnoddh(n3)
        WRITE(*,'(a18,2(4x,f6.3))') '        dQSchdh: ',
     +                                              dqhdh(n1), dqhdh(n3)
        WRITE(*,'(a18,2(4x,f6.3))') '      d2QSchdh2: ',
     +                                            d2qhdh(n1), d2qhdh(n3)
        WRITE(*,'(a18,2(1x,f9.7))') '      dSfnodedQ: ',
     +                                        dsfnoddq(n3), dsfnoddq(n3)
        WRITE(*,*)                  '******************'
        WRITE(*,'(a18,4(1x,i4))'  ) '     Gausspunkte: ', (i,i = 1, 4)
        WRITE(*,*)                  '******************'
        WRITE(*,'(a18,4(1x,f9.5))') '           hhint: ',
     +                                           (hhint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(3x,f7.4))') '        dhhintdx: ',
     +                                        (dhhintdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '           qqint: ',
     +                                           (qqint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         qqintdx: ',
     +                                         (qqintdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         areaint: ',
     +                                         (areaint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '      dareaintdh: ',
     +                                      (dareaintdh(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '     d2areaintdh: ',
     +                                     (d2areaintdh(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         daintdx: ',
     +                                         (daintdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '        d2aintdx: ',
     +                                        (d2aintdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '      d2aintdhdx: ',
     +                                        (d2aidhdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         qschint: ',
     +                                         (qschint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(3x,f7.4))') '        dqsintdh: ',
     +                                        (dqsintdh(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         d2qsidh: ',
     +                                         (d2qsidh(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '        dqsintdx: ',
     +                                        (dqsintdx(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '        s0schint: ',
     +                                        (s0schint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '           sfint: ',
     +                                           (sfint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '       dsfintdh1: ',
     +                                       (dsfintdh1(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '        vflowint: ',
     +                                        (vflowint(nn,i), i = 1, 4)
        WRITE(*,'(a18,4(1x,f9.5))') '         dvintdx: ',
     +                                         (dvintdx(nn,i), i = 1, 4)
      !end if
      !pause
      ENDIF
