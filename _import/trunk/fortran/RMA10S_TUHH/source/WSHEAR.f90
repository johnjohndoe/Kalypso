...
!
      DO  J=1,NP
!rrraug97         IF (WAVVEL(ND) > 0.01)  THEN
...
!      WRITE(75,998)
!      DO 60 J=1,NP
!        IF (WAVVEL(J) < .01)  GO TO 60
!        WRITE(75,999) J,WAVHIT(J), WAVPER(J), WAVVEL(J), RYNOLD,
!     &                        FW(J), WSHR(J), BSHEAR(J), DEP(J)
!   60 CONTINUE
!      endif
!
      RETURN
!
!  998 FORMAT(//,1(3X,'NODE',1X,'WHITE',2X,'WPER',2X,'WVEL',1X,
!     &            'REYNOLD'))
! 999  FORMAT(1(3X,I4,2(2X,F4.2),2X,F4.2,2X,F6.1,3F7.3,F7.1))
!
      END
!
!
...
!
 ...
 999     FORMAT(4(3X,I4,2(2X,F4.2),2X,F4.2,2X,F6.1),/)
      ENDIF
!
!      WRITE(*,*) '  >>>>   END OF DEBUG STATEMENTS IN JONFW  <<<<'
!
 201  CONTINUE
      RETURN
!
...
!ccr 1180 FORMAT('1',///,'    *****ENTER SUBROUTINE JONFW*****',//5X,
 1180 FORMAT(/5X,                                                       &
