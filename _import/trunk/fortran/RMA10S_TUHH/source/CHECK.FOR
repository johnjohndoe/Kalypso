...
cipk  last update Jan 21 1998
      ...
      type (LinkedNode), pointer :: tmpLinkedNode => null()
      type (LinkedNode), pointer :: prevNode => null()
...
!C-
!              NEL=NDEP(NA)-2
!              IF(NDEP(NC)-2 > NEL) NEL=NDEP(NC)-2
!              DO 500 M=1,NEL,2
!C-
!C...... Test for triangle
!C-
...
CIPK OCT98        MX = LMT(J)
     ...
     +       totalwsll,aves(j),avet(j),avesd(j)
            ...
            if (icyc == ncyc .OR. nita == 0) then
              write(uclq,'(a7)')'ENDDATA'
              write(uclh,'(a7)')'ENDDATA'
              CLOSE(uclq)
              CLOSE(uclh)
            end if
          end if
