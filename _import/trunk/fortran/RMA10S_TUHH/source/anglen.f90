!ipk  last update dec 2003   add iedsw dependence
!IPK  LAST UPDATE MAR 18 2003 add diffusion switch ( default of  0 uses old formulations
!ipk  last update Jan 12 2001 add test for IMAT
!IPK  LAST UPDATE dEC 21 2000 ALLOW FOR GATE STRUCTURE
!ipk  last update Dec 4 1998
!IPK  LAST UPDATE NOV 27 1997
!ipk  last updated April 23 1997
!ipk  last update Oct 1 1996 Add options for horizontal turbulence
      SUBROUTINE ANGLEN
      USE BLK10MOD
      USE BLK11MOD
!
...
!
!ipk dec03 add iedsw depedence
        LLL = abs(MOD(IMAT(LL),1000))
        IEDSW=IEDSW1(LLL)
!
!ipk OCT 96 add new options
!ipk nov97 clarify options see below      IF(IEDSW == 0  ) GO TO 90
!IPK OCT96      IF (EEXXYY(1,LL) >= 0.) GO TO 90  
...
!.... FIND NEXT LONGEST
 ...
 26   CONTINUE
      ...
      ENDIF
!
!.... DETERMINE LENGTH AND ANGLE OF MAJOR AXIS
      IF(NOP(LL,7) == 0) THEN
!.... TRIANGLES
...
!         T2=10.
         ...
         T2=0.
         T3=0.
!rrr August 1994 Experimental computation of minimum t1 and t2 not general
!                not currently used  i.e. t1=t2=0.
!          tt = sqrt(xla**2 + yla**2)
!          if (tt <= 0.0) stop
!          t1 = 02.5 * xla/tt
!          t2 = 02.5 * yla/tt
!
!          if (mod(imat(ll),1000) == 3)  then
!             t1 = 01.0 * xla/tt
!             t2 = 01.0 * yla/tt
!          endif
!
...
!ccc      ENDIF
 ...
 90   CONTINUE
!
!IPK NOV97     open(75,file='elemdir.dat')
         open(76,file='elemxycoef.dat')
         do 11  j=1,ne
!nis,may07           
           if (imat(j) /= 89) then
!-           
!ipk jan01 put in test
             if(imat(j) > 0) then
               angx = th(j)
!              if (angx < 0.)  angx = angx + 3.14159
!C             write(75,1500) j, (nop(j,k),k=1,8), nfixh(j),angx
...
!CC      stop  '   anglen complete '
!
!         write(76,*) 'anglen ivrsid',ivrsid
!
      RETURN
      END
