...
subroutine mk_potentialnose(temp_prof,pot_nos)
...
j = pot_nos%nextnode
d1 = temp_prof%prnode(j-1)%distance
d2 = temp_prof%prnode(j)%distance - d1
z1 = temp_prof%prnode(j)%elevation - temp_prof%prnode(j-1)%elevation
z2  = temp_prof%water_elev - temp_prof%prnode(j-1)%elevation
