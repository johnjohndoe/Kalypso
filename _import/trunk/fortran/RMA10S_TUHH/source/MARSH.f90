!     Last change:  NIS  14 May 2008    7:20 pm
!IPK  LAST UPDATE MAR 29 2005  RESET CORRECTION FOR CONSISTENCY WITH RMA2
!ipk  last update Mar 13 1998   fix initialization for no marsh case
!IPK  NEW ROUTINE NOVEMBER 13 1997
      SUBROUTINE MARSH
      USE BLK10MOD
      USE BLK11MOD
      USE BLKDRMOD
...
!
 ...
 6171   FORMAT(//'  DEFAULT VALUES FOR MARSH ELEMENTS USED',            &
     ...
     &  /  '      KAPPA      ', F8.2)
        ELSE
!
!     IDNOPT = -2  Read default/ individual marsh values
!
!
!ipk apr97 fix to read MP correctly
          IF (ID (1:3) == 'MP ') THEN
 ...
 6172     FORMAT(//'  INPUT VALUES FOR MARSH ELEMENTS USED',            &
     ...
     &    /, '      ABS A0     ', F8.2)
          ELSE
!
!       Define baseline defaults for no MP line
!
            AC3=0.04
            IF(GRAV < 10.) THEN
              AC1=1.5
...
!..... Define ADB temporarily as elevation at bottom of range
!..... Define ADT temporarily as elevation at top of range
!..... Define AKP as equivalent porosity
!..... Define ADO as elevation at bottom of slot
!
            ADB(N)=AO(N)-AC2/2.
            ADT(N)=ADB(N)+AC2
            AKP(N)=AC3
...
!*********************************************************************        
!
!...... Loop around nodes of an element to set ADO so that for an element
!       that is below adjacent AO
!
...
!
!             This adjusts ADB to account for storage below
!
          ENDIF
!
!..... Now adjust to make ADB and ADT depths
!
          ADB(N) = ADB(N) - ADO(N)
