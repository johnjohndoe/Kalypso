...
!ipk  last update jul 05 2006 use perim for hydraulic radius in friction
!IPK  LAST UPDATE MAY 30 2006 CORRECT FRICTION SUBSCRIPT AND H DEFINITION
!Nis  LAST UPDATE APR XX 2006 Adding flow equation of Darcy-Weisbach
!IPK  LAST UPDATE mar 23 2006 correct ice initial values 
...
!     Last change:  WP    2 Jun 2008    9:55 am
...
!         SSLOP2 side slope side 2
      SSLOP2=XM(1)*SS2(N1)+XM(2)*SS2(N2)
!
!         SSLOP1 side slope side 1
      SSLOP1=XM(1)*SS1(N1)+XM(2)*SS1(N2)
!
!       SSLOP total side slope 
...
!ipk jul06 define dhdx earlier         
      ...
      ENDIF
!
!           PERIM Wetted perimeter            
      PERIM=WID+H*(SQRT(1.+SSLOP1**2)+SQRT(1.+SSLOP2**2))
!           DPERMH rate of change of wetted perimeter wrt H
      DPERMH=SQRT(1.+SSLOP1**2)+SQRT(1.+SSLOP2**2)
!           WSRF water surface width
      WSRF=WID+SSLOP*H
!
!           WTOT total water surface width including overbank
      WTOT=WSRF+WIDSTR    
!
!           DACR rate of change of cross-section wrt X
      DACR=H*DWIDX+H**2/2.*DSLOX+DHDX*WSRF
!
!           ACR cross section area
      ACR=H*(WSRF+WID)*0.5
!
!           DACRH rate of change of cross-section wrt X wrt H
      DACRH=DWIDX+H*DSLOX+SSLOP*DHDX
!
!           DACRI rate of change of cross-section wrt X wrt DHDX
      DACRI=WSRF
!
!           DASHDX rate of change of (cross-sec area/H)wrt X times h**2/2 
      DASHDX=H**2/2.*(DWIDX+(DSLOX*H+SSLOP*DHDX)/2.)
!
!           DASHDXH rate of change of (cross-sec area)*H/2 wrt X wrt H
      DASHDXH=DWIDX*H+3*H**2*DSLOX/4.+SSLOP*DHDX*H/2.
!
!           DASHDXI rate of change of (cross-sec area)*H/2 wrt DHDX
      DASHDXI=H**2*SSLOP/4.
!
!           DAHDH2 = rate of change of (cross-sec area)*H/2 wrt H
...
!      DO I=1,12
!        IF(F(I) /= 0.) THEN
!          WRITE(235,7780) NN,I,F(I)
! 7780     FORMAT(2I8,1PE15.6)          
!        ENDIF
!        DO J=1,12
!      
!          IF(ESTIFM(I,J) /= 0.) THEN
!            WRITE(234,7779) NN,I,J,ESTIFM(I,J)
! 7779       FORMAT(3I8,1PE15.6)        
!          ENDIF
!        ENDDO
!      ENDDO
!
