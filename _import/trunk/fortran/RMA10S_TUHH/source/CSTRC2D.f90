!     Last change:  MD    2 Sep 2008    1:44 pm
!IPK  NEWLY ADDED TO RMA10S FROM RMA2  JUNE 27 2005
!IPK  LAST UPDATE JAN 11 2005 ALLOW FOR NEG HLOS IN OPTION 2
!IPK  LAST UPDATE SEP 06 2004 ADD ERROR FILE
!IPK  LAST UPDATE JULY 5 2004 ALLOW FOR ENGLISH UNITS
!ipk  last update oct 2 2003 compute embankment width
!IPK  LAST UPDATE SEP 23 2003 TEST FOR MISSING LEVEE DATA ADDED
!IPK  LAST UPDATE SEP 13 2002 ADJUST FOR DIST FUNCTION
!IPK  LAST UPDATE JUL 30 200 REVIE WEIR TESTING
!ipk  last update may 3 2000 add submerged weir element
!ipk  last update Apr 8 1997 fix integer*2 for msoft compiler
!ipk  last update Mar 9 1997  add more control structures
!ipk  last update Sept 9 1996  fix lhs error for type 4
!ipk  last update Apr 1 1996
!ipk  last update nov 10 1995
...
!-
!
!ipk may00 add cross velocity
!
      DIMENSION U(8),DAFL(8),V(8)
!ipk apr97
!ipk sep02      integer*2 n1,n2
      data pid2/1.5708/
      data pid32/4.7124/
      Dist(n1,n2)=sqrt((cord(n1,1)-cord(n2,1))**2 +                     &
     &                 (cord(n1,2)-cord(n2,2))**2)
!ipk mar97 3 lines above modified
!
!ipk mar00 initialize
      do j=1,32
        do k=1,32
          estifm(j,k)=0.
        enddo
...
!ipk nov95 remove iabs 2 lines
     ...
     &               -U(8-KK)/2.
          ELSE
            ...
            Q=(U(KK)*(VEL(3,N3)+VEL(3,N4))/2.                           &
     &       +U(8-KK)*(VEL(3,N5)+VEL(3,N6))/2.)/2.
!ipk mar96 CJ is missing        WS1=(HEL(N3)+AO(N3)+HEL(N4)+AO(N4))/2.
     ...
     &               -U(KK)/4.
!ipk sep96 fix actor of 2 error     +               -U(KK)/2.
            ESTIFM(NB,NA+6)=BJ(NM)*GAMJ(NM)*WS1**(GAMJ(NM)-1.)/2.       &
     &               -U(KK)/4.
!ipk sep96 fix factor of 2 error     +               -U(KK)/2.
            ESTIFM(NB,NB-2)=                                            &
     &               -U(8-KK)/4.
            ESTIFM(NB,NB+6)=                                            &
     &               -U(8-KK)/4.
...
!-
!...... Reversible Q = function of head loss of h1 if h >c
!......                =0   if h < c
!-
        DO 360 KK=1,3
!ipk nov95 remove iabs 2 lines
...
!            WRITE(75,*) 'C WS1,WS2,Q',NN,WS1,WS2,Q
...
!    Flow controlled from upstream N2
     ...
     &                       +AJ(NM)*GAMJ(NM)*WS2**(GAMJ(NM)-1.)
            ELSE
!    Flow controlled by H1-H2  Get new AJ
              IF(WS1 > WS2) THEN
                AJN=AJ(NM)*(WS1-WS2)**(GAMJ(NM)-CJ(NM))
                F(NB)=Q-AJN*(WS1-WS2)**CJ(NM)
!                write(75,*) 'ajn,q,F(NB)',ajn,q,F(NB)
                ESTIFM(NB,NA+2)=ESTIFM(NB,NA+2)                         &
...
!            WRITE(75,*) 'M,WS1,WS2,Q',NN,WS1,WS2,Q
            IF(WS1 <= 0.  .AND. WS2 <= 0.) THEN
!    No Flow case
              F(NB)=Q
            ELSEIF(WS1 > 0. .AND. WS2 <= 0.) THEN        
...
!c          f(nb)=f(nb)/2.
  370   CONTINUE
!ipk mar97 add new loss function
!-
      ELSEIF(NJT(NM) == 8) THEN
!-
!.......If Depth < set value
!...... Reversible Q = C1*AFL*Sqrt( h1 -h2)
!......       or   q = C1*AFL/w *sqrt(h1-h2)
!......            A = function of depth
!-
!...... If Depth > set value 
!.      Reversible Q = C2*AFL*Sqrt(h1 -h2)
!.            or   q = C2*AFL/w*sqrt(h1-h2)
!. ...             A = constant at set value depth
!-
!.....  Get w first by averaging 1 to 3 and 5 to 7
        w=(dist(nop(nn,1),nop(nn,3))+dist(nop(nn,5),nop(nn,7)))/2.
!.......Get A based on average water depth
        avedep=(vel(3,nop(nn,1))+vel(3,nop(nn,3))+                      &
     &          vel(3,nop(nn,5))+vel(3,nop(nn,7)))/4.
        wbas=cj(nm)
        ssl=gamj(nm)
        dnomin=dj(nm)
!        write(75,*) 'w,avedep,ssl,dnomin',w,avedep,ssl,dnomin
...
!        write(75,*) 'afl,dafl(1),cof1',afl,dafl(1),cof1
        DO 390 KK=1,3
          N1=NOP(NN,KK)
          N2=NOP(NN,8-KK)
          NA=(KK-1)*NDF+1
          ...
          IF(KK /= 2) THEN
            ...
            HLOS=ABS(WS1-WS2)
            HLD=SIGN(1.,WS1-WS2)
!            write(75,*) 'hlos Q',hlos,Q
!            if(hlos == 0.) hlos=0.0001
...
!                write(75,*) 'ajn,U,F(NB)',ajn,UM,F(NB)
                if(um == 0.) um=ucut
                ESTIFM(NB,NA)=+AJN*CJN*UM**(CJN-1.)/2.
                ESTIFM(NB,NB)=+AJN*CJN*UM**(CJN-1.)/2.
                ESTIFM(NB,NA+2)=-1.0
!C     +                          +AJN*CJN*UM**(CJN-1.)*U(KK)/2.
                ESTIFM(NB,NB+2)=+1.0
!C     +                          +AJN*CJN*UM**(CJN-1.)*U(8-KK)/2.
              ELSEIF(Q == 0.) THEN
                F(NB)=WS1-WS2
                ESTIFM(NB,NA)=0.
                ESTIFM(NB,NB)=0.
              ...
              ELSE
                ...
                ESTIFM(NB,NB)=-AJN*CJN*(-UM)**(CJN-1.)/2.
                ESTIFM(NB,NA+2)=+1.0
!C     +                        -AJN*CJN*(-Q)**(CJN-1.)*U(KK)/2.
                ESTIFM(NB,NB+2)=-1.0
!CC    +                        -AJN*CJN*(-Q)**(CJN-1.)*U(8-KK)/2.
              ENDIF
            ENDIF
          ELSE
            N3=NOP(NN,1)
...
!                write(75,*) 'ajn,q,F(NB)',ajn,q,F(NB)
     ...
     &                 +AJN*CJN*UM**(CJN-1.)/2.
                ESTIFM(NB,NA-2)=-0.5
!C     +                      +AJN*CJN*Q**(CJN-1.)*U(KK)/4.
                ESTIFM(NB,NA+6)=-0.5
!C     +                      +AJN*CJN*Q**(CJN-1.)*U(KK)/4.
                ESTIFM(NB,NB-2)=0.5
!C     +                      +AJN*CJN*Q**(CJN-1.)*U(8-KK)/4.
                ESTIFM(NB,NB+6)=0.5
!C     +                      +AJN*CJN*Q**(CJN-1.)*U(8-KK)/4.
              ELSEIF(Q == 0.) THEN
                F(NB)=WS1-WS2
