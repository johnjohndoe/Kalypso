...
!---------------------------------------------------------------------------------------------
function wald (h, a, dp, cwr,opt_aplha_lat) result (lambda_p)
...
REAL (KIND = 8), INTENT(IN), optional :: opt_aplha_lat    !Boeschungsneigung in Grad
...
! no plants
  ...
  ! Diameter of trees is larger than the distance
    ...
    stop
  END if
  if (present (opt_aplha_lat)) then
    alpha_lat = opt_aplha_lat *Pi / 180
  else
    alpha_lat = 0.0d0   !floodplain without lateral angle
  endif

  lambda_p = ( (4.0 * h * dp) / (a**2) ) * cwr * cos(alpha_lat)
end if
