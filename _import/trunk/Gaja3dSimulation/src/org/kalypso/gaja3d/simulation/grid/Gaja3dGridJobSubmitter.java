...
public class Gaja3dGridJobSubmitter {
	...
	public void submitJob(final FileObject workingDir,
		...
		try {
			m_externalInputs.put(EXEC_ZIP_URL.toURI(), null);
		...
		try {
			...
			final IProcess process = KalypsoCommonsExtensions.createProcess(
					processFactoryId, workingDir, EXEC_SCRIPT_URL, arguments
			...
			// stage-in files
			final FileSystemManager manager = workingDir.getFileSystem()
					.getFileSystemManager();
			for (final URI einput : m_externalInputs.keySet()) {
				final FileObject inputFile = manager
						.resolveFile(getUriAsString(einput));
				final String destName = m_externalInputs.get(einput);
				if (destName != null) {
					final FileObject destFile = workingDir
							.resolveFile(destName);
			...
			throw new SimulationException(
					"Could not create temporary files for stdout and stderr.",
					e);
