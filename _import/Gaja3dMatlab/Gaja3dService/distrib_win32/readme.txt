    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_Gaja3dService.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * Gaja3dService.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * Gaja3dService.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * Gaja3dService (application created by MATLAB Compiler)
        * run_Gaja3dService.sh (shell script that can be run to 
    ...
    All Platforms:
        * Gaja3dService.dll (shared library, file extension varies by 
          platform)
        * Gaja3dService.h (library header file)
        * Gaja3dService.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * Gaja3dService.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * Gaja3dService.xml (documentation files)
        * Gaja3dService.pdb (program database file - if DEBUG option 
          selected)
        * Gaja3dService.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * Gaja3dService.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Gaja3dService_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * Gaja3dService.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * Gaja3dService.ctf (Component Technology File archive, contains 
          compiled M-code)
        * Gaja3dService.xla (Optionally created and placed in
          D:\EigeneDateien\Matlab\gaja3d_15.10.08\Gaja3dService\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
