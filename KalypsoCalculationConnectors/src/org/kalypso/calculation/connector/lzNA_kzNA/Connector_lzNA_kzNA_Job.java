...
public class Connector_lzNA_kzNA_Job extends AbstractInternalStatusJob implements ISimulation
{
    ...
    /**
     *  This connector is just a copy of another one,
  ...
  {
    final URL urlModelLZNA = (URL) inputProvider.getInputForID( MODELSPEC_CONNECTOR_LZNA_KZNA.LZ_NA.name() );
    ...
    try
    {
      ...
      final File gmlModelFileNA = new File( urlModelLZNA.getPath() );
      if( gmlModelFileNA.exists() )
      {
        try
        {
          ...
          final GMLWorkspace workspaceNA = GmlSerializer.createGMLWorkspace( urlModelLZNA, null );
