...
final class Generate2dSectionRunnable implements ICoreRunnableWithProgress
{
  ...
  Generate2dSectionRunnable( IResultMeta[] results, LengthSectionParameters lengthSectionParameters, boolean isKmValues, IFolder scenarioFolder )
  {
    m_results = results;
    ...
    try
    {
      ...
      if( resultMeta instanceof IStepResultMeta )
      {
        ...
        if( lsObs.getResult().size() > 0 )
        {
          ...
          // how to delete them?
          String riverName = m_lengthSectionParameters.getSelectedRiverName();
          String lengthSecionFileName = "section_" + riverName + ".gml";
          String sectionName = riverName;
          ...
          IResultMeta[] childArray = children.toArray( new IResultMeta[children.size()] );
          for( int i = 0; i < childArray.length; i++ )
          {
            IResultMeta child = childArray[i];
            if( child instanceof IDocumentResultMeta )
            {
              final IDocumentResultMeta docResult = (IDocumentResultMeta) child;
