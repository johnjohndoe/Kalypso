...
public class ThiessenAreaOperation
{
   ...
   */
  public Map<Feature, GM_Surface< ? >> execute( final List< ? > stations, final IBoundaryCalculator boundaryCalculator, final IProgressMonitor monitor ) throws CoreException, GM_Exception
  {
    ...
    final Map<Feature, GM_Surface< ? >> changeMap = new HashMap<>();
    ...
    else if( geoIndex.size() == 1 )
    {
      ...
      final GM_Surface< ? > gmBoundary = (GM_Surface< ? >) JTSAdapter.wrap( thiessenBoundary, crs );
    ...
    else
    {
      ...
      {
        final GM_Surface< ? > affectedArea = (GM_Surface< ? >) JTSAdapter.wrap( polygon, crs );
