 ...
 */
                ...
                coord[0] = -(i * qx + xmin);
                coord[2] = (j+1) * qy + ymin;
                GM_Position pos1 = GeometryFactory.createGM_Position( new double[] {-coord[0],coord[2]} );
                GM_Position pos2 = GeometryFactory.createGM_Position( new double[] {(i * qx + xmin),j * qy + ymin} );
                GM_Position pos3 = GeometryFactory.createGM_Position( new double[] {(i+1) * qx + xmin,j * qy + ymin} );
                boolean in1 = ring.contains( pos1 );
                boolean in2 = false;
                if ( !in1 ) in2 = ring.contains( pos2 );
                in2 = ring.contains( pos2 );
                boolean in3 = false;
                if ( !in1 && !in2 ) in3 = ring.contains( pos3 );
                ...
                coord[0] = -((i+1) * qx + xmin);
                coord[2] = (j + 1) * qy + ymin;
                pos1 = GeometryFactory.createGM_Position( new double[] {-coord[0],coord[2]} );                
                pos2 = GeometryFactory.createGM_Position( new double[] {(i * qx + xmin),((j +1) * qy + ymin) } );                
                pos3 = GeometryFactory.createGM_Position( new double[] {((i+1) * qx + xmin),(j * qy + ymin)} );                
                in1 = ring.contains( pos2 );
                if ( !in1 ) in2 = ring.contains( pos1 );                
                if ( !in1 && !in2 ) in3 = ring.contains( pos3 );
                if ( in1 || in2 || in3 ) {
                    cc += 3;
