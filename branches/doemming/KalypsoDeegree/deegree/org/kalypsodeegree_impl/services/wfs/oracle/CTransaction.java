...
//        String releaseAction = tr.getReleaseAction();
    ...
    */
    ...
    private boolean performDelete(WFSDelete delete) throws Exception {
        ...
        try {
            ...
            if (!fIDs.isEmpty()) {
                ...
                PointRowSelector selector =
                    new PointRowSelector(
    ...
    private void performDelete(FeatureType ft, HashMap tables) throws Exception {
        while (!tables.isEmpty()) {
            ...
            while (it.hasNext()) {
                ...
                PointRowSelector selector =
                    (PointRowSelector) tables.get(name);
                ...
                while (it2.hasNext() && !isReferenced) {
                    ...
                    for (int i = 0; i < refs.length; i++) {
                        TableDescription targetTable =
                            ft.getTableByName(refs[i].getTargetTable());
                ...
                PointRowSelector[] referencedRows =
                    (PointRowSelector[]) selector.getReferencedRows();

                for (int i = 0; i < referencedRows.length; i++) {
                    tables.put(
                        referencedRows[i].getTableName(), referencedRows[i]
                    );
                }
    ...
    private Set getFeatureIDs(FeatureType ft, Filter filter, DBAccess osa) throws Exception {
        StringBuffer query =
            new StringBuffer("SELECT " + ft.getMasterTable().getIdField());
