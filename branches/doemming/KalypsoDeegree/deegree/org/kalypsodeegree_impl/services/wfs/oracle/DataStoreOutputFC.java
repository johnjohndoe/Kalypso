...
public class DataStoreOutputFC implements DataStoreOutputFormat {
	...
	/**
	 * @see org.deegree.services.wfs.DataStoreOutputFormat#format(java.util.HashMap, org.deegree.tools.ParameterList)
	 */
	public Object format( HashMap map, ParameterList parameter) throws Exception {        
		...
		FeatureCollection fc = (FeatureCollection)pl.getParameter( WFSConstants.FEATURECOLLECTION ).getValue();
		while( iterator.hasNext() ) {
			pl = (ParameterList)iterator.next();
			fc.appendFeatures( (FeatureCollection)pl.getParameter( WFSConstants.FEATURECOLLECTION ).getValue() );
		}
		return fc;
	}
                ...
                if ( !(row[i] instanceof ByteArrayOutputStream) )  {
                    if ( row[i] instanceof Table ) {
                        ...
                        FeatureCollection fc_ =
                            FeatureFactory.createFeatureCollection( td_.getTargetName(), 1000 );
        ...
        for (int i = 0; i < columnNames.length; i++) {
            ...
            if ( td.getGeoFieldIdentifier( columnNames[i] ) == null  ) {
                if ( columnTypes[i].equals( "org.deegree.model.table.Table") ) {
                    ...
                    // column
                    ftp[k++] = FeatureFactory.createFeatureTypeProperty( pn, columnTypes[i],
                                                                  true );
                }
            } else {
                // create a FeatureTypeProperty with geometry type
                ftp[k++] = FeatureFactory.createFeatureTypeProperty( pn, "org.deegree.model.geometry.GM_Object",
                                                              true );
            }
        }
    ...
    CS_CoordinateSystem srs)  throws Exception {
        ...
        switch ( getGeometryType( wkb ) ) {
            case 1: geo = GeometryFactory.createGM_Point( wkb, srs ); break;
            case 2: geo = GeometryFactory.createGM_Curve( wkb, srs); break;
            case 3: geo = GeometryFactory.createGM_Surface( wkb, srs, new GM_SurfaceInterpolation_Impl(1)); break;
            case 4: geo = GeometryFactory.createGM_MultiPoint( wkb, srs ); break;
            case 5: geo = GeometryFactory.createGM_MultiCurve( wkb, srs ); break;
            case 6: geo = GeometryFactory.createGM_MultiSurface( wkb, srs, new GM_SurfaceInterpolation_Impl(1) ); break;
            default: geo = null;
        }
