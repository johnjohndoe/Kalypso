 ...
 * within the deegree framework. A <tt>WCSService</tt> extends the <tt>OGCWebService</tt>
 * interface to act like a OGC web service. This means that a WCS is callable
 * through the <tt>doService</tt> -method inherited from <tt>OGCWebService</tt>.
 * 
 * 
...
public class GazetteerService_Impl extends OGCWebService_Impl implements OGCWebServiceClient {
    ...
    /**
     * implements the <tt>doService</tt> method inherited from the <tt>OGCWebService_Impl</tt>
     * class. The method receives a WFSG... request and delievers it to one or
     * more WFS's
     * 
     * @param event event object that contains the requests to be performed
     */
     ...
     * more WFS's
     * 
     * @param req requet to be performed
     */
     ...
     */
    private void handleGetCapabilities(WFSGGetCapabilitiesRequest request) throws WebServiceException {
        Debug.debugMethodBegin();       
        
        WFSGGetCapabilitiesResponse res = 
            WFSGProtocolFactory.createWFSGGetCapabilitiesResponse( request, (WFSGCapabilities)capabilities );
     ...
     * @throws  WebServiceException
     */
    private void handleDescribeFeatureType(WFSGDescribeFeatureTypeRequest request) throws WebServiceException {
        Debug.debugMethodBegin();
    ...
    private void handleGetFeatureRequest(WFSGGetFeatureRequest request) throws GazetteerException {
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            if (filter instanceof ThesaurusFilter) {
                ...
                queries[i] = WFSProtocolFactory.createQuery( null, null, request.getVersion(), 
                                                             queries[i].getTypeName(), 
                                                             queries[i].getFilter() );
            } else {
                this.hierarchyLevels = 0;
                this.relationType = "NT";
                locationtype = new SI_LocationType_Impl( queries[i].getTypeName(), null, 
                                                         queries[i].getTypeName(), null, 
                                                         null, null, null, null );
            }
            
            WFSGetFeatureRequest wfsrequest = 
                WFSProtocolFactory.createWFSGetFeatureRequest(request.getVersion(), 
                                                             requestid, null, 
                                                             request.getNative(), 
                                                             "FEATURECOLLECTION", 
                                                             request.getHandle(), 
                                                             null, request.getMaxFeatures(), 
                                                             request.getStartPosition(), 
                                                             new WFSQuery[] { queries[i] } );        
     ...
     * @param locationtype type of the request assigned to the root node
     */
    private void initResponseTree(String id, SI_LocationType locationtype) {
        ...
        DataObject dao =
            new DataObject( id, locationtype, FeatureFactory.createFeatureCollection( id, 100 ) );        
    ...
    private void performGetFeatureRequest(WFSGetFeatureRequest request ) throws GazetteerException {                  
        ...
        try {
            ...
            throw new GazetteerException("Exception occured while creating the "+ 
                                         "OGCWebService for the FeatureFilter-Request " +
                                         e.toString() );
        }
    }

    /**
     * Method from OGCWebServiceClient. All responses to requests will be passed to 
     * this method
     * 
     * @param result 
     */
    public void write(Object result) {
        ...
        if (wse.getResponse().getException() == null) {
            ...
            OGCWebServiceResponse res = 
                new OGCWebServiceResponse_Impl( wse.getRequest(), wse.getResponse().getException() );               
            OGCWebServiceEvent event_ = new OGCWebServiceEvent_Impl( this, res, "" );
     ...
     * reached.
     *
     * @param wse OGCWebServiceEvent containing a WFSGetFeatureResponse
     */
    ...
    /**
     * creates GetFeature requests to receive the locations instances for the
     * next hierarchy level related to the passed response
     */
    private synchronized void performHierachyRequests(WFSGetFeatureResponse response, Node node) throws GazetteerException {
        Debug.debugMethodBegin();
        ...
        if ( relationType.equals("RT") ) {
            ...
            if ( dao != null && dao.locationType != null && 
                dao.locationType.getChild() != null && dao.locationType.getChild().length > 0) {
     ...
     * @param locType
     */
    private void performWFSRequest(WFSGetFeatureResponse response, SI_LocationType locType) throws GazetteerException {
        Debug.debugMethodBegin();
        ...
        if ( response.getResponse() instanceof FeatureCollection) {
            ...
            for (int i = 0; i < fc.getSize(); i++) {
                ...
                WFSGetFeatureRequest wfsrequest = 
                    createGetFeatureRequest( node.getId(), locType.getName(), 
    ...
    /**
     * creates the FilterEncoding operation formulating the relation between
     * two location types  for Geometry type properties
     * 
     * @param prop geometry property acting as literal of the operation
     * @return
     * @throws GazetteerException
     */
    ...
    /**
     * creates the FilterEncoding operation formulating the relation between
     * two location types for none Geometry type properties
     * 
     * @param prop feature property acting as literal of the operation
     * @return
     * @throws GazetteerException
     */
    private Operation createOperation(Object prop) throws GazetteerException {
    	...
    	Operation operation = 
            new PropertyIsCOMPOperation( OperationDefines.PROPERTYISEQUALTO, 
    ...
    /**
     * creates a GetFeature request for accessing the locationinstances of the next
     * level. 
     */
    private WFSGetFeatureRequest createGetFeatureRequest(String id, String featureType,
    	...
    	WFSGetFeatureRequest wfsrequest = 
    		WFSProtocolFactory.createWFSGetFeatureRequest(wfsVersion, id, 
    				null, null, "FEATURECOLLECTION", null, null, 0, 0, query);
     ...
     * @param locType location type of the requested instances
     * @param feature feature assigned to the new node
     */
     ...
     *
     * @param feature source feature to re-create
     * @param propertyNames names of the properties to be consider
     */
    private Feature recreateFeature(Feature feature, String[] propertyNames, boolean leaf ) {
        ...
        if ( ts.size() == 0 || ts.contains( "geographicIdentifier" ) ) {
            ftp = FeatureFactory.createFeatureTypeProperty( "geographicIdentifier", "java.lang.String", false );
            ftpList.add( ftp );
        ...
        if ( ts.size() == 0 || ts.contains( "geographicExtent" ) ) {
            ftp = FeatureFactory.createFeatureTypeProperty( "geographicExtent", "org.deegree.model.geometry.GM_Object", true );
            ftpList.add( ftp );
        ...
        if ( ts.size() == 0 || ts.contains( "position" ) ) {
            ftp = FeatureFactory.createFeatureTypeProperty( "position", "org.deegree.model.geometry.GM_Point", true );
            ftpList.add( ftp );
        ...
        if ( !leaf ) {
            ...
            ftp = FeatureFactory.createFeatureTypeProperty( "children", "org.deegree.feature.FeatureCollection", true );        
            ftpList.add( ftp );
        ...
        FeatureTypeProperty[] ftpL = (FeatureTypeProperty[])ftpList.toArray( new FeatureTypeProperty[ftpList.size()] );
        FeatureProperty[] fpL = (FeatureProperty[])fpList.toArray( new FeatureProperty[fpList.size()] );
        FeatureType featureType = 
        	FeatureFactory.createFeatureType( null, null, feature.getFeatureType().getName(), ftpL );        
    ...
    private void finishGazetteerService() {
        ...
        OGCWebServiceResponse res =
            WFSGProtocolFactory.createWFSGGetFeatureResponse( this.event.getRequest(), 
                                                            null, null, fc);
