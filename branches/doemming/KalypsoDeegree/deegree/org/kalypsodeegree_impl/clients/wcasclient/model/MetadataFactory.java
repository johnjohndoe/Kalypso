...
public class MetadataFactory {
     ...
     * 
     * @param element DOM representation of a brief catalog response
     *
    ...
    public static BaseMetadata createISO19115Brief( Element element, String namespace )
        ...
        try {
            String fileIdentifier = 
                XMLTools.getRequiredStringValue( "fileIdentifier",  namespace,  element );            
            
            String metadataStandardName = 
                XMLTools.getRequiredStringValue( "metadataStandardName",  namespace,  element );

            String metadataStandardVersion = 
                XMLTools.getRequiredStringValue( "metadataStandardVersion",  namespace,  element );
            ...
            mdMetadata = new BaseMetadata( title, fileIdentifier, dateStamp, 
     ...
     * 
     * @param element DOM representation of a full catalog response
     *
        ...
        try {
            ...
            if ( el.getLength() > 0 ) {
                maintenanceAndUpdateFrequency = 
                    getMaintenanceAndUpdateFrequency( namespace, el.item(0) );
            }
            ...
            mdMetadata = new DetailedMetadata(title, fileIdentifier, dateStamp, 
                            metadataStandardName, metadataStandardVersion, 
                            tex[0], tex[1], abstract_, crs, lineageSrcDesc, 
                            lineageStatement, maintenanceAndUpdateFrequency, 
                            constraints, processStep, equivalentScale, keywords, 
                            contacts, onlineTransferOptions );
        } catch (Exception e) {
     ...
     */
    private static Contact[] getContact(String namespace, ElementList el) throws CatalogClientException {
        Debug.debugMethodBegin();
        ...
        try {            
            for (int i = 0; i < el.getLength(); i++) {
                String name = 
                    XMLTools.getStringValue( "individualName",  namespace,  el.item(i), "-" );
                String organisation = 
                    XMLTools.getStringValue( "organisationName",  namespace,  el.item(i), "-" );
                String role = 
                    XMLTools.getStringValue( "role",  namespace,  el.item(i), "-" );
                // get address
                ...
                String street = 
                    XMLTools.getStringValue( "deliveryPoint",  namespace,  ell.item(0), "-" );
                String postalCode = 
                    XMLTools.getStringValue( "postalCode",  namespace,  ell.item(0), "-" );
                String city = 
                    XMLTools.getStringValue( "city",  namespace,  ell.item(0), "-" );
                String country = 
                    XMLTools.getStringValue( "country",  namespace,  ell.item(0), "-" );
                String email = 
                    XMLTools.getStringValue( "electronicMailAddress",  namespace,  ell.item(0), "-" );
                // get phone 
                ...
                String phone = 
                    XMLTools.getStringValue( "voice",  namespace,  ell.item(0), "-" );
                String fax = 
                    XMLTools.getStringValue( "facsimile",  namespace,  ell.item(0), "-" );
                // get Online Resource
                ell = XMLTools.getChildElementsByName( "contactInfo" , namespace, el.item(i) );
                ell = XMLTools.getChildElementsByName( "onlineResource" , namespace, ell.item(0) );
                String onlineResource = 
                    XMLTools.getStringValue( "linkage",  namespace,  ell.item(0), "-" );

                // create contact information
                contacts[i] = new Contact( name, fax, phone, street, city, postalCode, 
                                           country, email, role, organisation, onlineResource );
            }
        } catch (Exception e) {
            throw new CatalogClientException( e.toString(), e );
        }
     ...
     */
    private static Calendar[] getTimeExtent(String namespace, Element element) throws CatalogClientException {
        Debug.debugMethodBegin();
        ...
        try {   
            ...
            if ( el != null && el.getLength() > 0 ) {      
                tex = new Calendar[2];
                String begin = XMLTools.getStringValue( "begin", "http://www.isotc211.org/iso19115/", el.item(0), null );
                String end = XMLTools.getStringValue( "end", "http://www.isotc211.org/iso19115/", el.item(0), null );
                if ( begin != null ) {              
        ...
        try {
            ...
            if ( el.getLength() > 0 ) {
                ...
                if ( el.getLength() > 0 ) {
                    ...
                    lineageSrcDesc = XMLTools.getStringValue( "description",  namespace, 
                                                              el.item(0), null );
                }
            }
        } catch (Exception e) {
    ...
    private static String getLineageStatement(String namespace, Element element ) 
        ...
        try {
            ...
            if ( el.getLength() > 0 ) {
                el = XMLTools.getChildElementsByName("LI_Lineage", namespace,  el.item(0) );
                lineageStatement = XMLTools.getStringValue( "statement",  namespace, 
                                                            el.item(0), null );
        ...
        try {
            ...
            if ( el.getLength() > 0 ) {
                ...
                if ( el.getLength() > 0 ) {
                    ...
                    processStep = XMLTools.getStringValue( "description",  namespace, 
                                                              el.item(0), null );
                }
            }
        } catch (Exception e) {
     ...
     */
    private static String getMaintenanceAndUpdateFrequency(String namespace, 
                                    Element element ) throws CatalogClientException {
        Debug.debugMethodBegin();
        
        String mauf = null;
        try {
            ElementList el = XMLTools.getChildElementsByName("MD_MaintenanceInformation", 
                                                             namespace,  element );
            if ( el.getLength() > 0 ) {                
                mauf = XMLTools.getStringValue( "maintenanceAndUpdateFrequency",  namespace, el.item(0), null );
            }
        } catch (Exception e) {
            throw new CatalogClientException( e.toString(), e );
        }
    ...
    private static String  getResourceConstraints(String namespace, Element element ) 
        ...
        String constraints = null;
        try {
            ElementList el = XMLTools.getChildElementsByName("MD_LegalConstraints", 
                                                             namespace,  element );
            if ( el.getLength() > 0 ) {
                constraints = XMLTools.getStringValue( "otherConstraints",  namespace, 
                                                       el.item(0), null );
            }
        } catch (Exception e) {
            throw new CatalogClientException( e.toString(), e );
    ...
    private static String getEquivalentScale( String namespace, Element element ) 
        ...
        String equivalentScale = null;
        try {
            ElementList el = XMLTools.getChildElementsByName("equivalentScale", 
                                                             namespace,  element );
            if ( el.getLength() > 0 ) {
                equivalentScale = XMLTools.getRequiredStringValue( "denominator",  
                                                        "http://www.isotc211.org/iso19115/", 
                                                        el.item(0));
            }
        } catch (Exception e) {
            throw new CatalogClientException( e.toString(), e );
    ...
    private static String[] getDescriptiveKeywords(String namespace, Element element ) 
        ...
        String[] keywords = null;
        try {
            ElementList el = XMLTools.getChildElementsByName("MD_Keywords", 
                                                             namespace,  element );
