...
public class WebMapContextFactory {
     ...
     * read from the file idenfied by the passed name
     *
     * @param fileName name of a file containing a web map context document
     *
     * @return 
     *
     * @throws IOException 
    ...
    /**
     * creates an instance of a ViewContext from the web map context document
     * read from the passed Reader
     *
     * @param reader reader enabling access to a web map context document
     *
     * @return 
     *
     * @throws IOException 
    ...
    private static General createGeneral( Element element ) throws XMLParsingException {
        ...
        try {
            general = new General( title, abstract_, rect, contact, bbox, 
                                   descriptionURL, logoURL, keywords, extension );
        } catch (Exception e) {
    ...
    private static ContactInformation createContactInformation( Element element )
        ...
        if ( element != null ) {
            ...
            String contactPosition = XMLTools.getStringValue( "ContactPosition", CNTXTNS, element, 
                                                              null );

            // optional: <ContactAddress>
            ContactAddress contactAddress = null;
            Element contactAddressElement = XMLTools.getChildByName( "ContactAddress", CNTXTNS, 
                                                                     element );
            ...
            String contactElectronicMailAddress = XMLTools.getStringValue( 
                                                          "ContactElectronicMailAddress", CNTXTNS, 
                                                          element, null );
    ...
    private static ContactAddress createContactAddress( Element element ) throws XMLParsingException {
        ...
        String stateOrProvince = XMLTools.getRequiredStringValue( "StateOrProvince", CNTXTNS, 
                                                                  element );
    ...
    private static Frontend createFrontend( Element element ) throws XMLParsingException {
        ...
        Frontend frontend = new JSPFrontend(controller, west, east, south, north, 
                                            center, style, buttons, commonJS,
                                            header, footer);
    ...
    private static ModuleConfiguration createModuleConfiguration(Element element) 
        ...
        if ( element != null ) {
            Element elem = 
                XMLTools.getRequiredChildByName( "OnlineResource",  DGCNTXTNS, element );
            URL onlineResource = createOnlineResource( elem );
    ...
    private static Layer createLayer(Element element) throws XMLParsingException {
        ...
        Layer layer = null;
        try {
            layer = new Layer( server, name, title, abstract_, srs, dataURL, 
                                 metadataURL, formatList, styleList, queryable, 
                                 hidden, extension );
        } catch (Exception e) {
     ...
     * @throws XMLParsingException 
     */
    private static LayerExtension createLayerExtension(Element element ) 
                                                    throws XMLParsingException {
        Debug.debugMethodBegin();
