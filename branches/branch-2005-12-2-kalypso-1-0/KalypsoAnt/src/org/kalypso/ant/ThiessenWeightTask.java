...
public class ThiessenWeightTask extends Task
{
  ...
  public void execute() throws BuildException
  {
    try
    {
      ...
      final GMLWorkspace workspace = GmlSerializer.createGMLWorkspace( m_gmlURL );
      if( workspace == null )
      ...
      final FeatureType rainfallWeightFT = workspace.getFeatureType( "RainfallWeight" );
      ...
      try
      {
        final UrlResolver resolver = UrlResolverSingleton.getDefault();
  ...
  private void calculateWeights( final Feature areaFeature, final Feature[] thiessenFeatures,
      final FeatureType rainfallWeightFT, final ILogger logger ) throws GM_Exception
  {
    logger.log( Level.INFO, LoggerUtilities.CODE_NONE, "Berechne Faktoren für: " + areaFeature.getProperty( "name" ) );

    final FeatureTypeProperty areaPT = areaFeature.getFeatureType().getProperty( m_areaPropertyPath );
    if( areaPT == null )
    ...
    for( int i = 0; i < thiessenFeatures.length; i++ )
    {
      ...
      final FeatureTypeProperty thiessenPT = thiessenFeature.getFeatureType().getProperty( m_thiessenPropertyPath );
      if( thiessenPT == null )
  ...
  private void createWeight( final Feature areaFeature, final Feature thiessenFeature,
      final FeatureType rainfallWeightFT, final BigDecimal factor )
  {
    final Feature weightFeature = FeatureFactory.createFeature( "weight", rainfallWeightFT, false );

    final FeatureTypeProperty weightPT = areaFeature.getFeatureType().getProperty( m_weightPropertyPath );
    if( weightPT == null )
      throw new BuildException( "Keine Property mit gefunden mit Namen: " + m_weightPropertyPath );

    final List weights = (List)areaFeature.getProperty( m_weightPropertyPath );
