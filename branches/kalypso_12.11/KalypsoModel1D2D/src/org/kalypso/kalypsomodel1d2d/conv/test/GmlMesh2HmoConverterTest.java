...
public class GmlMesh2HmoConverterTest extends TestCase
{
  ...
  {
    final URL gmlLocation = getClass().getResource( "/etc/testdata/conv/tinyDiscretization.gml" ); //$NON-NLS-1$
    assertNotNull( gmlLocation );

    final URL hmoLocation = getClass().getResource( "/etc/testdata/conv/tinyDis.hmo" ); //$NON-NLS-1$
    assertNotNull( hmoLocation );

    final GMLWorkspace disWorkspace = GmlSerializer.createGMLWorkspace( gmlLocation, null );
    final Feature rootFeature = disWorkspace.getRootFeature();
    final IFEDiscretisationModel1d2d dis = (IFEDiscretisationModel1d2d) rootFeature.getAdapter( IFEDiscretisationModel1d2d.class );

    final File result = FileUtilities.createNewUniqueFile( "hmoTest", FileUtilities.TMP_DIR ); //$NON-NLS-1$

    final GmlMesh2HmoConverter conv = new GmlMesh2HmoConverter( dis );
