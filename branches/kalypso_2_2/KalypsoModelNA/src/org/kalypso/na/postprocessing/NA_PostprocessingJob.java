...
/**
 * This class is the post processor for standard rainfall-runoff calculation.
 * The behaviour of it depends on the information given by model specification:
 * <ul>
 * <li>If the "CalculationNature" parameter is set to "PLC", this class produces
 * the set of data representing the difference in the discharge on its nodes.
 * The structure of the output is compatible with PlanerClient application.
 * <ul>
 * <li><b>TODO:</b></li>
 * <li>If the planing area is defined, only the nodes located in the planing
 * area and downstream nodes will be included in the result set; otherwise, all
 * the nodes are included.</li>
 * <li>Planing area is defined by setting the "planingAreaMember" property of
 * "suds.gml", a part of rainfall-runoff model. 
 * See {@link org.kalypso.planer.client.server.landuse.helper.AwMeasureHelper#setPlaningAreaMember}.</li>
 * </ul>
 * </li>
 * <li>If the value of optional parameter "CalculationNature" is
 * <code>null</code> or different than "PLC", the class does nothing.</li>
 * </ul>
...
public class NA_PostprocessingJob extends AbstractInternalStatusJob implements ISimulation {
	...
	if ("PLC".equals(calculationNature)) {
		...
		/*
		 * Create the shape root feature, we need it to create the
		 * children.
		 */
