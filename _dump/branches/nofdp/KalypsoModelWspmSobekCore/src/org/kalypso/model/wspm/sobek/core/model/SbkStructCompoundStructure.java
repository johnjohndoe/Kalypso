 ...
 * @author thuel2
 */
public class SbkStructCompoundStructure extends SbkStructure implements ISbkStructCompundStructure
{
  public SbkStructCompoundStructure( final IModelMember model, final Feature node )
  {
    super( model, node );
  }

  /**
   * @see org.kalypso.model.wspm.sobek.core.interfaces.ISbkStructCompundStructure#getContainedStructures()
   */
  public ISbkStructure[] getContainedStructures( )
  {
    final List<ISbkStructure> myStructs = new ArrayList<ISbkStructure>();

    final List< ? > structs = (List< ? >) getFeature().getProperty( ISobekConstants.QN_HYDRAULIC_SBK_STRUCTURE_COMPOUND_STRUCTURE_STRUCTURE_NODE_MEMBER );
    for( final Object object : structs )
    {
      if( !(object instanceof Feature) )
        continue;

      final Feature feat = (Feature) object;
      myStructs.add( (ISbkStructure) FNNodeUtils.getNode( getModelMember(), feat ) );
    }
