...
public class ProfileThroughputLocationCreator extends FeaturePropertyFunction
{
  ...
  public Object getValue( final Feature feature, final IPropertyType pt, final Object currentValue )
  {
    try
    {
      ...
      // FIXME: This string is just a copy of
      //IWspmTuhhConstants.MARKER_TYP_DURCHSTROEMTE
      // which cannot be used due to dependency limitations
      final IProfilPointMarker[] durchstroemte = profil.getPointMarkerFor( "urn:ogc:gml:dict:kalypso:model:wspm:profileMarkerComponents#DURCHSTROEMTE" );

      if( durchstroemte.length < 2)
        return null;

      final int left = profil.indexOfPoint( durchstroemte[0].getPoint() );
      final int right = profil.indexOfPoint( durchstroemte[1].getPoint() );
      ...
      for (int i = left; i <= right; i++)
      {
        ...
        {
          if( compBreite == -1 )
            throw new IllegalStateException( Messages.getString("org.kalypso.model.wspm.schema.gml.ProfileCacherFeaturePropertyFunction_0") );
