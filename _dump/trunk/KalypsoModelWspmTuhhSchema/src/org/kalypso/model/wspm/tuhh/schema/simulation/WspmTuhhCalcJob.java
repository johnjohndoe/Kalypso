...
public class WspmTuhhCalcJob implements ISimulation
{
  ...
  public void run( final File tmpDir, final ISimulationDataProvider inputProvider, final ISimulationResultEater resultEater, final ISimulationMonitor monitor ) throws SimulationException
  {
    ...
    try
    {
      ...
      switch( calcMode )
      {
        case WATERLEVEL:
        {
          ...
          //
          // overview map (generated by token replacement) either from OVW_MAP_SPECIAL or OVW_MAP_GENERAL (OvwMap)
          //
          if( ovwMapURL != null )
          {
            mapContent = ovwMapURL.openStream();
            final String inhalt = IOUtils.toString( mapContent );
            
            calculation.getReach().getFeature().getWorkspace().getFeaturepathForFeature(calculation.getReach().getFeature());
          }
          ...
          if( lenSecFile.exists() )
          {
            ...
            if( lengthSectionGmlFile.exists() )
            {
              ...
              {
                pwSimuLog.println( "‹berschwemmungslinie konnte nicht erzeugt werden: " + e.getLocalizedMessage() );
              }
            }
          }
    ...
    finally
    {
      ...
      IOUtils.closeQuietly( mapContent );
    }
  }
