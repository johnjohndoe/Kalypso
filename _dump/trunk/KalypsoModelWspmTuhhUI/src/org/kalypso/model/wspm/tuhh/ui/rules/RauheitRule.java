...
public class RauheitRule extends AbstractValidatorRule
{
  public void validate( final IProfil profil, final IValidatorMarkerCollector collector ) throws CoreException
  {
    ...
    for( final IRecord point : points )
    {
      ...
      if( value.isNaN() )
      {
        collector.createProfilMarker( IMarker.SEVERITY_ERROR, "unzul‰ssiger Datentyp f¸r "+ pointProp.getName(), "km " + Double.toString( profil.getStation() ), profil.indexOfPoint( point ), pointProp.getId(), pluginId );
        break;
      }
      else if( value <= 0.0 )
      {
        collector.createProfilMarker( IMarker.SEVERITY_ERROR, "unzul‰ssiger Rauheitswert [" + value + "]", "km " + Double.toString( profil.getStation() ), profil.indexOfPoint( point ), pointProp.getId(), pluginId );
        break;
      }
    }
  }
