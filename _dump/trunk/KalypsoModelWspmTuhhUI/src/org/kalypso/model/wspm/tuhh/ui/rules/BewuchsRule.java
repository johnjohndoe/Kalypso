    ...
    try
    {
      if( VorlandLhasValues | VorlandRhasValues && !Flussschl.isEmpty() )
      {
        ...
        for( final IRecord point : Flussschl )
        {
          ...
          if( ax + ay + dp != 0 )
            collector.createProfilMarker( false, "Bewuchsparameter im Flu�schlauch werden ignoriert", "", i, IWspmConstants.POINT_PROPERTY_BEWUCHS_AX, pluginId, new AbstractProfilMarkerResolution[] { new DelBewuchsResolution() } );
          i++;
        }
        ...
        if( VorlandLhasValues && (Double) points[lastIndex].getValue( ProfilObsHelper.getPropertyFromId( points[lastIndex], IWspmConstants.POINT_PROPERTY_BEWUCHS_AX ) ) == 0 )
          collector.createProfilMarker( false, "Bewuchsparameter an Trennfl�chen �berpr�fen", "", lastIndex, IWspmConstants.POINT_PROPERTY_BEWUCHS_AX, pluginId, new AbstractProfilMarkerResolution[] { new AddBewuchsResolution( 0 ) } );
        if( VorlandRhasValues && (Double) rightP.getValue( ProfilObsHelper.getPropertyFromId( rightP, IWspmConstants.POINT_PROPERTY_BEWUCHS_AX ) ) == 0 )
          collector.createProfilMarker( false, "Bewuchsparameter an Trennfl�chen �berpr�fen", "", rightIndex, IWspmConstants.POINT_PROPERTY_BEWUCHS_AX, pluginId, new AbstractProfilMarkerResolution[] { new AddBewuchsResolution( devider.length - 1 ) } );
      }
    }
