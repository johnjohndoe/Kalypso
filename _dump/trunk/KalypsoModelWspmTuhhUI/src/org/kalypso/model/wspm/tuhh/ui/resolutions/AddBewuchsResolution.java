  ...
  protected IProfilChange[] resolve( IProfil profil )
  {
    ...
    if( m_deviderIndex == 0 )
    {
      ...
      for( final IProfilPoint pt : leftPts )
      {
        ...
        try
        {
          final Double ax = pt.getValueFor( POINT_PROPERTY.BEWUCHS_AX );
          final Double ay = pt.getValueFor( POINT_PROPERTY.BEWUCHS_AY );
          final Double dp = pt.getValueFor( POINT_PROPERTY.BEWUCHS_DP );
          if( ax != 0.0 )
          {
            params[0] = ax;
          }
    ...
    changes[2] = new PointPropertyEdit( point, POINT_PROPERTY.BEWUCHS_DP, params[2] );
    return changes;
  }

  public AddBewuchsResolution( final Integer deviderIndex )
  {
    super( "Bewuchsparameter an der Trennfl‰che erzeugen", null, null );
