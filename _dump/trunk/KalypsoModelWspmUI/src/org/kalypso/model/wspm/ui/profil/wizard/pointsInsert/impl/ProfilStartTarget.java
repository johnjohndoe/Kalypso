...
public class ProfilStartTarget extends AbstractPointsTarget
{
  ...
  public void insertPoints( final IProfilEventManager pem, final IProfilPoints points )
  {
    ...
    try
    {
      ...
      final double deltaX = points.getPoints().getLast().getValueFor( POINT_PROPERTY.BREITE ) - targetPkt.getValueFor( POINT_PROPERTY.BREITE );
      final double deltaY = points.getPoints().getLast().getValueFor( POINT_PROPERTY.HOEHE ) - targetPkt.getValueFor( POINT_PROPERTY.HOEHE );
      int i = pointsCount-1 ;
...
//    try
//    {
//      final double deltaX = points.getPoints().getLast().getValueFor( POINT_PROPERTY.BREITE ) - pem.getProfil().getPoints().getFirst().getValueFor( POINT_PROPERTY.BREITE );
//      final double deltaY = points.getPoints().getLast().getValueFor( POINT_PROPERTY.HOEHE ) - pem.getProfil().getPoints().getFirst().getValueFor( POINT_PROPERTY.HOEHE );
