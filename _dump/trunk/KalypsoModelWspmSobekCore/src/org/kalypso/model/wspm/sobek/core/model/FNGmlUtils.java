...
import org.kalypso.model.wspm.sobek.core.interfaces.INodeTypes.NODE_BRANCH_TYPE;
...
public class FNGmlUtils
{
  ...
  {
    FNGmlUtils.createBranch( workspace, curve, new Feature[] {}, NODE_TYPE.eConnectionNode, NODE_TYPE.eConnectionNode );
  }
  ...
  private static Feature[] createBranch( final CommandableWorkspace workspace, final GM_Curve curve, final Feature[] nodes, final NODE_TYPE upperNodeType, final NODE_TYPE lowerNodeType ) throws Exception
  {
    ...
    final IRelationType rtBranchMember = (IRelationType) root.getFeatureType().getProperty( ISobekConstants.QN_HYDRAULIC_BRANCH_MEMBER );
    final IFeatureSelectionManager selectionManager = KalypsoCorePlugin.getDefault().getSelectionManager();

    final AtomarAddFeatureCommand command = new AtomarAddFeatureCommand( workspace, ftBranch, root, rtBranchMember, -1, null, selectionManager );
  ...
  public static Feature createNode( final CommandableWorkspace workspace, final NODE_TYPE nodeType, final GM_Point point, final Feature[] nodes ) throws Exception
  {
    ...
    final Feature root = workspace.getRootFeature();
    final IRelationType targetPropertyType = (IRelationType) root.getFeatureType().getProperty( ISobekConstants.QN_HYDRAULIC_NODE_MEMBER );
    ...
    values.put( nodeType.getTargetFeatureType( workspace ).getProperty( ISobekConstants.QN_HYDRAULIC_NODE_LOCATION ), point );
    ...
    final AtomarAddFeatureCommand command = new AtomarAddFeatureCommand( workspace, nodeType.getTargetFeatureType( workspace ), root, targetPropertyType, -1, values, selectionManager );
    workspace.postCommand( command );

    return command.getNewFeature();
  }

  private static String createNodeId( final GMLWorkspace workspace, final NODE_TYPE nodeType )
  {
  ...
  private static void deleteBranch( final GMLWorkspace workspace, final Feature branch ) throws Exception
  {
    ...
    final QName[] qn = new QName[] { ISobekConstants.QN_HYDRAULIC_BRANCH_UPPER_CONNECTION_NODE, ISobekConstants.QN_HYDRAULIC_BRANCH_LOWER_CONNECTION_NODE };
...
// delete branch
  ...
  private static void cleanUpNodes( final GMLWorkspace workspace, final Feature branch ) throws Exception
  {
    ...
    for( final Object object : myNodes )
    {
      ...
      final NODE_TYPE type = NODE_TYPE.getFeatureType( node );
      switch( type )
      {
        case eCrossSectionNode:
  ...
  private static void deleteConnectionNode( final Feature node ) throws Exception
  {
    ...
    final List< ? > outflowing = (List< ? >) node.getProperty( ISobekConstants.QN_HYDRAULIC_NODE_LINKED_OUTFLOWING_BRANCHES );
