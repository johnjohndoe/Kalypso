...
/**
 * Represents a curve made of lines, each of them being connected together, except (but
 * it can also be the case) for the first and the last lines.
 * 
 * <p>
 * NOTE: the class is currently under redesign, points are stored as Point2D and also as
 * coordinates. This is maybe not the best solution. But it allows us to perform some
...
public class PolyLine
{
     ...
     *
     * @param pts order is not important. Method sorts them on X coordinate.
     */
    ...
    /**
     * Shortens this polyline and returns new one. It only takes the values in the given
     * range of the X Axis. If from and/or to is/are not present (Double.NaN), it
     * computes the missing points and insert them into the new line.
     *
     * @param from
     * @param to
     *
    ...
    /**
     * Gibt Y Wert an einer bestimmten Stelle der PolyLine zurück. Der Wert wir linear
     * zwischen zwei Stützpunkten interpoliert. Funktioniert nur, wenn x-Wert austeigend
     * sortiert.
     *
     * @param X Für diesen x Wert wird der y-Wert gesucht
     * @param bExtrapol Für Punkte ausserhalb der Polylinie: falls true wird der Rand aus
     *        den beiden letzten/ersten Punkten extrapoliert, bei false wird die
     *        PolyLine wagerecht mit der Höhe des ersten/letzten Punktes fortgesetzt.
     *
    ...
    /**
     * UGLY: same method as getYFor(), but returns a Point2D instead. This code could be
     * ameliorated by mergin both methods.
     *
     * @param X
     * @param bExtrapol 
     *
     ...
     *
     * @param xe [null allowed] the refined X-coords of both p1 and p2, need to be
     *        sorted. When null is given, then xe is computed internally
     * @param p1
     * @param p2
     * @param mode
     *
    ...
    public static double area( PolyLine p1, PolyLine p2, int mode )
    {
        ...
        for( int i = 0 /*startX*/; i < ( xe.length - 1 ) /*( stopX - 1 )*/; i++ )
        {
            ...
            /*
             * TRICKY: compute area only when points' placement
             * verifies mode.
             */
            ...
            if( ( ( mode == OVER ) && ( dc.compare( y11, y12 ) >= 0 ) &&
                    ( dc.compare( y21, y22 ) >= 0 ) ) ||
                    ( ( mode == UNDER ) && ( dc.compare( y12, y11 ) >= 0 ) &&
                    ( dc.compare( y22, y21 ) >= 0 ) ) || ( mode == ALL ) )
            {
     ...
     *
     * @param other PolyLine to intersect
     *
    ...
    public double[] intersect( PolyLine other )
    {
        ...
        for( int i = 0; i < ( m_x.length - 1 ); i++ )
        {
            ...
            int startJ =
                Math.max( 0, Math.abs( Arrays.binarySearch( other.m_x, x11 ) ) - 2 );
            int stopJ =
                Math.min( other.m_x.length,
                    Math.abs( Arrays.binarySearch( other.m_x, x12 ) ) + 2 );
    ...
    /**
     * Returns the intersections (PolyLines) where pOther intersects pMaster. Only the
     * intersections of pOther that correspond to mode in regard to pMaster will be
     * returned.
     *
     * @param pMaster
     * @param pOther
     ...
     * @return
     */
    public static PolyLine[] intersectAsPolyLines( PolyLine pMaster, PolyLine pOther,
        int mode )
    {
        ...
        {
            /*
             * no intersection points, let check the position of the Y-coord
             * for one valid point and decide whether or not to build polygone
             */
        ...
        Vector vPols = new Vector( ints.length );
        ...
        return (PolyLine[])vPols.toArray( new PolyLine[vPols.size(  )] );
    }

    /**
     * Gibt die Vereinigung der X-Werte zweier Polylines sowie deren Schnittpunkte
     * zurück.
     *
     *
     * @return die (unsortierten) x-Werte: alle x-Werte von p1, p2 und vom Schnitt der
     *         beiden Polygone
     */
    public static double[] refinePolylines( final PolyLine p1, final PolyLine p2 )
    {
        ...
        // get all points from the union (on X) (use double[] instead of SortedSet because of performance)
        double[] xe =
            new double[( Math.max( p1.getX(  ).length, p2.getX(  ).length ) * 2 ) +
            interx.length];
    ...
    /**
     * builds the necessary polygones to represent the area between the two polylines.
     * Depending on the mode, it might either use all the points of the polylines or
     * just the ones that correspond to the Y-coord positioning.
     *
     * @param p1 the first polyline
     * @param p2 the second polyline
     * @param mode position of p1 in regards to p2 (
     *
     * @return
     *
     * @throws IllegalArgumentException when mode is not one of ALL, OVER, UNDER.
     *
    ...
    public static PolyGone[] spaceBetween( final PolyLine p1, final PolyLine p2, int mode )
    {
        ...
        else if( ( mode == PolyLine.OVER ) || ( mode == PolyLine.UNDER ) )
        {
            ...
            return (PolyGone[])vPols.toArray( new PolyGone[vPols.size(  )] );
        }
        else
    ...
    private static boolean isModeVerified( int mode, PolyLine p1, PolyLine p2 )
    {
        ...
        for( int i = 0; i < xs.length; i++ )
        {
            ...
            if( ( mode == ALL ) ||
                    ( ( mode == PolyLine.OVER ) && ( dc.compare( y1, y2 ) > 0 ) ) ||
    ...
    /**
     * Tells if the mode is verified according to the placement of the given polylines,
     * in the range from-to.<br>
     *
     * @param mode
     * @param p1
     * @param p2
        ...
        double xFrom, double xTo )
    {
        ...
        /*
         * take a point somewhere in the middle of the current segment
         * 
         * it is possible that this trick won't work all the time, for instance
         * if y1 == y2 at x, then we will return false if mode == OVER or UNDER.
         * This could be improved, for instance by trying more that one x-coord.
         */
    ...
    /**
     * Computes the length of the polyline. The length is the sum of the length of each
     * segment that build this polyline: <br>
     * L = S1 + ... + Si + ... + Sn<br>
    ...
    public double length(  )
    {
        ...
        for( int i = 0; i < ( m_y.length - 1 ); i++ )
            length += PointUtilities.segmentLength( m_x[i], m_y[i], m_x[i + 1], m_y[i +
                1] );
