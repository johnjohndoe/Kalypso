...
public class SobekResultModelHandler implements ISobekResultModel, IWorkspaceCache
{
  ...
  public IBranchLengthSectionModel getBranchHydrographModel( ) throws CoreException
  {
    try
    {
      if( m_branchesCommandableWorkspace == null )
      {
        ...
        if( m_branchesWorkspace instanceof CommandableWorkspace )
          m_branchesCommandableWorkspace = (CommandableWorkspace) m_branchesWorkspace;
        else
  ...
  public IResultTimeSeries getNodeTimeSeries( INode node ) throws CoreException
  {
    ...
    try
    {
      ...
      registerWorkspaces( node.getId(), gmlWorkspace, commandable );
      ...
      if( empty )
      {
        ...
        NodeTimeSeriesHandler handler = new NodeTimeSeriesHandler( commandable.getRootFeature(), node );

        final ResultWorker worker = new ResultWorker( commandable, binding, node, handler, m_lastfall.getLastfallStart(), m_lastfall.getLastfallEnd() );
      ...
      return new NodeTimeSeriesHandler( commandable.getRootFeature(), node );
    }
    catch( final Exception e )
    {
      throw new CoreException( StatusUtilities.createErrorStatus( e.getMessage() ) );
