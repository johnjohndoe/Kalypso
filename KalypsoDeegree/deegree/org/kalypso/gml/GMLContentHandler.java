...
public class GMLContentHandler implements ContentHandler, FeatureTypeProvider
{
	...
	private final static QName XSD_SCHEMALOCATION = new QName(NS.NS_XSD,
			"schemaLocation");
	...
	public GMLContentHandler(final XMLReader xmlReader,
			final URL schemaLocationHint, final boolean useGMLSchemaCatalog,
			final URL context)
	{
		m_xmlReader = xmlReader;
			...
			Attributes atts)
	{
			...
			if (m_schemaLocationHint != null)
				try
				{
					if (m_useSchemaCatalog)
						schema = GMLSchemaCatalog
								.getSchema(m_schemaLocationHint);
					else
						schema = GMLSchemaFactory
								.createGMLSchema(m_schemaLocationHint);
		...
		case START_PROPERTY_END_FEATURE:
		{
			...
			if (pt instanceof IValuePropertyType)
			{
				...
				m_propParser.setContent(parentFE, vpt, m_xmlReader, uri,
	...
	private String getSchemalocation(Attributes atts)
	{
		...
		{
			final QName attQName = new QName(atts.getURI(i), atts
					.getLocalName(i));
