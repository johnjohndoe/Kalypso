...
class MapView_Impl implements MapView {
    ...
    public double getScale (Graphics g) {
        scale = calcScale (g.getClipBounds().width,
    ...
    public void swapThemes(Theme first, Theme second) {
        ...
        if ( themesL.contains( first ) &&
             themesL.contains( second ) ) {
    ...
    /**
     * @see org.deegree_impl.graphics.MapView_Impl#getBoundingBox()
    ...
    private double calcScale(int mapWidth, int mapHeight) {
        ...
        try {
            ...
            GM_Position min = GeometryFactory.createGM_Position( bbox.getMin().getX() + dx*(mapWidth/2d-1), 
                                          bbox.getMin().getY() + dy*(mapHeight/2d-1) );
            GM_Position max = GeometryFactory.createGM_Position( bbox.getMin().getX() + dx*(mapWidth/2d), 
