...
public class WMSHelper
{
  ...
  private static void internalTransformation( final Graphics2D g2d, final GeoTransform projection, final TiledImage rasterImage, final RectifiedGridDomain gridDomain, final CS_CoordinateSystem targetCS ) throws Exception
  {
    ...
    final BufferedImage buffer = new BufferedImage( (int) buffImageEnv.getWidth(), (int) buffImageEnv.getHeight(), BufferedImage.TYPE_INT_ARGB );
  ...
  public static GM_Envelope getTransformedEnvelope( final GM_Envelope serverEnv, final CS_CoordinateSystem serverCRS, final CS_CoordinateSystem local )
  {
    try
    {
  ...
  /**
   * This method transformes an image from a source to a target coordiante system and paints it on the submitted graphic
   * context g.
   * 
   * @param remoteImage
  ...
  public static void transformImage( final TiledImage remoteImage, final GM_Envelope env, final CS_CoordinateSystem localCSR, final CS_CoordinateSystem remoteCSR, final GeoTransform worldToScreenTransformation, final Graphics g ) throws Exception
  {
    final int height = remoteImage.getHeight();
    final int width = remoteImage.getWidth();

    final OffsetVector offsetX = new OffsetVector( (env.getMax().getX() - env.getMin().getX()) / width, 0.0 );
    final OffsetVector offsetY = new OffsetVector( 0.0, (env.getMax().getY() - env.getMin().getY()) / height );
