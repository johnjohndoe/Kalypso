...
public class GC_GridCoverage_Impl extends CV_Coverage_Impl implements GC_GridCoverage {
     ...
     * reference system, an envelope describing the spatial extension of
     * of the coverage and the coverages metadata.
     * @param descriptor descrition of the GC
     * @param isEditable indicates if the grid coverages data can be edited
     */
    public GC_GridCoverage_Impl( CVDescriptor descriptor, boolean isEditable )
        ...
        gt = new WorldToScreenTransform( envelope.minCP.ord[0], envelope.minCP.ord[1], 
                                         envelope.maxCP.ord[0], envelope.maxCP.ord[1], 
                                         envImg.minCP.ord[0], envImg.minCP.ord[1], 
    ...
    */
     ...
     *
     */
    public void setPackedDataBlock( GC_GridRange gridRange, byte[] values )
                            throws RemoteException {
    }
     ...
     *
     */
    public void setDataBlockAsInteger( GC_GridRange gridRange, int[] values )
                               throws RemoteException {
    }
     ...
     *
     */
    public void setDataBlockAsByte( GC_GridRange gridRange, byte[] values )
                            throws RemoteException {
    }
     ...
     * @param env bounding box of the area to extracted
     */
     ...
     * (coordinates at the gc's CRS) as <tt>BufferedImage</tt>
     * @param env bounding box of the area to extracted
     * @param width width of the returned raster
     * @param height height of the returned raster
     * @param imageType (color model) type of the image will only be used if the
     *                  result is a <tt>BufferedImage</tt> @see java.awt.image.BufferedImage
     */
    public Object getRaster( GM_Envelope env, int width, int height, int imageType, 
                             RangeParamList rangeParams ) throws IOException, 
     ...
     * If no CVRange matches, the upper level Level is returned -- FIXME check in default values instead
     *
     * TODO only first-level Ranges are explored. We must recurse into deeper levels.
     *
     * @param    rangeParams           a  Map of RangeParam; key is the name
     * @param    rangeCollector   will be filled with the selected ranges
     *
    ...
    /**
     * reads a grid coverage from image sources and returns it as <tT>BufferedImage</TT>
     */
    private BufferedImage getBufferedImage( GM_Envelope env, int width, int height, int imageType, 
                                            Tile[] tiles, RangeParamList rangeParams, 
                                            List usedRanges ) throws IOException {
        BufferedImage bi = new BufferedImage( width, height, imageType );

        GeoTransform gtd = new WorldToScreenTransform( env.getMin().getX(), env.getMin().getY(), 
                                                       env.getMax().getX(), env.getMax().getY(), 0, 0, 
                                                       width, height );
            ...
            if ( senv != null ) {
                ...
                if ( url.getFile().toLowerCase().endsWith( ".ecw" ) ) {
                    try {
                        ...
                        //get image using intersection of dest-env & tile-env and width and height of dest-rect
    ...
    public Object getRaster( GM_Envelope env, int width, int height ) throws IOException {
        ...
        try {
            o = getRaster( env, width, height, BufferedImage.TYPE_INT_ARGB, null ); //ETJ FIXME
        } catch ( Exception e ) {
            throw new IOException( e.toString() );
        }
...
/* ********************************************************************
   ...
   Revision 1.31  2004/03/26 11:19:29  poth
   no message
