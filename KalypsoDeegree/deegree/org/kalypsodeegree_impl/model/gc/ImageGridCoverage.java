...
public class ImageGridCoverage extends CV_Coverage_Impl implements GC_GridCoverage
{
     ...
     */
    public ImageGridCoverage(BufferedImage image, GM_Envelope bbox, 
                             CS_CoordinateSystem crs, boolean isEditable)
    {
        super( crs );       
    ...
    public byte[] getDataBlockAsByte(GC_GridRange gridRange) throws RemoteException {                
        ...
        if ( hi.length != lo.length ) throw new RemoteException( "Invalid Parameter" );
        
        int c1 = lo[0]; int c2 = hi[0];
        int r1 = 0; int r2 = (int)(envImg.maxCP.ord[0] - envImg.minCP.ord[0]);
        if ( hi.length > 1 ) {
            r1 = lo[1]; r2 = hi[1];            
        }
