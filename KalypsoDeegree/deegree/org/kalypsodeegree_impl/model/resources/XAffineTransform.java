...
public abstract class XAffineTransform extends AffineTransform
{
   ...
   * @param sy
   *          Echelle le long de l'axe des <var>y </var>.
   * @param x
  ...
  public static void round( final AffineTransform zoom )
  {
        ...
        && Math.abs( ( m10 = Math.rint( r = zoom.getShearY() ) ) - r ) <= EPS )
    {
      ...
      {
        double m02 = Math.rint( r = zoom.getTranslateX() );
        if( !( Math.abs( m02 - r ) <= EPS ) )
          m02 = r;
        double m12 = Math.rint( r = zoom.getTranslateY() );
        if( !( Math.abs( m12 - r ) <= EPS ) )
          m12 = r;
