...
* specialized by derived classes <code>GM_Polygon</code> for example
*
* ------------------------------------------------------------
* @version 11.6.2001
* @author Andreas Poth 
...
abstract class GM_SurfacePatch_Impl implements GM_GenericSurface, Serializable {
    ...
    protected GM_SurfacePatch_Impl( GM_SurfaceInterpolation interpolation, 
                                    GM_Position[] exteriorRing, GM_Position[][] interiorRings, 
     ...
     * used for this GM_SurfacePatch. This mechanism uses the control 
     * points and control parameters defined in the various subclasses 
     * to determine the position of this GM_ SurfacePatch.
     */
	...
	//    return true;
        ...
        if ( interiorRings != null ) {
            ...
            for ( int i = 0; i < interiorRings.length; i++ ) {
                if ( !Arrays.equals( interiorRings[i], 
                                     ( (GM_SurfacePatch)other ).getInteriorRings()[i] ) ) {
                    return false;
                }
            }
        } else {
