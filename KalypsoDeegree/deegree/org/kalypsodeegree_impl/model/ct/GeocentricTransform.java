...
class GeocentricTransform extends AbstractMathTransform implements Serializable
{
      ...
      int dstOff, int numPts )
  {
    ...
    while( --numPts >= 0 )
    {
      ...
      final double sin_B0 = T0 / S0; // sin(B0), B0 is estimate of Bowring aux
      // variable
      ...
      // of
      // cos(phi1)
      final double S1 = Math.sqrt( T1 * T1 + sum * sum ); // corrected estimate
      // of horizontal
      // component
      ...
      if( computeHeight )
      {
        final double rn = a / Math.sqrt( 1 - e2 * ( sin_p1 * sin_p1 ) ); // Earth
        // radius
        // at
        // location
        if( cos_p1 >= +COS_67P5 )
  ...
  static final class Provider extends MathTransformProvider
  {
    ...
    public Provider( final boolean inverse )
    {
      ...
      putInt( "dim_geoCS", 3, DIM_RANGE ); // Custom parameter: NOT AN OPENGIS
      // SPECIFICATION
