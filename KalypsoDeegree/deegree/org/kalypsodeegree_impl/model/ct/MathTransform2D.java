...
/**
 * Transforms two-dimensional coordinate points. {@link CoordinateTransformation#getMathTransform}may returns instance
 * of this interface when source and destination coordinate systems are both two dimensional.
 * <code>MathTransform2D</code> extends {@link MathTransform}by adding some methods for easier interoperability with
 * <A HREF="http://java.sun.com/products/java-media/2D/">Java2D </A>. If the transformation is affine, then
 * <code>MathTransform</code> shall be an immutable instance of {@link AffineTransform}.
 * 
 * @version 1.00
...
public interface MathTransform2D extends MathTransform
{
   ...
   * is <code>null</code>, a new {@link Point2D}object is allocated and then the result of the transformation is
   * stored in this object. In either case, <code>ptDst</code>, which contains the transformed point, is returned for
  ...
  /**
   * Transform the specified shape. This method may replace straight lines by quadratic curves when applicable. It may
   * also do the opposite (replace curves by straight lines). The returned shape doesn't need to have the same number of
   * points than the original shape.
   * 
   * @param shape
   *          Shape to transform.
   ...
   * @return The derivative at the specified point as a 2&times;2 matrix. This method never returns an internal object:
