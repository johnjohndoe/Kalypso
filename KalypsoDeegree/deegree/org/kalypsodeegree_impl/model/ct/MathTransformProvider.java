...
public abstract class MathTransformProvider
{
      ...
      final Range range ) throws IllegalStateException
  {
    ...
    if( defaultValue != null && range != null )
    {
      ...
      Object check;
      if( defaultValue.equals( check = range.getMinValue() ) )
        defaultValue = check;
  ...
  private static Object wrap( final double value )
  {
    ...
    if( value == Latitude.MIN_VALUE )
      return LATITUDE_RANGE.getMinValue();
    if( value == Latitude.MAX_VALUE )
      return LATITUDE_RANGE.getMaxValue();
    if( value == Longitude.MIN_VALUE )
      return LONGITUDE_RANGE.getMinValue();
    if( value == Longitude.MAX_VALUE )
