...
final class TransverseMercatorProjection extends CylindricalProjection {
    ...
    protected Point2D transform( double x, double y, final Point2D ptDst )
        ...
        if ( isSpherical ) {
            ...
            y = ( ak0 * ( mlfn( y, sinphi, cosphi ) + 
                    sinphi * al * x * FC2 * ( 1.0 + 
                        FC4 * als * ( 5.0 - t + ( n * ( 9.0 + 4.0 * n ) ) + 
                            FC6 * als * ( 61.0 + ( t * ( t - 58.0 ) ) + 
                                ( n * ( 270.0 - 330.0 * t ) ) + 
    ...
    protected Point2D inverseTransform( double x, double y, final Point2D ptDst )
        ...
        if ( isSpherical ) {
            ...
            y = ( y < 0.0 )     ? ( -phi ) : phi;
            x = ( Math.abs( d ) > EPS10 || Math.abs( t ) > EPS10 )
                ? ( Math.atan2( d, t ) + centralMeridian ) : centralMeridian;
        } else {
            ...
            if ( Math.abs( phi ) >= ( Math.PI / 2.0 ) ) {
                ...
                double ds = d * d;
                y = phi - 
                    ( ( con * ds / ( 1.0 - es ) ) * FC2 * ( 1.0 - 
                        ds * FC4 * ( 5.0 + ( t * ( 3.0 - 9.0 * n ) ) + ( n * ( 1.0 - 4 * n ) ) - 
                            ds * FC6 * ( 61.0 + ( t * ( 90.0 - ( 252.0 * n ) + 45.0 * t ) ) + 
                                ( 46.0 * n ) - 
                                ds * FC8 * ( 1385.0 + 
                ...
                x = ( ( d * ( FC1 - 
                            ds * FC3 * ( 1.0 + ( 2.0 * t ) + n - 
                                ds * FC5 * ( 5.0 + ( t * ( 28.0 + ( 24 * t ) + 8.0 * n ) ) + 
                                    ( 6.0 * n ) - 
                                    ds * FC7 * ( 61.0 + 
                                        t * ( 662.0 + t * ( 1320.0 + 720.0 * t ) ) ) ) ) ) ) / cosphi ) + 
                    centralMeridian;
            }
        }
    ...
    public boolean equals( final Object object ) {
        ...
        if ( super.equals( object ) ) {
            ...
            return ( Double.doubleToLongBits( this.false_easting ) == Double.doubleToLongBits( 
                                                                              that.false_easting ) ) && 
