...
public class GMLGeoProperty_Impl extends GMLProperty_Impl implements GMLGeoProperty {
    ...
    public static GMLGeoProperty createGMLGeoProperty( Document doc, int geoType )
        ...
        switch ( geoType ) {
            ...
            case MULTILINESTRING:
                elem = doc.createElementNS( GMLGeometricMapping.GMLNS, 
            ...
            case MULTIGEOMETRY:
                ...
                break;
            default:
                throw new GMLException( geoType + 
                                        ": isn't a valid identifier for a geometry property!" );
        }
    ...
    /**
     * validate if the submitted geometry is the same type as
     * the geometry tag
     */
        ...
        switch ( geoType ) {
            ...
            case MULTILINESTRING:
            {
                if ( !( value instanceof GMLMultiLineString ) ) {
                    throw new GMLException( 
                            "A multiLineStringProperty only can contain multilinestrings!" );
                }
            ...
            case MULTIPOLYGON:
            {
                if ( !( value instanceof GMLMultiPolygon ) ) {
                    throw new GMLException( 
                            "A multiPolygonProperty only can contain multipolygons!" );
                }
            ...
            case MULTIGEOMETRY:
            {
                if ( !( value instanceof GMLGeometryCollection ) ) {
                    throw new GMLException( 
                            "A multiGeometryProperty only can contain geometry collections!" );
                }
 ...
 * no message
 *
 * Revision 1.5  2003/11/26 17:05:35  poth
 * no message
 *
 * Revision 1.4  2003/11/26 07:47:18  poth
 * no message
 *
 * Revision 1.3  2003/04/23 15:44:39  poth
 * no message
 *
 * Revision 1.2  2003/02/24 09:34:42  poth
 * no message
 *
 * Revision 1.1.1.1  2002/09/25 16:01:02  poth
 * no message
 *
 * Revision 1.8  2002/08/19 15:59:29  ap
 * no message
 *
 * Revision 1.7  2002/08/05 16:11:02  ap
 * no message
 *
 * Revision 1.6  2002/08/01 08:56:56  ap
 * no message
 *
 */
