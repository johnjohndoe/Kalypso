...
public class FeatureTypeBuilder
{
    ...
    public FeatureTypeBuilder(GMLSchema schema, Node node) throws Exception
    {
        ...
        if (!element.hasAttribute("name"))
            ...
            throw new Exception("no valid elementnode (name is missing): "
                    + XMLHelper.toString(node));

        // set Name
        SchemaAttribute nameAttribute = new SchemaAttribute(m_schema, XMLHelper
                .getAttributeNode(node, "name"));
        m_name = nameAttribute.getValue();

        // set substitutiongroup
        final Node substitutionGroupNode = XMLHelper.getAttributeNode(node,
                "substitutionGroup");

        if (substitutionGroupNode != null)
        {
            final SchemaAttribute substitueAttribute = new SchemaAttribute(
                    m_schema, substitutionGroupNode);
        ...
        { // complexType is inner definition
            Node cNode = XMLHelper.getFirstChildElement(node,
                    XMLHelper.XMLSCHEMA_NS, "complexType", 1);
        ...
        // complexType
        {
            SchemaAttribute typeAttribute = new SchemaAttribute(schema,
                    attributeNode);
    ...
    public void setOccurency(String name, String namespace, Node node)
    {
        ...
        if (minNode != null)
        {
            SchemaAttribute minOccursAttribute = new SchemaAttribute(m_schema,
                    minNode);
        ...
        if (maxNode != null)
        {
            SchemaAttribute maxOccursAttribute = new SchemaAttribute(m_schema,
                    maxNode);
    ...
    public void setTypeName(SchemaAttribute typeAttribute) throws Exception
    {
        ...
        final ITypeHandler typeHandler = TypeRegistrySingleton
                .getTypeRegistry().getTypeHandlerForTypeName(typename);
        if (typeHandler != null)
        {
        ...
        if ("complexType".equals(((Element) cNode).getLocalName()))
        {
            ...
            SchemaAttribute innerTypeAttribute = new SchemaAttribute(schema,
                    XMLHelper.getAttributeNode(custoumNode, "type"));
        ...
        } else if ("simpleType".equals(((Element) cNode).getLocalName()))
        {
            ...
            throw new Exception("content of element not found in Schema "
        ...
        if ("FeatureAssociationType".equals(m_typeName))
        {
            if (m_featureProtoTypes.size() > 0)
            {
                ...
                if (associateableFeatureType instanceof FeatureType)
                {
                    ...
                    return new FeatureAssociationTypeProperty_Impl(m_name,
                            m_namespace, m_typeName, true, m_schema, ftp,
                            m_annotationMap);
                }
                if (associateableFeatureType instanceof FeatureTypeBuilder)
                {
                    ftp = ((FeatureTypeBuilder) associateableFeatureType)
                            .toFeatureType();
                    return new FeatureAssociationTypeProperty_Impl(m_name,
                            m_namespace, m_typeName, true, m_schema, ftp,
                            m_annotationMap);
                ...
                {
                    return new FeatureAssociationTypeProperty_Impl(m_name,
                            m_namespace, m_typeName, true, m_schema,
            ...
            {
                if (XMLHelper.GMLSCHEMA_NS.equals(m_namespace)
                        && "featureMember".equals(m_name))
                    return new FeatureAssociationTypeProperty_Impl(m_name,
                            m_namespace, m_typeName, true, m_schema, m_schema
                                    .getFeatureType("_Feature"),
                            m_annotationMap);
            }
            throw new UnsupportedOperationException();
        }
        if (m_enumeration.size() > 0)
            return new EnumerationFeatureTypeProperty(m_name, m_namespace,
                    m_typeName, true, m_enumeration.toArray(), m_annotationMap);

        // if( m_typeName == null )
        // System.out.println( "debug" );

        if (m_isCutoumType)
            return new CustoumFeatureTypeProperty(m_name, m_namespace,
                    m_typeName, true, m_annotationMap);
        return FeatureFactory.createFeatureTypeProperty(m_name, m_namespace,
                m_typeName, true, m_annotationMap);
    }
    ...
    public FeatureType toFeatureType()
    {
        ...
        for (int i = 0; i < m_featureProtoTypes.size(); i++)
        {
            ...
            if (o instanceof FeatureTypeBuilder)
            {
                ...
                if (builder.isFeaturePropertyType())
                    featureTypePropertyList
                            .add(builder.toFeatureTypeProperty());
        ...
        {
            String key = ftProperties[i].getNamespace() + ":"
                    + ftProperties[i].getName();
        ...
        final FeatureType realFeatureType = FeatureFactory.createFeatureType(
                m_name, m_namespace, ftProperties, minOccurs, maxOccurs,
                m_substitutionGroup, m_annotationMap);
