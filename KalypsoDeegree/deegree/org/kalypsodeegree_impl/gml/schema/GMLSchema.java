...
public class GMLSchema
{
   ...
   * @throws Exception Falls GML nicht geladen werden kann
   */
  ...
  private void setImportedSchemas()
  {
    ...
    NodeList nl = m_schemaDoc.getElementsByTagNameNS( XMLHelper.XMLSCHEMA_NS, "import" );
    for( int i = 0; i < nl.getLength(); i++ )
    {
      try
      {
        Node attributeNode = XMLHelper.getAttributeNode( nl.item( i ), "schemaLocation" );
        String schemaLocation = attributeNode.getNodeValue();

        GMLSchema schema = null;
        if( m_url == null )
        {
//          URL url = new URL( schemaLocation );
//          if( "http://troubadix.wb.tu-harburg.de/doemming/test/feature.xsd".equals( url.toString() ) )
//          {
//            // TODO remove this workaround
//            URI uri = new URI( url.toExternalForm() );
//            File schemaFile = new File(
//                "/home/doemming/eclipse_workspace/KalypsoNA/src/schema/feature.xsd" );
//            schema = new GMLSchema( uri, XMLHelper.getAsDOM( schemaFile ) );
//          }
//          else
//          {
//            final InputStream stream = url.openStream();
//            URI uri = new URI( url.toExternalForm() );
//            schema = new GMLSchema( uri, XMLHelper.getAsDOM( stream ) );
//          }
        }
        else
        {
          ...
          final URL url = new URL( m_url, uri.getPath() );
