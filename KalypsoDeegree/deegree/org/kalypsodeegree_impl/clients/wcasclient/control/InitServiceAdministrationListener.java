...
/**
 * This <tt>Listener</tt> reacts on 'initServiceAdministration' - events, queries
 * the WCAS and passes the service data on to be displayed by the JSP.   
 * <p>
...
public class InitServiceAdministrationListener extends AbstractSecuredListener {
	 ...
	 */
	public static Set getBriefDescriptions (URL catalogURL) throws IOException, 
		...
		String briefRequest = 
			...
			" startPosition=\"-1\">" +
			"<Query typeName=\"Service\">" +
			"<PropertySet setName=\"Full\"/>" +
			"</Query>" +
			"</GetRecord>";
		...
		Element searchResultElement = XMLTools.getRequiredChildByName (
			"searchResult", null, doc.getDocumentElement ());
		ElementList serviceElements = XMLTools.getChildElementsByName (
			"ISO19119", null, searchResultElement);

		for (int i = 0; i < serviceElements.getLength (); i++) {
			Element serviceElement = serviceElements.item (i);
			String serviceId = XMLTools.getRequiredStringValue (
				"fileIdentifier", null, serviceElement);
			String serviceType = XMLTools.getRequiredStringValue (
				"serviceType", null, serviceElement);
			Element citationElement = XMLTools.getRequiredChildByName (
				"citation", null, serviceElement);
			String serviceName = XMLTools.getRequiredStringValue (
				"title", null, citationElement);
	...
	public static String [] getFullDescription (URL catalogURL, String serviceId) throws Exception {
		...
		details [0] = serviceId;
		details [1] = XMLTools.getRequiredStringValue (
			"serviceType", null, serviceElement);
		details [2] = XMLTools.getRequiredStringValue (
			"serviceTypeVersion", null, serviceElement);

		Element citationElement = XMLTools.getChildByName (
			"citation", null, serviceElement);
		if (citationElement != null) {
			details [3] = XMLTools.getStringValue (
				"title", null, citationElement, "");
			details [4]= XMLTools.getStringValue (
				"abstract", null, serviceElement, "");

			Element pointOfContactElement = XMLTools.getChildByName (
				"pointOfContact", null, serviceElement);
			if (pointOfContactElement != null) {
				details [5] = XMLTools.getStringValue (
					"individualName", null, pointOfContactElement, "");
				details [6] = XMLTools.getStringValue (
					"positionName", null, pointOfContactElement, "");
				details [7] = XMLTools.getStringValue (
					"organizationName", null, pointOfContactElement, "");
				details [8] = XMLTools.getStringValue (
					"onlineResource", null, pointOfContactElement, "");

				Element contactInfoElement = XMLTools.getChildByName (
					"contactInfo", null, pointOfContactElement);
				if (contactInfoElement != null) {
					Element addressElement = XMLTools.getChildByName (
						"address", null, contactInfoElement);
					if (addressElement != null) {			
						details [9] = XMLTools.getStringValue (
							"deliveryPoint", null, addressElement, "");
						details [10] = XMLTools.getStringValue (
							"city", null, addressElement, "");
						details [11] = XMLTools.getStringValue (
							"postalCode", null, addressElement, "");
						details [12] = XMLTools.getStringValue (
							"country", null, addressElement, "");
						details [13] = XMLTools.getStringValue (
							"electronicMailAddress", null, addressElement, "");
					}
				}
			}
		}
