...
public class MapViewListener extends AbstractListener {    
    ...
    public void actionPerformed( FormEvent e ) {
        ...
        try {
            ...
            gotoErrorPage( "Invalid Request: " +    
                           StringExtend.stackTraceToString( ee.getStackTrace() ) );
        ...
        if ( !msg.equals( "" ) ) {
            ...
            if ( gmr != null ) {
                ...
                this.getRequest().setAttribute( org.deegree_impl.clients.wmsclient.model.Constants.WMSGETMAPREQUEST, mapReq);
            }            
    ...
    private HashMap getWMSGetMapRequests(RPCWebEvent webEvent) {
        ...
        try {
            ...
            gotoErrorPage( "Error: " +
                           StringExtend.stackTraceToString( ex.getStackTrace() ) );
        }         
     ...
     * served by them. for each catalog the returned HashMap contains an 
     * ArrayList containing the associated IDs
     */
    ...
    */
     ...
     * @param ids IDs/layers to find the serving WMS
     * @param bbox bbox of the resulting WMS GetMap request(s)
     */
    private HashMap getWMSGetMapCalls(String catalog, String[] ids, GM_Envelope bbox) 
        ...
        try {
            String casReq = 
                ISO19119RequestFactory.createRequest( "Full", "WMS", ids, "GetMap" );
        ...
        WMSClientConfiguration wmsConf = CSWClientConfiguration.getInstance().getWMSClientConfiguration();
        
        HashMap map = new HashMap();
        map.put( "REQUEST", "GetMap" );
		map.put( "SERVICE", "WMS" );
		map.put( "VERSION", version );
		map.put( "BBOX", bbox.getMin().getX() + "," + bbox.getMin().getY() +"," + 
    ...
    /**
    * calculates the common bounding box of all submitted boxes and returns it 
    * in the coordinate reference system of the dataset
    */
    private GM_Envelope calcBoundingBox( RPCWebEvent event ) throws Exception {
        ...
        GM_Envelope bbox = GeometryFactory.createGM_Envelope( min.getX(), min.getY(), 
        													  max.getX(), max.getY() );
