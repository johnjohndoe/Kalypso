...
public class ThesaurusRequestFactory {
    /**
     * creates a GetFeature request against a thesaurus. the passed term will
     * be compared with the TH_TERM field
     *
     * @param pattern 
     *
     * @return 
    ...
    public static WFSGetFeatureRequest createRequest( String pattern ) {
        ...
        Operation op = new PropertyIsLikeOperation( new PropertyName( "TH_TERM" ), 
                                          new Literal( "*" + pattern + "*" ), 
                                          '*', '?', '/' );
        Filter filter = new ComplexFilter( op );
        
        WFSQuery query = WFSProtocolFactory.createQuery( new String[] { "TH_TERM", "TH_DESC_NO" }, 
                                         null, "1.0.0", "Thesaurus", filter );
        IDGenerator idg = IDGenerator.getInstance();
        WFSGetFeatureRequest request = 
        	WFSProtocolFactory.createWFSGetFeatureRequest( "1.0.0", ""+idg.generateUniqueID(), 
