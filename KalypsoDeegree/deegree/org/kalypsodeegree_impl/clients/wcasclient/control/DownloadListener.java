...
//        Debug.level = Debug.ALL;
    ...
    */
    private HashMap getCatalogIdAssociations(RPCWebEvent event) throws CatalogClientException, Exception {
        Debug.debugMethodBegin();
     ...
     * feature template. 
     *
     * @param catalog catalog where the required ISO 19119 data are stored
     * @param ft feature template to find the serving WFS
     */
    private HashMap getWFSGetFeatureCalls(String catalog,FeatureTemplate[] ft ) 
        ...
        try {
            ...
            for (int z = 0; z < ft.length; z++) {
                String[] ids = new String[] { ft[z].getId() };
                String casReq = 
                    ISO19119RequestFactory.createRequest( "Full", "WFS", ids, "Query" );
    ...
    private WFSGetFeatureRequest createGetFeatureRequest( FeatureTemplate ft, String version ) 
        ...
        try {
            ...
            gfr = WFSProtocolFactory.createWFSGetFeatureRequest( "1.0.0", 
                                                ""+idg.generateUniqueID(), 
                                                null, null, "GML2", null, null, -1, 0, 
                                                new WFSQuery[] { query } );
        } catch (Exception e) {
            Debug.debugException( e, "" );
            throw new CatalogClientException( "couldn't create GetFeature request " +
                                              "for: " + ft.getId() + "\n" + ft.getEnvelope(),
                                              e );
        }
     ...
     * WFS. The class also informs the user who had requested the data via 
     * email that the data are ready for download if everything had worked fine.
     * otherwise the the user and the administrator will be informed about
     * the problem that had occured.
     */
    private class LoadController extends Thread {
        ...
        /**
         * initialize the LoadController with an association of web addresses 
         * with GetFeature requests.
         */
        ...
        public void run() {
            ...
            while ( iterator.hasNext() ) {
                ...
                try { 
                    ...
                    rollback( zipFiles );
                    Debug.debugException( e, "create shape file: " + 
                                             getFeat.getQuery()[0].getTypeName());
                    try {
        ...
        /**
         * stores the passed feature collection as zipped shape and returns the 
         * file name
         *
         * @param fc feature collection to be stored
         * @param typeName name of the feature type requested from the WFS
         */
        private String storeFC(FeatureCollection fc, String typeName ) throws CatalogClientException {
            ...
            try {                
                ...
                String shapeBaseName = 
                    typeName + System.currentTimeMillis() + "" + l;
                ...
                fileNames = new String[] { shapeBaseName+".shp", 
                                           shapeBaseName+".shx", 
                                           shapeBaseName+".dbf" };
        ...
        /**
        * validates the result of the catalog request and returns a <tt>Document</tt>
        * or a message string depending on the results content.
        */
         ...
         *
         * @param zipFiles list of zip files associated to the download order
         */
        ...
        /**
         * sends a mail to inform the administrator and the user about an error
         * that raised performing the data access
         */
        private void sendErrorMail(Exception e, String addr, String target) throws SendMailException {
            ...
            MailMessage mm = new EMailMessage( download.getMailFrom(), mailAddress, 
                                               download.getMailSubject(), message );
            MailHelper.createAndSendMail( mm, download.getMailHost() );
            
            // send message to administrator
            mm = new EMailMessage( download.getMailFrom(), download.getMailFrom(), 
                                   download.getMailSubject(), message );
            MailHelper.createAndSendMail( mm, download.getMailHost() );
        ...
        /**
         * sends a mail to the user that the data download succeded and informs
         * him where to download the created files
         *
         * @param files list of created files (names)
         *
...
//            String mailAddress = u.getEmail();
            ...
            // send message to the user
            MailMessage mm = new EMailMessage( download.getMailFrom(), mailAddress, 
                                               download.getMailSubject(), sb.toString() );
            MailHelper.createAndSendMail( mm, download.getMailHost() );
