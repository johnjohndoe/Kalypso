...
public class TakeSearchBoxListener extends AbstractListener {    
    ...
    /**
     *
     * @param gmr GetMap request
     * @param minx pixel minx ccordinate
     * @param miny pixel miny ccordinate
     * @param maxx pixel maxx ccordinate
     * @param maxy pixel maxy ccordinate
     *
     * @return CRS coordinates corresponding to the pixel coordinates
     */
    private GM_Envelope calcualteSearchBox(WMSGetMapRequest gmr , double minx, 
        ...
        GeoTransform gti = 
            new WorldToScreenTransform( bbox.getMin().getX(), bbox.getMin().getY(),  
                                        ...
                                        0, 0, gmr.getWidth()-1, gmr.getHeight()-1 );
     ...
     *
     * @param searchBox search area defined by the user
     */
    private void handle(GM_Envelope searchBox) {
        ...
        RPCParameter[] detailedsearchParams = 
            (RPCParameter[])session.getAttribute( Constants.SESSION_DETAILEDSEARCHPARAM );
        ...
        bbox.addMember( mem );
        mem = new RPCMember_Impl( Double.class, ""+searchBox.getMin().getY(), 
                                                Constants.RPC_BBOXMINY );
        bbox.addMember( mem );
        mem = new RPCMember_Impl( Double.class, ""+searchBox.getMax().getX(), 
                                                Constants.RPC_BBOXMAXX );
        bbox.addMember( mem );
        mem = new RPCMember_Impl( Double.class, ""+searchBox.getMax().getY(), 
