...
public class WMSClientConfiguration implements Marshallable {
     ...
     * is an instance of the default map client configuration. if one of the 
     * other parameters submitted to the constructor isn't set, the value(s)
     * of the default configuration will be used. this inculdes that if the 
     * default configuration is created all parameters must be set! Therefore
     * an instance of a <tt>WMSClientConfiguration</tt> shall be created using
     * a factory.
     *
     * @param defaultConfig default map client configuration
     * @param wmsCapabilities map of the capabilities of the WMS known by the client
     * @param offeredMapSizes list of map sizes offered by the client
     * @param offeredMapFormats list of map formats offered by the client
     * @param offeredInfoFormats list of info formats offered by the client
     * @param offeredMapOperations list of map operations offered by the client
     * @param offeredZoomFactors list of zoom factors offered by the client
     * @param offeredPanFactors list of pan factors offered by the client
     * @param minScale min map scale handled by the client
     * @param maxScale maximum map scale handled by the client
     * @param gmr initial GetMap request (first map the user will see)
     */
    WMSClientConfiguration( WMSClientConfiguration defaultConfig, HashMap wmsCapabilities, 
                            MapSize[] offeredMapSizes, Format[] offeredMapFormats, 
                            Format[] offeredInfoFormats, MapOperation[] offeredMapOperations, 
                            MapOperationFactor[] offeredZoomFactors, 
                            MapOperationFactor[] offeredPanFactors, double minScale, 
                            double maxScale, WMSGetMapRequest gmr, Project[] offeredProjects ) {
    ...
    public Object clone() {
        ...
        for (int i = 0; i < ms.length; i++) {
            ...
            ms[i] = new MapSize( m.getWidth(), m.getHeight(),  m.isSelected(), 
                                 m.isFree() );
        }
        ...
        return new WMSClientConfiguration( defaultConfig, wmsCapabilities, ms, mapFormat, 
                            infoFormat, mapOp, zoom, pan, minScale, maxScale, gmr,
                            projects );
    }
     ...
     *
     * @param id ID of the WMS which capabilities shall be returned
     *
     ...
     *
     * @param id ID of the WMS
     * @param url reference to the capabilities of the WMS
     */
     ...
     *
     * @param id ID of the WMS
     * @param capabilities capabilities of the WMS
     */
    ...
    /**
     * returns the <tt>MapSize</tt> that is marked as selected. If no <tt>MapSize</tt>
     * is marked, the first <tt>MapSize</tt> will be returned.
     */
    ...
    /**
     * returns the map format that is marked as selected. If no map format
     * is marked, the first map format will be returned.
     */
