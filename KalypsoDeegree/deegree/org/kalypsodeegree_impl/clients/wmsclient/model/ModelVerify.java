...
public class ModelVerify {
    /**
     * validates the bounding box of the model against the maximal
     * possible bounding box. If the bounding box of the model exceeds
     * the limiting bounding box it will be corrected 
     */
     ...
     * if the scale of the model is smaller then minScale the model will
     * be rescaled.
     */
    public synchronized static WMSGetMapRequest setToMinScale( WMSGetMapRequest mrm, 
                                                               double minScale ) {
        ...
        if ( scale < minScale ) {
            GM_Position center = 
            	GeometryFactory.createGM_Position( ( ( maxx - minx ) / 2.0 ) + minx, 
                                                   ( ( maxy - miny ) / 2.0 ) + miny );
    ...
    /**
     * returns the scale of the requested map. the scale is measured as defined
     * at the WMS 1.0.0 specifications of the OGC
     */
