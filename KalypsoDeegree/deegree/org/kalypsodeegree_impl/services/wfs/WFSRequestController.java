...
/**
 * the class administers all request that are in progress by the WFS
 * (Dispatcher)
 * <p>
 * --------------------------------------------------------
 * </p>
...
final public class WFSRequestController implements RequestController
{
  ...
  public void addRequestPart( Object obj )
  {
    ...
    this.handler.put( obj, Boolean.FALSE );
  ...
  public void addResponse( OGCWebServiceResponse response )
  {
    ...
    if( response instanceof WFSGetFeatureResponse || response instanceof WFSTransactionResponse
  ...
  /**
   * creates a complete response object from the partial responses of different
   * data store to a DescribFeatureType Request
   */
      ...
      throws DispatcherException
  {
    ...
    try
    {
      ...
      StringReader sr = new StringReader( "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
          ...
          + "=\"qualified\" attributeFormDefault" + "=\"unqualified\"><xsd:import "
          + "schemaLocation=\"http://www.deegree.org/xml" + "/schemas/wcts/feature.xsd\"/>"
          + "<xsd:element name=\"ResultCollection\" "
          + "substitutionGroup=\"gml:_FeatureCollection\">" + "</xsd:element></xsd:schema>" );
    ...
    WFSDescribeFeatureTypeResponse response_ = WFSProtocolFactory
        .createWFSDescribeFeatureTypeResponse( request, afft, excep, schema );
  ...
  /**
   * creates a complete response object from the partial responses of different
   * data store to a GetFeature Request
   */
  ...
  /**
   * formats the result of a GetFeature request to GML and adds it to the
   * submitted <tt>GMLFeatureCollection</tt>
   */
  ...
  private FeatureCollection formatFC( FeatureCollection fc, WFSGetFeatureResponse response )
  {
    ...
    if( fc == null )
    {
      fc = FeatureFactory.createFeatureCollection( request.getId(), handler.size() );
    }
  ...
  private OGCWebServiceResponse createTransactionResponse()
  {
    ...
    OGCWebServiceResponse response = WFSProtocolFactory.createWFSTransactionResponse( request,
        getAffectedFeatureTypes(), excep, ir, "SUCCESS12", handle );
