...
class CGetFeature extends DBGetFeature
{
    ...
    try
    {
      ...
      for( int i = 0; i < queries.length; i++ )
      {
        ...
        if( parent.isKnownFeatureType( queries[i].getTypeName() ) )
        {
          ...
          hm.put( queries[i].getTypeName(), pl );
          DataStoreOutputFC form = new DataStoreOutputFC();
          FeatureCollection fc = (FeatureCollection)form.formatTable( hm, pl );
  ...
  private String getQuery( Filter filter, WFSQuery query, boolean indexed ) throws Exception
  {
   ...
   *         Exception
   */
  //    private boolean isIndexed(OracleSpatialAccess jos, String datasource)
  // throws Exception {
  //        try {
  ...
  //            String query = null;
  //            if (ds.length == 1) {
  //            	// no schema + "." note potential for two identically named tables in
  // different schemas
  //				query = "select count (*) from all_indexes where ITYP_NAME like
  // 'SPATIAL_INDEX' "+
