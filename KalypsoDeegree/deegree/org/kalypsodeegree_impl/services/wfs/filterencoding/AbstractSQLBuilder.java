 ...
 * FIXME: - featureFilter2SQL () lacks configurability (should be moved to the
 * concrete implementation maybe) - function2SQL () lacks function name checks
 * (might break the generated SQL)
 * 
...
public abstract class AbstractSQLBuilder
{
  ...
  /**
   * Generates a SQL-fragment for the given object. As this depends very much on
   * the handling of geometry data by the concrete database in use, this method
   * is abstract and must be overwritten by any implementation.
   */
  public abstract StringBuffer spatialOperation2SQL( SpatialOperation operation ) throws Exception;

  /**
   * Generates a SQL-fragment for the given object.
   */
  public StringBuffer comparisonOperation2SQL( ComparisonOperation operation ) throws Exception
  {
    StringBuffer sb = new StringBuffer( 200 );
  ...
  /**
   * Generates a SQL-fragment for the given object. Replacing and escape
   * handling is based on a finite automaton with 2 states: (escapeMode) - ' is
   * appended as \', \ is appended as \\ - every character (including the
   * escapeChar) is simply appended - unset escapeMode (escapeMode is false) - '
   * is appended as \', \ is appended as \\ - escapeChar means: skip char, set
   * escapeMode - wildCard means: append % - singleChar means: append ?
   */
  ...
  {
    StringBuffer sb = new StringBuffer( 200 );
    sb.append( expression2SQL( operation.getLowerBoundary() ) ).append( " <= " ).append(
        propertyName2SQL( operation.getPropertyName() ) ).append( " AND " ).append(
        propertyName2SQL( operation.getPropertyName() ) ).append( " <= " ).append(
        expression2SQL( operation.getUpperBoundary() ) );
    return sb;
  }
  ...
  public StringBuffer logicalOperation2SQL( LogicalOperation operation ) throws Exception
  {
    ...
    if( not )
    {
      ...
      case OperationDefines.AND:
        opId = OperationDefines.OR;
        break;
      case OperationDefines.OR:
        opId = OperationDefines.AND;
        break;
      }
    }
  ...
  private StringBuffer getRelatedTableSQL( String targetTable, String property )
  {
    ...
    sqlFrag.push( td.getName() + "." + ref[index].getTableField() + " IN (SELECT " + targetTable
        + "." + ref[index].getTargetField() + " FROM " + targetTable + " WHERE " );
  ...
  public StringBuffer literal2SQL( Literal literal )
  {
    ...
    if( tp == Types.BIT || tp == Types.CHAR || tp == Types.LONGVARCHAR || tp == Types.OTHER
        || tp == Types.VARCHAR )
    {
      sb.append( "'" ).append( literal.value ).append( "'" );
    }
    else if( tp == Types.DATE )
    {
      sb.append( formatDate( literal.value ) );
    }
    else if( tp == Types.TIME )
    {
      sb.append( formatTime( literal.value ) );
    }
    else if( tp == Types.TIMESTAMP )
    {
   ...
   * Generates a SQL-fragment for the given object.
   */
  public StringBuffer arithmeticExpression2SQL( ArithmeticExpression expr ) throws Exception
  {
    StringBuffer sb = new StringBuffer( 200 );
   ...
   * name is constructed like this: table.propertyname or
   * schema.table.propertyname. if no table is specified with a property name,
   * the name of the feature types master table will be returned.
   */
