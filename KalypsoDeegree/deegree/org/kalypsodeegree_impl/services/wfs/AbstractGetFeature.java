...
abstract public class AbstractGetFeature extends WFSMainLoop
{
  ...
  /**
   * the method returns a comma-seperated list of fields, that are affected by a
   * <tt>WFSQuery</tt> against the subbmitted table. If all fields of the
   * submitted table shall be selected to method just returns "*".
   * <p>
   * </p>
   * example:
   * <p>
      ...
      throws Exception
  {
    ...
    {
      throw new Exception( "getAffectedFields: No valid target field for SQL statement selected" );
      //s = " * ";
    }
  ...
  /**
   * create a response object from the GMLFeatureCollections store into the
   * submitted HashMap
   */
      ...
      throws Exception
  {
    ...
    for( int i = 0; i < affectedFeatureTypes.length; i++ )
    {
      ...
      {
        String[] s = new String[]
        { affectedFeatureTypes[i] };
        responses[i] = WFSProtocolFactory.createWFSGetFeatureResponse( request, s, null, fc );
      }
      catch( Exception e )
      {
        Debug.debugException( e, " - " );
        OGCWebServiceException exce = new OGCWebServiceException_Impl( e.toString(),
            "DBDataStore createResponse idx1" );
        responses = new OGCWebServiceResponse[1];
        responses[0] = WFSProtocolFactory.createWFSGetFeatureResponse( request, null, exce, null );
        Debug.debugMethodEnd();
      }
    }
