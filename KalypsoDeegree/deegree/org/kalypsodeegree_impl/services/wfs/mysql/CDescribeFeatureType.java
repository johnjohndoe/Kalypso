...
class CDescribeFeatureType extends AbstractDescribeFeatureType
{
  ...
  protected Document createSchema( String featureType ) throws Exception
  {
    ...
    try
    {
      ...
      String query = "select column_name, data_type, nullable from "
          + "sys.all_tab_columns where OWNER = '" + getOwner( featureType )
          + "' and TABLE_NAME = '" + getTableName( featureType ) + "'";
  ...
  private Document tableToSchema( String featureType, Table table ) throws Exception
  {
    Debug.debugMethodBegin( this, "createResponse" );

    boolean nullable = false;
    org.deegree.model.feature.FeatureType[] ft = new org.deegree.model.feature.FeatureType[1];
    ...
    for( int i = 0; i < table.getRowCount(); i++ )
    {
      ...
      ftp[i] = FeatureFactory.createFeatureTypeProperty( table.getValueAt( i, 0 ).toString(), type,
          nullable );
    }
    // create feature type with the properties taken from the table
    ft[0] = FeatureFactory.createFeatureType( null, null, featureType, ftp );
  ...
  private String getType( String oType )
  {
    ...
    else if( oType.equalsIgnoreCase( "Varchar2" ) )
    {
      return "java.lang.String";
    }
    else if( oType.equalsIgnoreCase( "Varchar" ) )
    {
      return "java.lang.String";
    }
