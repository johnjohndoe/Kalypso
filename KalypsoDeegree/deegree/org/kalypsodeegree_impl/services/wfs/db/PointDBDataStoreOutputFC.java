...
public class PointDBDataStoreOutputFC implements DataStoreOutputFormat {
            ...
            for ( int i = 0; i < row.length; i++ ) {
                ...
                if ( gfi == null ) {
                    if ( row[i] instanceof Table ) {
                        ...
                        FeatureCollection fc_ = FeatureFactory.createFeatureCollection( 
                    ...
                    if ( !baseStack.contains( base ) ) {
                        ...
                        if ( ( row[x] != null ) && ( row[y] != null ) ) {
                            ...
                            fp[fp.length - 1] = FeatureFactory.createFeatureProperty( pn, geo );
                        } else {
                            fp[fp.length - 1] = FeatureFactory.createFeatureProperty( pn, null );
                        }
                    }
                }
            }
    ...
    private org.deegree.model.feature.FeatureType createFeatureType( FeatureType ft, Table table ) {
        ...
        int k = 0;
        ArrayList baseStack = new ArrayList(50);
        ...
        for ( int i = 0; i < columnNames.length; i++ ) {
            ...
            if ( gfi == null ) {
                ...
                if ( !baseStack.contains( base ) ) {
                    ...
                    // create a FeatureTypeProperty with type of the current
                    // column
                    ftp[k++] = FeatureFactory.createFeatureTypeProperty( pn, "org.deegree.model.geometry.GM_Object", true );
                }
            }
        }
    ...
    private GM_Object getGeometry( String x, String y, String crs ) {
        ...
        return GeometryFactory.createGM_Point( Double.parseDouble( x ), 
        									   Double.parseDouble( y ), cs );
    }
}