...
/**
 * In response to a transaction request, the web feature server shall generate
 * an object document indicating the termination status of the transaction. In
 * addition, if the transaction request includes <Insert>operations, then the
 * web feature server must report the feature identifiers of all newly created
 * features. In the event that the transaction fails to execute, a web feature
 * server shall also indicate this in the response.
 * 
 * <p>
 * --------------------------------------------------------
 * </p>
 ...
 */
class WFSTransactionResponse_Impl extends WFSBasicResponse_Impl implements WFSTransactionResponse,
    Marshallable
{
  ...
  WFSTransactionResponse_Impl( OGCWebServiceRequest request, String[] affectedFeatureTypes,
      Document exception, WFSInsertResult[] insertResults, String status, String handle )
  {
 ...
 * 
 * Revision 1.3 2003/12/05 09:34:17 poth no message
 * 
 * Revision 1.2 2003/04/07 07:26:57 poth no message
 * 
 * Revision 1.1.1.1 2002/09/25 16:01:24 poth no message
 * 
 * Revision 1.3 2002/08/15 10:01:40 ap no message
 * 
 * Revision 1.2 2002/08/09 15:36:30 ap no message
 * 
