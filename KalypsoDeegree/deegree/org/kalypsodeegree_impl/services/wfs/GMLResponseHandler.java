...
public class GMLResponseHandler implements GetFeatureResponseHandler
{
      ...
      WFSCapabilities capabilities ) throws Exception
  {
    ...
    // using http get
    URL[] url = null;
    DCPType[] dcp = capabilities.getCapability().getRequest().getDescribeFeatureType().getDCPType();
    ...
    if( url != null )
    {
      ...
      if( url[0].getPort() != -1 )
        port = ":" + url[0].getPort();
      String s = "http://www.deegree.org " + url[0].getProtocol() + "://" + url[0].getHost() + port
          + url[0].getPath();
    ...
    OGCWebServiceResponse resp = WFSProtocolFactory.createWFSGetFeatureResponse( response
        .getRequest(), aff, null, result );
