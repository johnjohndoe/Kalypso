 ...
 *          <p>
 */
public class SDEDataStoreOutputGML implements DataStoreOutputFormat
{
  ...
  public Object format( HashMap map, ParameterList paramList ) throws Exception
  {
    ...
    gml.append( "<gml:Collection xmlns:gml=\"http://www.opengis.net/gml\">" );
    ...
    while( iterator.hasNext() )
    {
      ...
      org.deegree.services.wfs.configuration.FeatureType ft = null;
      ft = (org.deegree.services.wfs.configuration.FeatureType)pl.getParameter(
          WFSConstants.FEATURETYPE ).getValue();

      TableDescription td = ft.getMasterTable();
      // get feature type description of the submitted data features
      org.deegree.model.feature.FeatureType ftype = feature[0].getFeatureType();
      FeatureTypeProperty[] ftp = ftype.getProperties();

      // transform features to GML
      for( int i = 0; i < feature.length; i++ )
      {
        Object oo = feature[i].getProperty( td.getIdField() );
        String id = null;
    ...
    if( p != null )
    {
      String s = DOMPrinter.nodeToString( ( (GMLFeatureCollection_Impl)fc ).getAsElement(), "" );
  ...
  /**
   * the method formats a slash-seperated property name like /cities/ID to a
   * dot-seperated xml-valid name --> cities.ID
   */
