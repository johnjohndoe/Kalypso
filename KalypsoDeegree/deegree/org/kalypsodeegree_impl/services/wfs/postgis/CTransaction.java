...
class CTransaction extends WFSMainLoop
{
   ...
   */
  public ArrayList performInsert( InsertTree tree, PostgisDBAccess osa ) throws Exception
  {
    ...
    for( int ii = 0; ii < idValues.size(); ii++ )
    {
      ...
      stmt.append( "INSERT INTO " ).append( table ).append( " " ).append( fieldList ).append( " " )
          .append( valueList );
  ...
  /**
   * Updates FeatureType-instances from the datastore that are specified by the
   * given <tt>WFSUpdate</tt>.
   * <p>
   * 
   * @param update
  ...
  private boolean performUpdate( WFSUpdate update ) throws Exception
  {
    ...
    try
    {
      ...
      if( !fIDs.isEmpty() )
      {
        ...
        PostgisRowSelector selector = new PostgisRowSelector( masterTable,
            masterTable.getIdField(), fIDs, osa, ft );
        ...
        while( it.hasNext() )
        {
          ...
          if( !ft.isPropertyKnown( name ) )
          {
            throw new Exception( "Property '" + name.replace( '/', '.' )
                + "' is not known to Feature '" + ft.getName() + "' (not mapped?)" );
          }
  ...
  boolean performUpdate( PostgisRowSelector selector, HashMap changes ) throws Exception
  {

    PostgisRowSelector[] referencedRows = (PostgisRowSelector[])selector.getReferencedRows();
