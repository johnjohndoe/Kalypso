...
class CTransaction extends WFSMainLoop
{
  ...
  private boolean performUpdate( WFSUpdate update ) throws Exception
  {
    ...
    try
    {
      ...
      if( !fIDs.isEmpty() )
      {
        ...
        RowSelector selector = new RowSelector( masterTable, masterTable.getIdField(), fIDs, osa,
            ft );
  ...
  {
    RowSelector[] referencedRows = selector.getReferencedRows();
  ...
  public ArrayList performInsert( InsertTree tree, DBAccess osa ) throws Exception
  {
    ...
    //deleteFeature( osa.getConnection(), table, idField + " like '" +
    // idValues.get(0) + "'" );
      ...
      while( it.hasNext() )
      {
        ...
        else
        {
          ...
          case Types.LONGVARCHAR:
          {
            ...
            {
              valueList.append( "'" ).append( ( (String)vals.get( ii ) ).trim() ).append( "'" );
            }
          ...
          {
            valueList.append( SQLTools.formatDate( dbVendor, ( (String)vals.get( ii ) ).trim() ) );
            break;
          }
