...
public class InsertTree
{
  ...
  /*
   * Builds an InsertTree-object from a GMLFeature and a FeatureType. This
   * structure is recursive (it has sons that are InsertTrees, too) and every
   * InsertTree corresponds to a TableDescription object (a table of the
   * database). @param gmlFeature the concrete instance to use @param ft the
   * configuration-object that describes this FeatureType's structure and
   * mappings @return the constructed tree
   * 
   * @see GMLFeature
   * @see FeatureType
   */
  public static InsertTree buildFromGMLFeature( GMLFeature gmlFeature, FeatureType ft )
  ...
  /**
   * Constructs a new InsertTree-object (only for class-internal use). Iterates
   * on the tables referenced by the given (constructed) table:
   * <ul>
   * <li>ref. table is a standard-table -> ref. table is added to the
   * lookup-table & ref. table is added as a child</li>
   * <li>ref. table is a jointable -> ref. table is added to the lookup-table
   * </li>
   * </ul>
   * 
   * @param table
   *          the table associated to the new InsertTree
   * @param ft
   *          the FeatureType this InsertTree corresponds to
   */
  private InsertTree( TableDescription table, FeatureType ft ) throws InsertException
  {
    ...
    if( table.isIdFieldAutoIncremented() )
    {
      throw new InsertException( "The use of autoincremented IDs is not "
          + "implemented yet (used in " + "configuration of table '" + table.getName() + "')" );
    }
   ...
   * object) If the table to add is a jointable, several children with the same
   * name are possible, non-jointable sons have to have unique names.
   * 
   * @param targetTable
   *          the table to add
   * @param childIsJT
  ...
  /**
   * Adds a field to the structure. The field slips recursively through the tree
   * until a InsertTree with a matching name is found. The recursion does not
   * descend to subtrees that are connected via jointables, only "normally"
   * connected subtrees are searched.
   * 
   * @param value
  ...
  private boolean addField( String value, String mappedTable, String mappedField )
  {
    ...
    else
    {
      ...
      while( it.hasNext() )
      {
        ...
        if( tree.addField( value, mappedTable, mappedField ) )
          return true;
      }
    }
    return false;
  ...
  /**
   * Builds a new InsertTree object recursively. Recurses on both the
   * GMLFeature-parameter and the InsertTree-instance. Flat properties are
   * simply added by calling addField (). Complex properties require that a new
   * child-InsertTree to be added, recursion descends into the new child and the
   * property.
   * 
   * @param gmlFeature
  ...
  private InsertTree buildFromGMLFeature( GMLFeature gmlFeature ) throws InsertException
  {
    ...
    for( int i = 0; i < gmlProperties.length; i++ )
    {
      ...
      else
      {
        ...
        if( !found )
          throw new InsertException( "No interpretation found for '" + name
              + "' in context of table '" + table.getName() + "'!" );
      }
    }
  ...
  /**
   * Extracts the two parts of a concatenated String of the form [a.]b (part a
   * is optional). Method is null-save.
   * 
  ...
  private StringBuffer toStringRecurse( String indent )
  {
    ...
    sb.append( "Table: " ).append( table.getName().toUpperCase() ).append( ", " );
