...
/**
 * 
 * A WCAS implementns the <tt>WCASService</tt> interface to act like a OGC web
 * service. This means that a WCAS is callable through the <tt>doService</tt>
 * -method defined at <tt>OGCWebService</tt>.
 * <p>
 * </p>
 * Because the <tt>WCASService_Impl</tt> acts as proxy he also implements the
 * OGCWebServiceClient interface to receive the response(s) from the WFS's he
 * had called. So the WCAS is nothing else than a client to a WFS.
 * 
 * <p>
 * -----------------------------------------------------
 * </p>
...
public class WCASService_Impl extends OGCWebService_Impl implements Handler
{
   ...
   * <tt>OGCWebService</tt> interface. The method receives a WCAS... request
   * delievers it to one or more WFS's
   * 
   * @param event
  ...
  private void handleGetRecordResponse() throws Exception
  {
    ...
    for( int i = 0; i < results.size(); i++ )
    {
      if( results.get( i ) instanceof WFSGetFeatureResponse )
      {
        ...
        WFSGetFeatureRequest req = (WFSGetFeatureRequest)( (WFSGetFeatureResponse)results.get( i ) )
            .getRequest();
    ...
    OGCWebServiceResponse resp = CASProtocolFactory.createCASGetRecordResponse( inRequest, null, sb
        .toString() );
  ...
  /**
   * handles the creation and send of a describeRecordType response back to the
   * client
   */
  private void handleDescribeRecordTypeResponse()
  {
    ...
    OGCWebServiceResponse resp = CASProtocolFactory.createCASDescribeRecordTypeResponse( inRequest,
        null, s );
    OGCWebServiceEvent event = new OGCWebServiceEvent_Impl( this, resp, "" );
    ...
    for( int i = 0; i < results.size(); i++ )
    {
      ...
      else
      {
        ...
        if( ir != null )
        {
          ...
          {
            cir[k] = CASProtocolFactory.createCASInsertResult( ir[k].getHandle(), ir[k]
                .getFeatureIds() );
          }
        }
        ...
        OGCWebServiceResponse resp = CASProtocolFactory.createCASTransactionResponse( inRequest,
            ex, cir, res.getStatus(), res.getHandle() );
        OGCWebServiceEvent event = new OGCWebServiceEvent_Impl( this, resp, "" );
        dest.write( event );
      }
    }
  ...
  /**
   * handles the creation and send of a registerService response back to the
   * client
   */
  ...
  /**
   * transforms a getRecord request to one or more getFeature requests and
   * encapsulates them into one or more <tt>OGCWebServiceEvent</tt> s
   */
  private OGCWebServiceEvent[] transformGetRecord( CASGetRecordRequest request ) throws Exception
  {
    Debug.debugMethodBegin();
    ...
    for( int i = 0; i < queries.length; i++ )
    {
      ...
      WFSQuery q = WFSProtocolFactory.createQuery( props, queries[i].getHandle(), queries[i]
          .getVersion(), request.getOutputRecType(), queries[i].getFilter() );

      req = WFSProtocolFactory.createWFSGetFeatureRequest( queries[i].getVersion(), request.getId()
          + "-" + i, null, null, //request.getOutputFormat(),
          propertySetName, propertySetName, request.getFilter(), request.getMaxRecords(), request
              .getStartPosition(), new WFSQuery[]
          { q } );
  ...
  private OGCWebServiceEvent[] transformDescribeRecordType( CASDescribeRecordTypeRequest request )
  {
    ...
    {
      if( types[i].equalsIgnoreCase( "product" ) || types[i].equalsIgnoreCase( "collection" ) )
      {
    ...
    OGCWebServiceRequest req = WFSProtocolFactory.createWFSDescribeFeatureTypeRequest( request
        .getVersion(), request.getId(), null, null, request.getOutputFormat(), types );
  ...
  private OGCWebServiceEvent[] transformTransaction( CASTransactionRequest request )
  {
    ...
    OGCWebServiceRequest req = WFSProtocolFactory.createWFSTransactionRequest(
        request.getVersion(), request.getId(), request.getLockId(), wfsop, request.getHandle(),
        null );
  ...
  /**
   * performs a registerService request. this will be done by requesting the
   * service describing XML document from the specified address and transforming
   * it into one or more (WFS) transaction requests encapsulated into one or
   * more <tt>OGCWebServiceEvent</tt>s.
   */
  ...
  /**
   * handles a get capabilities request by reading the capabilities document
   * from its source.
   */
  ...
  private void performCascadeGetRecord( CASGetRecordRequest request ) throws Exception
  {
    ...
    for( int i = 0; i < fc.length; i++ )
    {
      ...
      WFSGetFeatureResponse res = WFSProtocolFactory.createWFSGetFeatureResponse( request, null,
          null, doc );
   ...
   * 
   * @param event
   *          event object that contains the request to be performed
   */
   ...
   * 
   * @param event
   *          event oject that contains the response to a request
   */
  public void handleResponse( OGCWebServiceEvent event )
  {
    ...
    if( results.size() == expectedResponses )
    {
      try
      {
        ...
        default:
        {
          OGCWebServiceException ex = new OGCWebServiceException_Impl(
              "WCASService_Impl:handleResponse", "not known request type" );
          OGCWebServiceResponse resp = CASProtocolFactory.createCASTransactionResponse( inRequest,
              ex, null, null, null );
      ...
      catch( Exception e )
      {
        e.printStackTrace();
        OGCWebServiceException ex = new OGCWebServiceException_Impl(
            "WCASService_Impl:handleResponse", e.toString() );
        OGCWebServiceResponse resp = CASProtocolFactory.createCASTransactionResponse( inRequest,
            ex, null, null, null );
        event = new OGCWebServiceEvent_Impl( this, resp, "" );
        dest.write( event );
   ...
   * @return true if the service is interested in handling the request/response
   *         contained within the event object
   */
   ...
   * 
   * @param handler
   *          handler to be registered to the service
   */
   ...
   * @param handler
   *          handler to be removed from the service
   */
