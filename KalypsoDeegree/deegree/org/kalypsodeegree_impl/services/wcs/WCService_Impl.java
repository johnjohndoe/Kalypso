...
public class WCService_Impl extends OGCWebService_Impl implements Handler {
     ...
     *
     * @param capabilities capabilites of the WCS
     *
    ...
    /**
     * returns a copy of the WCService_Impl using the same capabilities 
     * reference
     *
     * @return
     */
    ...
    private WCSGetCoverageResponse handleGetCoverageRequest( OGCWebServiceEvent event ) {
        ...
        // ETJ FIXME this gcr may be originated by WMS: maybe not all WMS ranges are meaningful for WCS.
		
//		RangeSetDescription    rsd = ((GridCoverageLayer)desc.getCoverageLayer()).getRangeSetDescription();
...
//            response = createErrorResponse( event.getRequest(), nsfe, "handleGetCoverageRequest" );
        } catch ( Exception e ) {
            e.printStackTrace();
            response = createErrorResponse( event.getRequest(), e, "handleGetCoverageRequest" );
        }
