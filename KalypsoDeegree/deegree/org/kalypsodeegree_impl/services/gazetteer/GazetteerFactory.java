...
public class GazetteerFactory {
     ...
     * <tt>Document</tt>
     * 
     * @param root <tt>Element</tt> containing an XML representation of one ore more 
     *              <tt>SI_LocationInstance</tt>s 
     * @return
     ...
     * 
     * @param element containing an XML representation of a <tt>SI_LocationInstance</tt>
     * @return
     * @throws GazetteerException
     */
                ...
                if ( elem.getNodeName().endsWith( "GM_Envelope" ) ) {
                    ...
                    try {
                        ...
                        if ( name.equals( "position" ) ) {
                            value = new GM_Point[] { (GM_Point)value };
                        } else {
                            value = new GM_Object[] { (GM_Object)value };
                        }
                    } catch (Exception e) {
    ...
    public static SI_LocationInstance createLocationInstance(HashMap map) 
                                                    throws GazetteerException {
        Debug.debugMethodBegin();
        SI_LocationInstance li = 
            new SI_LocationInstance_Impl( (String)map.get("geographicIdentifier"), 
                                          (String)map.get("alternativeGeographicIdentifier"), 
                                          (String)map.get("identifier"), 
                                          (Date)map.get("begin"), (Date)map.get("end"), 
                                          (GM_Point[])map.get("position"), 
                                          (GM_Object[])map.get("geographicExtent"), 
                                          (CitedResponsibleParty)map.get("administrator"), 
                                          (SI_LocationInstance[])map.get("parents"), 
                                          (SI_LocationInstance[])map.get("children"), 
                                          (SI_LocationType)map.get("locationType" ) );
        Debug.debugMethodEnd();
        return li;
    }
...
/* ********************************************************************
   ...
   Revision 1.6  2004/03/26 16:42:18  poth
   no message

   Revision 1.5  2004/03/26 11:19:29  poth
   no message

   Revision 1.4  2004/03/24 08:12:20  poth
   no message

   Revision 1.3  2004/03/16 08:07:24  poth
   no message

   Revision 1.2  2004/03/15 16:55:29  poth
   no message

   Revision 1.1  2004/03/15 07:39:00  poth
   no message
