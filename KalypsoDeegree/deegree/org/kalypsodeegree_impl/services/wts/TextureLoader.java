...
System.out.println( "length: " + boxes.length);
            ...
            for ( int i = 0; i < boxes.length; i++ ) {
                for ( int j = 0; j < boxes[0].length; j++ ) {                    
                    wcsReq = WCSProtocolFactory.createWCSGetCoverageRequest( 
                                                           "1.0.0", layerId + "-" + i + "-" + j, 
                                                           null, layer.getName(), request.getSrs(), 
                                                           request.getSrs(), 
                                                           boxes[i][j].getEnvelope(), null, 
                                                           dw, dh, -1, 
                                                           conf.getFormatName( layer.getName() ), 
                                                           null, "application/vnd.ogc.se_xml" );                                    
                    if ( i > boxes.length-2 ) {                        
                        ...
                        WCSGetCoverageResponse res = WCSProtocolFactory.createGetCoverageResponse( wcsReq, image );
