...
public class WTService_Impl extends OGCWebService_Impl implements OGCWebServiceClient {
    ...
    private static ViewPoint createViewPoint( WTSGetViewRequest request ) {
        ...
        ViewPoint vp = new ViewPoint_Impl( request.getYAW(), request.getPitch(), 
                                           request.getDistance(), poi_, request.getAOV() );
    ...
    private void handleWCSResponse( WCSGetCoverageResponse response ) throws Exception {
        ...
        if ( req.getFormat().equalsIgnoreCase( "IMG" ) ) {
            ...
            // it's a texture tile
            try {
//                SeekableStream sst = new ByteArraySeekableStream( (byte[])response.getResponse() );
...
//                sst.close();
     ...
     * 
     * @param gmlFc feature collection; each feature represents a building
     * @param scale scale of the buildings (1 = original scale)
     ...
     * 
     * @param fc feature collection containing the TINs
     * @param scale scale of the TINs (1 = original scale)
     * @param index index of the tiles the result will be assigned to
     * @throws ConfigurationException
     */
    ...
    private void forceGetViewResponseToClient() {
        ...
        try {
                            ...
                            if ( textureTiles[0][i][j] != null ) {
                                ...
                                for ( int k = 1; k < textureTiles.length; k++ ) {
                                    if ( textureTiles[k][i][j] != null ) {
                                        g.drawImage( (BufferedImage)textureTiles[k][i][j], 0, 0, 
                                                     null );
                                    }
                                }
                                ...
                                textureTiles[0][i][j] = new BufferedImage( 2, 2, 
                                                                           BufferedImage.TYPE_INT_ARGB );
                            }
                        }
            ...
            // create 3D scene            
            WTSScene scene = WTSFactory.createWTSScene( shapes, blocks, vp, req.getDate(), null, 
            ...
            OGCWebServiceException exc = new OGCWebServiceException_Impl( this.getClass().getName(), 
                                                                          e.toString() );
            response = WTSProtocolFactory.createGetViewResponse( 
                               ( (Marshallable)exc ).exportAsXML(), req );
        }
