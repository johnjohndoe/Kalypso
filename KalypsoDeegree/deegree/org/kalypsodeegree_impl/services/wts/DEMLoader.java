...
class DEMLoader extends Thread
{
  ...
  public void run()
  {
    try
    {
      ...
      if( ( srcType == WTSConfiguration.LOCALWCS ) || ( srcType == WTSConfiguration.REMOTEWCS ) )
      {
        handleWCS( boxes );
      }
      else if( ( srcType == WTSConfiguration.LOCALWFS ) || ( srcType == WTSConfiguration.REMOTEWFS ) )
      {
        handleWFS( boxes );
      }
    }
  ...
  /**
   * handles the creation and performing of a GetCoverage request to access the
   * DEM underlying the 3D scene to be created
   */
  private void handleWCS( GM_Surface[][] boxes ) throws Exception
  {
    ...
    for( int i = 0; i < boxes.length; i++ )
    {
      for( int j = 0; j < boxes[0].length; j++ )
      {
        ...
        WCSGetCoverageRequest wcsReq = WCSProtocolFactory.createWCSGetCoverageRequest( "1.0.0", i
            + "-" + j, null, layer, request.getSrs(), request.getSrs(), boxes[i][j].getEnvelope(),
            null, sx, sy, -1, conf.getFormatName( layer ), null, request.getExceptions() );
  ...
  /**
   * handles the creation and performing of a GetFeature request to access the
   * DEM underlying the 3D scene to be created as a TIN
   */
