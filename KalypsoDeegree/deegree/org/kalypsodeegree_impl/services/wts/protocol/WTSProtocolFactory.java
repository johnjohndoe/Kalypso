...
public class WTSProtocolFactory {
    /**
     * creates a request object from a model (key-value-pair encoding) of a
     * WTS request and a unique ID.
     *
     * @param id ID of the request
     * @param model key-value-pair encoding of the request
     *
     * @return 
     *
    ...
    /** creates a <tt>WTSGetViewRequest</tt> from a <tt>HashMap</tt> that contains
     ...
     * @throws WTSProtocolException will be thrown if mandatory parameter are missing or parameters don't contain valid values
     */
    public static WTSGetViewRequest createGetViewRequest( String id, HashMap model )
        ...
        return createGetViewRequest( version, id, vendorSpecificParameter, format, ls, crs, width, 
                                     height, bgColor, aov, distance, pitch, poi, yaw, exceptions, 
                                     date, scale, background, dem, ftCollections );
    }

    /** creates a <tt>WTSGetViewRequest</tt> from a set of parameters
     * @param id request id
     * @param vendorSpecificParameter vendorSpecificParameter parameter(s)
     * @param format format the result of the request shall be returned
     * @param layers layers that are requested
     * @param crs coordinate reference system that scene's data are requested at
     * @param width width scene in pixel
     * @param height height scene in pixel
     * @param bgColor background color of the scene
     * @param aov viewers angle of view
     * @param distance distance between viewer and target on an imaginery horizontal ground in height
     * of the target
     * @param pitch -
     * @param poi target where the viewer is looking to
     * @param yaw -
     * @param exceptions format how to return exceptions
     * @param date month, day and hour for which he scene shall be created (light-conditions)
     * @param scale vertical scale of the dem and the objects in the scene (default = 1)
     * @param background background used for scene (cloud images form example)
     * @param dem name of the dem that shall be used
     * @param ftCollections named feature collections that shall be superimposed on the dem
     * @return an instance of <tt>WTSGetViewRequest</tt>
     */
    public static WTSGetViewRequest createGetViewRequest( String version, String id, 
                                                          ...
                                                          String crs, int width, int height, 
                                                          Color bgColor, double aov, 
                                                          double distance, double pitch, 
                                                          GM_Position poi, double yaw, 
                                                          String exceptions, Calendar date, 
                                                          double scale, Object background, 
                                                          String[] dem, 
                                                          WTSGetViewRequest.Layer[] ftCollections ) {
        return new WTSGetViewRequest_Impl( version, id, vendorSpecificParameter, format, layers, 
                                           crs, width, height, bgColor, aov, distance, pitch, poi, 
                                           yaw, exceptions, date, scale, background, dem, 
                                           ftCollections );
    }

    /** creates a responseobject to a getViewRequest
     * @param view created scene
     * @param request request from which the scene was created
     * @return response object
     */
    public static WTSGetViewResponse createGetViewResponse( Object view, 
     ...
     * @param exception exception that has been raised performing the request
     * @param request request that has been performed when exception raised
     * @return response object
     */
