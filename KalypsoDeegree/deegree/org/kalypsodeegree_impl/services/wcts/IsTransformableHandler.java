...
/**
 * This class handles the IsTransformable-request and creates the response
 * which returns only the true or false attribute.
 * If the creating of this answer failed, an exception is included in the
 * response.
 *
 * <p>--------------------------------------------------------------------</p>
...
class IsTransformableHandler implements Handler {
    ...
    public void handleRequest( OGCWebServiceEvent request ) {
        ...
        if ( !req.getVersion().equals( "1.0.0" ) ) {
            ...
            CS_CoordinateSystem s_srs_cs = ( (IsTransformableRequest)request.getRequest() ).getSourceCRS();
            CS_CoordinateSystem t_srs_cs = ( (IsTransformableRequest)request.getRequest() ).getDestinationCRS();
