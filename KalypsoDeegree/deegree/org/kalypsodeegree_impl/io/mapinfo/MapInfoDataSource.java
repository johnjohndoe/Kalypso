...
public class MapInfoDataSource {
     ...
     *
     * @throws Exception if file doesn't exist or is not readable etc
     */
    ...
    private void setUpFactories() throws Exception {
        ...
        for ( int i = 0; i < hColumnsNames.size(); i++ ) {
            ...
            FeatureTypeProperty ftp = FeatureFactory.createFeatureTypeProperty( 
                                              (String)hColumnsNames.get( i ), typeClass.getName(), 
                                              true );
            colAttribs.add( ftp );
        }
        ...
        try {
            FeatureTypeProperty[] ftps = (FeatureTypeProperty[])colAttribs.toArray( 
                                                   new FeatureTypeProperty[colAttribs.size()] );
            pointType = FeatureFactory.createFeatureType( null, null, filename.toString() + 
                                                       "_point", ftps );
        ...
        try {
            FeatureTypeProperty[] ftps = (FeatureTypeProperty[])colAttribs.toArray( 
                                                   new FeatureTypeProperty[colAttribs.size()] );
            lineType = FeatureFactory.createFeatureType( null, null, filename.toString() + "_line", 
                                                      ftps );
        } catch ( Exception schexp ) {
        ...
        // Add default attribute type
        ftp = FeatureFactory.createFeatureTypeProperty( "GEOM", 
        ...
        try {
            FeatureTypeProperty[] ftps = (FeatureTypeProperty[])colAttribs.toArray( 
                                                   new FeatureTypeProperty[colAttribs.size()] );
            polygonType = FeatureFactory.createFeatureType( null, null, filename.toString() + 
                                                         "_poly", ftps );
        } catch ( Exception schexp ) {
        ...
        try {
            while ( ( readMifLine( mifReader ) != null ) && 
                ...
                if ( clause( line ).equalsIgnoreCase( CLAUSE_DELIMETER ) ) {
                    ...
                    hDelimeter = line.replace( '\"', ' ' ).trim()
                                     .substring( line.trim().indexOf( ' ' ) ).trim();
     ...
     *
     * @throws Exception For all data source errors.
     */
    public void getFeatures( FeatureCollection collection, WFSQuery query )
                     throws Exception {
        Filter filter = null;
