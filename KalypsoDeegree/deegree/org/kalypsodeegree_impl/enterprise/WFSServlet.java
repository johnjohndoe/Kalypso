...
public class WFSServlet extends AbstractOGCServlet
{
  ...
  private class WFS implements OGCWebServiceClient
  {
     ...
     * constructor
     */
    WFS( HttpServletRequest servletRequest, HttpServletResponse servletResponse ) throws Exception
    {
      finished = false;
    ...
    WFS( HashMap model, HttpServletResponse servletResponse ) throws Exception
    {
      ...
      if( req.equals( "DescribeFeatureType" ) )
      {
        ...
        {
          org.deegree.services.wfs.capabilities.FeatureType[] ft = wfsCapabilities
              .getFeatureTypeList().getFeatureTypes();
      ...
      {
        OGCWebServiceException exception = new OGCWebServiceException_Impl( "WFSServlet", req
            + " is " + "no valid WFS request" );
    ...
    /**
     * the method stops the processing for the submitted amount of minutes. The
     * processing will also be continued if a other method sets the finished
     * parameter to true. if an error raises while waiting or the loop is
     * finished by exceeding the time the method returns <tt>false</tt>
                ...
                || res instanceof WFSGetFeatureWithLockResponse )
            {
              ...
              else
              {
                ...
                if( o instanceof GMLFeatureCollection )
                {
                  ...
                  String s = DOMPrinter.nodeToString( gfc.getAsElement().getOwnerDocument(),
                      "UTF-8" );
