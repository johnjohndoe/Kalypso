...
public class WTSServlet extends AbstractOGCServlet
{
  ...
  private HashMap mapParams( HttpServletRequest request )
  {
    ...
    while( enum.hasMoreElements() )
    {
      ...
      key = key.toUpperCase();
      if( key.equals( "TEXTURE" ) || key.equals( "STYLES" ) || key.equals( "ELEVATION_MODELS" )
          || key.equals( "FEATURE_COLLECTIONS" ) || key.equals( "POI" ) )
      {
   ...
   * configuration file was altered.
   */
  protected void initService()
  {
    ...
    try
    {
      ...
      {
        Debug
            .debugSimpleMessage( "Parameter 'maxInstances' has invalid format. Using default value." );
      }
      ...
      {
        Debug
            .debugSimpleMessage( "Parameter 'initInstances' has invalid format. Using default value." );
      }
    }
    catch( Exception e )
    {
  ...
  private class WTS implements OGCWebServiceClient
  {
     ...
     */
    private WTS( HashMap paramMap, HttpServletResponse resp ) throws InconsistentRequestException,
        WebServiceException
    {
      this.resp = resp;
    ...
    /**
     * Stops the processing for the given number of minutes. The processing will
     * be continued if another Thread sets the finished parameter first. FIXME:
     * Better use signals instead of polling.
     * 
     * @param mins
