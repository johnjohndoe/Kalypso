...
public class WMSServlet extends AbstractOGCServlet {
        ...
        try {
            ...
            try {
                ...
                if ( maxInstancesStr == null ) {
                    getServletContext()
                        .log( "Parameter 'maxInstances' " + "unspecified. Using default value." );
                } else {
                    servicePool.setMaxInstances( Integer.parseInt( maxInstancesStr ) );
                }
            } catch ( NumberFormatException e ) {
                getServletContext()
                    .log( "Parameter 'maxInstances' has " + 
                          "invalid format. Using default value." );
            }
            ...
            try {
                ...
                if ( initInstancesStr == null ) {
                    getServletContext()
                        .log( "Parameter 'initInstances' " + "unspecified. Using default value." );
                } else {
                    servicePool.fill( Integer.parseInt( initInstancesStr ) );
                }
            } catch ( NumberFormatException e ) {
                getServletContext()
                    .log( "Parameter 'initInstances' has invalid " + 
    ...
    private class WMS implements OGCWebServiceClient {
         ...
         * it doService(..) method.<p>
         * If the request can't be performed an exception will be written to
         * the output stream to the client. The exception format is taken from
         * the request (default: application/vnd.ogc.se_xml).
         */
        ...
        */
         ...
         * @param response 
         */
        private void handleFeatureInfoResponse( WMSFeatureInfoResponse response )
                                        throws Exception {
         ...
         *
         * @param location location where the exception occurs
         ...
         *
         * @param code code of te expetion as defined in the OGC WMS 1.1.1 spec and
         *              the WMS 1.1.1 test suite
         * @param report detailed description of the exption
         */
        public void writeServiceExceptionReport( String code, String report ) {
            ...
            if ( exceptionFormat.equals( "application/vnd.ogc.se_inimage" ) ) {
                ...
                // default --> application/vnd.ogc.se_xml
                ServiceExceptionReport exce = new ServiceExceptionReport_Impl( "1.1.1", code, 
                                                                               report );
        ...
        private void writeImage( Object output, String mime ) {
            try {
                ...
                if ( mime.equalsIgnoreCase( "image/gif" ) ) {
                    ...
                    os = resp.getOutputStream();
                    Encoders.encodeJpeg( os, (BufferedImage)output, 
                                         wmsCapabilities.getDeegreeParam().getMapQuality() );
        ...
        private String xsltTransformGetFeature( String capa, URL xslt ) {
            ...
            try {
                ...
                Transformer transformer = tFactory.newTransformer( 
                                                  new StreamSource( xslt.openStream() ) );
...
*****************************************************************************
   ...
   Revision 1.51  2004/03/12 15:56:47  poth
   no message
