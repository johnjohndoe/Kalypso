...
public abstract class AbstractOGCServlet extends HttpServlet {
     ...
     * <p>
     * NOTE: Everything that gets called by this method should not use
     *       Debug.xxx () to produce messages, but 'getServletContext.log ()'.
     * <p>
     ...
     * servlet is being taken out of service. Stops the Reloader Thread, if
     * running.
     */
    public void destroy () {
        if (reloader != null) {
     ...
     * @param request source of the data
     ...
     * parameter "capabilities" in the servlet container. Monitor frequency
     * can be adjusted by the parameter "updateFrequency".
     */        
    protected void enableReloader () {
        if (reloader != null) { reloader.decease (); }
    ...
    protected void handleError(Exception ex, HttpServletResponse response) {
        ...
        OGCWebServiceException wex = 
            new OGCWebServiceException_Impl( this.getClass().getName(), tmp );
        try {
     ...
     * (capabilities-file). If the file is altered, the service is signalled
     * to reinitialize itself by a call to initService ().
     */
    protected class Reloader extends Thread {
         ...
         * thread.
         * @param capabilities file that should be monitored
         * @param freqStr check interval in seconds (as String)
         */
        protected Reloader (String capabilities, String freqStr) {

            // evaluate interval String
            if (freqStr == null) {
                getServletContext ().log ("No frequency (interval) specified, defaulting to 60 "
                   						+ "seconds.");
            } else {            
                try {                
                    freq = Integer.parseInt (freqStr) * 1000;
                } catch (NumberFormatException e) {
                    getServletContext ().log ("Invalid frequency (interval) specification: "
                       						+ freqStr + ", defaulting to 60 seconds.", e);
                }
            }
            ...
            try {            
                ...
                if (!file.exists ()) {
                    getServletContext ().log ("File to monitor: ('" + capabilities + "') "
            ...
            getServletContext ().log ("Reloader-Thread started. Monitoring file: '" + capabilities
               						+ "', Check interval: " + freq + " (milliseconds)");  
            reloader = this;
        }
        ...
        public void run () {   
            ...
            while (!doExit) {
                try {
                    ...
                    if (lastTS != currTS) {
                        getServletContext ().log ("Configuration alteration detected: reloading "
                           						+ "service... ");
                        System.out.println("Configuration alteration detected: reloading "
                           						+ "service... ");
                        lastTS = currTS;
                        initService ();
                    }
