 ...
 * of mathematical functions valid over a set of polynomials.</LI>
 * </OL>
 * 
...
public interface CV_Coverage extends Remote
{
  ...
  /**
   * This specifies the coordinate system used when accessing a coverage or grid coverage with the <code>evaluate</code>
   * methods. It is also the coordinate system of the coordinates used with the math transform (see
   * {@linkorg.opengis.gc.GC_GridGeometry#getGridToCoordinateSystemgridToCoordinateSystem}).
   * 
   * This coordinate system is usually different than the grid coordinate system of the grid. grid coverage can be
   * accessed (re-projected) with new coordinate system with the {@link org.opengis.gp.GP_GridCoverageProcessor}
  ...
  /**
   * The bounding box for the coverage domain in coordinate system coordinates. For grid coverages, the grid cells are
   * centered on each grid coordinate. The envelope for a 2-D grid coverage includes the following corner positions.
   * 
   * <blockquote>
   * 
   * <pre>
   ...
   * </blockquote>
   * 
   * If a grid coverage does not have any associated coordinate system, the minimum and maximum coordinate points for
   * the envelope will be empty sequences.
   * 
  ...
  /**
   * Returns the source data for a grid coverage. If the {@link GC_GridCoverage}was produced from an underlying dataset
   * (by <code>createFromName</code> or <code>createFromSubName</code> for instance) the {@link #getNumSources}
   * method should returns zero, and this method should not be called.
   * 
   * If the <code>GC_GridCoverage}</code> was produced using {link org.opengis.gp.GP_GridCoverageProcessor} then it
   * should return the source grid coverage of the one used as input to <code>GP_GridCoverageProcessor</code>. In
   * general the source() method is intended to return the original <code>GC_GridCoverage</code> on which it depends.
   * 
   * This is intended to allow applications to establish what <code>GC_GridCoverage</code> s will be affected when
   * others are updated, as well as to trace back to the "raw data".
   * 
   ...
   * vector. The default interpolation type used when accessing grid values for points which fall between grid cells is
   * nearest neighbor. The coordinate system of the point is the same as the grid coverage coordinate system (specified
   * by the {@link #getCoordinateSystem}).
   * 
   * @param point
   ...
   * between grid cells is nearest neighbor. The coordinate system of the point is the same as the grid coverage
   * coordinate system.
   * 
   * @param point
  ...
  /**
   * Return a sequence of double values for a given point in the coverage. A value for each sample dimension is included
   * in the sequence. The default interpolation type used when accessing grid values for points which fall between grid
   * cells is nearest neighbor. The coordinate system of the point is the same as the grid coverage coordinate system.
   * 
   * @param point
   *          Point at which to find the grid values.
