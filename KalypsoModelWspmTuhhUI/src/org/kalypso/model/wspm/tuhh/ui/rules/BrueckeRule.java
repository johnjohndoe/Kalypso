  ...
  public void validate( final IProfil profil, final IValidatorMarkerCollector collector ) throws CoreException
  {
    ...
    try
    {
      ...
      for( int i = outerLeft; i < outerRight; i++ )
      {
        ...
        if( h - okB > delta )
          collector.createProfilMarker( IMarker.SEVERITY_ERROR, Messages.getString( "org.kalypso.model.wspm.tuhh.ui.rules.BrueckeRule.23" ), "km " + Double.toString( profil.getStation() ), i, IWspmConstants.POINT_PROPERTY_BREITE, pluginId, new EditPointResolution( i, profil.getPointPropertyFor( IWspmTuhhConstants.POINT_PROPERTY_OBERKANTEBRUECKE ), h ) ); //$NON-NLS-1$ //$NON-NLS-2$
        if( h - ukB > delta )
