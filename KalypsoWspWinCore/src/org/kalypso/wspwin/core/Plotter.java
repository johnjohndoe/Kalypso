...
public class Plotter
{
  ...
  /**
   * Check if the plotter exe is configured. If not the user is asked to choose it in a file dialog.
   */
  ...
  {
    final IPreferenceStore preferences = getPlotterPreferences();
    final String preferencePlotterPath = preferences.getString( WSPWIN_PLOTTER_PATH );
    if( !StringUtils.isBlank( preferencePlotterPath ) )
      return new File( preferencePlotterPath );

    /* try to fall back to global configuration if preferences is not set */
    final String globalPlotterPath = FrameworkUtilities.getProperty( WSPWIN_PLOTTER_PATH, preferencePlotterPath );
    if( !StringUtils.isBlank( globalPlotterPath ) )
      return new File( globalPlotterPath );
