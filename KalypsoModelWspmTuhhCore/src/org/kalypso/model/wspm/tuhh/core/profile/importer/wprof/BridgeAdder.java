 ...
 */
class BridgeAdder extends GelaendeAdder
{
  private final IWProfPoint[] m_ukPoints;

  private final IWProfPoint[] m_okPoints;

  private final IWProfPoint[] m_bridgeWidthPoints;

  public BridgeAdder( final IWProfPoint[] soilPolygon, final IWProfPoint[] ukPoints, final IWProfPoint[] okPoints, final IWProfPoint[] bridgeWidthPoints )
  {
    super( soilPolygon );

    m_ukPoints = ukPoints;
    m_okPoints = okPoints;
    m_bridgeWidthPoints = bridgeWidthPoints;
  }

  /**
   * @see org.kalypso.model.wspm.tuhh.core.profile.importer.wprof.GelaendeAdder#configure(org.kalypso.model.wspm.core.profil.IProfil,
  ...
  {
    super.configure( profile, transformer );
  ...
  private void addBridgeObject( final IProfil profile )
  {
    ...
    final Double width = findBridgetWidth( profile );
    if( width != null )
    {
      final BigDecimal widthScaled = new BigDecimal( width.doubleValue() ).setScale( 2, BigDecimal.ROUND_HALF_UP );
  ...
  private Double findBridgetWidth( final IProfil profile )
  {
    if( m_bridgeWidthPoints == null || m_bridgeWidthPoints.length == 0 )
      return null;

    final IWProfPoint widthPoint = m_bridgeWidthPoints[0];
    ...
    return location.distance( profileLine );
  }

  private void cleanupSoil( final IProfil profile )
  {
