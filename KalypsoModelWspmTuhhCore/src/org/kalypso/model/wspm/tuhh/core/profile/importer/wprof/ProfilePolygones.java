...
class ProfilePolygones extends HashMap<String, ProfilePolygon>
{
  ...
  public IWProfPoint[][] findPoints( final String... keys )
  {
    final IWProfPoint[][] polys = new IWProfPoint[keys.length][];
    for( int i = 0; i < keys.length; i++ )
    {
      final ProfilePolygon foundPolygon = get( keys[i] );
      if( foundPolygon == null )
        return null;

      polys[i] = foundPolygon.getPoints();
    }
