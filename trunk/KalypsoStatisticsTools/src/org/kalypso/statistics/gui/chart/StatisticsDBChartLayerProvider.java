...
import org.kalypso.chart.ext.observation.DefaultObservationProvider;
import org.kalypso.chart.ext.observation.IObservationProvider;
...
public class StatisticsDBChartLayerProvider extends AbstractLayerProvider
{
  ...
  public TupleResultDomainValueData< ? , ? > getDataContainer( )
  {
    ...
    try
    {
      ...
      final long from = Long.parseLong( fromStr );
      final long to = Long.parseLong( toStr );
      final IObservation<TupleResult> observation = createObservation( type, uid, from, to );
      if( observation != null && domainComponentName != null && targetComponentName != null )
      {
        final IObservationProvider provider = new DefaultObservationProvider( observation );

        return new TupleResultDomainValueData<>( provider, domainComponentName, targetComponentName );
      }
    }
  ...
  private IObservation<TupleResult> createObservation( final String type, final int uid, final long from, final long to )
  {
    ...
    final ETimeseriesType timeseriesType = ETimeseriesType.valueOf( type );
    final IComponent componentDouble = new Component( timeseriesType.getAbbreviation(), timeseriesType.getLabel(), null, timeseriesType.getUnit(), null, XmlTypes.XS_DOUBLE, null, null );
    ...
    try
    {
      ...
      final PreparedStatement stmt = connection.prepareStatement( "SELECT entryDate, entryValue FROM TimeSerieEntry WHERE timeserieProfile_UID = ? AND entryDate >= ? AND entryDate <= ? ORDER BY entryDate ASC" );
      stmt.setInt( 1, uid );
      stmt.setLong( 2, from );
      stmt.setLong( 3, to );
      ...
      stmt.close();
      return new Observation<>( "ime 1", "opis 1", result );
    }
