...
/**
 * An <code>ITableViewTemplate</code> designed to be used with an <code>IObservation</code>.
 * 
 * @author schlienger
 */
public class ObservationTableViewTemplate implements ITableViewTemplate
{
  ...
  public void setObservation( final IObservation obs, final boolean editableColumns, final IVariableArguments args )
  {
    ...
    for( int i = 0; i < axes.length; i++ )
    {
      ...
      {
        final DefaultTableViewColumn col = new DefaultTableViewColumn( axes[i].getLabel() + " - " + axes[i].getUnit(),
