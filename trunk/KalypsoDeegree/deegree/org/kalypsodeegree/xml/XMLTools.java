...
public class XMLTools
{
	...
	public static StringBuffer validateCDATA(String cdata)
	{
		...
		if (cdata != null
				&& (cdata.length() > 1000 || cdata.indexOf('<') >= 0
	...
	/**
	 * Returns the specified child element of the given elemen. If there are
	 * more than one with the same name, the first one is returned.
	 * <p>
	 * 
	 * @param name
	...
	public static Element getRequiredChildByName(String name, String namespace,
			Node node) throws XMLParsingException
	{
	 ...
	 *            current element
	 * @param defaultValue
	 *            value to be used if the specified element is missing or it's
	 ...
	 * 
	 * @throws XMLParsingException
	 *             specified child element is missing or the contained text does
	 *             not denote a double value
	 */
			...
			Node node) throws XMLParsingException
	{
		...
		if (value == null)
		{
			throw new XMLParsingException("Required attribute '" + namespace
					+ ":" + name + "' of element '" + node.getNodeName()
					+ "' is missing.");
		}
		return value;
	}
	 ...
	 * @throws SAXException
	 */
	public static Document parse(final URL resource) throws IOException,
			SAXException
	{
	...
	public static Node copyNode(Node source, Node dest)
	{
		...
		if (attr != null)
		{
			...
			{
				((Element) dest).setAttribute(attr.item(i).getNodeName(), attr
						.item(i).getNodeValue());
			}
		}
	 ...
	 * 'meta'-schema. for this it is nessecary that all into schemas uses the
	 * same prefix for namespace: http://www.w3.org/2001/XMLSchema. the result
	 * schema won't have a target namespace because the input schemas may use
	 * different target namespace (it's very probably that they will).
	 */
	...
	/**
	 * Appends a node and it's children to the given StringBuffer. Indentation
	 * is added on recursion.
	 */
