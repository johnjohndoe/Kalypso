...
public class LabelFactory {
	...
	public static Label createLabel (String caption, Font font, Color color, LineMetrics metrics,
									 ...
									 double rotation, double anchorPointX, double anchorPointY,
									 double displacementX, double displacementY) {
		if (rotation == 0.0) {
			return new HorizontalLabel (caption, font, color, metrics, feature, halo, 
                                        x, y, w, h, new double [] {anchorPointX, anchorPointY}, 
                                        new double [] {displacementX, displacementY});				
		} else {	
			return new RotatedLabel (caption, font, color, metrics, feature, halo, 
                                     x, y, w, h, rotation, new double [] {anchorPointX, anchorPointY}, 
                                     new double [] {displacementX, displacementY});
		}
	}
	...
	public static Label [] createLabels (LabelDisplayElement element, GeoTransform projection, Graphics2D g)
		...
		if (geometry instanceof GM_Point) {
			...
			labels = new Label [] {
				createLabel (caption, font, sldFont.getColor (feature), metrics,
						     feature, symbolizer.getHalo(), x, y, w, h, rotation*Math.PI,
			...
			if (getDistance (boxStartX, boxStartY, x, y) >= w) {
				...
				switch (placementType) {
					...
					case LinePlacement.TYPE_ABOVE: {
						label = createLabel (caption, font, sldFont.getColor (feature), metrics, feature, symbolizer.getHalo(), boxStartX, boxStartY, (int) width, (int) height, rotation, 0.0, 0.5, (w - width) / 2, delta + deviation [0]);
						break;
					}
					case LinePlacement.TYPE_BELOW:
					case LinePlacement.TYPE_AUTO: {
						label = createLabel (caption, font, sldFont.getColor (feature), metrics, feature, symbolizer.getHalo(), boxStartX, boxStartY, (int) width, (int) height, rotation, 0.0, 0.5, (w - width) / 2, -delta - deviation [1]);
						break;
					}
	...
	/**
	 * Calculates the maximum deviation that points on a linestring have to
	 * the ideal line between the starting point and the end point.
	 * <p>
