...
public class GMLSchema
{
    ...
    private void setImportedSchemas()
    {
        ...
        NodeList nl = m_schemaDoc.getElementsByTagNameNS(
                XMLHelper.XMLSCHEMA_NS, "import");
        for (int i = 0; i < nl.getLength(); i++)
        {
            try
            {
                Node attributeNode = XMLHelper.getAttributeNode(nl.item(i),
                        "schemaLocation");
                ...
                if (m_url == null)
                {
                    ...
                    //          if(
                    // "http://troubadix.wb.tu-harburg.de/doemming/test/feature.xsd".equals(
                    // url.toString() ) )
                    //          {
                    ...
                    // );
                    //            schema = new GMLSchema( uri, XMLHelper.getAsDOM(
                    // schemaFile ) );
                    //          }
                    //          else
                    //          {
    ...
    private void setNameSpaces()
    {
        ...
        for (int i = 0; i < nodeMap.getLength(); i++)
        {
            try
            {
                ...
                if ("http://www.w3.org/2000/xmlns/".equals(attribute
                        .getNamespaceURI()))
                    m_ns
                            .put(attribute.getLocalName(), attribute
                                    .getNodeValue());
            } catch (Exception e)
            {
                e.printStackTrace();
