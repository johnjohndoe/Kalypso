...
public class OracleSpatialAccess extends DBAccess {
     ...
     * @param sdoVersion version of the spatial extension
     */
     ...
     * @throws Exception
     */
    public OracleSpatialAccess( final String driver, final String logon, 
                                final Properties properties, final String sdoVersion )
                        throws SQLException, Exception {
    ...
    public Object performQuery( String query, final int startFeature, final int maxFeatures )
        ...
        if ( start != -1 ) { //it's present at least one ordinate array
    ...
    /**
     * reformates the select statement to an prepared statement. this method
     * will be called if the select statement contains a geometry operator 
     */
    private PreparedStatement reformatStatement(String query) throws SQLException {
        ...
        sdo_ordinate = StringExtend.extractString( query.toUpperCase(), 
        ...
        //convert the string representing the coordinates in double and load it into an ARRAY
        for ( int i = 0; i < count; i++ ) {
    ...
    /**
     * Performs a query against a oracle spatial database. If the type of the query result isn't aren't
     * wkb a Exception will be thrown.
     *
     * @param query the query parameter contains the query to perform.
     * @return the wkb's corresponding to the submitted query.
     *         If no wkb matches the query null will be returned.
     * @exception SQLException will be thrown if the result aren't a wkb's
     */
    ...
    /**
     * Performs a query against a oracle spatial database. If the type of the query result isn't aren't
     * wkb a JOSException will be thrown.
     *
     * @param query the query parameter contains the query to perform.
     * @param maxFeatures the maximum amount of features that should be
     *          returned by the request. if <tt>maxFeatures</tt> is <= 0 all
     ...
     * @return the wkb's corresponding to the submitted query.
     *         If no wkb matches the query null will be returned.
    * @exception Exception will be thrown if the result aren't a wkb's
     */
