...
public class DBaseIndex
{
	...
	*/
	...
	Used by createIndex()
	*/
	private DBaseIndex(String name, int startingPageNo, int numberOfPages, int sizeOfKeyRecord, int keyLength, int noOfKeysPerPage,
		int keyType, boolean uniqueFlag, RandomAccessFile file)
	{
		fileName = name;
	...
	*/
	...
	Create a new index
	*/
	public static DBaseIndex createIndex(String name, String column, int keyLength, boolean uniqueFlag, boolean numbers) throws IOException
	{
		RandomAccessFile file = new RandomAccessFile(name + ".ndx", "rw");

		int startingPageNo = 1, numberOfPages = 1,
			sizeOfKeyRecord, noOfKeysPerPage, keyType = numbers ? 1 : 0;
	...
	*/
	public void addKey(Comparable key, int record) throws IOException, KeyAlreadyExistException, InvalidKeyTypeException,
		KeyTooLongException
	{
		if(key == null)
			throw new NullPointerException();
