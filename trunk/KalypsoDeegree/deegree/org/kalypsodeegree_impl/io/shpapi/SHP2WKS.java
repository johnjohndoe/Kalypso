...
public class SHP2WKS {
    ...
    private boolean ccw(SHPPoint[] ring) {
        ...
        do {
            ...
            /*
             * p2 have not to be checked like p0. because of the loop
             * used in getRange() p2 can't be larger then ring.numPoints-1.
             */
            p2 = p1 + 1;
        ...
        for (int i = 0; i < ring.length; i++) {
            ...
            if ((ring[i].getX() == point.getX()) && (ring[i].getY() <= point.getY())) {
                ...
                while (ring[z].getX() == point.getX()) {
                  z += 1;
                  if (z == ring.length) z = 0;
                }

                //find previous point on ring with different x
                int zz = i - 1;
                if (zz < 0) zz = ring.length - 1;
    ...
    //                ring[j] = GeometryFactory.createGM_Position(shppolygon.rings.points[i][j].x,
