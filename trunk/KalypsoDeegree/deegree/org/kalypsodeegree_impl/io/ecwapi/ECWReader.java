...
public class ECWReader {
   ...
   * @param fileName full pathname of the ECW-file
   */
  ...
  /**
   * read part from ECW-file which falls within env and return this part
   * as BufferedImage with dimenions width and height
   * @param env bounding box in world coordinates of requested part
   * @param width width of the returned image
   * @param height height of the returned image
   */
  public BufferedImage getBufferedImage(GM_Envelope env, int width, int height) throws JNCSException {
    ...
    if (dWorldBRX > (ecwFile.originX + ((ecwFile.width - 1) * ecwFile.cellIncrementX))) // Huh? ECW does not except the full width
    ...
    // Set the view
    ecwFile.setView(ecwFile.numBands, bandlist, dWorldTLX, dWorldTLY, dWorldBRX, dWorldBRY, viewWidth, viewHeight);
