...
/**
 * factory class to wrap from binding geometries to GM_Object geometries and
 * visa versa
 * 
 * @author doemming
 */
public class GML3BindingGM_ObjectAdapter
{
	...
	final static Adapters m_csAdapter = org.kalypsodeegree_impl.model.cs.Adapters
			.getDefault();
	...
	static
	{
		...
		gm_object2BindingHash.put(GM_MultiCurve.class,
				MultiLineStringType.class);
		gm_object2BindingHash
				.put(GM_MultiSurface.class, MultiPolygonType.class);
	...
	public static GM_Object createGM_Object(final Object bindingObject)
			throws GM_Exception
	{
		...
		if (bindingObject instanceof AbstractGeometryType)
		{
			...
			final CS_CoordinateSystem cs = getCS_CoordinateSystem(null,
					bindingTypeObject);
			...
			if (bindingTypeObject instanceof LineStringType)
				return createGM_LineString((LineStringType) bindingTypeObject,
						cs);
			if (bindingTypeObject instanceof MultiPolygonType)
				return createGM_MultiSurface(
						(MultiPolygonType) bindingTypeObject, cs);
			if (bindingTypeObject instanceof MultiLineStringType)
				return createGM_MultiLineString(
						(MultiLineStringType) bindingTypeObject, cs);
			if (bindingTypeObject instanceof MultiPointType)
				return createGM_MultiPoint((MultiPointType) bindingTypeObject,
						cs);
		}
		// else
		// {
		...
		throw new UnsupportedOperationException(bindingObject.getClass()
				.getName()
				+ " is not supported");
	}

	private static GM_MultiPoint createGM_MultiPoint(MultiPointType type,
	...
	private static GM_MultiCurve createGM_MultiLineString(
			MultiLineStringType type, CS_CoordinateSystem cs)
			throws GM_Exception
	{
		final CS_CoordinateSystem co = getCS_CoordinateSystem(cs, type);
		final List<LineStringPropertyType> lineStringMember = type
				.getLineStringMember();
		final GM_Curve[] curves = new GM_Curve[lineStringMember.size()];
		final Iterator<LineStringPropertyType> iterator = lineStringMember
				.iterator();
		int i = 0;
		while (iterator.hasNext())
		{
			final LineStringPropertyType lineStringPropType = iterator.next();
			final LineStringType lineString = lineStringPropType
					.getLineString();
	...
	private static GM_MultiSurface createGM_MultiSurface(MultiPolygonType type,
			CS_CoordinateSystem cs) throws GM_Exception
	{
		final CS_CoordinateSystem co = getCS_CoordinateSystem(cs, type);
	...
	private static GM_Surface createGM_Surface(PolygonType type,
			CS_CoordinateSystem cs) throws GM_Exception
	{
		final CS_CoordinateSystem co = getCS_CoordinateSystem(cs, type);
		...
		final List<JAXBElement<AbstractRingPropertyType>> interior = type
				.getInterior();
		final Iterator<JAXBElement<AbstractRingPropertyType>> iterator = interior
				.iterator();
		final List<GM_Position[]> interiorList = new ArrayList<GM_Position[]>();
		while (iterator.hasNext())
		{
			final AbstractRingPropertyType abstractRingPropertyType = iterator
					.next().getValue();
			final GM_Position[] positions = createGM_PositionFrom(abstractRingPropertyType
					.getRing().getValue());
			interiorList.add(positions);
		}
		final GM_Position[][] interiorRings = interiorList
				.toArray(new GM_Position[interiorList.size()][]);
		return GeometryFactory.createGM_Surface(exteriorRing, interiorRings,
				null, co);
	}

	private static GM_Position[] createGM_PositionFrom(
	...
	private static GM_Curve createGM_LineString(LineStringType type,
			CS_CoordinateSystem cs) throws GM_Exception
	{
		final CS_CoordinateSystem co = getCS_CoordinateSystem(cs, type);
	...
	private static GM_Point createGM_Point(PointType type,
			CS_CoordinateSystem cs)
	{
		final CS_CoordinateSystem co = getCS_CoordinateSystem(cs, type);
		...
		final CoordinatesType coordinates = type.getCoordinates();
		final GM_Position[] positions = createGM_Positions(coordinates);
		return GeometryFactory.createGM_Point(positions[0], co);
	}
	...
	private static GM_Position[] createGM_Positions(CoordinatesType coordinates)
	{
		...
		final StringTokenizer tuppleTokenizer = new StringTokenizer(value,
				tuppleSeparator, false);
		while (tuppleTokenizer.hasMoreTokens())
		{
			final String tupple = tuppleTokenizer.nextToken();
			final StringTokenizer coordinatesTokenizer = new StringTokenizer(
					tupple, coordinateSepearator, false);
	...
	public static CS_CoordinateSystem getCS_CoordinateSystem(
			CS_CoordinateSystem defaultCS, AbstractGeometryType geom)
	{
	...
	private static String getCSName(final GM_Object geometry,
			String csNameDefault)
	{
		final CS_CoordinateSystem coordinateSystem = geometry
				.getCoordinateSystem();
	...
	public static EnvelopeType createBindingGeometryType(
			final GM_Envelope envelope)
	{
		...
		final CoordinatesType coordinatesType = createCoordinatesType(new GM_Position[]
		{ min, max });
		envelopeType.setCoordinates(coordinatesType);
		return envelopeType;
	}

	public static AbstractGeometryType createBindingGeometryType(
			final GM_Object geometry) throws GM_Exception
	{
		...
		if (geometry instanceof GM_MultiCurve)
			return createMultiLineStringType((GM_MultiCurve) geometry,
					csNameDefault);
		if (geometry instanceof GM_MultiSurface)
			return createMultiPolygonType((GM_MultiSurface) geometry,
					csNameDefault);
	...
	private static MultiPolygonType createMultiPolygonType(
			GM_MultiSurface multiSurface, String csNameDefault)
	{
		final String csName = getCSName(multiSurface, csNameDefault);
		final MultiPolygonType multiPolygonType = gml3Fac
				.createMultiPolygonType();
		final List<PolygonPropertyType> polygonMember = multiPolygonType
				.getPolygonMember();
		...
		for (int i = 0; i < allSurfaces.length; i++)
		{
			...
			final PolygonPropertyType polygonPropertyType = gml3Fac
	...
	private static MultiLineStringType createMultiLineStringType(
			GM_MultiCurve multiCurve, String csNameDefault) throws GM_Exception
	{
		final String csName = getCSName(multiCurve, csNameDefault);
		final MultiLineStringType multiLineStringType = gml3Fac
				.createMultiLineStringType();
		final List<LineStringPropertyType> lineStringMember = multiLineStringType
				.getLineStringMember();
		final GM_Curve[] allCurves = multiCurve.getAllCurves();
		for (int i = 0; i < allCurves.length; i++)
		{
			final GM_Curve curve = allCurves[i];
			final LineStringPropertyType lineStringPropertyType = gml3Fac
					.createLineStringPropertyType();
			final LineStringType lineStringType = createLineStringType(curve,
					csName);
	...
	private static MultiPointType createMultiPointType(
			GM_MultiPoint multiPoint, String csNameDefault)
	{
		...
		final List<PointType> pointList = multiPointType.getPointMembers()
				.getPoint();
	...
	private static PolygonType createPolygonType(GM_Surface surface,
			String csNameDefault)
	{
		...
		final CoordinatesType coordinatesType = createCoordinatesType(exteriorRing
				.getPositions());
		linearRingType.setCoordinates(coordinatesType);
		final JAXBElement<LinearRingType> linearRing = gml3Fac
				.createLinearRing(linearRingType);

		final AbstractRingPropertyType abstractRingPropertyType = gml3Fac
				.createAbstractRingPropertyType();
		abstractRingPropertyType.setRing(linearRing);

		final Exterior exterior = gml3Fac
				.createExterior(abstractRingPropertyType);

		polygonType.setExterior(exterior);
		// interior

		final List<JAXBElement<AbstractRingPropertyType>> interiorContainer = polygonType
				.getInterior();
		for (int i = 0; i < interiorRings.length; i++)
		{
			final GM_Ring ring = interiorRings[i];
			final LinearRingType interiorLinearRingType = gml3Fac
					.createLinearRingType();

			final CoordinatesType interiorCoordinatesType = createCoordinatesType(ring
					.getPositions());
			linearRingType.setCoordinates(interiorCoordinatesType);
			final JAXBElement<LinearRingType> interiorLinearRing = gml3Fac
					.createLinearRing(interiorLinearRingType);

			final AbstractRingPropertyType interiorAbstractRingPropertyType = gml3Fac
					.createAbstractRingPropertyType();
			abstractRingPropertyType.setRing(interiorLinearRing);

			final Interior interior = gml3Fac
					.createInterior(interiorAbstractRingPropertyType);
	...
	private static LineStringType createLineStringType(GM_Curve lineString,
			String csNameDefault) throws GM_Exception
	{
	...
	private static PointType createPointType(GM_Point point,
			String csNameDefault)
	{
