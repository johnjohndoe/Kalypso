...
public class CVDescriptorFactory
{
   ...
   */
  public static CVDescriptor createCVDescriptor( URL resource ) throws IOException, SAXException,
   ...
   */
  public static CVDescriptor createCVDescriptor( Document document ) throws IOException,
      InvalidAxisDefinitionException
  {
    ...
    {
      if( nl.item( i ) instanceof Element && nl.item( i ).getNodeName().endsWith( "Property" ) )
      {
    ...
    //		if(levelNode == null)
    //            throw new XMLParsingException( "Range node requires a Level child.");

    Level level = createLevel( levelNode );
    System.out.println( "*** Level:: " + level.toString() ); // ETJ ext level
  ...
  {
    Debug.debugMethodBegin( "CVDescriptorFactory", "createRange" );
    ...
    Node levelNode = XMLTools.getNamedChild( node, NSCV, "Level" );
    Level level = createLevel( levelNode );
    System.out.println( "*** Range(" + name + ")/Level:: " + level.toString() ); // ETJ
    // ext
    // level
  ...
  private static Level createLevel( Node node ) throws IOException
  {
    ...
    Node dir = XMLTools.getNamedChild( node, NSCV, "Directory" );

    if( dir != null )
    {
      tiles = createTilesFromDirectory( dir );
    }
    else
    {
    ...
    Level level = new Level_Impl( minScale, maxScale, properties, tiles );
  ...
  /**
   * creates an array of <tt>Tile</tt> instances from an array of
   * Tile-elements. Each element may represent one single Tile (file) or a set
   * of Tiles using '*' as wild card for file (not directory) name
   */
  ...
  /**
   * creats an array of <tt>Tile</tt> instances from a node that represents a
   * directory at the local or a remote file system
   */
  private static Tile[] createTilesFromDirectory( Node node ) throws IOException
  {
    Debug.debugMethodBegin( "CVDescriptorFactory", "createTilesFromDirectory" );
    ...
    int width = -1;
    int height = -1;
    String s = XMLTools.getAttrValue( node, "tileWidth" );
    if( s != null )
      width = Integer.parseInt( s );
    s = XMLTools.getAttrValue( node, "tileHeight" );
    if( s != null )
      height = Integer.parseInt( s );

    s = XMLTools.getAttrValue( node, "extensions" );
    ...
    // get names of all valid files within the directory
    File file = new File( dir );
    File[] files = file.listFiles( new CVFileFilter( extensions ) );

    // if the directory doesn't contain any files, the files are stored at
    // one or more direct sub-directories
    if( files == null || files.length == 0 )
    {
      String[] dirs = file.list( new CVFilenameFilter() );
      ArrayList list = new ArrayList( 2000 );
      for( int i = 0; i < dirs.length; i++ )
      {
        file = new File( dir + "/" + dirs[i] );
        files = file.listFiles( new CVFileFilter( extensions ) );
    ...
    Node lnode = XMLTools.getNamedChild( node, NSCV, "Level" );
    Level level = null;
    if( lnode != null )
      level = createLevel( lnode );

    // create tiles from the ilst of valid files
    ...
    for( int i = 0; i < files.length; i++ )
    {
      ...
      if( i % 500 == 0 )
        System.gc();
      double[] bbox = worldFile( files[i].getPath(), width, height );
      GM_Envelope env = GeometryFactory.createGM_Envelope( bbox[0], bbox[1], bbox[2], bbox[3] );
      list.add( new Tile_Impl( new URL( "file:///" + files[i].getPath() ), env, level ) );
    }
  ...
  private static double[] worldFile( String filename, int width, int height ) throws IOException
  {
     ...
     * beginning index, inclusive - and extends to the character at index
     * endIndex (position of '.') - the ending index, exclusive.
     */
    String fname = null;
    ...
    /*
     * Reads character files. The constructors of this class (FileReader) assume
     * that the default character encoding and the default byte-buffer size are
     * appropriate. The BufferedReader reads text from a character-input stream,
      ...
      throws InvalidAxisDefinitionException, MalformedURLException
  {
    ...
    GridCoverageLayer gcl = new GridCoverageLayer_Impl( id, title, abstract_, keywords, llbbox,
        crs, null, metadataURLs, formats, interpolationMethods, ged, descriptorResource, rsd );
  ...
  /**
   * creates a list URLs referencing metadata assigned to a
   * <tt>GridCoverageLayer</tt>
   */
  ...
  /**
   * creates a list of formats that are supported by the
   * <tt>GridCoverageLayer</tt>
   */
  ...
  /**
   * creates the <tt>GridExtentDescription</tt> for the
   * <tt>GridCoverageLayer</tt>
   */
      ...
      throws InvalidAxisDefinitionException
  {
    ...
    //
    GridExtentDescription ged = new GridExtentDescription_Impl( se, dim, null, null, grid, gad,
        null );
  ...
  private static SpatialExtent createSpatialExtent( Element element )
  {
    ...
    // TODO
    //        elem = (Element)element.getElementsByTagNameNS( NSWCS, "ZExtent"
    // ).item(0);
  ...
  private static ExtentType createExtent( Element element )
  {
    ...
    String s = element.getElementsByTagNameNS( NSWCS, "min" ).item( 0 ).getFirstChild()
        .getNodeValue().trim();
    double min = Double.parseDouble( s );
    s = element.getElementsByTagNameNS( NSWCS, "max" ).item( 0 ).getFirstChild().getNodeValue()
        .trim();
    double max = Double.parseDouble( s );
    double res = 0;
    try
    {
      s = element.getElementsByTagNameNS( NSWCS, "res" ).item( 0 ).getFirstChild().getNodeValue()
          .trim();
  ...
  private static GridRangeDescription[] createGridRangeDescription( NodeList nl )
  {
    ...
    for( int i = 0; i < grd.length; i++ )
    {
      ...
      //TODO
      // optional parameter
      grd[i] = new GridRangeDescription_Impl( rangeID, title, null, null, null, null, null, null,
          null );
    }
  ...
  private static class CVFileFilter implements FileFilter
  {
    ...
    CVFileFilter( String[] extensions )
    {
