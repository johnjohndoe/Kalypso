...
public class Layer implements Marshallable
{
   ...
   * @param dataURL
   *          contains a link to an online resource where data corresponding to
   *          the layer can be found.
   * @param metadataURL
   *          contains a link to an online resource where descriptive metadata
   *          corresponding to the layer can be found.
   * @param formatList
   *          parent element containing the list of available image format for
   *          this layer.
   * @param styleList
   *          parent element containing the list of available styles for this
   *          layer.
   * @param extension
   *          container tag in which arbitrary vendor specific information can
   *          be included
   * 
  ...
  public Layer( Server server, String name, String title, String abstract_, String[] srs,
      ...
      boolean queryable, boolean hidden, Marshallable extension ) throws ContextException
  {
    setName( name );
    setTitle( title );
  ...
  /**
   * The element defining the service from which the named Layer may be
   * requested
   * 
   * @return
   */
  ...
  /**
   * The abstract of the selected layer (extracted from Capabilities by the
  ...
  /**
   * This element contains a link to an online resource where descriptive
   ...
   * WMS 1.1.1 Specification. <p/>A FormatList shall include at least one Format
   * 
   * @return
   */
  ...
  public String exportAsXML()
  {
    ...
    if( dataURL != null )
    {
      sb.append( "<DataURL width='" ).append( " format='" ).append( dataURL.getFormat() ).append(
          "'>" );
      sb.append( "<OnlineResource  xlink:type='simple' xlink:href='" ).append(
          NetWorker.url2String( dataURL.getOnlineResource() ) ).append( "'/>" );
      sb.append( "</DataURL>" );
    }

    if( metadataURL != null )
    {
      sb.append( "<MetadataURL width='" ).append( " format='" ).append( metadataURL.getFormat() )
          .append( "'>" );
      sb.append( "<OnlineResource  xlink:type='simple' xlink:href='" ).append(
          NetWorker.url2String( metadataURL.getOnlineResource() ) ).append( "'/>" );
