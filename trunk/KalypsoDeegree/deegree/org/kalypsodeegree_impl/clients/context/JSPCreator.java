    ...
    private static StringBuffer createInitCalls( GUIArea guiarea ) {
        ...
        for ( int i = 0; i < module.length; i++ ) {  
            ...
            sb.append( "var " ).append( module[i].getName().toLowerCase() ).append( " = new " )
              .append( module[i].getName() ).append( "('" ).append(module[i].getName());
...
System.out.println("commonJS: " + commonJS[i]);        	
    ...
    /**
     * returns a matrix containing an array of layer for each server defined
    ...
    public static String[][] getServiceRequests( ViewContext viewContext ) throws ContextException {
        ...
        GM_Envelope env = 
        	GeometryFactory.createGM_Envelope( bbox[0].getX(), bbox[0].getY(), 
        									   bbox[1].getX(), bbox[1].getY() );
        String crs = null;
     ...
     */
      public static String getLayerAsHTMLTable( ViewContext viewContext ) {
        ...
        sb.append( "<tr bgcolor='#E2E2EC' ><td colspan='3'><p><b>&nbsp;Grünplan</b></p></td><!--td colspan='2'></td--></tr>\n");
        ...
        for ( int i = serv.length-1; i >= 0; i-- ) {
            ...
            for(int k = 0; k < layers.length; k++) {
                ...
                sb.append( "value='").append( layers[k].getName() )
                  .append("|").append( layers[k].getStyleList().getCurrentStyle().getName())
                  .append( "'/></td>\n" );
                // title section 
    ...
    public static void main( String[] args ) throws Exception {
        ViewContext vc = WebMapContextFactory.createViewContext( 
                                 "C:/Projekte/UmweltInfo/fachschalen/WEB-INF/xml/mapcontext.xml" );                       
