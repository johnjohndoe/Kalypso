...
/**
 * will be called if the client forces a zoomin action. the zoomin will be
 * performed by setting the map boundaries to the rectangle selected by the
 * client or centering the map onto the point the user had mouse-clicked to. the
 * point or the rectangle is defined within the settings that are contained
 * within the <tt>MapClientEvent</tt>.
 * <p>
 * -------------------------------------------------------
 * </p>
 * 
...
{
  /**
   * the method will be called by the <tt>MapListener</tt> if a zoomin
   * action/event occurs.
   */
  ...
  /**
   * the method perfomrs a "zoom in" on the map model. This means the map will
   * be zoomed and recentered onto a specified point or rectangle . Usually this
   * will be the point the user clicked on the map or a rectangle he had drag
   * with the mouse.
   */
  ...
  /**
   * The method performs a "zoom in by rectangle" to the map model. This means
   * that the map is set to a new bounding box that is smaller then the previous
   * one. Usually the user defines the new bounding box by dragging a rectangle
   * on the map.
   */
  private WMSGetMapRequest modifyModelToZoomInByRect( WMSClientConfiguration config, HashMap model )
      throws InconsistentRequestException, XMLParsingException, MalformedURLException
  {
