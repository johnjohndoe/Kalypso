...
/**
 * A the top level of the ContentMetadata section of the Capabilities XML document
 * for WCS is a CoverageLayerList element, which lists coverage layers in any
 * order. 
 *
 * <p>-----------------------------------------------------------------------</p>
 *
...
class WCSCapabilities_Impl extends OGCWebServiceCapabilities_Impl implements WCSCapabilities {
    ...
    /** returns the list off <tt>CoverageLayer</tt> that are available through the
     * WCS.
     *
     */
                ...
                if ( cl[i] instanceof GridCoverageLayer ) {
                    ...
                    for (int j = 0; j < ga.length; j++) {
                        ...
                        if ( ga[j].getDescription() != null ) {
                            sb.append( "<wcs:description>" + ga[j].getDescription() + 
                                       "</wcs:description>" );
                        }
                        int o = ga[j].getOrientation();
                        String s = null;
                        switch( o ) {
                            case GridAxis.BACK: s = "back";
                            case GridAxis.DOWN: s = "down";
                            case GridAxis.FRONT: s = "front";
                            case GridAxis.LEFT: s = "left";
                            case GridAxis.RIGHT: s = "right";
                            case GridAxis.UP: s = "up";
                        }
