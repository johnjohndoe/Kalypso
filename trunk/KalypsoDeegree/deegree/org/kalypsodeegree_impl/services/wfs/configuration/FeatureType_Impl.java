...
public class FeatureType_Impl implements FeatureType
{
  ...
  /**
   * returns the names of the datastore fields a property of the feature type
   * will be mapped to
   */
  ...
  /**
   * Returns the type of the datastore field. If has not been mapped explicitly,
   * the metadata of the underlying database is queried (i.e. the table
   * description).
   * <p>
   * 
   * @param datastoreField
   *          fieldname in the form 'TABLE.FIELD' (uppercased)
  ...
  {
    return ( properties.get( property ) != null || ( properties.isEmpty() && dbFieldTypes
        .get( property ) != null ) );
  }
  ...
  public void setMasterTable( MasterTable masterTable )
  {
    ...
    {
      referencedBy.put( ref[i].getTargetTable().toUpperCase(), masterTable.getName().toUpperCase() );
    }
  ...
  /**
   * returns a description of all tables that are related direct or indirect to
   * the master table. if no related tables are defined the method returns a
   * zero length array
   */
  ...
  public void addRelatedTable( RelatedTable relatedTable )
  {
    ...
    {
      referencedBy
          .put( ref[i].getTargetTable().toUpperCase(), relatedTable.getName().toUpperCase() );
    }
  ...
  /**
   * returns a description object for the table identified by the submitted
   * name. if no table with the submitted can be found <tt>null</tt> will be
   * returned.
   */
  ...
  /**
   * returns the name of the coordinated reference system the geo spatial data
   * are stored. this is mandatory for ORACLESPATIAL, POINTDB and SHAPEFILES.
   */
   ...
   * the data source. This code may differs from the EPSG code stored in element
   * <CRS>
   */
  ...
  /**
   * returns the list (in correct order) of tables that are used to reach the
   * submitted property
   *  
   */
