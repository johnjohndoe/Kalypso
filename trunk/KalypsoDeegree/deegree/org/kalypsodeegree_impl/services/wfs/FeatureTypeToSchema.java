...
/**
 * The class transforms a list of FeatureTypes to their corresponding XML-Schema
 * base on the GML-schema definition.
 * 
 * <p>
 * ------------------------------------------------------------------------
 * </p>
...
public class FeatureTypeToSchema
{
  ...
  /**
   * Only public method of the class. It recieves an array of feature types and
   * transform them to their corresponding GML based XML-schema. Depending on
   * the used constructor the returned root- <tt>Node</tt> is a
   * <tt>org.w3c.dom.Document</tt> or a simple <tt>org.w3c.dom.Node</tt>.
   */
  public Document[] createFeatureTypeSchema( FeatureType[] types ) throws Exception
  {
    ...
    for( int i = 0; i < types.length; i++ )
    {
      ...
      for( int j = 0; j < ftp.length; j++ )
      {
        ...
        else if( ftp[j].getType().equals( "java.lang.Float" ) )
        {
          createFloat( sequence, ftp[j] );
        }
        else if( ftp[j].getType().equals( "java.lang.Double" )
            || ftp[j].getType().equals( "java.math.BigDecimal" ) )
        {
          createDouble( sequence, ftp[j] );
        }
        else if( ftp[j].getType().equals( "java.lang.String" ) )
        {
          createString( sequence, ftp[j] );
        }
        else if( ftp[j].getType().startsWith( "java.lang.Object" ) )
        {
          createObject( sequence, ftp[j] );
        }
        else if( ftp[j].getType().startsWith( "org.deegree.model.geometry.GM_" ) )
        {
  ...
  /**
   * creates an elements for desribing a property with data type = integer and
   * adds it to the submitted parent
   */
  private void createInteger( Element parent, FeatureTypeProperty ftp )
  {
    ...
    createXSDPrecision( restriction, 10 );
  }

  /**
   * creates an elements for desribing a property with data type = float and
   * adds it to the submitted parent
   */
  ...
  /**
   * creates an elements for desribing a property with data type = double and
   * adds it to the submitted parent
   */
  ...
  /**
   * creates an elements for desribing a property with data type = string and
   * adds it to the submitted parent
   */
  private void createString( Element parent, FeatureTypeProperty ftp )
  {
    ...
    Element simpleType = createXSDSimpleType( element );
    createXSDRestriction( simpleType, "xsd:string" );
  }

  /**
   * creates an elements for desribing a property with data type = object and
   * adds it to the submitted parent. This method will/should be used if the
   * data type of a property can't be determined exactly.
   */
  private void createObject( Element parent, FeatureTypeProperty ftp )
  {
    ...
    Element simpleType = createXSDSimpleType( element );
    createXSDRestriction( simpleType, "xsd:object" );
  }

  /**
   * creates an elements for desribing a property with data type = geometry and
   * adds it to the submitted parent.
   * <p>
  ...
  private void createGeometry( Element parent, FeatureTypeProperty ftp )
  {
    if( ftp.isNullable() )
    {
      createXSDElement( parent, ftp.getName(), "gml:_Geometry", true, 0, 1 );
    }
    else
    {
      createXSDElement( parent, ftp.getName(), "gml:_Geometry", false, 1, 1 );
    }
  }

  /**
   * creates the schema tag of the describtion. If the root is a
   * <tt>org.w3c.dom.Document</tt> returned <tt>Element</tt> will be the
   * root element.
   */
  ...
  /**
   * creates a <xsd:restriction>tag. it is used within simple type definitions
   */
  ...
  /**
   * creates a <xsd:precision>tag. it is used to define the precision of numeric
   * types
   */
  ...
  /**
   * creates a <xsd:element>tag that contains the definition of one feature type
   * property
   */
  private Element createXSDElement( Node parent, String name, String type, boolean nullable,
  ...
  /**
   * creates a <xsd:sequence>tag that contains a element tag for each property
   * definied for a feature type
   */
  ...
  /**
   * creates a <xsd:complexContent>tag that contains a feature type definition
   */
  ...
  /**
   * creates a <xsd:complexType>tag that contains a feature type definition (
   * <xsd:complexContent>).
   */
