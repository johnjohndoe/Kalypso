...
public class PointRowSelector extends RowSelector
{
  ...
  public PointRowSelector( TableDescription table, String keyfield, Set keys, DBAccess osa,
  ...
  public RowSelector[] getReferencedRows() throws SQLException
  {
    ...
    for( int i = 0; i < refs.length; i++ )
    {
      ...
      if( fKeys.size() > 0 )
      {
        rowSelectors.add( new PointRowSelector( targetTable, targetField, fKeys, osa, ft ) );
      }
    }
    return (PointRowSelector[])rowSelectors.toArray( new PointRowSelector[rowSelectors.size()] );
  }
  ...
  public void updateRows( ArrayList changeList ) throws SQLException
  {
    ...
    for( int i = 0; i < changeList.size(); i++ )
    {
      ...
      if( type == FeatureType.GEOMETRY )
      {
        ...
        {
          throw new SQLException( "Error parsing GMLPoint-fragment: " + e.getMessage() );
        }
      ...
      {
        setFieldValue( stmt, index++, ft.getDatastoreFieldType( fullName ), value, name );
      }
    }
