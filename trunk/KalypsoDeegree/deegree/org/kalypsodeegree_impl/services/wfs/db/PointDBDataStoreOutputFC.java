...
public class PointDBDataStoreOutputFC implements DataStoreOutputFormat
{
  ...
  /**
   * returns the index of the column with the submitted name. if the column
   * isn't contained within the columnNames array -1 will be returned
   */
  ...
  /**
   * returns a <tt>GM_Point</tt> object created from the submitted x, y value
  ...
  private GM_Object getGeometry( String x, String y, String crs )
  {
    ...
    return GeometryFactory.createGM_Point( Double.parseDouble( x ), Double.parseDouble( y ), cs );
  }
}