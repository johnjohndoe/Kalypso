...
class CGetFeature extends DBGetFeature
{
  ...
  protected OGCWebServiceResponse[] performRequest( OGCWebServiceRequest request )
  {
    ...
    try
    {
      ...
      for( int i = 0; i < queries.length; i++ )
      {
        ...
        {
          org.deegree.services.wfs.configuration.FeatureType ft = config.getFeatureType( queries[i]
              .getTypeName() );
          ...
          {
            o = FilterTools.extractFirstBBOX( (ComplexFilter)filter );
            if( o[0] != null )
            {
              filter = (Filter)o[0];
            }
          ...
          String query = getQuery( filter, queries[i] );
          ...
          tmp.put( queries[i].getTypeName(), pl );
          PointDBDataStoreOutputFC form = new PointDBDataStoreOutputFC();
          FeatureCollection fc = (FeatureCollection)form.format( tmp, pl, (Filter)o[1] );
          pl.addParameter( WFSConstants.FEATURECOLLECTION, fc );
    ...
    catch( Exception e )
    {
      ...
      OGCWebServiceException exce = new OGCWebServiceException_Impl( "CGetFeature: performRequest",
  ...
  private String getQuery( Filter filter, WFSQuery query ) throws Exception
  {
    ...
    org.deegree.services.wfs.configuration.FeatureType ft = config.getFeatureType( query
        .getTypeName() );

    StringBuffer query_s = new StringBuffer( "SELECT " );
    String g = null;
    if( ft.getMasterTable().getGeoFieldIdentifier() != null
        && ft.getMasterTable().getGeoFieldIdentifier().length > 0 )
    {
      g = ft.getMasterTable().getGeoFieldIdentifier()[0].getDatastoreFieldBaseName();
    }
    String s = getAffectedFields( ft.getMasterTable().getName(), ft, query );
    if( g != null )
    {
      s = StringExtend.replace( s, ft.getMasterTable().getName() + "." + g, ft.getMasterTable()
          .getName()
          + "." + g + "_X," + ft.getMasterTable().getName() + "." + g + "_Y", true );
    }
    query_s.append( s );
