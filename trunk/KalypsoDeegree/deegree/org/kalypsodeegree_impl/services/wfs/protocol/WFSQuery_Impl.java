...
/**
 * Each individual query packaged in a GetFeature request is defined using the
 * query value. The query value defines which feature type to query, what
 * properties to retrieve and what constraints (spatial and non-spatial) to
...
public class WFSQuery_Impl implements WFSQuery
{
   ...
   * support feature versioning. A value of ALL indicates that all versions of a
   * feature should be fetched. Otherwise an integer can be specified to return
   * the n th version of a feature. The version numbers start at '1' which is
   * the oldest version. If a version value larger than the largest version is
   * specified then the latest version is return. The default action shall be
   * for the query to return the latest version. Systems that do not support
   * versioning can ignore the parameter and return the only version that they
   * have.
   */
  ...
  /**
   * The typeName attribute is used to indicate the name of the feature type or
   * class to be queried.
   */
 ...
 * empty log message *** Revision 1.5 2004/02/09 08:00:22 poth no message
 * 
 * Revision 1.4 2003/08/18 07:23:26 poth no message
 * 
 ...
 * 
 * Revision 1.5 2002/05/29 16:09:38 ap no message
 * 
 * Revision 1.4 2002/05/14 14:39:51 ap no message
 * 
