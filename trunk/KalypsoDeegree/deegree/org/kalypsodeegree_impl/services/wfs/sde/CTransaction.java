...
class CTransaction extends AbstractTransaction
{
        ...
        if( parent.isKnownFeatureType( ftName ) )
        {
          ...
          for( int j = 0; j < gmlFeatures.length; j++ )
          {
            ...
            for( int k = 0; k < gmlProperties.length; k++ )
            {
              ...
              {
                throw new Exception( "no SDE field for property: " + name + " could be found!" );
              }
              row.put( s[0], gmlProperties[k].getPropertyValue() );
              System.out.println( "prop: " + name + "   SDE field: " + s[0] + "   value: "
                  + gmlProperties[k].getPropertyValue() );
            }
            System.out.println( "----------" );
            try
            {
  ...
  private boolean performUpdate( WFSUpdate update ) throws Exception
  {
    ...
    org.deegree.services.wfs.configuration.FeatureType featureType = config.getFeatureType( update
        .getTypeName() );
  ...
  private boolean performDelete( WFSDelete delete )
  {
    ...
    {
      org.deegree.services.wfs.configuration.FeatureType featureType = config
          .getFeatureType( delete.getTypeName() );
