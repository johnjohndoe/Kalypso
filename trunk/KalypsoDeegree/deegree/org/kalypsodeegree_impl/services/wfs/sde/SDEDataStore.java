 ...
 * shape-file in memory to provide a mauch fast access to them. The cache
 * can be configured throw the configuration XML-document that also contains
...
public class SDEDataStore extends AbstractDataStore {
    ...
    private class CGetFeature extends AbstractGetFeature {
            ...
            try {
                ...
                for ( int i = 0; i < queries.length; i++ ) {
                    if ( isKnownFeatureType( queries[i].getTypeName() ) ) {
                        ...
                        Feature[] features = getFeatures( queries[i], (GM_Object[])o[1], 
                                                          (Table)o[0], startPosition, 
                                                          maxFeatures );
                ...
                response[0] = WFSProtocolFactory.createWFSGetFeatureResponse( request, affectedFeatureTypes, 
                                                                   exce, null );
            }
        ...
        /**
         * returns the name of the datastore fields (SDE table column names)
         * that shall be returned by the query or null if all columns shall be 
         * returned
         */
            ...
            if ( props != null && props.length > 0  ) {
                ...
                for (int i = 0; i < props.length; i++) {                
                    ...
                    if ( s == null || s.length == 0 ) {
                        throw new Exception( "no field for property: " + props[i] +
                                             " can be found!" );
                    }
                    fields[i] = s[0];
                }
        ...
        private Object[] getDataFromSDE( WFSQuery query, String[] cols ) throws Exception {
            ...
            sq.setSpatialFilter( bbox.getMin().getX(), bbox.getMin().getY(), 
         ...
         */
        private Feature[] getFeatures( WFSQuery query, GM_Object[] deegree_gm_obj, 
                                       Table table, int startPosition, 
            ...
            // create feature type
            org.deegree.model.feature.FeatureType ftt = 
                createFeatureType( table, ft, deegree_gm_obj.length == table.getRowCount());
            ...
            if ( deegree_gm_obj.length == table.getRowCount() ) {
                ...
                for ( int i = startPosition; i < table.getRowCount(); i++ ) {
                    ...
                    for ( int j = 0; j < ( fp.length - 1 ); j++ ) {
                        ...
                        fp[j] = FeatureFactory.createFeatureProperty( s, 
                                                     table.getValueAt( i - startPosition, j ) );
                    }
                    ...
                    fp[fp.length - 1] = 
                    	FeatureFactory.createFeatureProperty( s, deegree_gm_obj[i - startPosition] );
        ...
        private SpatialQuery openSpatialQuery() throws Exception {
            ...
            SpatialQuery sq = new SpatialQuery( server, port, database, 
                                                config.getConnection().getUser(), 
        ...
        private org.deegree.model.feature.FeatureType createFeatureType(Table table, 
                                                        FeatureType ft, boolean spatialField) {
            Debug.debugMethodBegin();
            ...
            if ( spatialField ) {
                ...
                for ( int i = 0; i < table.getColumnCount(); i++ ) {
                    ...
                    ftp[i] = FeatureFactory.createFeatureTypeProperty( s, 
                                                                table.getColumnType( i ), true );
                }
            ...
            org.deegree.model.feature.FeatureType ftt = 
            	FeatureFactory.createFeatureType( null, null, ft.getMasterTable().getName(), ftp );
            Debug.debugMethodEnd();
            return ftt;
        }
