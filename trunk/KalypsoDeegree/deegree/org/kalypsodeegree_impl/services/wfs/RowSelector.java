 ...
 * but for Geometry-fields, the <tt>updateRows()</tt> -method has to be
...
public class RowSelector
{
  ...
  public RowSelector( TableDescription table, String keyfield, Set keys, DBAccess osa,
      FeatureType ft ) throws SQLException
  {
  ...
  /**
   * Creates new <tt>RowSelector</tt> -objects that represent the rows in
   * other tables that are referenced from the current table's rows.
   * <p>
   * 
  ...
  public RowSelector[] getReferencedRows() throws SQLException
  {
    ...
    ArrayList rowSelectors = new ArrayList();
    for( int i = 0; i < refs.length; i++ )
    {
      ...
      StringBuffer sql = new StringBuffer( "SELECT " ).append( refs[i].getTableField() ).append(
          " FROM " ).append( table.getName() ).append( " WHERE " ).append( keyfield ).append(
          " IN (" );
      ...
      // referenced table (but only if rows are actually selected)
      if( fKeys.size() > 0 )
      {
        rowSelectors.add( new RowSelector( targetTable, targetField, fKeys, osa, ft ) );
      }
    }
    return (RowSelector[])rowSelectors.toArray( new RowSelector[rowSelectors.size()] );
  }
  ...
  public void deleteRows() throws SQLException
  {
    ...
    StringBuffer sql = new StringBuffer( "DELETE FROM " ).append( table.getName() ).append(
        " WHERE " ).append( keyfield ).append( " IN (" );
  ...
  public void updateRows( ArrayList changeList ) throws SQLException
  {
    ...
    for( int i = 0; i < changeList.size(); i++ )
    {
      ...
      setFieldValue( stmt, index++, ft.getDatastoreFieldType( name ), value, name );
    }
  ...
  public String toString()
  {
    ...
    sb.append( "Table: '" ).append( table.getName() ).append( "', Field: '" ).append( keyfield )
        .append( "', Values: [" );
  ...
  /**
   * Convenience method that calls the right setXXX ()-method for the given
   * <tt>PreparedStatment</tt>.
   * <p>
   * 
   * @param stmt
   *          the <tt>PreparedStatement</tt> to be "filled"
   * @param i
   *          index of the field to be set
   * @param type
   *          <tt>java.sql.Types</tt> -constant for the field type
   * @param value
   *          new String value for the field
   * @throws SQLException
   *           if the conversion of the String failed (for example in the case
   *           of <tt>Integer</tt> s or <tt>Date</tt>s)
   */
  protected void setFieldValue( PreparedStatement stmt, int i, int type, String value, String field )
      throws SQLException
  {
    ...
    try
    {
      ...
      {
        stmt
            .setDate( i, new java.sql.Date( TimeTools.createCalendar( value ).getTime().getTime() ) );
        break;
      }
      default:
      {
        throw new SQLException( table.getName() + "." + field + " has invalid type: " + type );
      }
      }
    }
    ...
    {
      e.printStackTrace();
      throw new SQLException( "Value '" + value + "' is not suitable for " + "field '" + field
