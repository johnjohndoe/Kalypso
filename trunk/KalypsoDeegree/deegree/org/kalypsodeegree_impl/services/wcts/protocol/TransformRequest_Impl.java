...
/**
 * <p>
 * The transform-schema describes the only mandatory request, which is to adapt
 * from the Transformation-Web-Service
 * <p>
 * The basic elememts of the reqest are the coordinate-system of the data
 ...
 * <p>
 * The determination of a transformation-sequence is optional. Normally several
 * single-transformations are needed for the transformation of coordinates from
 * one system into another. With the &lt;TransformationSequence&gt;- element it
 * is possible to define (a) in which order (b) which transformations should be
 * executed.
 * <p>
 * It is to notice, that the transformationservice is based on the OpenGIS
 * implemantation specification &quot;Coordinate Transformation Service
 * 1.0.0&quot;. I. e. that the transformation of a geometry results from the
 * transformation of its particular coordinates. Splines and other not-linear
 * interpolations get not explicit transformed. So, for the the incoming data
 * only point-geometries and linear interpolated geometries are acceptable. In
 * addition to the GML-coded geometries a coding of &quot;Well Known Text&quot;
 * (WKT) is possible. A WKT-geometry is transported as a string.
 * <p>
 * ----------------------------------------------------------------------
 * </p>
 * 
...
public class TransformRequest_Impl extends OGCWebServiceRequest_Impl implements TransformRequest
{
  ...
  TransformRequest_Impl( String version, String id, HashMap vendorSpecificParameter,
      ...
      CS_CoordinateSystem destinationCRS, TransformationSequence[] transformationSequence,
      GMLGeometry[] data )
  {
  ...
  /**
   * gets DestinationCRS-element. Unique description of the Destination
   * coordinate-system
   */
  ...
  /**
   * gets the TransformationSequence-element. Sequence of the transformation
   * steps. Each transformation step is to be described completely by means of
   * XML.
   */
  public TransformationSequence[] getTransformationSequence()
  {
    return (TransformationSequence[])transformationSequence
        .toArray( new TransformationSequence[transformationSequence.size()] );
  }
