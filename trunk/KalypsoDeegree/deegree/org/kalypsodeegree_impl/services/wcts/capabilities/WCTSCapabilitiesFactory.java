...
public final class WCTSCapabilitiesFactory {
    /**
     * factory method for creating a <tt>WCTSCapabilities</tt> object from
     * a file that contains a WCTS-specification conform XML capabilities document
     */
    public static synchronized CTS_Capabilities createCapabilities( Reader reader )
                                                            throws Exception {
    ...
    /**
     * factory method for creating a <tt>WCTSCapabilities</tt> object from
     * a WCTS-specification conform XML capabilities document
     */
    public static synchronized CTS_Capabilities createCapabilities( Document doc )
                                                            throws Exception {
        Debug.debugMethodBegin( "WCTSCapabilitiesFactory", "createCapabilities" );
    ...
    */
     ...
     * the WCTS capabilities.
     * <p>Contains the two attributes <tt>userDefinedCoordinateSystems</tt>
     * and <tt>userDefinedTransformations</tt>
     */
    private static Capability getCapability( Element capElement ) throws Exception {
        ...
        KnownCoordinateReferenceSystem[] kcrs_array = new KnownCoordinateReferenceSystem[nlkcrs.getLength()];
        ...
        Capability capability = new Capability_Impl( userDefinedCoordinateSystems, 
                                                     userDefinedTransformations, request, ktt_array, 
                                                     kcrs_array, null );
    ...
    private static KnownTransformationType getKnownTransformationType( Element kttElement ) {
        ...
        KnownTransformationType knowntransformationtype = new KnownTransformationType_Impl( 
                                                                  authority, code );
        Debug.debugMethodEnd();
     ...
     */
    private static KnownCoordinateReferenceSystem getKnownCoordinateReferenceSystem( Element kcrsElement ) {
     ...
     * IsTransformable, DescribeTransformation</tt> are built in the same
     * way. This method uses this to summarize the ChildNodes.
     */
        ...
        for ( int k = 0; k < nl.getLength(); k++ ) {
            ...
            try {
                ...
                for ( int i = 0; i < getL.getLength(); i++ ) {
                    String u = XMLTools.getAttrValue( XMLTools.getFirstElement( getL.item( i ) ), 
                                                      "href" );
                    getOR[i] = new URL( u );
                }
                ...
                for ( int i = 0; i < postL.getLength(); i++ ) {
                    String u = XMLTools.getAttrValue( XMLTools.getFirstElement( postL.item( i ) ), 
                                                      "href" );
                    postOR[i] = new URL( u );
                }
