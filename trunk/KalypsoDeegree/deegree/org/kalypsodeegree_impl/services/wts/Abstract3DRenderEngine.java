...
abstract public class Abstract3DRenderEngine implements RenderEngine
{
   ...
   * 
   * The Canvas3D class provides a drawing canvas for 3D rendering. The Canvas3D
   * object extends the Canvas object to include 3D-related information such as
   * the size of the canvas in pixels, the Canvas3D's location, also in pixels,
   * within a Screen3D object, and whether or not the canvas has stereo enabled.
   * Because all Canvas3D objects contain a reference to a Screen3D object and
   * because Screen3D objects define the size of a pixel in physical units, Java
   * 3D can convert a Canvas3D size in pixels to a physical world size in
   * meters. It can also determine the Canvas3D's position and orientation in
   * the physical world.
   * 
  ...
  {
    //This class is used to obtain a valid GraphicsConfiguration that can be
    // used by Java 3D.
    //It instantiates objects and then sets all non-default attributes as
    // desired.
  ...
  private TransformGroup createTransformGroup()
  {
    ...
    //Specifies that the node allows access to its object's transform
    // information.
    view_tg.setCapability( TransformGroup.ALLOW_TRANSFORM_READ );

    //Specifies that the node allows writing its object's transform
    // information.
    view_tg.setCapability( TransformGroup.ALLOW_TRANSFORM_WRITE );

    //Specifies that this Node allows read access to its local coordinates to
  ...
  protected void setView( View view, BranchGroup view_group )
  {
    ...
    //The ViewPatform class is used to set up the "view" side of a Java 3D
    // scene graph.
    ViewPlatform camera = new ViewPlatform();

    //RELATIVE_TO_FIELD_OF_VIEW tells Java 3D that it should modify the
    // eyepoint position so it is located
    //  at the appropriate place relative to the window to match the specified
    // field of view.
    //This implies that the view frustum will change whenever the application
    ...
    translation.setTranslation( new Vector3d( -vp.getObserverPosition().x,
        vp.getObserverPosition().y, vp.getObserverPosition().z ) );

    //creates the POI (Point of View)
    Transform3D angle = new Transform3D();

    //rotY: Sets the value of this transform to a counter clockwise rotation
    // about the y axis.
    angle.rotY( mapAngle( vp.getHDirection() ) );

    Transform3D angle2 = new Transform3D();

    //rotX: Sets the value of this transform to a counter clockwise rotation
    // about the x axis.
    ...
    // The View object contains all parameters needed in rendering a three
    // dimensional scene
    // from one viewpoint.
  ...
  protected void addBackround( ViewPoint vp, Group world_group, String background )
  {
    ...
    //bounds = new BoundingBox(new Point3d(-200,-10,-140), new
    // Point3d(200,0.1,60) );
  ...
  public BranchGroup createBackground( ViewPoint vp, String background )
  {
    ...
    float[] vertices =
    { (float)-fp[1].x, (float)fp[1].y - 200, (float)fp[1].z, (float)-fp[0].x, (float)fp[0].y - 200,
        (float)fp[0].z, (float)-fp[0].x, (float)fp[0].y + 5000, (float)fp[0].z, (float)-fp[1].x,
        (float)fp[1].y + 5000, (float)fp[1].z };
