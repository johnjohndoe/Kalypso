...
class GetFeatureInfoHandler_Impl implements GetFeatureInfoHandler
{
  ...
  private double calcScale() throws Exception
  {
    ...
    GM_Position min = GeometryFactory.createGM_Position( bbox.getMin().getX() + dx
        * ( getMapRequest.getWidth() / 2d - 1 ), bbox.getMin().getY() + dy
        * ( getMapRequest.getHeight() / 2d - 1 ) );
    GM_Position max = GeometryFactory.createGM_Position( bbox.getMin().getX() + dx
        * ( getMapRequest.getWidth() / 2d ), bbox.getMin().getY() + dy
        * ( getMapRequest.getHeight() / 2d ) );
  ...
  public WMSGetFeatureInfoResponse performGetFeatureInfo() throws WebServiceException
  {
    ...
    for( int i = 0; i < qlayers.length; i++ )
    {
      ...
      {
        throw new LayerNotQueryableException( "Layer: " + qlayers[i] + " is not " + "queryable" );
      }
      if( !layerList[i].isSrsSupported( getMapRequest.getSrs() ) )
      {
        throw new InvalidSRSException( "SRS: " + getMapRequest.getSrs() + "is not known by layer: "
            + qlayers[i] );
      }
    }
   ...
   * or the time limit has been reached. If the waiting is terminated by
   * reaching the time limit an <tt>WebServiceException</tt> will be thrown to
   * indicated that the request couldn't be performed correctly.
   * 
   * @throws WebServiceException
  ...
  /**
   * Inner class for accessing the data of one layer and creating a GML document
   * from it. The class extends <tt>Thread</tt> and implements the run method,
   * so that a parallel data accessing from several layers is possible.
   * 
   * @version $Revision$
   * @author <a href="mailto:poth@lat-lon.de">Andreas Poth </a>
   */
  private class ServiceInvoker extends Thread implements OGCWebServiceClient
  {
    ...
    public void run()
    {
      ...
      if( ds != null )
      {
        ...
        {
          Debug.debugException( e, " - " );
          OGCWebServiceException exce = new OGCWebServiceException_Impl( "ServiceInvoker: "
    ...
    /**
     * calculates the target area for the getfeatureinfo request from the maps
     * bounding box, the its size and the image coordinates of interest. An area
     * is calculated instead of using a point because to consider uncertainties
     * determining the point of interest
     * 
     * @param ds
     *          <tt>DataSource</tt> of the layer that is requested for feature
    ...
    private GM_Envelope calcTargetArea( DataSource ds ) throws WebServiceException
    {
      ...
      try
      {
        ...
        GeoTransform gt = new WorldToScreenTransform( bbox.getMin().getX(), bbox.getMin().getY(),
            bbox.getMax().getX(), bbox.getMax().getY(), 0, 0, width - 1, height - 1 );
        ...
        tBbox = GeometryFactory.createGM_Envelope( target[0], target[1], target[2], target[3] );
      }
      catch( Exception e )
      {
    ...
    /**
     * creates a describe Coverage request The request will be encapsualted
    ...
    /**
     * creates a GetFeatureInfo request for requesting a cascaded remote WMS The
     * request will be encapsualted within a <tt>OGCWebServiceEvent</tt>.
     * 
    ...
    private OGCWebServiceEvent createGetFeatureInfo( DataSource ds ) throws XMLParsingException
    {
      ...
      gmr = WMSProtocolFactory.createGetMapRequest( version, "" + idg.generateUniqueID(), lys,
          elevation, sampleDim, format, getMapRequest.getWidth(), getMapRequest.getHeight(),
          getMapRequest.getSrs(), getMapRequest.getBoundingBox(), getMapRequest.getTransparency(),
          bgColor, getMapRequest.getExceptions(), time, null, null, vendorSpecificParameter );
    ...
    public void write( Object result )
    {
      ...
      try
      {
        ...
        if( res.getException() != null )
        {
          ...
          OGCWebServiceException exce = new OGCWebServiceException_Impl( "ServiceInvoker: "
              + layer.getName() + "\n" + location.getFirstChild().getNodeValue(), message
     ...
     * <tt>DisplayElement</tt> and a <tt>Theme</tt> from it
     */
    private void handleDescribeCoverageResponse( WCSGetCoverageResponse response ) throws Exception
    {
