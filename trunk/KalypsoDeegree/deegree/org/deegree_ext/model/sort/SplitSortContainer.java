...
public class SplitSortContainer
{
	...
	*/  
	...
	else // has no subContainer && myObjects.size() >=MAX_OBEJCTS
	    {
		createSubContainers();
		//		if(size()>RESORT_SIZE)
		resort();
    ...
    {
	return myEnvelope.contains(env);
    }
    */
	...
	GM_Envelope env[]={GeometryFactory.createGM_Envelope(minX,minY,midX,midY),
	...
	for(int i=0;i<4;i++)
		...
		if(mySubContainer[i]!=null
		   &&
		   mySubContainer[i].getEnvelope().contains(env))
		    {
	...
	for(int i=0;i<4;i++)
		...
		if(mySubContainer[i]!=null
			   &&
		   mySubContainer[i].getEnvelope().intersects(env))
		    mySubContainer[i].remove(env,object);
	    }
	...
	*/
	...
	if(hasSubContainers())
		...
		mySubContainer=new SplitSortContainer[]{null,null,null,null};
	    }
	if(hasSubContainers())
	    return;
	if(size()>0)
    ...
    {		
		...
		g.drawRect((int) (g1x < g2x ? g1x:g2x),
			   (int) (g1y < g2y ? g1y:g2y),
			   (int) Math.abs((g2x-g1x)),
