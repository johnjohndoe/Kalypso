 ...
 */
public class WProfImportMarkerPage extends WizardPage
{
  private final Map<String, int[]> m_markerMap = new HashMap<String, int[]>();

  public WProfImportMarkerPage( final String pageName )
  {
    super( pageName );
  }

  public WProfImportMarkerPage( final String pageName, final String title, final ImageDescriptor titleImage )
  {
    super( pageName, title, titleImage );
  }

  public void setDefaultValues( final String markerType, final int[] values )
  {
  ...
  public void createControl( final Composite parent )
  {
    ...
    createMarkerRow( group, "Trennflächen", IWspmTuhhConstants.MARKER_TYP_TRENNFLAECHE, false );
    createMarkerRow( group, "Bordvollhöhe", IWspmTuhhConstants.MARKER_TYP_BORDVOLL, true );
    createMarkerRow( group, "Durchströmte Bereiche", IWspmTuhhConstants.MARKER_TYP_DURCHSTROEMTE, true );
  ...
  protected void handleTextChanged( final String label, final String currentText, final String markerType, final boolean isOptional )
  {
    ...
    try
    {
      ...
      {
        if( !isOptional )
          setErrorMessage( String.format( "'%s' must be set", label ) );
        return;
      }
    ...
    {
      setErrorMessage( String.format( "Invalid input: %s (%s)", currentText, e.getLocalizedMessage() ) );
    }
  }
