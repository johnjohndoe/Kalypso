...
public class StationsByStationsStrategy
{
  ...
  {
    /* 1. Level: Stations */
    final Collection<TimeseriesNode> nodes = new ArrayList<TimeseriesNode>();

    final IFeatureBindingCollection<Station> stations = m_stations.getStations();
    for( final Station station : stations )
    {
      final TimeseriesNode stationNode = buildStationNode( station );
      nodes.add( stationNode );
    }

    return nodes.toArray( new TimeseriesNode[nodes.size()] );
  }

  private TimeseriesNode buildStationNode( final Station station )
  {
