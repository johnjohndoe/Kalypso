...
public class ThemePainter
{
  ...
  {
    /* Find all visible themes to be painted. */
    final IKalypsoThemePredicate visiblePredicate = new IKalypsoThemePredicate()
    {
      public boolean decide( final IKalypsoTheme theme )
      {
        // Also filter out CascadinKalypsoThemes (aka IMapModell's), they would just paint their children.
        return theme.isVisible() && !(theme instanceof IMapModell);
      }
    };
    final KalypsoThemeVisitor visitor = new KalypsoThemeVisitor( visiblePredicate );
    model.accept( visitor, IKalypsoThemeVisitor.DEPTH_INFINITE );
    final IKalypsoTheme[] visibleThemes = visitor.getFoundThemes();
