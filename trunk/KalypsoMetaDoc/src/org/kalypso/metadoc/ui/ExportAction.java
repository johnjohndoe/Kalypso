...
{
  private final static class ExportWizard extends Wizard
  {
    private final Shell m_shell;
    private final WorkspaceModifyOperation m_operation;
    private final IExportTarget m_target;

    private ExportWizard( final IExportTarget target, final IExportableObjectFactory factory, final Shell shell ) throws CoreException
    {
      m_target = target;
      m_shell = shell;
      ...
      m_operation = new WorkspaceModifyOperation()
      {
            ...
            InterruptedException
        {
          try
          {
            ...
            final IStatus status = stati.length > 1 ? new MultiStatus( KalypsoMetaDocPlugin.getId(), 0, stati, "siehe Details", null ) : stati[0];
            if( !status.isOK() )
  ...
  public void run()
  {
    ...
    try
    {
      final Wizard wizard = new ExportWizard( m_target, factory, shell );
