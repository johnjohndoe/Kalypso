...
{
  private Collection<HydrotopeBean> m_result;
  ...
  public IStatus execute( final IProgressMonitor monitor )
  {
    ...
    final Map<String, HydrotopeBean> hash = new HashMap<>();
    ...
    for( int i = 0; i < intersectionsSize; i++ )
    {
      ...
      final HydrotopeBean hydrotope = (HydrotopeBean) polygon.getUserData();
      final String key = hydrotope.getHydrotopeHash();

      /* Add or merge element */
      final HydrotopeBean existingBean = hash.get( key );
      if( existingBean == null )
        hash.put( key, hydrotope );
      else
      {
        final HydrotopeBean newBean = existingBean.mergeGeometries( hydrotope );
    ...
    return new Status( IStatus.OK, ModelNA.PLUGIN_ID, m_logMessage );
  }

  public Collection<HydrotopeBean> getResult( )
  {
